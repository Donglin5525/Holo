...
1. **搜索栏**（顶部固定，见 §11-6）。报告 Tab 定位为**只读档案**：不在本 Tab 常驻发起入口（发起统一回对话页，见 §11-8）。
...
### 4.4 发起的两条路（殊途同归，§11-8 修订）

对话胶囊 / 直接说话问分析 —— 走同一条生成链路、归入同一份档案。聊天流照旧落一张分析卡，卡上新增「已存入报告 ↗」回执 chip（点击跳报告 Tab）。空态橱窗 CTA 属于第三条「软路径」：切回对话并预填话术，仍由用户确认发送。
...
6. **搜索**：搜索栏 + 300ms 防抖；**匹配范围=提问/标题/摘要/正文（观察与建议全文）+回放内容**；实现为 SQLite `CONTAINS[C]` 粗筛 + 内存精确匹配，结果上限 50；搜索态列表平铺不分月，空结果有引导文案。
7. **布局**：档案按月分组（「2026年8月」月头 + 分隔线），给「档案越来越厚」以时间节奏；搜索态、生成中卡不参与分组。
8. **撤除报告 Tab 发起区（2026-08-22，东林拍板）**：「发起新分析」按钮与范围胶囊整体移除——与对话胶囊入口重复，且点击即发送、用户没有确认输入的机会，不符合对话产品预期。报告 Tab 定位为**只读档案**；发起统一走对话（胶囊或自己输入，额度预检沿用聊天流既有逻辑）。空态橱窗 CTA 改为「去对话发起我的第一份分析」：切回对话 Tab + 预填话术到输入框、**不自动发送**，确认权在用户。随发起区一并移除：范围胶囊、额度角标、双池预检与内联拦截卡（其存在前提是「本 Tab 发起」）。
