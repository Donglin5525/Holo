...
| 文件 | 说明 |
|------|------|
| `CoreDataStack.swift` | 已更新，添加所有 Todo 实体定义 |
...
| `Color+Hex.swift` | 已更新，支持 3/6/8 位十六进制颜色 |
...
1. **测试数据模型**：验证 Core Data 实体是否正确创建和保存
2. **完善通知服务**：将 TodoNotificationService 集成到任务创建/编辑流程
3. **添加空状态处理**：优化 EmptyStateView 的展示逻辑
4. **修复已知问题**：
   - RepeatRuleView 中的 untilCount 被注释，需要启用
   - TaskDetailView 中的重复规则显示需要完善
...
1. **AI 智能排序**：基于优先级和截止时间的智能排序
2. **AI 每日总结**：生成任务完成情况报告
3. **Widget 小组件**：今日任务桌面组件
4. **Siri Shortcuts**：语音创建任务
...
1. **编译测试**：在 Xcode 中编译验证
2. **数据测试**：测试 CRUD 操作是否正常
3. **UI 测试**：验证所有视图的展示和交互
4. **集成测试**：验证与 HomeView 的集成
