...
> 2026-07-15 更新：已完成 HoloAI 领域记忆与跨域融合的本地实现和后端生产发版；默认处于 shadow 阶段，等待真机与真实指标灰度验收。
...
| 能力 | 当前行为 |
|------|----------|
| 分领域理解 | 财务、观点、健康、习惯、任务、目标、对话和个人信息分别形成高信号记忆 |
| 跨域融合 | 仅连接共同时间、共同主题且有独立底层证据的多个领域；关联不会写成确定因果 |
| 问题路由 | “最近状态如何”优先组合记忆；“最近 14 天吃了多少麦当劳”仍查询明细 |
| 时间权重 | 以最后一次有效证据动态衰减，较新的结论优先；评分细节不向普通用户展示 |
| 静默更新 | 启动、回前台和数据变化时后台检查；数据未变化时跳过 AI 调用，不阻塞业务主流程 |
| 用户控制 | 可确认、纠正、停用、忘记和查看来源；忘记后通过语义墓碑阻止近义重生 |
| 隐私边界 | 普通记忆可进入 iCloud 私有库；健康及包含健康证据的派生记忆只保存在本机 |
| 发布控制 | 默认 shadow，只生成和统计、不注入回答；萃取、融合、回答注入可独立熔断 |
| 开发验证 | Debug 的“AI 记忆实验室”可检查领域链路、跨域门禁、查询路由和脱敏指标；Release 不包含该能力 |

用户侧只需要理解“自动形成记忆”和“记忆辅助回答”两个开关，不需要理解置信度、新鲜度、相关度、版本链或灰度阶段。
...
- **完整逐字对话长期记忆**：不会保存每轮原文；只允许用户明确表达且通过敏感与稳定性校验的高信号记忆
