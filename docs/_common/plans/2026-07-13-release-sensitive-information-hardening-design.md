...
**状态：** 第一阶段已完成；2026-07-13 App Store 审核复查后，批准追加构建级诊断隔离
...
东林作为内部负责人，可以在 Xcode Debug 或显式启用 `INTERNAL_DIAGNOSTICS` 的内部构建中查看完整 AI 调用日志，但必须先通过 Apple 登录完成后端身份验证。App Store Release 不编译日志入口、日志拉取和内部会话交换能力，避免形成审核员无法访问的隐藏功能。

内部日志能力是独立的运维权限，不属于 Plus 权益，也不允许通过客户端本地开关、隐藏手势或静态白名单获得。服务端白名单继续保留，形成“构建条件 + Apple 身份验证”双重边界。

## 2. 成功标准

1. 普通用户和 Plus 用户无法从正式版 UI、Core Data、iCloud、公开后端接口获取内部 AI 配置或完整 Prompt。
2. 非内部账号不会生成、持久化或同步完整 LLM 请求日志。
3. 东林用已授权 Apple 账号登录内部构建后，可以在真机长按 AI 回复查看对应调用的完整日志。
4. 内部权限退出登录、Apple 凭证失效或令牌过期后立即失效。
5. Prompt 由后端根据 `purpose` 选择并注入，iOS 正式版不下载托管 Prompt 正文。
6. 生产公开接口只返回实现业务所需的最小信息。
7. App Store Release 产物不包含“查看日志”入口，也不会向内部会话端点交换 Apple identity token。
8. 所有后端变更部署到 ECS，并通过本机、服务器本机和公网三层验收。
...
虽然可以阻止普通用户读取日志，但日志功能仍作为审核员无法访问的隐藏能力存在于 App Store 二进制中，增加 2.3.1 审核风险。因此必须再增加编译条件。

### 3.4 仅使用编译条件、不做身份验证

内部构建仍可能被复制或安装到非本人设备。编译条件只能区分分发渠道，不能证明操作者身份，因此后端 Apple 白名单与短期令牌仍然保留。
...
内部日志相关 UI、Apple 内部会话交换、后端日志拉取和本机诊断仓库只在 `DEBUG || INTERNAL_DIAGNOSTICS` 条件下编译。标准 App Store Release 不定义这两个条件。

日常 Xcode 真机调试继续使用 Debug。需要接近 Release 优化行为的内部包时，可在受控构建命令中显式传入 `INTERNAL_DIAGNOSTICS`；该条件不得加入 App Store Archive 配置。
...
后端为每次 AI 请求生成不可预测的 `requestId`。

内部账号的 AI 调用完成后：

1. iOS 从响应头取得 `requestId`；
2. iOS 检查内部权限；
3. 使用 Holo 会话令牌立即请求 `GET /v1/internal/ai-logs/:requestId`；
4. 后端再次验证 `internalDiagnostics`；
5. 后端从短期热缓存返回该请求的完整诊断日志；
6. iOS 将完整日志写入仅本机的内部诊断仓库。

长按 AI 回复时，iOS 再次检查内部权限，从本机内部诊断仓库读取对应日志，并在现有日志页展示和复制。内部拉取失败不影响 AI 主功能，也不通过普通接口重新获取 Prompt 或日志。

任何普通令牌、Plus 状态、设备 ID 或客户端开关都不能访问该接口。

### 6.3 日志保留与本地存储

- 后端完整日志只停留在现有短期热缓存；SQLite 继续只保存截断脱敏摘要，不新增完整正文持久化；
- iOS 不把完整日志写入 Core Data 或 CloudKit；
- iOS 使用独立的本地内部诊断仓库保存 `messageId`、`requestId` 和完整日志，默认保留 7 天；
- 内部诊断文件使用系统完整文件保护，不进入备份、共享容器或导出数据；
- 退出登录、Apple 凭证撤销和清除缓存时删除全部内部诊断文件；
- 普通账号不会创建该仓库，也不会收到内部日志内容。
...
- App Store Release 源码边界覆盖内部会话、日志捕获、日志页面和菜单；
- 普通账号不显示日志菜单；
- Plus 账号不显示日志菜单；
- 内部权限有效时显示日志菜单；
...
8. 扫描 Release 产物确认不存在“查看日志”和内部日志页面特征；
9. 东林账号在 Debug/Internal 真机验证可查看一次真实 AI 请求日志；
10. 退出登录后再次验证入口消失；
11. App Review Notes 只描述审核员可访问的 Release 功能，不引用开发者 AI 设置或内部日志。
