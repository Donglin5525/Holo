...
## 九、追加改版：「提醒时刻」弹层交互重做（2026-09-23 东林二次批评后调研立项）

**东林批评两点**：① 已添加的时刻只能删不能改；② 全天任务弹层没有「提前15分钟」这类快捷档位。经六产品调研（Apple/Todoist/滴答清单/微软To Do/Things3/谷歌Tasks，见会话记录）证实这两点均违反业界通用约定。

**调研结论**（共性）：预设 chips 是一等入口、滚轮收末位；已设条目「点行即改」绝不删重建；全天任务默认「当天 9:00」是三巨头共识（我们预填撞对）；「准时/提前N分钟」只适用于带时刻任务。

**改版方案**（东林已看过可交互 HTML 原型 docs/prototypes/2026-09-23-reminder-picker-prototype.html 后拍板，档位取三个不加中午档）：
1. 绝对模式加锚定预设 chips：当天 09:00 / 当天 18:00 / 前一天 20:00，点选即加、再点即删、可多选、选中态与条目联动；落在过去的档位置灰（防止建永不响的提醒）。
2. 点行即改：任意已设条目点行内展开日期+时刻滚轮，删除按钮保留；替代「只有最新一条临时滚轮」的旧实现（newAbsoluteReminderId → editingReminderId）。
3. 详情页统一：全天任务的提醒区从「提前N分钟」相对 chips（语义不成立：全天无截止时刻，提前15分钟=00:00前几分钟）改为锚定绝对档位，与确认卡一致；带时刻任务维持相对 chips 不动。
4. 保留：预填 9:00（业界共识）、锚定添加默认、空选=显式不要提醒。

**实施**：TaskPendingDefaults.anchoredPresetDate 纯函数（单测锁定）+ TaskReminderEditor 绝对模式重写 + TaskDatePickerSheet 模式判定改为「带时刻才相对」。编译绿，TaskPendingEditTests 18/18。

**QA 冒烟 PASS（5 断言全过，无崩溃）**：chips 点选/再点取消/多选/计数联动 ✓；点行即改行内展开双滚轮 ✓；删除与 chip 熄灭联动 ✓；预填 9:00 且对应 chip 选中 ✓；详情页全天任务已切锚定档位、带时刻任务相对 chips 未被打坏 ✓。QA 报的「先建任务在列表消失」疑点经查库+二进制哈希比对定性为**并行会话装包互踩**（19:39:06 外来包覆盖安装清空 AppGroup 库，与记忆 Simulator-package-overwrite-phantom-defect 同类），非本改动缺陷。遗留小项：相对 chips 已选项高亮不明显（原状，非本次引入），设计侧后续定。
