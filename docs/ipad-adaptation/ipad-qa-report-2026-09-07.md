...
- 全量 HoloTests：912 条，905 过 / 7 失败。**7 个失败全部是 ThoughtRichContentTests（想法富文本域），错误为 Core Data「模型配置不兼容」**——与本批改动（习惯/长廊/布局常量）零代码交集；且套件从上轮的 899 涨到 912 条，正是并行会话刚把 PersonalContext 系测试接入 pbxproj（字母序 PersonalContext 先于 Thought 执行），失败模式与「集成测试在同进程注册共享 CoreData 栈污染后续测试类」的在档陷阱完全吻合。判定：并行会话在途测试的跨类污染，非本批引入，已如实转告。
- ⚠️ **工程文件事故记录**：02:50 前后 project.pbxproj 被并行会话编辑至「xcodebuild 无法打开（Project damaged）」状态（plutil 语法层 OK、语义层解析失败）。本批所有验证与取证均完成于损坏发生前；提交内容不含 pbxproj。后续会话若无法构建，需等该会话完成其工程文件手术或自行修复。

## 五、后续待办（真机侧）
1. iPad 真机安装本包（release/1.0.2 @ 4d7341a23）登录 iCloud 验证习惯数据出现。
2. 妙控键盘 ⌘1-9 直跳 / 轴档长按拖选门，模拟器无通道，留真机首验。

## 六、拍板项实施批（2026-09-07 中午，东林五条拍板后）

东林拍板：①习惯同步他晚上真机验收；②轴档做多泳道展开；③想法双栏开工；④设置双栏开工；⑤长廊 920 先保留看效果。②③④已实施并模拟器取证（/tmp/holo_ipad_audit/v10-*）：

### ②轴档多泳道展开（TimelineAxisLayout + TimelineReplayView）
- 组内按时间重叠贪心分配泳道（重叠各占一条、释放即复用），任务/日程两组宽度按泳道数比例分配——1:1 退化为旧的对半布局，重叠越多该组展得越开。
- 种子器补「准备复盘材料 15:30–16:45」与「完成产品复盘 16:00–17:30」重叠夹具，今天的轴档天然呈现多泳道（v10-01）。
- 单测：TimelineAxisLayoutTests 新增 6 条泳道分配契约（无重叠单泳道/重叠分道/释放复用/三重重叠/接龙共用/空输入退化），16/16 绿。

### ④设置页宽屏双栏（SettingsView）
- expanded 档改为「左分组导航（252pt：用户信息卡+11 个分组项）+ 右侧只渲染所选分组」；窄屏/iPhone 保持单列长滚动不变。
- 分组与单列 section 一一对应，零内容重组；右栏切组重置滚动（v10-02，亦见 v8-l-14）。

### ③想法列表-详情双栏（ThoughtListView + ThoughtsView）
- expanded 档想法 tab 改「左列表 46% + 右详情 54%」：轻点左列表卡片右栏即展详情，不再全屏跳转；无选中时右栏显示引导位（设计稿②）。
- 编辑仍走全屏编辑器；知识树（主题）tab 本批未动双栏，为下一批（见遗留）。
- 宽屏新建入口上移列表头部（右上「+ 新建」），右下角 FAB 在宽屏退役；手机/竖屏 FAB 不变（设计稿④）。
- 竖屏回归（v10-05）：单列列表+FAB 在场，行为与改前一致。

### 遗留（下一批）
- 知识树（主题）tab 的双栏化（设计稿③第 3 条）：需单独重构 ThoughtKnowledgeTreeView，未混入本批。
- iPhone 真机/模拟器 spot-check：本批改动均按宽度档位门控（expanded 才生效），竖屏 iPad 回归已过；iPhone 端建议东林真机顺手看一眼想法/习惯/轴档。
