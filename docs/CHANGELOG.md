...
### Features
- **AI 思考过程透明化**：AI 回复期间不再只显示跳动的圆点或「正在分析中…」，而是如实告诉用户此刻在查阅哪些数据，让用户切实感受到 AI 在翻阅「我的」数据来回答
  - **深度分析**：把内部已有的工具调用记录（调了账单/记忆/任务/想法等）翻译成用户可见文案，从泛化的「正在读取本地数据」升级为具体的「正在翻阅账单（12 条）、回顾记忆（3 条）」；数字来自工具实际读取的原始记录条数（账单笔数/想法条数/记忆条数等），不是估算；点击卡片可展开查看完整调用日志时间线
