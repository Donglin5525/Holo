...
2. `MessageBubbleView`：当消息有 `analysisContext` 时，改为渲染单个 `AnalysisCompactChatCard`，不再直接铺开 `analysisCards + bubbleContent`。
...
| `Views/Chat/MessageBubbleView.swift` | 有 `analysisContext` 时渲染 `AnalysisCompactChatCard` |
