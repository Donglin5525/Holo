...
Agent 可以按自然日对齐健康与财务、健康与习惯数据，现场计算相关性、条件均值和分组差异，不再依赖预先写死的跨域指标。

### 变更
- **受控跨域计划**：新增 `crossDomainPlan`，首批开放健康 × 财务、健康 × 习惯，禁止任意 join 和未注册字段访问。
...
- **Prompt v6**：iOS 与后端同步跨域工具选择、计划格式及安全边界。
