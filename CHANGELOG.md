...
## [2026-09-20] 健康模块日期筛选升级：点日期弹日历任意跳历史 + 左右滑动切天

> 纯 iOS 改动，无 HoloBackend 发版。

### 新增
- **日期胶囊可点出日历**：健康看板与各指标详情页顶部日期胶囊的日期文案（新增小日历图标）点击弹出「选择日期」日历弹层，跨月翻页、点选任意历史日期立即生效并关闭、未来日期置灰、右上「今天」一键回当天——替代此前只能一天一天点箭头的低效筛选。
- **左右滑动切天**：看板页与详情页内容区左滑切后一天、右滑切前一天（滑出+弹入两段动画），今天封顶不越界（再左滑回弹）；上下滑动仍正常纵向滚动。

### 改进
- **右滑返回零冲突**：切天手势起步避开屏幕左缘 24pt（右滑返回手势的专属接管区），左缘右滑行为不变；既有两条边缘右滑 UITest 回归用例继续在册锁定。
- 账本页验证过的 DaySwipeContainer 滑动容器共享复用，新增「未来方向封顶」与「左缘排除带」两个参数（账本页走默认值行为零变化）；「不越过今天」边界规则收敛为 HealthDateNavigator.steppedDate 单一实现，箭头/日历/滑动三条路径共用。
- 切天滑动过程中的点击防误触守卫：指标卡、身体状态入口、趋势切换、洞察卡、数据源卡、运动会话行同步读滑动状态兜底。

### 验证
- Debug 全量编译与 UITest 测试构建双绿；UITest 新增 3 条用例（日历跳转+一键回今天 / 看板切天+今天封顶 / 详情切天与边缘右滑返回共存）已入册；东林人工验收通过。
...
### 修复
- **问答断点续答**：进行中的目标问答（轮次/已答/草案）落 UserDefaults（GoalPlanningSessionStore），切 Tab、杀 App 后回来继续回答自动接上上下文；超过 2 小时未完成的问答作废（防陈旧会话吞新消息）；确认/取消/额度终态即清。
- **草案期发送无响应**：草案就绪后再发消息，此前只写一个无界面消费点的 errorMessage，用户看到的是「点了没反应」；改为落到聊天气泡告知先处理草案卡，输入文字不再吞掉。
- **草案卡归属收紧**：草案就绪期间只有最新一条 AI 规划消息渲染成草案卡，历史追问不再全部变卡（会话恢复场景会放大此缺陷，顺手收紧）。
- **拦截提示不堆卡**：草案拦截提示气泡保持普通消息类型——若标成 goalPlanning 类型会自己成为「最新一条 AI 规划消息」撞草案卡归属条件，拦截一次堆一张卡（QA 复验实锤后修正）。

### 验证
- 模拟器（iPhone 17 Pro Max 无头 QA 子代理四场景）：切 Tab 续答 PASS（回答继续目标问答，不再弹任务卡）；杀 App 草案卡恢复 PASS（单卡不重复）；目标共创新流程入口（GOAL_WORKSHOP_FORCE_ON）PASS；五大入口冒烟无闪退。二轮复验：草案拦截气泡正常提示、输入保留、草案确认保存走通后聊天链路恢复（全 PASS）；三轮复验拦截不堆卡。
- 注：分支 HEAD 自 8866c0ca8 起因小组件 HoloWidgetIntents.swift 漏提交而不可独立编译（在途会话待补），本提交不涉及。
...
## [2026-09-17] 想法卡片点正文进编辑器真机根治 + 「…」编辑兜底 + 编辑器工具栏参考改版；详情页砍除批次收口入库

> 「真机无法编辑历史想法」P0 根治（探针实证根因）+ 工具栏按 flomo 参考改版 + 详情页砍除批次随车入库。纯 iOS 零后端发版。

### P0 修复：点卡片正文进编辑器（真机无法编辑历史想法事故）
- **根因**（swiftc 单文件探针 7 变体二分实证）：正文 Button 的 label 内容全是禁命中层（禁交互 UITextView 正文 + 禁命中测宽层），而 `contentShape` 声明挂在 **Button 外**——iOS 26 上 Button 外的 shape 不参与 Button 自身的点击识别，Button 因无可命中实体整体失效（真机手指与模拟器合成触摸同样死、同屏其他按钮正常）。
- **修法**：`contentShape(Rectangle())` 移入 **label 内部**（探针 V1 实证命中）；正文入口由纯 onTapGesture 改为 Button 承载；Button 外不再包 `.accessibilityElement(children: .ignore)`（实证吞触摸）。引用列表卡片（ReferenceCardView）同款错误写法一并修正。
- **「…」菜单补「编辑」兜底项**：正文触摸链路万一再失效，历史想法仍有保底编辑入口。
- **推翻旧结论**：「合成触摸点不透正文=模拟器限制、勿改代码」不成立——真机同样死，合成触摸反而忠实复现真机此故障、可作验证通道；踩坑速查表已新增条目。

### 工具栏改版（参考 flomo）
- 发送键正圆 → 横向胶囊（常规 46×32 宽高比 1.44 / 紧凑 42×30），纸飞机图标小而居中；麦克风→发送拉开 12pt、任务→语音 8pt；左组 6 工具占位 38/40→32、图标 17→15pt，省出的宽度变成左右组间弹性留白。
- ViewThatFits 两档自适应：402pt+ 走呼吸档（组间留白实测约 40pt）、375pt 极窄屏自动紧凑档防溢出。
- QA 像素级实测两档全过（胶囊比例 1.44/1.41、右缘无裁剪、三元素无粘连）。

### 随车入库（详情页砍除批次收口）
- `ThoughtDetailView` 删除，编辑器（`ThoughtEditorView`）承接阅读+编辑双职能（纸飞机=新建 / ✔=编辑双态）；引用列表卡片新组件 `ThoughtReferenceListView`；两个守门测试摘除详情页豁免/扫描项。
- `HomeView` / `ThoughtsView` 编辑器调用改显式 `onSave:`（编辑器新增多个可选闭包参数后 trailing closure 会绑错参，致纸飞机退出失效）。
- 筛选按钮补无障碍标签「筛选」（原被系统兜底误标）。
- 「编辑」「筛选」词条词表已有，零 xcstrings 改动。

### 验证
- 探针 7 变体定位（V0-V5 全命中 / V6 生产复刻 0/5）；修复后 QA 终验 8/8 PASS（点正文文字/点空白/第二张卡/端到端改字保存回读/菜单兜底/防误触/无崩溃）。
- 五大入口冒烟过（HoloAI 发消息因无头模拟器授权门未走完，待真机）；工具栏 402/375 双机全过；worktree 独立验证编译绿。

---

## [2026-09-17] 想法卡片「点击查看全文」原地展开：不跳编辑页、零重建切换

> 纯 iOS 零后端发版。模拟器两轮走查通过（含对照实验定性），待真机验收。

### 新增
- **「点击查看全文」原地展开**：列表卡片点「点击查看全文 ›」不再跳编辑页，卡片原地向下展开完整正文（0.2 秒过渡动画），原位置变「收起 ˄」点击收回；点正文其余区域仍进编辑页（详情页砍除后的主路径不变）。多张卡可各自展开，滚出屏幕后展开态自然复位；「待确认」队列同款卡片用同一组件，自动获得相同交互。

### 性能
- **行数摘出渲染重建触发器**：正文渲染层「输入不变跳过全量重建」的判定原含行数维度，直接加展开会令每次切换都触发长文富文本全量重建（长文几十毫秒、点击瞬间掉帧）。行数本只是 textContainer 属性（updateUIView 每次同步），摘出后展开/收起零重建、仅一次 UIKit 重排，动画由渲染层完成——不卡顿的核心保障。
- **展开态封闭在正文预览组件内部**：外层卡片（ThoughtContentBody 只依赖内容字符串两份输入）不感知展开态，父级状态型重算依旧整体跳过正文（保住「…」卡顿修复的成果）；卡片滚出 LazyVStack 即销毁，无驻留成本。

### 入库说明
- `ReadOnlyRichTextView.swift` 同文件随车入库在途的测宽层禁命中修复（`Color.clear.allowsHitTesting(false)`，正文 Button 命中链恢复的配套件），两改动功能咬合且经同一组合包验收。
- 「收起」词条词表已有，零 xcstrings 改动。

### 验证
- 编译绿；QA 专用模拟器两轮走查：展开/收起 5 次切换终态全对、不跳页、滚动无异动、全程无崩溃。
- 「展开态点正文进编辑页」在模拟器无法端到端验证：合成触摸点不透想法卡片正文为既有通道限制（收起态同样点不进，对照实验定性，非本次回归）；该路径代码本次未动，待真机顺手确认。
...
## [2026-09-16] 图片快捷记账功能收口入库 + 「待确认」通知押后 5 分钟

> 功能本体（1.0.5 切割遗留）首次入 1.0.6 分支；通知押后为东林 2026-09-16 拍板（5 分钟）。纯 iOS 零后端发版。

### 新增 · 图片快捷记账（快捷指令自动记账，2026-09-14 方案）
- **快捷指令系统动作「识别图片并记账」**：把支付截图/小票交给 Holo 识别（金额/商户/日期/支付通道），安全时自动入账；Siri 短语「用 Holo 记账」。处理方式两档：「安全时自动记账」（后台完成不打扰）与「始终先确认」（全部生成待复核项并打开复核页）。
- **门禁三态**（v2 契约，字段级置信度）：单笔+已完成+各字段置信达标才允许自动写；金额/方向/日期不确定、多笔、疑似重复、快捷指令账户/项目失效等一律转待复核；转账/理财/未支付/外币/无关图诚实拒识。自动入账总闸 `autoCommitEnabled` 暂固定关闭（M1 灰度），当前实际全部走待复核。
- **原子入账 + 幂等**：来源键（规范化图片摘要）+条目键与交易同次落库，同图连按/系统重试/回执丢失重跑只产生一笔；撤销后同图允许重新入账。
- **复核流闭环**：待复核草案（App Group JSON+受保护证据图，7 天过期启动清理）；通知点击/设置页「图片自动记账」入口（带待复核角标）直达复核页；确认走分类链重解析后原子落账，支持改金额/账户/项目/分类。
- **聊天图片记账同链路收口**：聊天发图识别改走公共协调器（压缩/摘要/识别/账户解析一条链），AI 来源字段与交易同次保存，路由期间被杀可凭来源键对账（interrupted confirmation 修复通道）。

### 优化 · 「待确认」通知押后 5 分钟（即记即扰根治）
- 快捷指令选「始终先确认」时记账后不再立即弹「有一笔账需要你确认」：通知押后 5 分钟，用户在打开的复核页当场确认/放弃即撤回，一次不打扰；一直未确认才到点弹提醒兜底。不打开 App 的场景（通知是唯一反馈）与结果类通知（已记账/已记过/失败）维持立即投递。
- **通知标识稳定化**：待复核通知标识从发时随机 UUID 改为绑定草案 ID；撤回挂在删草案必经之路 `discardDraftFiles`（确认/放弃）与 7 天过期清理，顺带根治「确认后通知仍挂通知栏、点进去只提示已处理」旧缺陷。历史随机标识通知无法追溯撤回，仅对新通知生效。

### 验证
- `ReceiptBookingKernelTests` 30 条全绿（门禁/幂等/原子写入/金额转换/结果文字/草案往返/通知押后 5 分钟行为锁定），iPhone 17 模拟器。
- 测试栈基础设施：`CoreDataTestSupport` 收编进程唯一内存容器 + 实体清空工具（R4-1 测试污染根治的落点，ReceiptBookingKernelTests 落库三测已按此迁移），随本次一并入库。
- 提交为外科手术式收口：工作区其他在途改动（iPad 改造/记忆/固定支出等）未混入；`Localizable.xcstrings` 为共享词表整文件入库（含并行会话已完成的少量词条，闲置无害）。
