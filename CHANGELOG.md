...
## [2026-07-23] Phase 0–5 安全与可靠性整改上线

本轮把代码审查中确认的高风险问题从“文档结论”推进到可运行、可验证的产品能力，并将最新 HoloBackend 发布到生产。重点不是增加可见功能，而是降低用户数据泄露、重复执行、错误结论、启动卡顿和发布不可追溯的风险。

### 后端与 AI Agent
- **成本与数据安全边界**：补齐请求体/上传限制、日志脱敏、管理员登录限速与安全 Cookie，关键生产配置缺失时启动即失败；增加 `/v1/live` 与 `/v1/ready`，支持优雅停机。
- **Agent 幂等与可信结论**：Agent step 响应改为加密持久化，并用 generation CAS 防止并发覆盖；统一校验结构化响应、证据与结论，区分“没有数据”“数据不可用”和“结果被截断”。
- **App Attest 闭环地基**：新增 challenge、key 注册、计数器防重放、实例会话和数据库迁移，iOS 已接入会话认证；生产当前保持灰度关闭，待 Apple production 证书、Capability 与 TestFlight 真机验证完成后再强制启用。
- **发布身份可信**：rsync 发布显式绑定已推送的完整 Git SHA，同时记录源码摘要，避免 ECS 本地旧 Git HEAD 冒充本次线上版本。

### iOS、隐私与工程质量
- **启动与导入稳定性**：应用启动任务分层，避免非关键初始化阻塞首屏；CSV 导入增加字段、类型与异常值校验。
- **本地 Agent 数据语义**：统一空值、不可用、截断状态，持久化和结论校验按同一契约处理，减少“没有数据”被误说成“状态正常”。
- **合规真相源**：建立官网隐私政策/支持入口及 App Store 生产配置说明；补齐统一测试清单与 CI 门禁。

### 生产发布与验证
- 生产数据库已在部署前在线备份；启动时完成 migration #7（Agent CAS）与 #8（App Attest 状态表），并额外生成迁移前备份。
- HoloBackend 本地测试 `151/151` 通过；生产 `/health`、`/live`、`/ready`、公开及管理员 release status 全部通过。
- 生产真实请求验证通过：记账日期契约、任务提醒契约、健康分析路由三类意图均通过。
- 线上发布身份：commit `b69fd72ab14fc417eb6cf55c80681fc792c767c9`，source digest `58eb1e17366e1645c6289d7fff94a8aaf2d97f4c5e7c7cdac0a51b40e9aa7e05`。

### 尚未完成
- App Attest 生产强制校验尚未开启：需补齐 Apple production 根证书/Team ID/Capability，并完成 TestFlight 或 Release 真机灰度。
- iOS 全量模拟器 XCTest 仍受本机 `simdiskimaged` 异常阻塞；相关目标代码已完成定向解析、类型检查和 standalone 回归验证。
- 官网尚未实际部署，App Store 产品页 URL 尚未回填；目标历史快照仍保留为后续独立的数据保留能力。
