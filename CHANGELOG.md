...
## [2026-09-15] 新用户激活一期——第一步行动卡 + 全模块空态一键直达 + 首次记录庆祝

### 新增
- **首页「第一步行动卡」**：新用户（启动时全库无任何记录）首页出现行动气泡，引导完成第一句 AI 记录；产生首条记录自动消失，点 × 手动关闭落盘不再打扰（A 横条形态实现后经复议替换为 C 气泡定稿）。
- **空态一键直达（激活方案 §3.2）**：任务/想法/财务/看板四模块空态从陈述句升级为行动按钮——任务「告诉 Holo 要做什么，或手动创建」、想法「一闪而过的念头，都值得留下来」一键开编辑器、财务「记一笔」、看板 calm 态「对 Holo 说」。
- **「记录今天」改指向 AI 并落焦**：今日看板快捷记录原误指想法编辑器，改为进 AI 对话且光标直接落在输入框等用户开口；不再预填示例句（「午饭花了 35 元」曾让用户误以为已记假账），placeholder「告诉 Holo 发生了什么…」承担提示职责。AI 页常驻场景的外部预填/聚焦信号消费链路（onChange 兜底）一并补齐。
- **首次记录庆祝浮层**：空库用户的人生第一笔记录触发全屏庆祝（一次即封，启动时已有记录的老用户升级后永不误弹），庆祝后引导去记忆长廊看第一颗星。

### 验证
- NewUserActivationState 5 条单测绿；A/B 两轮模拟器 QA 过；纯 iOS 无发版；待真机验收。

---

## [2026-09-15] 深度分析报告——提问入口 + 分享卡

### 新增
- **报告提问卡**：深度分析报告详情页内可就报告内容继续提问，追问与原报告同源展示（提问数据本就随报告存储，本次补齐入口与呈现）。
- **报告分享卡**：报告详情可生成分享图（复用想法分享卡骨架双档样式），一键保存/分享。

### 验证
- 模拟器走查过；App Store 种子器 busyWeek 场景补 agentResult 报告样本；纯 iOS 无发版；待真机验收。

---

## [2026-09-15] 固定支出保存即落账 + 科目管理换记一笔分类网格 + 月历缓存刷新根治

### 变更
- **固定支出保存即落账**：固定支出保存从「仅存模板」改为同时落一笔账（东林拍板），不再需要手动再记。
- **科目管理页改用记一笔同款分类网格**：新建/编辑科目使用与记一笔一致的分类选择体验，全 App 口径统一。
- **月历缓存刷新根治**：固定支出页落账后切回账本，月度卡片残留旧数字——CalendarState 统一监听财务数据变化通知并清缓存重载，不再依赖账本视图恰好在屏；跨月选日同步重载该月汇总与环比（顶部「本月支出」不再残留上月数字）。

### 验证
- 两轮 QA 模拟器全过；纯 iOS 无发版；待真机验收。

---

## [2026-09-15] 小组件「想法记忆」大卡字号根治 + ASC 截图 v5 素材入库

### 修复
- **想法记忆大卡字太大根治**：大尺寸卡片正文截断上限 72→220 字、标签字号下调至 10.5pt——卡片变大是多显行数、不是字变大；小组件快照服务同步扩上限。三轮 QA 过。

### 变更
- **ASC 截图 v5**：新增咖啡/甜点/天空三张场景素材，截图渲染脚本更新，App Store 截图重制。
...
## [2026-09-15] 相册「仅限选中的照片」细分提示 + 想法编辑器拍照/相册入口分拆

### 修复
- **受限权限细分提示**：相册权限为「仅限选中的照片」且所选图片不在授权子集时（选择器里看得到、加载却失败），提示从笼统「检查网络」改为明确自救指引（系统设置 > Holo > 照片改为「所有照片」后重试）；想法/任务/反馈三处图片入口共用同一口径；三层加载（裸数据/系统转码/iCloud 原图）每层补失败日志。
- **想法编辑器图片入口分拆**：工具栏「添加图片」拆为「拍照」（走相机权限流程）与「从相册选择」（前置相册读取权限）两个直达入口，去掉中转来源选择弹层（弹层曾与键盘焦点互相打架）。
