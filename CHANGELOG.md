...
### 验证
- 模拟器（iPhone 17 Pro Max 无头 QA 子代理四场景）：切 Tab 续答 PASS（回答继续目标问答，不再弹任务卡）；杀 App 草案卡恢复 PASS（单卡不重复）；目标共创新流程入口（GOAL_WORKSHOP_FORCE_ON）PASS；五大入口冒烟无闪退。draftReady 拦截气泡为复验项见下轮记录。
