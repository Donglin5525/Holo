...
## [2026-07-30] 任务模块体验大改：时间统一/砍规划日标签/只读详情页/想法转任务

任务模块此前交互割裂（日期时间提醒重复散在四个弹窗、截止日与规划日概念重叠、标签与清单两套分类打架、看板任务点不开、查看即误改），本次从结构和交互两层做了系统性重构。

### 结构改造
1. **砍掉规划日（plannedDate）**：一个任务只有一个日期概念。今日待办重新定义为「今天到期 + 已逾期、未完成」，首页「加入今日」改为设置截止日为今天。日历「计划」维度随之移除。
2. **隐藏标签体系**：任务分类只靠「清单」一套系统，标签入口全部移除。历史标签数据完整保留（仅隐藏不删除），避免 iCloud 同步冲突。
3. **时间四入口合并为统一面板**：原来的日期页、时间页、提醒页、重复页合并成一个面板，从上到下一次设完。不再来回切换。
4. **提醒可脱离截止日独立存在**：新增绝对时间提醒模式——「周五下午 3 点提醒我打电话」这种没有截止日、只想定点提醒的需求，现在一步能设。老任务的相对提醒（截止前 N 分钟）行为不变。
5. **想法转任务**：想法详情页新增「转为任务」，点一下把想法变成待办并建立来源关联；任务详情页显示「来自想法」的跳转入口。打通「灵感 → 行动」链路。

### 交互改进
6. **新建只读详情页（TaskDetailView）**：点击任务先查看（只读），想改再点编辑。解决了「一进来就是编辑框、误碰就改、改了就自动保存不问」的问题。列表、搜索、首页看板、AI 卡片点开任务的行为统一到只读详情页。
7. **首页今日看板任务可点开详情**：此前看板任务行只能勾选/加入今日、点不开详情（最大的体验断层），现已修复。
8. **完成撤回机制全局化**：任务完成的 3 秒撤回状态从各页面独立管理提升到全局共享，列表与看板行为一致，切屏后撤回 banner 仍在、撤回有效。
9. **卡片信息精简**：列表卡片默认只显示截止日 + 紧急度 + 清单，描述折成 1 行，子任务进详情页查看，一屏看到更多任务。
10. **其他打磨**：新建任务时标题自动聚焦弹键盘；保存失败有可见提示；删除/归档有过渡动画；左滑展开后点空白处自动收起；预设筛选状态跨退出保持记忆。

### 性质
- 纯 App 端改动（本地数据 + UI），不需要后端发版。
- 删字段（plannedDate）走 Core Data 轻量迁移，标签数据保留不删。

---

## [2026-07-30] 语音「改任务」支持改时间，通知跟随截止时间重排

此前通过语音/聊天修改已有任务（如「把换水的任务改到明晚」）时，链路是通的（找到原任务再更新，而非删了重建），但质量远低于创建任务，最痛的「改时间」几乎不可用：相对时间解析无兜底、全天标记不同步、改了时间后手机通知仍在旧时间响。本次把「改任务」拉齐到「建任务」同等水平。

### 改动
1. **后端指令补强**（`defaultPrompts.json` `intent_recognition`）：原 update_task 指令只有一行，大模型不知道改时间要填 dueDate、不知道用户没说的字段不能动；现明确「必填 taskKeyword 定位任务、改什么填什么（dueDate/title/priority）、未提及字段不填」，与 create_task 时间规则一致。配套把长度红线测试从 4300 调到 4350（update_task 补全属功能必需内容）。
2. **复用强时间解析**（`IntentRouter.handleUpdateTask`）：改任务改用与 create_task 同款的 `resolveTaskDueDate`（含原文兜底），「明晚」「今晚10点」等相对时间不再因 LLM 漏填而解析失败；同时改 dueDate 时同步 `isAllDay`，避免全天标记与时间错位。
3. **通知跟随重排**（`TodoRepository.updateTask`）：改截止时间后自动重排本地通知（先取消旧的、再按新时间重建），根治「时间改了但通知仍按旧时间响」的问题；这是数据层根治，所有调用 updateTask 改时间的路径都受益。
4. **回执更明确**（`AIResponseTextBuilder.taskUpdated`）：从干巴巴的「已更新任务」改为「已更新任务：给猫换水（改到 7月31日 20:00）」。

### 验证
- 后端测试 159/159 全部通过（含 intent_recognition 长度红线 4350）
- iOS 全工程编译通过（Holo target，iOS Simulator）

### 性质
- 分两端：后端 prompt 已部署生效（commit 792b16d，Docker 重建完成，生产健康检查通过）；iOS 三处改动已提交（commit f5b980c），随下次 App 发版生效
- 完整可用需前后端都上线：当前 iOS 未发版前，大模型返回正确但 iOS 端时间解析与通知重排仍是旧逻辑
