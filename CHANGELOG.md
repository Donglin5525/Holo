...
## [2026-07-29] 按数据量门槛控制自动 AI 调用：没有新输入就不调 API

此前每次开机 App 都会触发一批自动 AI 调用——生成今日洞察、提取各领域记忆、扫描历史回放摘要——即使今天没有输入任何新数据。从服务器真实用量看，这类自动触发是 token 消耗的重要来源（记忆提取、回顾摘要合计占约 40% 成本），且很多调用的「性价比」很低：单条数据变化就调一次 AI、今天零记录也照常生成洞察。

本次按「没有新输入就不该调 API，新增太少也不值得调」的原则，给**自动触发**路径加数据量门槛。用户主动发起的操作（聊天、手动生成洞察、Agent）不受任何门槛，始终全量数据。

### 改动
1. **日洞察：今天有记录才生成**：开机自动补生成今日洞察前，先查今天是否有任何有效记录（记账/待办/习惯/想法任一）。零记录直接跳过，不调 API。月洞察有天然的「本月不足 7 天自动回退上月」门槛且一月最多一次，成本极低，保持不动。
2. **记忆提取：每领域 ≥3 条信号才调 API**：此前只要某领域有 1 条信号变化就触发 AI 提取，导致一次开机 7 个领域各调一次。现在门槛提到 3 条，零星 1-2 条变化攒着等下次批量提取。低于门槛时调度器走既有的 `.belowMaterialThreshold` 跳过逻辑，无需额外改动。
3. **回顾摘要：去掉开机扫描历史**：此前每次开机都扫描全部历史洞察、最多调 4 次 API 合并进长期摘要（第三大开销）。现在去掉开机和回前台的扫描调用，改为被动模式——只在有新洞察生成成功后，自动合并那一条。从「主动扫历史」变成「跟着新洞察走」。
4. **用户路径零影响**：自动路径和用户路径在代码里本来就是分开的。用户手动生成洞察、聊天触发洞察、周期回放直接调 `generateInsight`，不经过前台补偿；聊天/Agent 只读现有记忆，不走提取路径。三处门槛全部只加在自动路径上。

### 验证
- 全工程构建成功（Holo Debug + Release，iPhone 17 Simulator，`BUILD SUCCEEDED`）
- 记忆提取门槛纯逻辑独立验证通过：6 项断言（0/1/2 条不提取，3/10 条提取）
- HoloTests target 受项目既有配置问题阻塞（3 个测试文件引用 main app 中不存在的类型，与本次改动无关），核心逻辑由独立编译验证

### 性质
- 纯 App 端改动，随下次 App 发版生效，不涉及后端发版
- 基于服务器真实用量估算，自动触发场景的 API 消耗预计下降约 30%

---

## [2026-07-29] Agent 连续追问：在已完成分析上继续提问

深度分析的结果此前是一次性的——用户想就同一份结论追问、换范围或纠正口径，只能重新发起一次独立提问，系统不会把上下文带过来。本次新增「连续追问」能力：用户可在已完成的分析结果上直接继续提问，系统建立父子血统关系、继承上下文、自动识别追问意图，并把追问结果正确标注展示。覆盖显式追问、隐式相邻追问、结果转待办三条主路径。

### 改动
1. **显式追问闭环**：分析卡片与详情页新增「继续追问」入口；点击后输入框出现锚定条，下一条消息作为子任务承接父分析，结果卡标注追问关系与变化标签（如「已按新范围重新分析」）。
2. **血统（lineage）模型**：每条追问冻结不可变的血统记录，含 root/parent 身份、追问关系、链深度（上限 20、防环）。旧数据无血统字段按独立 root 兼容，不阻塞迁移。
3. **上下文继承基础设施**：父分析的关键事实（结论摘要、证据引用、时间范围）冻结为快照；注入前由 Context Guard 复用 V2 Verifier 的 orphan/archived 语义校验证据有效性，Compiler 用隔离数据块包裹（防 prompt injection）。
4. **追问意图路由**：新增确定性 Follow-up Router，按问法自动判定 8 类关系（解释/深挖/纠正/换范围/跨域/转待办/新话题/模糊），不调用模型；其中「转待办」直接把建议转成待办确认卡，无需重新分析。
5. **自然相邻追问**：用户不点按钮直接接着问（如「那步数呢」），系统自动识别为对最近分析的追问；带三层安全护栏——确定性规则、4 小时确认窗口、误判后 circuit breaker 自动关闭。
6. **多 store 事务一致性**：saveProgress 新增事务闸门与 journal，崩溃后 Reconciler 启动时精确前滚恢复；cleanup 尊重三开关；InputSnapshot hash 升级 v2（纳入血统标识），旧 checkpoint 按版本平滑重算不误判输入变化。

### 验证
- 血统核心契约独立编译测试通过：9 项（root 传播、深度递增、上限、防环、未知关系容错、Codable round-trip、快照构造）
- Context Compiler/Guard 独立编译测试通过：8 项（授权/拒绝/缺失、claim 过滤、隔离标记、空快照）
- Follow-up Router 独立编译测试通过：13 项（8 类关系规则匹配与边界）
- 结果有效性枚举逻辑测试通过
- Holo Debug + Release（iPhone 17 Pro Simulator）全工程构建成功
- HoloTests target 受项目既有多个 `@main` 配置阻塞，核心逻辑由独立编译验证

### 性质
- 纯 App 端改动，随下次 App 发版生效，不涉及后端发版
- 血统等字段仅本地持久化，不含跨设备 CloudKit 同步

---

## [2026-07-29] Agent 接入个人档案与分层记忆，并公开本次读取来源

此前 Holo 已经保存个人档案、近期观察与长期记忆，但深度 Agent 是否读取取决于多层灰度、问题路由和临时上下文，用户在真实回答中很难确认这些信息是否真正参与。本次把档案与记忆统一接入 Agent 执行链，并在结果卡中直接展示实际读取来源。

### 改动
1. **主动加载个人档案**：状态判断、规划、诊断和建议类问题会在首次推理前读取个人档案、当前关注和偏好边界；精确明细查询不强行注入，避免个性化上下文干扰数据答案。
2. **统一分层记忆语义**：Agent 按「近期观察 / 当前阶段 / 长期规律 / 长期事实」使用统一记忆查询服务；原“短期记忆”结构明确为一次性即时上下文快照，不再形成平行记忆源。
3. **结果可解释**：分析卡片和详情页新增「本次读取」，持久展示个人档案及各层记忆的实际读取数量；不展示记忆正文，兼顾可感知性与隐私。
4. **收敛内部灰度规则**：Debug 和内部诊断构建默认进入内部召回范围，正式版继续受控；用户设置中的「让记忆辅助回答」仍是授权真相源，关闭时不会绕过。
5. **避免上下文污染结论**：档案与记忆只参与个性化推理，不进入数据覆盖率、空结果原因、规律挖掘或兜底答案的统计，避免把“档案字段数”误当成用户数据结论。

### 验证
- Agent 运行时定向测试通过：覆盖状态类问题预加载、精确明细查询不预加载、来源持久化
- 记忆策略定向测试通过：38 个断言，覆盖内部/正式版灰度与总开关
- 结果展示定向测试通过：覆盖档案和分层记忆来源文案
- Holo Debug 通用 iOS Simulator 全工程构建成功
- 完整 XCTest target 仍受项目既有多个 `@main` 与测试成员配置阻塞，本次不将其表述为全量测试通过
...
## [2026-07-29] 聊天滚动警告收敛、想法卡片操作菜单、日历事件跳原模块

三项独立的 App 端体验改进，均为前端改动，需 App 发版生效，不涉及后端。

### 改动
1. **聊天滚动警告收敛**（`ChatScrollController`）：聊天页滚动时控制台偶发「Publishing changes from within view updates is not allowed」警告。根因是滚动状态（viewport）的发布散落在多条路径（滚动、缩放、手势、定位、attach），其中部分会落在 SwiftUI 更新界面的周期内同步发布，触发警告。改为把发布统一收敛到 `updateViewport` 一处、统一跳一帧异步执行，从根上消除，不再依赖各调用点各自记得异步化。

2. **想法卡片「更多操作」上线**（`ThoughtCardView` / `ThoughtListView`）：想法列表卡片右下角的「⋯」按钮此前是个空壳，点击无反应。本次接入真正的操作菜单：**移入主题 / 归档 / 删除**，并接通列表页已有的主题选择、归档、删除能力。仅当至少有一个可用操作时才展示按钮，避免空菜单。

3. **日历事件「在原模块打开」**（`CalendarEventDetailSheet`）：在记忆画廊日历里点开一条事件后，新增「在 XX 模块打开」按钮，可一键跳回该事件来源模块的详情页（如待办详情、想法详情）。此前因画廊弹层遮住无法直接路由而遗留为 TODO，本次通过「先关掉画廊再触发跳转」绕开，并先接入待办、想法两类（路由已就绪），其余模块后续扩展。

### 性质
- 三项均为 App 端改动，需 App 发版生效，不涉及后端发版
- 第 3 项通过 originID 回查实体再跳转，回查放在后台 context，不阻塞 UI

---

## [2026-07-29] 修复趋势分析结论全废与习惯数据噪音，提升 Agent 回复人味儿

用户点击「最近分析」分析趋势时，要么提示「没有形成可信结论」，要么数据明显错误（如「每天健身」实际只打卡一次）。经排查为数据校验层与数据源层的两个独立根因，同时顺带提升了 Agent 回复的人性化程度。

### 根因
1. **趋势分析结论被全部判废**：趋势分析走 `dynamic_query` 路径，产出的数据标签是 `dynamic.health.steps.avg.all` 这种动态格式，但 AI 写结论用的标签是固定名 `health.steps.average`。核验器要求两者逐字相等，永远对不上 → 每条结论都被判 critical 拒绝 → 「没有可信结论」。
2. **习惯数据噪音导致 AI 胡说**：习惯工具把近 14 天全部日期（含没打卡的 count=0 的天）都生成了证据（如「健身 完成 0 次」）。AI 收到 14 条同名证据，误以为用户频繁打卡，写出「每天坚持健身」。实际只有 1 次打卡。
3. **数字容差过严**：核验器要求结论数字与数据数字误差不超过 0.01，但数据是精确值（7999.8 步），AI 习惯取整（8000 步），差 0.2 就被废。

### 修复
1. **核验器 metricKey 判断**（V1+V2）：从「逐字相等」改为「数据域重叠」——提取标签的核心 token，≥2 个重叠即视为同域（如 `health.steps.average` 与 `dynamic.health.steps.avg.all` 同属步数域）。跨域误引（如 `finance.total` 引 `health.steps`）仍被拦截。9 种场景验证通过。
2. **习惯证据过滤**（`HoloHabitTool`）：正向/负向习惯事件只为 count > 0 的天生成证据，0 值天不产生噪音。完成率指标不受影响（仍用完整 14 天数组计算）。
3. **数字分档容差**：比例类（|值|≤2，如百分比、相关系数）严容差 0.05；其余取绝对 0.5 和相对 1% 的较大者，容忍 AI 对步数/睡眠/支出的合理取整，同时拒绝明显错误。
4. **英文泄漏修复**：6 处数据工具的 excerpt 用英文 rawValue（steps/sleep/stand/learning/happy 等）改为中文映射；展示层加裸英文工具词检测兜底。
5. **人味儿提升**：渲染层反转优先级，LLM 自然语言清洗后优先于模板事实句；合成器从「覆盖」改为「分工」保留 LLM 叙事；兜底文案简化；卡片收尾跟着具体建议走。

### agentLoop v17（前后端）
6. 后端 agentLoop prompt 加 `title`/`narrativeSummary` 顶层字段，让 LLM 产出有人味儿的标题和摘要（已部署生产，版本 16→17）。
7. 前端全链路贯穿：解析 → `HoloAgentResult` → `HoloRenderedAgentResult` → 详情页 `openingTitle`/`openingBody` 优先消费 LLM 标题摘要。

### 性质
- 后端 v17 已部署生产（commit `b971568`），立即生效
- App 端改动（核验器、习惯工具、渲染层、英文修复、UI）随本次提交发布，需 App 发版
- 核验器修复同时影响 V1（简单查询）和 V2（复杂分析）路径

### 待办（数据可信度，下一步）
- 数据一致性自检：AI 出结论前，系统自动核对结论描述与原始数据是否一致，矛盾时拦截或降级
- 透明展示原始数据：每条结论旁边明确展示基于哪些原始数据，可点击核对

---

## [2026-07-28] 修复周期回放历史摘要回填刷屏报错与额度争抢

启动 App 或切回前台时控制台刷屏 `回填单期失败：回放摘要返回格式不正确` 和 `今天的 AI 使用次数已达上限`。这是一个本应在后台静默跑完的"老用户历史摘要归纳"任务退化成了每次启动都空转、还挤占了用户正常聊天的 AI 额度。

### 根因
三个问题叠加：
1. **格式不正确**：后端 `replayDigest` 路由 `maxTokens=1024`，而它用的是推理模型（deepseek-v4-flash，带思维链）。模型把全部 1024 token 花在 `reasoning_content` 上，还没开始写正式 `content` 就因 `finish_reason=length` 被截断 → App 拿到空字符串 → 报"格式不正确"。生产数据库近 8 条调用里 7 条命中此症状。
2. **额度达上限**：`replayDigest` 没有独立配额，与主聊天共用全局 50/天。历史回填每次启动/回前台都跑，迅速把聊天额度耗光。叠加后端"日切"按 UTC（北京 08:00）算，导致凌晨也显示"已达上限"。
3. **刷屏重试**：回填撞 429 后不止损，下次启动从头再扫一遍、再撞一遍，日志无限循环。

### 修复
1. **后端·replayDigest maxTokens 1024 → 4096**（`HoloBackend/src/config.js`）：与 insight 等同类复杂归纳任务对齐，模型有足够预算写完 content。根治"格式不正确"。
2. **后端·replayDigest 独立配额桶**（`HoloBackend/src/config.js`）：新增 `requestLimits: { perMinute: 10, perDay: 30 }`，不再与聊天抢那 50/天。
3. **App·回填 429 冷却**（`HoloReplayDigestService.swift`）：撞 `rateLimited` 整批立即 break，并写入 25h 冷却（持久化到 `HoloReplayDigest.json`，重启不丢）。冷却期内启动/回前台直接跳过扫描，不再刷屏。

### 性质
- 后端两项（maxTokens、独立配额）已部署生产，立即生效，无需用户更新 App
- App 冷却机制随下次 App 发版生效；但有了根因修复，格式错不再发生，冷却更多是兜底

---

## [2026-07-28] 周期回放接入 HoloAI 对话并补齐可恢复生成与阅读体验

周期回放现在从 HoloAI 对话直接发起、展示和恢复。用户不再只看到标题与摘要；生成完成后会得到完整卡片，退出重进、网络波动或系统暂停后也能在原消息上继续。

### 生成与恢复
- 每份回放持久化独立任务状态，覆盖生成中、等待网络、等待前台恢复、失败和完成；同一周期的未完成任务会复用原消息，避免重复卡片
- 流式连接收到部分内容后若中断，不再把半截 JSON 与重试结果拼接；网络恢复、回到前台或冷启动后从安全位置继续
- 用户主动生成优先于自动补齐；历史累计摘要改为单通道排队和断点回填，避免后台任务抢占当前回放
- 生成失败保留原请求，并提供“继续生成”入口

### 展示与交互
- 聊天消息持久化完整回放结构，首次生成、退出重进和加载历史消息都渲染同一张周期回放卡片
- 默认展示标题、摘要和洞察目录，展开后显示正文、证据和追问建议
- 图标改为与标题文字垂直居中，增加段落行距和卡片间留白；异常图标降低警报感，长内容更易扫读
- 清理 `warning`、误拼写 `waring`、`critical` 等内部严重级别，不再向用户暴露机器状态词
- 展开/收起改为标准按钮并取消高度动画；监听消息列表真实内容高度，在收起后直接校正越界滚动位置，修复内容区偶发整屏空白、必须手动滑动才恢复的问题

### 验证
- 内部严重级别清洗定向样例通过
- `git diff --check` 通过
- 通用 iOS Debug 全工程构建成功
- 签名 Simulator Debug 全工程构建成功；模拟器未稳定复现真机偶发空白，保留为真机回归项

### 性质
- 客户端与回放展示改动随本次提交发布
- 回放生成 Prompt 已通过独立提交 `3acce6b` 部署生产；该提交仍需同步推送到 GitHub `main`
...
### 根因与修复
- **主因｜转场中途插入视图**：预算数据在 `.task` 中写入 `@State`，而 `.task` 在首帧渲染之后才执行，预算卡是在入场转场动画进行中被插入视图树的——SwiftUI 对祖先转场期间新插入的视图不跟随共享动画，直接满透明度弹出（录屏实锤：收支卡半透明滑动中，预算卡已满透明度钉在最终位置）。修复：预算数据改为在 `FinanceLedgerView.init` 中**同步计算作为 `@State` 初始值**，预算卡从第一帧就在视图树里，随整页一起滑入淡入
- **内容分批到达**：`.task` 内多个 `@Published` 逐帧更新，各区域分批弹入。修复：`isInitialContentReady` 门控——预算 + 日历数据全部就绪前内容区只占位不可见（底板/日历正常参与转场），就绪后一次性淡入
- **冷启动竞态**：Core Data store 后台异步加载，未就绪时同步 fetch 静默返回空，数据后补导致内容分批/缺失出现。修复：`FinanceLedgerView` / `TaskListView` / `ThoughtListView` / `DailyKanbanView` 首次加载前统一 `await CoreDataStack.shared.waitUntilReady()`；`onAppear` 同步加载加 `isReady` 守卫
- **空态闪现**：习惯/任务/想法三个列表页入场时先闪现空态再被列表替换。修复：`hasLoadedOnce` 门控，首次加载完成前不渲染空态

### 验证
- 模拟器录屏抽帧对比：修复前预算卡提前满透明度弹出；修复后与收支卡以相同透明度一起淡入，无 pop-in
- Holo Simulator Debug 完整构建通过
...
### 卡片视觉统一
- **新增共享 `.holoCard()` 修饰符**（`DesignSystem.swift`）：卡片底色 + 16pt 圆角 + 0.5pt 半透明描边 + 轻投影，以明细趋势图卡为基准沉淀为统一标准，深色模式自动适配
- **财务总览**：周期汇总卡 / 柱状图卡 / TOP 排行卡统一接入；汇总卡金额改 17pt rounded 数字字体并支持自动缩放，分隔线减淡与描边呼应
- **类别页**：饼图与分类图例列表装进卡片容器，页面节奏与总览一致
- **习惯统计**：可展开统计卡、习惯统计卡容器统一接入 `.holoCard()`

### 图表手势对齐「明细」
- **问题**：总览柱状图用 `DragGesture(minimumDistance: 0)`，触摸一落下即被图表认领，手指按在卡片上无法纵向滚动页面；饼图旧覆盖层手势同时识别，横滑时页面抖动
- **改造**：明细趋势图的方向手势方案抽为共享组件 `DirectionalChartGestureOverlay`（UIKit pan 在识别开始前按速度判方向，横向图表独占、纵向/模糊方向交还父级 ScrollView；新增 `onTap` 可选回调区分点按与拖动结束）
- **柱状图**：横向拖动查看收支/余额 tooltip（逻辑不变），纵向拖动直接滚动页面
- **饼图**：横向拖动高亮扇区、点按选中、纵向滚动页面，iPad 指针悬停保留；删除旧 `PieChartTouchOverlay`

### 习惯打卡规律图重设计
- **热力图**：去掉固定高度改正方形小格（GitHub 风格），「今天待打卡」改为淡橙底 + 品牌橙描边环，底部新增「已完成/未完成/今天」图例
- **月历 / 周条**：移除右下角 checkmark/xmark 小图标，改为颜色填充语义——完成日实心填充习惯色 + 白色日期，今天描边环，超限红底

### 验证
- Holo Simulator Debug 完整构建通过（两次改动分别验证）
...
## [2026-07-25] 修复对话页「加载更早消息」不跟手

上滑加载更多历史消息时，加载出来的内容会自动把视图向前滚动到最新消息，而不是停在用户当时看的那条，无法顺着往上翻。对齐微信/iMessage 体验。

### 修复
- **主因｜流式输出无条件抢滚动**：`onChange(streamingText)` 一旦流式文本变化就 `scrollToBottom`，加载历史时若 AI 正在回复会把锚点抢走。新增 `isApplyingEarlierSession` 标志（覆盖「加载 + 视图刷新」全窗口），期间 `onChange(streamingText / isStreaming / messages)` 一律不触发自动滚到底
- **时序竞争｜锚点 scrollTo 跑在数据刷新前**：仓库 prepend 后 `messages` 经 `receive(on: .main)` 异步刷新，原实现紧跟 await 直接 scrollTo，LazyVStack 还没插入新行，定位不准。改为把锚点存入 `pendingEarlierSessionAnchor`，由 `onChange(messages)` 等数据真正刷新后再 `scrollTo(.top)`
- **锚点语义｜改为钉死原位置**：仓库返回值从「新批次最后一条」改为「加载前首条」（用户当时看的那条），加载后视图顶部对齐这条，新内容出现在屏幕上方屏幕外
- `isApplyingEarlierSession` 的复位延迟 0.35s，确保覆盖 prepend → 插入渲染 → 锚点 scrollTo 全窗口（`isLoadingEarlierSession` 的 defer 复位过早，挡不住视图刷新那一拍）
...
## [2026-07-25] Agent 深度分析：修复「2026年体重趋势」查不到数据

问「2026年体重趋势是怎样的」时，Agent 明明有体重记录却回答「数量 0 条 / 没能形成可信结论」。彻查后定位到四层叠加原因并全部修复。

### 修复
- **裸年份时间解析缺失（根因）**：时间语义解析器只认「今年/去年/2026年7月」，不认裸年份「2026年」→ 问题时间窗为 nil → 所有数据源回退到「近 14 天」，更早的体重记录完全不可见。新增 `explicitYear` 解析：「2026年」→ 2026-01-01 ~ 2027-01-01，label「2026年」；不影响「2026年7月」「今年」既有行为
- **域识别没有「体重」**：「体重/体脂」不在任何域关键词里，问题路由不到习惯工具。已将「体重、体脂」加入 habit 域
- **habit.daily 行日期整体错位一天**：`dayOffset` 锚点是数据源参考日（时间窗 end），还原日期时却用「end 减一天」，导致每条记录早一天、窗口边界记录被过滤。现统一用同一锚点还原，两种场景（显式窗口 / 默认近 14 天）日期往返均验证正确
- **测量型习惯被错误累加**：体重等 LATEST 聚合习惯一天多条记录会被求和（两次称重变 140kg）。现按习惯自身的聚合类型取「当日最后一条记录值」；数值型习惯单位（kg）随记录透传，证据文案从「体重 70 次」修正为「体重 70 kg」

### 验证
- 新增 standalone 测试：裸年份解析全年窗口、label、起止边界；「2026年7月」「今年」回归
- 既有 standalone 套件全部通过：TimeSemanticResolver / PlanAndSemanticFrame / HabitTool / MetricSemantic / FixedToolSemantic / DeterministicAnswerComposer / AnswerCrossDomain
- 日期还原数学用独立脚本按两种场景逐项验证（修复前恒早一天，修复后往返一致）
- Holo Simulator Debug 完整构建通过

### 性质
- 纯 App 端改动，不涉及后端接口 / Prompt / 数据契约，**无需后端发版**
...
### 修复
- **趋势图绘图区留白**：X/Y 轴增加独立 plot padding，折线、零基线与选中点不再贴住卡片边缘；Y 轴改用 1/2/5 的常规刻度上取整，避免 5000 等峰值被错误扩到 1 万
- **视觉层级收敛**：弱化面积渐变、网格、卡片描边与阴影，保留清晰趋势线和选中态，提升紧凑空间里的数据辨识度
- **拖动实时联动**：趋势图在 pan 的 began/changed 阶段持续换算日期，下方明细随手指移动实时滚到日期锚点，不再等松手后才更新
- **保留全量明细**：选择趋势日期只改变列表可见位置，不再过滤成单日数据；标题和笔数始终对应当前范围内的完整交易
- **横纵手势互斥**：横向意图确认后图表独占、页面位置固定；纵向或方向模糊时图表立即让出，由页面滚动接管，趋势图仍可正常随整页划走
- **排序冲突保护**：金额排序状态下从趋势图按日期导航，会自动回到时间降序分组，确保日期锚点稳定且含义明确

### 验证
- 财务交互 standalone 测试 **15 条断言通过**：横/纵/模糊方向仲裁、禁止同时识别、日期导航排序回退与四种排序稳定性
- Holo Simulator Debug 完整构建通过；使用项目内置虚构财务数据实测全量 17 笔交易仍保留、横滑联动日期、纵滑可划走趋势图
- `git diff --check` 通过
...
### 新增
- **合计卡片变期次选择器**：「本期合计/对比期合计」点击直接切换下方明细列表（选中态 holoPrimary 边框高亮），替代原来的"两期明细上下堆叠"；原跳统计页功能收拢为汇总卡右上角「统计分析 ›」入口，时间段跟随选中期次
- **科目对比模式**：新增「明细 | 科目对比」分段切换（仅有对比期时显示），按一级科目对齐两期支出，三列展示本期/对比期/差额，差额按支出语义配色（多支出红、少支出绿），行内可展开二级科目；仅一期有的科目标「新增」，未分类自动归组
- **对比排序切换**：科目对比支持三种排序——按差额（默认，|差额| 降序）、金额从高到低、金额从低到高，一级与展开的二级科目同步生效
- **本地聚合零查询**：`CategoryComparisonBuilder` 纯函数聚合页面内存中的两期交易，不新增 Core Data 查询，天然尊重证据页关键词过滤

### 测试
- `CategoryComparisonBuilderTests`（standalone 桥接模式）40 断言通过：共有科目差额、仅本期/仅对比期、二级归父级、未分类归组、差额/金额三种排序、空输入
...
### 修复
- **取消独立解析重试上限**：解析故障统一使用任务的 LLM 轮数预算，不再固定重试 2 次后提前结束；只有真正触达用户设置的轮数或运行时限制才停止。
- **响应故障分层恢复**：坏 Agent JSON、SSE 非法帧、SSE 未完整结束、空响应和截断响应都进入受控恢复；模型暂不可用则进入可恢复等待态，不把任务标记为失败。
- **跨重启保持进度**：结构纠错次数、纠错上下文和待请求 step 会落盘；冷启动后复用未完成 step，成功后清理临时恢复状态，避免同一坏响应被幂等缓存反复重放。
- **解析兼容加固**：兼容模型把 `dynamicPlan` / `crossDomainPlan` 错放进 `parameters`、数值字段输出为字符串等常见协议漂移，并拒绝让字符串日期覆盖端侧确定性时间窗口。
- **响应契约 v14**：前后端后备 Prompt 明确 `final_claims`、`metricAssertions`、证据 ID 和动态计划层级；后端遇到无法解析的 Agent 输出时返回受控恢复信封，不再直接以 502 终止客户端任务。
- **避免重复放大请求**：响应结构和流完整性错误不在 HTTP 层原样盲重放，由 Agent Runtime 使用新 step 和纠错上下文恢复；真正的瞬时上游故障仍保留网络重试。

### 验证
- 后端完整测试 155/155 通过；iOS Response Parser 与 Local Agent Runtime standalone 回归通过，Holo Simulator Debug 全工程构建通过。
- 生产 Prompt v14、坏 JSON 恢复信封、step 幂等命中和最终 claims 契约均已活体验证。
- 原始问题「这个月消费比上个月多在哪儿」在生产返回 HTTP 200，并生成按分类分组、环比计算和排序的动态查询计划。
- 后端改动已部署生产并完成公网验收。
...
## [2026-07-23] 固定支出升为底部独立 Tab

订阅、会员等周期性支出与耐用品一次性购买的管理，从「统计 → 总览 → 深层 sheet」里搬出来，成为财务模块底部第四个平等 Tab，并新增全局悬浮「记一笔」入口。此前这个功能藏在四层入口之下，且与「统计分析」这一事后复盘页在概念上错配——它本质是管理未来的承诺清单，不是分析已发生的交易。

### 变更
- **新增「固定支出」Tab**：财务底部导航从「统计 / 账本 / 设置」三栏扩展为「账本 / 统计 / 固定支出 / 设置」四栏，图标用循环符号 `repeat`。原寄居在统计总览里的长期成本入口卡片移除，统计页回归纯粹的收支分析。
- **全局悬浮「记一笔」**：原中间「账本 / ＋」复用按钮拆分——4 个 Tab 统一渲染，记一笔改为始终悬浮在右下角的圆形按钮（FAB），在统计、固定支出、设置页也能直接记账，不再需要先切回账本。
- **代码独立成文件**：长期成本的 6 个视图（主列表、详情、结束条件、一次性编辑、添加、分类/账户菜单）从 `OverviewTabView.swift` 抽到独立的 `SpendingProjectsView.swift`，与其作为一等 Tab 的定位匹配。工程使用 Xcode 16 文件系统同步分组，新文件自动纳入编译，无需改动 pbxproj。

### 性质
- 纯 UI 重组 + 文件搬家，未触碰 `SpendingProjectRepository`、CoreData 模型、自动生成流水与暂停/恢复等任何数据层和逻辑层。
- 「固定支出」Tab 右上角的「＋」是添加固定支出项目（订阅/耐用品），与悬浮 FAB 的「记一笔」刻意区分，避免混淆。

### 验证
- Holo iOS Simulator Debug 全工程编译通过（`BUILD SUCCEEDED`）。

## [2026-07-22] 修复对比类问题返回「没有形成可信结论」
...
### 修复
- **后端流式拉取**：agent_loop 非流式请求改为内部用流式（SSE）从上游模型拉取——边生成边传，连接始终保持活跃，不再触发 30 秒空闲墙切断。流式超时上限为 120 秒（原非流式 60 秒）。收集完整响应后拼成与非流式相同的结构返回，客户端零改动。

### 验证
- 部署后直接测试 agent_loop 对比类请求：HTTP 200，17.5 秒完成，completion_tokens 1642（含 1294 reasoning tokens），完整收集无超时。
- 部署前同类请求 72% 失败率（30 秒超时）；修复后日志零 AbortError、零 500。
## [2026-07-22] 修复对比类问题返回「没有形成可信结论」

问 AI「这个月比上个月消费多在哪」时，即使本月、上月都有真实数据，也会返回「没有形成可信结论」。根因是对比期（baseline）时间窗从问题解析到工具参数的整条传递链路都断了——baseline 被错误回退成「前 14 天」而非「上月日历窗口」，且 Claim 校验器浮点严格相等误杀了合法结论。

### 修复
- **对比时间窗解析**：时间语义解析器新增双窗对比解析，按「本月/上月」「本周/上周」「今年/去年」词法配对，一次性解析出本期 + 对比期两个日历窗口，不再只取最早的一个词而丢弃另一个。
- **对比期管道打通**：`HoloAgentJob` 新增 baseline 字段，对比期窗口从 job 一路注入到工具请求和动态查询计划（`dynamicPlan.baseline`），数据源终于能收到正确的「上月」窗口。旧数据 Codable 自动兼容。
- **Claim 校验器容差**：指标数值比对从严格相等改为浮点容差（差异 > 0.01 才拒绝），不再因四舍五入差异误杀合法结论。
- **对比查询引导**：Agent Loop prompt 补充对比型动态查询规则与示例（按分类分组求环比、排序定位涨幅最大类），引导 LLM 主动用 `dynamic_query` 处理对比问题。

### 性质
- 这是系统性修复而非打补丁——打通的是所有对比类问题（月/周/年环比、同比、vs）的公共基础设施，不依赖枚举问法。动态查询引擎原有的分组求环比能力（`groupBy` + `percentageChange` + `sort`）此前因 baseline 窗口缺失而无法生效，现已连通。

### 验证
- 新增 8 项时间解析器对比测试（月/周/年配对、词序颠倒、非对比回退、窗口边界、label 正确性），全部通过。
- Claim 校验器既有测试通过；`HoloAgentJob` Codable round-trip 测试通过（新字段不破坏序列化）。
- 后端 `defaultPrompts.json` 对比引导已同步部署到线上（容器内验证），134 项后端测试通过。

## [2026-07-22] 修复模拟器测试反复崩溃

启动习惯、想法收敛与健康洞察测试时，不再因为主线程对象释放过程异常而导致 Holo 测试宿主反复崩溃、模拟器持续刷错。

### 修复
- **仓库析构安全化**：习惯数据操作继续由主线程保护，但无清理任务的析构过程不再切换主执行器，避开 XCTest 宿主中的 Swift 并发兼容析构重复释放。
- **收敛拒绝仓库加固**：明确主线程隔离并安全处理无清理任务的析构；测试 Fixture 持有完整内存数据库容器，避免上下文提前失效。
- **健康缓存测试隔离**：测试期间保留缓存实例到测试进程退出，避开 iOS 26.3 hosted XCTest 释放 MainActor 对象时的系统兼容问题。
- **测试正式纳入 target**：将 `HabitRepositoryCalendarTests` 登记到 `HoloTests`，避免构建显示成功但实际执行 0 条测试。

### 验证
- iPhone 17 Pro / iOS 26.3 模拟器真实执行习惯日历测试 12/12 通过，原崩溃用例通过且测试宿主未再次退出。
- 想法收敛拒绝仓库 12/12、健康洞察缓存 7/7 通过；与日历回归合并验证共执行 31 项，0 失败。
- Holo Debug 构建安装并正常启动，启动后进程持续存活。

## [2026-07-22] 修复后台记忆观察偶发闪退

Holo 在后台自动整理习惯等领域记忆时，不再因为多条信号共享同一份有效证据而毫无征兆退出；重新打开 App 后也不会因同一任务重试而再次触发相同闪退。

### 修复
- **允许合法证据复用**：同一份结构化证据可以同时支持快照、趋势等多条信号，不再使用要求 key 绝对唯一的字典初始化方式。
- **冲突证据安全关闭**：相同 evidence ID 如果对应不同内容，会被校验器明确拒绝，不静默覆盖，也不终止 App。
- **同类入口一并加固**：既有记忆索引采用相同的无损冲突检测，避免异常重复 ID 形成另一处运行时崩溃入口。

### 验证
- 真机两份崩溃报告均确认原因为 `Dictionary(uniqueKeysWithValues:)` 重复 evidence ID 触发 `EXC_BREAKPOINT / SIGTRAP`。
- 新增合法复用与冲突拒绝回归测试，领域记忆安全测试 21/21 通过。
- Holo iOS Simulator Debug 全工程编译通过（`BUILD SUCCEEDED`），scoped diff 检查通过。

## [2026-07-22] 记一笔固定显示金额与名称

记账时向下浏览和选择科目，不再把已经填写的金额与名称滚出视野，减少选完科目后需要回头确认输入内容的割裂感。

### 变更
- **金额与名称固定在顶部**：输入区从滚动内容中移出，位于收支类型下方，选择科目、账户和日期时始终可见。
- **其余表单独立滚动**：科目网格、补充信息和编辑态删除入口继续正常滚动，原有数字键盘、快捷标签与下拉保存行为不变。

### 验证
- `AddTransactionSheet.swift` Swift 语法解析通过，变更 diff 检查通过。
- Holo iOS Simulator Debug 全工程编译通过（`BUILD SUCCEEDED`）。
