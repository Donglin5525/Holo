...
## [2026-09-26] 个人页 Plus 卡删「已生效」角标：不再压住 Holo Plus 标题

> 纯 iOS 无发版，随 1.0.9 入库；待东林真机验收。

**背景**：东林实报个人页 Plus 卡右上角「已生效」胶囊直接叠在 "Holo Plus" 大标题尾字母上。原因：角标挂在卡片右上角（ZStack topTrailing），而标题是固定单行大字、一直顶到卡片右缘，两个元素空间重叠，系统大字号档位下更甚。

**处理**：整体删除角标、不挪位——「已开通 Plus」的信息由副标题「更高额度已为你开启」完整承载，胶囊属重复表达；免费态本无角标、零变化。会员中心页同主题徽章（badgeBg/badgeText 主题色）另有用途，保留不动。

**验证**：Debug 模拟器全量编译绿（纯删除式改动，删除的 Text 不再入视图树即不可能渲染）。

---

## [2026-09-25] AI 记账分类根治：云端直选科目 + 匹配链容错（酸汤肥牛落待分类）

> 后端已发版（v38 提示词，部署提交 f55414923）对存量 1.0.8 立即生效；iOS 容错随 1.0.9 入库待真机验收。背景：东林实报「酸汤肥牛 234」落待分类（22:13 记录），四层排查（生产日志 AI 回传完好 / 1.0.8 支出路径与主干零差异 / 复现单测绿 / 分类管理页齐全）后断点钉死在本地匹配阶段——根因是账本分类数据的父子挂接断裂（CloudKit 历史遗留孤儿/重复形态）叠加匹配链两处「按数据库连线硬查」的结构弱点，导致「名字明明对得上却全链失配」。东林拍板根治方向：AI 直接判到二级、拒绝挨个菜名打补丁、21 点归夜宵口径不动。

### 两层根治

1. **云端 AI 直选科目（后端 v38，已发版生效）**：记账意图必填 primaryCategory+subCategory，从标准收支科目表（餐饮 11 子类/交通 14/购物 8/娱乐 8/居住 11/医疗 7/学习 8/人情 7/其他 15 + 收入四类）里直选，禁止编表外名；餐饮按当前时间选餐次（05-10 早/10-16 午/16-21 晚/21 后夜宵），用户原话点名科目时优先按点名。菜名（酸汤肥牛/日式拉面/韩国拌饭）从此不依赖本地词表猜测。生产端到端实测回传「餐饮 > 夜宵」✓。提示词长度护栏随结构扩容 6750→8000（重划红线惯例入注释）。
2. **本地匹配链容错（iOS，随 1.0.9）**：`primaryMatches` 对孤儿子类（parentId 悬空）不再误当一级分类比较（原逻辑必判失败），改信名字放行；分类解析三处子类查找（学习映射/严格科目/语义兜底）在 parentId 失配时退化为按名查找——「实体在、名字对」即落位，库连线损伤不再静默掉「待分类」。`matchCategory` 拆出 categories 注入版供单测构造异常库形态。

### 验证

- 后端 470 测试全绿（含意图注册表两端对拍）；iOS 新增 MatchCategoryResilienceTests 四场景（孤儿/一级重复/AI 直选+孤儿/健康库回归）+ 酸汤肥牛复现测试（逐字段复刻生产回传+确认卡污染字段加强版）+ 识图记账 ReceiptBookingKernelTests 36 项回归，全绿。
- 生产已验证：提示词 v38 落库、端到端 curl 实测「酸汤肥牛 234」→ AI 直选「餐饮 > 夜宵」。

### 验收（东林手机 1.0.8，30 秒）

再记一笔「酸汤肥牛 234」：落「餐饮·夜宵」= 账本数据健康、全链闭环；仍落「待分类」= 账本连线损伤实锤（后端答案已给对、1.0.8 旧校验拦下），1.0.9 容错上线后兜住，届时复测同句应落「餐饮·夜宵」。

---

## [2026-09-25] 动画二期·场景动效六项：三环扫开/切天滑入/拍立得落定等

> 纯 iOS 无发版，随 1.0.9 入库待真机验收。动画一期四批交付后东林圈定八个场景动效候选全部通过，本轮落地六个，三个因目标文件正被并行会话在途改动而避让（记账表单金额过渡、想法编辑器工具栏按压、账本切天滑入——健康页同款已先行落地，其入库后补账本版）。

### 六项内容

1. **健康三环出场扫开**：进入健康页三环从 0 依次扫到当日进度（外环先动、错峰 0.08s、easeOut 0.8s），Apple 活动手表同款语言；中心身体分数加数字滚动。
2. **健康页切天方向性滑入**：左右滑/箭头切日期，内容按方向滑入（切明天从右缘进、昨天从左缘进，旧内容淡出），箭头与滑动两条路径统一走 switchDay；滚动位置随切天重置回顶部。
3. **底部导航选中动效**：个人/记忆长廊选中图标轻弹（symbolEffect bounce），中央 AI 按钮小黄点出现带 snappy 缩放。
4. **拍立得卡出场落定**：记忆长廊日回放的照片卡像「拍」上册页——略小、略斜、透明，弹簧回正到常态。
5. **AI 规划运行卡呼吸**：运行中的 sparkles 图标 1.4s 轻呼吸（暂态卡与转圈指示器同生命周期，完成即止）。
6. **纪念日 Hero 天数滚动**：大卡 54pt 天数数字变化用 numericText 平滑滚动。

记账数字键盘的按压动效（缩放+品牌色叠加）经核实已存在，未重复实现。

### 验证

- 全量编译绿；模拟器 idb 走查：健康页三环渲染、切天到 9月24日 数据正确切换、纪念日页、长廊日档拍立得渲染，截图判读通过（切天「前进」方向在今天是业务禁用属正确行为）。

### 验收清单（真机 30 秒）

进健康页看三环扫开+身体分数滚动；健康页左右滑切天看内容按方向滑入；首页底部导航切「记忆长廊」看图标轻弹+AI 黄点出现；长廊日回放看拍立得照片卡落定；发起一次个性化规划看 sparkles 呼吸（可选）。

---

## [2026-09-25] 全 App 动画体系一期落地：修硬切+参数收敛+微动效+高级过渡四批交付

> 纯 iOS 无发版，随 1.0.9 入库待真机验收。东林发起「让 App 更精致」的动画规划，拍板三原则：克制精致风（跟 iOS 系统手感，不做大幅度弹跳）、分批交付每批验收、存量 250 处散落动画参数顺带批量收敛。首批交付后二三四批连夜合并交付，模拟器走查 13 图判读全过。

### 四批内容

1. **修硬切**（4b699a0d3，13 文件）——最高频的「瞬间跳变」全补上过渡：iPhone 三 Tab 切换淡入淡出；想法删除/归档、交易删除行右滑淡出（交易删除改「先让行动画滑走、后台再删库，失败自动刷回」）；财务/健康/习惯 5 个 Charts 图表切日期时柱与线平滑插值（ChartDataPoint 的 id 是随机 UUID 不可作 diff，改用金额序列指纹）；饼图 Canvas 出场从 12 点方向扫开 0.45s 播完停帧（TimelineView 驱动，零常驻逐帧开销）；账户余额/净资产/月概览大号数字滚动（系统原生 numericText）。
2. **存量参数收敛**（df79b3863，70 文件 188 处）——散落的内联魔法数（15 种时长、11 种弹性参数）按容差原则（时长差≤0.05s、spring response 差≤0.05 且 damping 差≤0.08）等值映射归一到 HoloAnimation 规范表，规范表补 enter 档位（easeOut 0.2）兜住入场类；超容差的一次性手调值、长过渡、repeatForever 装饰动画、裸 withAnimation 全部原样保留——手感零变化，只统一参数来源。
3. **细节微动效**（748124316）——6 处分段选中胶囊改 matchedGeometryEffect 滑动（财务/任务/习惯顶部切换条、对话「对话|报告」双 Tab、想法「想法|主题」双模式、长廊「日|周|月|轴」四档，此技术全 App 首批落地）；习惯磁贴「逐行浮现」手感抽成共享修饰符 HoloStaggeredAppear 铺到想法列表/报告档案/纪念日列表（账本日明细有意不加：DaySwipe 高频切天时行行重播是噪音）；任务勾选、报告收藏星标、分段图标加 symbolEffect 单发 bounce。
4. **高级过渡**（同笔）——长廊四档与想法双模式切换内容按方向滑入（按档序定方向：日→周从右进、周→日从左进，旧内容纯淡出避免双向位移打架）；Toast 检查后确认已有统一进出语言未动；共享元素 zoom 试点评估后放弃（部署目标 17.0 需 iOS 18 双轨包装、报告详情多入口共用路由传 Namespace 耦合大、小卡与全屏长文形态差异大「缩略图放大」隐喻不成立），不硬塞。

### 验证

- 全量编译绿；模拟器 idb 走查（财务统计/类别饼图/想法双模式/长廊四档/对话双 Tab/任务）13 张截图只读判读 13/13 PASS。
- 自审两轮：并行在途文件零触碰（收敛脚本排除+提交严格限定文件范围）；收敛后残留分布复核全部为超容差保留项。

### 验收清单（真机 30 秒）

财务顶部「账本|账户|统计」点来点去看橙色胶囊滑动；AI 页切「报告」再切回看白胶囊滑动；想法页切「主题」看内容从右滑入；记忆长廊切 日/周/月/轴 看档位块滑动+内容按方向滑入；进想法列表看卡片逐行浮现；勾掉一个任务看勾图标弹一下。

---

## [2026-09-25] 统计总览趋势图「余额线飞天」改常规双轴：铺满全图穿柱而过

> 纯 iOS 无发版，随 1.0.9 入库待真机验收。东林实报「这是什么鬼？飞到天上去了？」——8-30 改版把柱子和余额线拼成上下两个互不接触的楼层（分层双轴），第一眼像渲染事故；拍板改回理财 App 通用语言：常规双轴同图。

**背景**：余额（万级）与单日收支（千级）数量级差十倍，同画一把尺必然一边趴地、一边出天际。8-30 版本为此把余额线折进图表上方约三分之一的独立条带、柱子只占下半区，本意「两把尺明着分开不误导」；实际效果是线悬在天上、右轴刻度挤在右上角，防误导没做到，先输给了看不懂。

### 改法
整张图内部本就画在一把 0~100 的虚拟尺子上，改三个常量完成布局重构（`TrendChartView.swift`）：柱轴上限映射 55→90（柱子近全高）、余额线映射带 62~95→8~94（按右轴铺满全绘图区，最低点贴底、从柱间穿过）。左右刻度、断口标记、网格线均按常量自动跟随：两把尺各自从下到上铺满，左轴管柱、右轴管线；相关旧设计注释同步修正。

### 验证
- 全量编译绿（主工作区被并行在途文件挡编译，按惯例 worktree 检出 HEAD + 重放己方文件隔离验证）。
- 种子数据模拟器走查「统计→总览」：截图判读五项全过（线铺满全高 / 线柱交叠穿过 / 柱子八成高以上 / 左右刻度各自拉开 / 无裁切无叠印）。

### 验收清单（真机 30 秒）
统计→总览：1. 蓝线贴绘图区上下铺满、从柱子间穿过；2. 柱子明显变高；3. 左千级 / 右万级刻度各自从下到上拉开；4. 按住某天，明细气泡跟手不遮挡。

---

## [2026-09-25] 统计分析饼图混层根治：回收站数据沙箱隔离红线设立

> 纯 iOS 无发版，随 1.0.9 入库待真机验收。东林实报「饼图把一级分类跟二级分类放一起了」，根因不在饼图代码，定性为软删机制的沙箱隔离缺口；同日拍板原则「回收站里面的东西不能影响线上的内容」并固化为质量红线 §7。

**背景**：财务统计分析的类别饼图出现一级分类与二级分类混在同一层。排查定性：饼图/统计页/聚合代码均未被改错（近期改动仅动画参数替换），真因链在数据态——9-19 分类删除治理把分类纳入软删后，回收站恢复引擎的父子联动分支表漏了分类，部分恢复留下「活二级+死一级」孤儿；聚合层的老兜底（父一级不在活缓存时让二级冒充一级，iCloud 同步上线时引入）把这些孤儿放大成饼图混层。

### 修复（三件，灭数据态而非读路径打补丁）
- **恢复联动补口**：`RecycleBinRestoreEngine.restoreLinkedParents` 补 Category→父分类分支——恢复二级分类时其软删的一级分组一并拉活，堵住新孤儿入口。
- **存量孤儿自愈**：新增 `CategoryOrphanRepair` 挂启动修复链（`runRepairPass`）——「活子+死父」自动复活父一级；父行彻底悬空的只计数暴露不擅自动数据；同 id 多行（iCloud 副本）优先活行，不复活软删副本制造新副本。装新包启动一次，存量孤儿自动归位。
- **读路径留痕**：聚合兜底命中孤儿时按分类名去重打日志，异常态不再静默。

### 原则固化与治理规划
- **质量红线 §7 数据沙箱隔离**（`docs/standards/quality-redlines.md`，INDEX.md 挂索引）：动软删/回收站/同步副本机制时三侧盘查缺一不可——删除侧子级去向显式处置、恢复侧父链联动、读路径找不到父时明确语义（归桶/跳过留日志/触发自愈），禁止静默冒充；新增可软删实体自检三问。
- **治理规划**（`docs/plans/2026-09-25-数据沙箱隔离治理规划.md`）：全 App 28 个软删实体的三侧现状盘点，P1 补联动分支表（已发现预算→分类/账户、目标指标→目标两个真实缺口）/ P2 读路径全量审计 / P3 自愈链扩展分期排期。

### 验证
- 7 个 Core Data 集成单测全绿（孤儿复活/父已活不动作/悬空不崩/副本优先活行/软删子不判定/恢复联动两级），新测试文件挂测试 target 四处经 plutil 校验。
- 方案调整留档：原拟聚合层「父缺失归未分类桶」，实做发现需动展示层结构，为自愈后≈0概率的中间态不值，改为「自愈灭数据态+读路径日志暴露」。

### 验收清单（真机 30 秒）
1. 装新包启动一次（触发自愈）；2. 财务→统计分析→类别：饼图恢复只显示一级分类；3. 点一级下钻看二级、返回正常；4. （可选）回收站恢复一个二级分类，其一级分组一并回来。

---

## [2026-09-25] 深度分析问句时间窗四层防线：问「近一周/最近两个月」不再答成近180天

> 后端已发版四次（agent_loop v24 数字纪律 / 冻结块无窗兜底 / validator label 可选 / 时间窗解析员），容器逐项验证；iOS 侧随 1.0.9 入库待真机复验。东林同问句三次实锤后定的方向：从「模型自觉」升级为「定岗定责」。

**背景**：东林问「最近一周我的睡眠…」，报告答成近 180 天；同症状历史上已修过一次（1.0.8 快捷查询链路）。自查+生产日志取证定案：时间窗症状按链路分布，修一条≠全好；本轮把深度分析链路的断口全部堵死并建评测闸门。

### 根因（多断点叠加，逐个实锤）
1. 聊天自由打字问句的时间词从未接进冻结任务（只有预设场景卡带窗），云端按快照默认 180 天全窗分析；生产日志 8 轮查询 timeRange 全 null。
2. 后端校验层静默丢弃模型传的无 label 时间窗（提示词只教 start/end，校验却硬要求 label）——模型传了也被吞。
3. 「两」字缺口：时间解析器正则字符类与中文数字映射表都缺「两」，「最近两个月」整句解析失败。
4. 词表外口语表达（入秋以来/上上个礼拜/三个季度）永远枚举不完，此前依赖模型查询时自觉（同问句时对时错）。

### 修复（四层防线，全部确定性优先）
- **iOS 词表**：自由问句时间词冻结成任务窗（复用 HoloAgentTimeSemanticResolver，与本地 Agent 同源）；场景卡问句时间词优先于场景默认窗；「今年/本月」等含未来段语义截到快照截止；词表补「两」。
- **服务端时间窗解析员**（questionTimeResolver.js）：客户端词表没中且问句非空时，任务开始前一次小 LLM 调用（思考档 none、千级 token、8s 超时保护、故障静默回落）把问句时间确定性解析成窗口，校验后作为主时间范围（此后 executor 护栏硬约束 + taskRange 回显 + iOS 范围标签显示「问句解析：xxx」）；窗口与快照窗边界截断、完全超窗保留原窗（防答非所问）。存量旧客户端无 answerTask 同样受益，iOS 零改动。
- **validator**：时间窗 label 改可选（不再静默吞掉提示词约定格式的窗口）。
- **executor 双护栏**：任务窗存在时 timeRange 缺省自动默认到任务窗（工具结果 warning 提示，显式宽窗查个人基线不夹紧）；冻结块无窗时注入确定性指令「问句含时间表述必须按它换算、回答开头注明实际范围」。

### 验证与可观测
- 真实生产模型评测 10/10 全绿（入秋以来=24天窗 / 上上个礼拜起点精确14天前 / 三个季度270天正确截断 / 9月和8月比取主窗 / 无时间词不瞎猜 / 指令注入防住），评测脚本 scripts/eval-question-time-resolver.mjs 常驻可复跑；后端 470 测试全绿（含解析员 5 场景集成测试）；iOS CloudTwoMonthsFreezeTests/CloudFreeformQuestionTimeRangeTests 全绿。
- 每个任务日志记「时间窗解析 source=client词表/resolver/none」——无窗比例可观测，词表缺口主动发现。
- 空态诚实化：云端有结论但被显示防线滤光时标记 unverifiable（副标题如实说「已查到数据未过核验」）+ 日志留 rawClaims 数，不再与「没数据」混淆。

### 验收清单（真机）
问「最近两个月花了多少钱」→ 范围标签「问句指定：最近两个月」；问「入秋以来我的睡眠有变化吗」（无需装新包，服务端解析员已生效）→ 标签「问句解析：入秋以来」+ 窗口约 8 月入秋至今。

---

## [2026-09-24] 深度分析报告可读性三件套 + 图表组件按 iOS 设计规范重写

> 后端 agent_loop v24 数字纪律已发版（对比必须写差额/幅度禁并排原始值、单句≤1数字、字段间数字分工）；iOS 三件套随 1.0.9 入库。图表组件经「HTML 高保真原型 → 东林过目批准 → 实现 → 两轮 QA 对照原型验收」全流程。

**背景**：东林反馈深度分析报告「一堆数据堆砌看不懂」+ 首版图表被否（单值被画成无轴无刻度的橙色大色块=「一坨」）。审计定性：盒中盒中盒三层容器、图表数字比正文还小、9.5pt 轴标签低于可读下限、橙色滥用四种语义、无动态字体、Y 轴隐藏失真。

### 交付内容
- **指标行**：依据卡头部核心数字提为大字（title2 圆体）+ 单位 + 变化徽章；
- **水平对比条**（成对断言）：标签/本期橙条/上期灰条/右对齐数值，条长按真实比例，↓57% 胶囊徽章只给方向和幅度不预设好坏；
- **单值大字数字行**：单值断言永不画柱（根除装饰色块），数字行贴正文左缘无面板底色；
- **趋势速览卡**：标准 16 圆角卡 + 图表蓝折线 2.25pt + 渐变面积 + Y 轴 4 刻度网格（不再隐藏）+ 末端圆点 + 「本机数据·逐日」标注；数据由本机按数据集标识+报告时间窗实时回查（HoloReportTrendDataResolver，>62 天自动按周聚合，scope 无起止或序列<3 点静默降级）；财务逐日支出补零画「哪些天没花钱」，健康序列过滤无记录日（断线比假 0 诚实）。
- 设计纪律：语义文本样式（动态字体）+ Asset Catalog 语义色（自动深色）+ 数据线统一 holoChart1 蓝、橙只留强调；VoiceOver 整卡合并为有意义描述；后端 comparison 字段的 up/down 方向枚举过滤（QA 实锤英文漏出）。

### 数据通道（图表只对新报告生效的口径修正）
metricAssertions 后端 v23 起随协议回传且经交付核验，但 iOS 合成层此前未写进落库 JSON——本轮透传后**只有装新包后新生成的报告有图表**（协议有字段≠落库有，中间隔着合成层）；趋势图另需报告有冻结时间窗，与时间窗修复（见上条）联动接通。

### 验证
- 后端 470 测试全绿；iOS 全量编译绿 + 图表兼容/趋势聚合/徽章/旧 JSON 解码兼容 16 项断言绿；
- QA 两轮对照原型验收全绿：对比条比例与徽章/单值行形态/趋势卡 Y 轴刻度网格末端点/深色可读/无障碍描述/依据区指标行/全程无崩溃；趋势线含种子交易=本机全库回查正常口径，种子假数字与现场造账不一致=种子自洽性非缺陷。
- 原型归档 docs/prototypes/2026-09-24-报告图表组件重写原型-v1.html（浅深双栏，两轮机器渲染验证）。

### 验收清单（真机）
装新包后问「最近一周我花了多少钱」：范围标签正确 + 观察卡水平对比条或大字数字（无橙色坨）+ 趋势速览蓝折线卡 + 依据区展开指标行；深色模式扫一眼。

---

## [2026-09-24] 周期回放健康域放开 + 素材扩到深度分析同级

> 后端回顾模板 v11 已发版（health 卡片类型 + 健康语义口径段），容器验证；iOS 随 1.0.9 入库。云端与本地回落两条生成轨道共用同一服务端模板，一次发版全生效。

**背景**：健康数据在代码上早已接入回顾快照组装，但被 InsightFeatureFlags.healthContextEnabled（Release 默认关、无用户开关）拦在正式包外；且隐私同意 v2 文案一直承诺「回顾素材含健康与活动摘要」——文案与行为不符。素材本身只有日均睡眠/步数两个数，站立/运动硬编码空值。

### 交付内容
- 健康开关单独默认开（只动 healthContext 一个 flag，不误开其他 5 个内部试验开关），文案与行为对齐；
- 回顾健康素材扩到深度分析同级：睡眠分期（深睡/REM/效率，无 Apple Watch 分期数据保持 nil 不当 0 解读）/平均就寝起床时刻（凌晨折算+圆均值，跨午夜不平均成中午）/站立/活动分钟/千卡/距离/运动次数与主要类型/上期环比字段（供「比上期少睡40分钟」叙事）/有数据天数诚实口径；信号扩到六条（睡眠偏少/步数低/站立少/周期无运动/就寝晚/效率低）；
- 后端回顾模板 v11：type 枚举加 health + 健康语义口径（不做医学判断、就寝晚只描述现象、分期缺失不提不写 0、环比用差额表达、recordedDayCount 支撑诚实口径）+ memoryCandidate 适用类型补 health。

### 验证
后端 464→470 测试全绿；iOS 富字段 Codable 兼容测试绿（旧素材 JSON 继续解码、新字段 nil 不炸）。

### 验收清单（真机）
做一次周回顾：应出现健康卡片（如「这周平均睡得比上周少」类叙事），无手表分期数据时不说「深睡为 0」。

---

## [2026-09-24] 想法卡片预览修复：「by 纯银」签名行不再被挤出 + 「点击查看全文」临界漏判根治

> 纯 iOS 无发版，随 1.0.9 入库；待东林真机验收。

**背景**：东林输入「#标签 + 正文 + 空行 + by 纯银」发送后，列表卡片上「by 纯银」消失、正文下方多出一行空白，且该卡内容明显被截却没有「点击查看全文」提示（下方长文卡有）。

### 根因
- **签名行消失**：列表卡片预览上限 7 行（ReadOnlyRichTextPreview → UITextView maximumNumberOfLines）。该内容 tag(1)+正文(5)+空行(1)=7 行满，「by 纯银」第 8 行被截；空行占第 7 行渲染成底部空白。数据未丢（编辑器内完好）。
- **提示不出现（排查中新挖出的实锤 bug）**：溢出判定与实际渲染是两套排版引擎——判定走手动 NSLayoutManager（TextKit 1）、渲染走 UITextView（TextKit 2），对中文折行存在临界差。正文末行仅「片。」两字时，测量判 7 行整「不溢出」、渲染实际 8 行截成 7 行（诊断测试实测 width=300 判溢出、width=320 判不溢出）。只有临界内容踩坑，长文两引擎都判溢出故正常。

### 修复
- **预览空行压缩**（`RichContentSerializer.previewNodes` + `ReadOnlyRichTextPreview` 新增 `compressesBlankLines` 开关 + `ThoughtCardView` 启用）：卡片预览压掉零长度空行，7 行配额让给有效文字——签名行进入预览、底部空行留白消失、也不再需要截断提示，一处修三症状。**只含空格的行保留占位**（东林拍板：用户敲的留白不在预览里消失，避免「我敲的内容去哪了」新疑问）；行内/行首/行尾空格原样不动。展开态与详情页/编辑器还原原文格式；主题确认队列另一调用点未启用，行为不变。
- **溢出判定换同引擎测量**（`ReadOnlyRichTextLayoutMetrics`）：手动 NSLayoutManager 管线换成离屏 UITextView（iOS 17 起与渲染同为 TextKit 2），消除两引擎折行临界差；private → internal 供测试直调。

### 验证
- 新增 3 条回归测试（预览压缩多场景断言 / 行内空格保留 / 签名场景压缩后各宽度判定不溢出）；MarkdownTextViewNodePipelineTests 49 用例全绿 + ThoughtRichContentTests 13 绿。首版压缩函数漏删中间空行被测试当场抓住后补齐。
- 模拟器复现实锤：编辑器内「by 纯银」完好（数据未丢）；带标签卡与普通卡「时间行→正文」间距一致（差 <1pt）。
- 未修两项待议：卡片上方空隙定性为设计间距观感（是否收紧待东林拍板）；发送后「tag 先出文字慢刷」模拟器未复现（同帧出现），定性真机性能观感（新卡首帧集中解码+富文本构建+溢出测量叠加编辑器收起动画）。

### 验收清单（真机）
1. 新建「#标签 + 正文 + 空行 + by 纯银」发送：卡片应显示「by 纯银」、无底部空行留白、无「点击查看全文」。
2. 点开编辑器：原文空行/空格行完好。
3. 长文卡：「点击查看全文」出现 → 展开 → 收起正常。
...
1.0.8 提醒弹层重做（锚定档位 chips + 点行即改）落库时 9 个新增文案只有简中，英/繁环境会直接漏出简中原文。本批补齐翻译目录（英文 + 繁中，繁中按台湾用语如「設定」），涉及：快捷档位说明头、无具体时间引导语、暂无提醒、提醒明细、截止前提醒、提醒时刻、独立提醒及其说明、截止时间前提醒。

随批盘点出两处翻译欠账，记 1.0.9 待办：
- chips 标签（当天 09:00 / 前一天 20:00 等）是 String 直传，不走翻译目录，需小改代码才能本地化；
- 全库存量 228 键缺英/繁（含隐私政策整段 HTML），历史欠账需单独立项批量补。

---

## [2026-09-24] AI 任务确认卡「提醒时刻」弹层重做：锚定档位 + 点行即改 + 全天任务详情页统一

> 纯 iOS，无后端发版，无 CloudKit 部署。TaskPendingEditTests 18/18 绿 + QA 模拟器冒烟 5 断言全过；随 1.0.8 入库，待东林真机验收。

**背景**：东林二次批评两点——已添加的提醒时刻只能删不能改；全天任务弹层没有「提前 15 分钟」这类快捷档位。经六产品调研（Apple 提醒事项 / Todoist / 滴答清单 / 微软 To Do / Things3 / 谷歌 Tasks）证实这两点均违反业界通用约定；东林过目可交互原型后拍板实施。

### 改版内容
- **锚定预设 chips**：绝对模式新增「当天 09:00 / 当天 18:00 / 前一天 20:00」三个快捷档位，点选即加、再点即删、可多选，选中态与已设条目联动；落在过去的档位置灰，防止建出永不响的提醒。
- **点行即改**：任意已设提醒点行内展开日期+时刻双滚轮直接改，删除按钮保留；替代「只有最新一条可调临时滚轮」的旧实现。
- **详情页统一**：任务详情页全天任务的提醒区从「提前 N 分钟」相对档位（全天任务无截止时刻，提前语义不成立）改为锚定绝对档位，与 AI 确认卡完全一致；带时刻任务维持相对档位不变。
- **保留**：新建提醒预填 9:00（三巨头共识）、空选=显式「不要提醒」。

### 验证
- TaskPendingDefaults.anchoredPresetDate 纯函数单测锁定；TaskPendingEditTests 18/18 绿。
- QA 冒烟 5 断言全过：chips 点选/再点取消/多选/计数联动、点行即改行内展开双滚轮、删除与 chip 熄灭联动、预填 9:00 且对应 chip 选中、详情页全天任务已切锚定档位。
- QA 报「先建任务在列表消失」经查库+二进制 md5 比对定性为并行会话装包互踩（外来包覆盖安装清空 AppGroup 库），非本改动缺陷。
- 可交互原型 docs/prototypes/2026-09-23-reminder-picker-prototype.html 随批归档。

---

## [2026-09-24] 生活理解端到端第二轮五连修：个性化候选进规划 prompt 主链全绿

> 纯 iOS。模拟器主链 testAskOnlyQ0 五项 PASS（方案卡+「因你的情况」+宠物照护项+红线零违反，prompt 691 字符带候选）；standalone 11/12 绿（唯一红 ExtractorOrchestrator=并行会话「五路决策」未收口已知项，stash 对照实证 HEAD 同红、非本批引入）。随 1.0.8 入库，待东林真机验收（链路文档 docs/_common/plans/2026-09-23-真机验收链路.md）。

**背景**：「生活理解与主动筹备」主链（四域观察→萃取→情境库→检索→方案生成）上一轮遗留「个性化候选没有进入规划生成」。第二轮排障定位出 1 个环境根因 + 5 个代码根因，全部修复。

### 五个根因与修复
- **recurrence 自由文本容错**：LLM 把循环规则写成「每天」等自由文本时，原整条候选被静默丢弃，且空批 receipt 永久拦截重试；现按频率词映射成规则、映射不出置 nil 保留原文（Codable 套件 +8 断言锁定）。
- **轮转调度饥饿**：caught-up（已追平）的域也扣 LLM 预算，把 more 域（想法）系统性饿死；现只有实际干了活的批扣预算，指针两圈回绕封顶。
- **分域落库域校验炸**：落库硬编码 domain:.thought，与仓库「primaryDomain 必须等于 domain」校验冲突，habit/task 候选反复 invalidRecord；现按 primaryDomain 分组落库（组键 batchKey#域，父键收尾保幂等）。
- **temporaryScopeNarrowed 误杀**：temporal.kind==event 即判一次性事件 observeOnly，把「曾请人上门喂猫」这类带 applicability 条件的条件化经验全挡在建议门外；现带 conditionText 的 event 不做最窄化。
- **lastSupportedAt 缺失秒归档**：萃取新建/合并记录不设该字段，freshness 恒 0，压缩服务一跑就把全部情境候选归档；现新建/合并都补 now（二分实证：回退版完整旅程复现归档回归，恢复版主链五项全 PASS）。

### 配套
- preparePrompt 全链诊断日志（DEBUG）：selected/excluded 分布、检索命中、prompt 字符数一行打尽（本轮 5 个代码根因中 5 个靠它定位）。
- 首因定性：17:11 卸载重装把 App 容器+AppGroup 容器全清（种子+候选+开关全丢）——「候选没进 prompt」的环境根因，非代码 bug。
...
**背景**：东林识图记账一张美团外卖订单页（已付款、配送中，合计 ¥20.9）被拒识；回 Holo 时昨日测试留下的 ¥65 旧草稿被自动弹出，被误当成「这张图识别成了 65 元」；且识别失败全程无任何提示。诊断确认三个叠加问题 + 一处误导性代码默认值。

### 问题与修复
- **已付款外卖订单被拒识（后端，已发版生效）**：DeepSeek 视觉模型对「商家已接单/配送中」的已付款订单页输出自相矛盾的结果——`paymentStatus=completed` 却 `imageType=pending_order`，伴随交易被图型红线清空，整单按未支付拒识。修：识别提示词升 v5（明示外卖「已接单/配送中/预计送达」=已付款、附完整输出示例）；理解单新增一致性护栏（模型亲口判 completed 时不得再按未支付拒识，防同类矛盾）。发版后同一张图复现验证：正确识别出 ¥20.9 支出，护栏未触发（模型直接服从提示词）。
- **拒识静默无反馈（iOS，待装包）**：识别失败/拒识原本不发任何通知——快捷指令后台或自动化运行时，其结果文字用户根本看不到。修：拒绝类一律发通知说清原因，文案与快捷指令结果共用同一份口径；仅用户主动取消不打扰。
- **历史草稿冒充本次结果（iOS，待装包）**：「确认页必达」回前台兜底不区分草稿新旧，历史遗留草稿被自动弹出且页面不显示时间，用户误当成这次识别的金额。修：回前台只自动弹 10 分钟内新建的草稿（识别失败则什么都不弹），旧草稿只走复核列表与绑定该草稿的兜底通知；确认详情页与待复核列表必显「识别于 今天 HH:mm / 昨天 / M月d日」。
- **误导性默认值（后端）**：代码默认识图模型 qwen3-vl-plus 与生产实际（DeepSeek deepseek-v4-flash-vision-exp）不一致，按东林拍板「模型定版 DeepSeek 不换」对齐删除；上游瞬断错误归一为 502，让 iOS 既有的一次自动重试能接住（生产实锤 11:31 一次上游瞬断未触发重试直接失败）。

### 验证
- 后端 462/462 测试全绿（新增 6 条：一致性护栏四态 + 外卖提示词条款 + 默认模型对齐）。
- iOS 新增 11 条单测（新鲜度闸门边界、旧草稿不弹、拒识通知文案、识别时间格式），44/44 绿。
- ios-qa 模拟器 5 验收点全过：新鲜草稿回前台自动弹、详情页与列表「识别于」展示、旧草稿不自动弹（反向）、全程无闪退。
- 生产复现验证：修复前同图拒识 → 修复后识别出 ¥20.9 支出。
- xcstrings 新词条（这笔没有入账/识别于 %@/今天/昨天）简中即源文案即时生效，繁/英随下批翻译统一补。

---

## [2026-09-22] 记忆长廊点击卡顿根治：高亮检测批量化 + 首屏重活后台化（8-22 遗留的「另一半」）

> 纯 iOS 修复，无 HoloBackend 发版，无数据迁移，无需 CloudKit 部署。

**背景**：东林报「点记忆长廊有明显卡顿」。8-22「打开卡顿 1-2 秒」治理当时只修了日历侧（首屏 ±20 天 + 全量 ±60 天错峰），CHANGELOG 明确遗留「洞察 Tab 的加载链是打开卡顿的另一半来源」，本轮根治。

### 根因（主线程三件重活，真机数据量下数百至上千次小查询串行挤主线程）
- **高亮检测是 N+1 放大器**：消费异常每个日期查 2 次交易（全对象拉回只为求和）、习惯全勤每个日期×每个习惯查 1 次、重要任务每个日期查 1 次。
- **聚合统计与热力图拉全对象**：「N 天生活证据·M 条记录」与 13 周热力图为了数天数，把全部交易/想法/任务的完整记录搬进内存。
- **交易关系逐笔惰性加载**：每笔交易临时回查分类、账户名（每笔 2 次小查询）。

### 修复
- **检测器批量化**：HighlightDetector 拆 `detectBatch`（三类检测改为窗口级一次列查询 + 内存按日分组，查询次数与日期数无关；谓词口径与原逐日实现完全一致）与 `detectStreakAchievements`（依赖 @MainActor HabitRepository，保留主线程，量级=活跃习惯数）。共用同一检测器的日历侧窗口加载（CalendarViewModel）与 AI 洞察上下文构建（MemoryInsightContextBuilder）签名未动、自动受益。
- **首屏重活后台化**：MemoryGalleryViewModel 新增 `computeSnapshot`，在后台 context 一把算完记忆条目+批量高亮+聚合统计+热力图（值类型回主线程）；统计/热力图改 `dictionaryResultType` 只取日期列；交易查询补分类/账户关系预取。
- **口径保持**：7 日均值分母恒为 7（沿用原实现）；全勤/重要任务检测谓词历史上不带 deletedAt 过滤，保持同口径未顺手改。习惯 streak 计算链未动（动有习惯模块回归风险、量级小，若真机仍卡单独立项）。

### 验证
- 新增 HighlightDetectorBatchTests 12 条全绿（检测器此前零覆盖：消费异常检出/不检出/对账流水排除、全勤三态、任务优先级两档+紧急副题、空库、多日一次检测各自独立判定），pbxproj 已挂载。
- ios-qa 模拟器冒烟通过：安装新包 → 进长廊日历 tab（大日期/时刻卡完整渲染）→ 切洞察 tab（统计与卡片正常）→ 无崩溃无异常日志。
- 真机验收待东林覆盖安装后确认点击卡顿消除。

### 附带结论（东林同问「长廊字体怪怪的」）
排查确认 **9 月无长廊字体改动**（衬线谱 25 处自 8-22 起稳定、iPad 1.25x 排印在 iPhone 恒不生效、全局字体 token 未动）。QA 视觉实锤为**衬线使用规则不统一**：日/周/月当大标题用衬线、周/轴档坐标刻度小字也用衬线、洞察页「理解」却是特粗黑体、月档「9月」为衬线数字拼无衬线汉字。属 8 月下旬册页风多批次排印累积（线上 1.0.2 已含），设计规范问题待东林拍板方向后另行修复，本批未动任何字体。

---

## [2026-09-21] 习惯类型无痛切换：编辑改类型保存生效 + 历史记录平滑转换

> 纯 iOS 修复，无 HoloBackend 发版，无数据迁移，无需 CloudKit 部署。

### 修复
- **编辑类型保存生效**：根治「数值型习惯编辑切打卡型、保存后仍是数值型」的问题。根因是编辑保存通道的更新参数里没有习惯类型这一项，类型选择器改了也白改；现保存时真正写入类型，并同步刷新提醒调度。

### 改进
- **历史记录无痛架桥转换**：切换类型时历史记录原地平滑转换，不丢数据、可随时切回——数值转打卡：当天记过数值即视为已完成，数值原样保留；打卡转数值：历史打卡按每次 1 计入，勾选状态保留；两个方向都会清理「勾了又取消」的无数据空记录，避免切换后完成率虚高。
- **类型切换预告提示**：编辑页改动类型时出现提示条，写明历史记录将如何转换，保存前心里有数。
- **目标字段平滑衔接**：数值转打卡未填目标次数时按每日 1 次统计（既有兜底），打卡转数值允许无目标。

### 验证
- 新增转换单测 6 条全绿（双向转换、空记录清理、来回切无损、幂等、类型未变不触发），既有习惯累计口径单测 8 条全绿无回归；worktree 隔离验证通过。
- 模拟器全链走查通过：新建数值习惯记数据 → 切打卡型（磁贴变打卡形态、当天自动已完成、连续天数接上）→ 切回数值型（数值无损）→ 改类型后取消正确弹「放弃修改」确认，全程无闪退。
- 真机验收待东林覆盖安装后进行。

---

## [2026-09-21] 想法编辑器中文输入法列表续行稳定性修复

> 纯 iOS 编辑器修复，无 HoloBackend 发版。

### 修复
- **根治列表续行回滚**：修复点击无序/有序列表后，中文拼音候选上屏、回车换行或开始输入第二行时，列表前缀消失、正文回退、光标跳到开头的问题。
- **统一编辑器状态所有权**：编辑会话内以 UITextView 当前内容为正文事实源，忽略 SwiftUI 回放的旧快照；工具栏动作移出视图刷新过程并保证只执行一次，避免状态循环和重复插入。
- **保留空列表项**：允许 `• `、`2. ` 等等待用户继续输入的临时列表项正常解析和恢复。

### 验证
- 编辑器专项 XCTest 46/46 通过，包含真实 SwiftUI 宿主回归：点击列表 → 第一行中文 → 回车 → 第二行中文 → 多代旧状态回放；Xcode 模拟器构建与测试通过，`git diff --check` 通过。
- 真机当前未连接，中文输入法真机覆盖安装与实际候选上屏仍需连接东林设备后完成。
...
### 改进
- **启动插画重绘**：保留 Holo 原有暖色线稿、女性侧脸、流动波纹与品牌标识，移除头部区域让用户感到不适的白色曲线装饰，恢复干净、平静的视觉焦点。
- **品牌文案统一**：启动页副文案由「持续记录，更了解你」调整为「你的人生数据库」，与 Holo 当前产品定位保持一致。

### 验证
- 新启动图保持原资源规格 1290×2796；Xcode Asset Catalog 编译通过。完整工程编译仍受工作区既有 `TaskDetailView` / `TaskListView` 编译问题阻塞，本次未把资源验证写成整包构建通过。

---

## [2026-09-19] 用户头像全局一致：圆形裁剪、私人 iCloud 同步与 Apple 账号卡联动

> 纯 iOS 与私人 CloudKit 数据模型改动，无 HoloBackend 发版。Production CloudKit schema 仍需在正式发版前单独部署。

### 新增
- **统一个人资料入口**：设置页与个人页共用头像/昵称编辑器，支持从相册选择或拍照；上传后，设置账号卡、个人页、Holo AI 用户消息中的头像同步更新。Apple 登录不提供头像，账号卡改为消费 Holo 头像，并保留登录状态角标。
- **圆形头像裁剪**：选择图片后进入圆形蒙版裁剪页，支持拖动定位、双指缩放，并同时预览对话 32pt、账号 56pt、个人页 80pt 三种真实显示尺寸。
- **私人跨设备同步**：新增 `UserAvatarEntity` 与统一仓库，头像通过 Core Data 私有 CloudKit 镜像同步；移除头像保存为可同步状态，避免旧设备把历史头像重新带回。退出 Apple 登录保留头像，删除账号与数据会一并删除。

### 隐私与图片处理
- 原图先降采样，最终只保存 512×512、最多 512KB 的 JPEG；重新渲染剥离 GPS/EXIF，不保存原始照片，也不发送给 AI 或 Holo 后端。
- 相机权限说明和应用内隐私政策已补充头像用途、存储位置、同步方式及更换/移除权利。

### 验证
- Holo Debug / generic iOS Simulator 完整构建通过，覆盖主 App、HoloWidgets 与共享 Core Data 模型。
- 裁剪几何 standalone 16 项断言全部通过；头像数据模型与图片输出专项 XCTest 2/2 通过；改动格式检查通过。
- 未冒充完成的发版门：真机相册/相机授权旅程、双设备 iCloud 同步、正式多语言词条与 Production CloudKit schema 部署。

---

## [2026-09-18] 删除分类不再死胡同：明细清单交用户处置 + 分类删除根因修复

> 用户反馈：删除分类提示被记账占用不让删，把账目明细删光后依然删不掉；东林定方向——不该卡住，应列出明细让用户决定怎么处理。另排查「右滑看不到删除按钮」反馈。纯 iOS 零后端发版。

### 修复
- **「删光明细仍删不掉分类」根治（根因两处）**：① 判占用的查询不过滤回收站标记（deletedAt），软删明细也占位；② iCloud 同步天然产生的同 id 重复行，删除/判定只摸到其中一行，幽灵行继续占位。修法 = 占用判定只数有效明细 + 按 `categoryId` 匹配（覆盖挂在重复行上的明细）+ 删除分类/子分类时按 id 清除全部行。「清理导入分类」的占用判定同步改口径。
- **删除流程按东林定稿改造**：删除仍有明细的分类不再弹「无法删除」死胡同，改为弹出「明细处理」弹层——列出明细笔数、合计金额与明细清单（最多预览 100 笔），用户可选「明细转移到其他分类（默认选中「待分类」）」或「连同明细一起删除」，确认后完成删除。
- **iOS 26 渲染崩溃双修**：① 分类管理页「清理导入分类」按钮误用了 SF Symbols 不存在的图标名 `broom`，渲染时直接 SIGTRAP，换成 `trash`；② 删除落库后埋在弹层底下的账本页/记账页仍持有并渲染已删除的明细与分类对象（开发规范 13 节场景 C），读属性强桥接 SIGTRAP——lldb 附着复现抓到真实堆栈（`FinanceLedgerView.transactionListView` ForEach 读已删 Transaction 的 UUID），修法 = 渲染边界过滤失效对象（`managedObjectContext == nil` 即已删）：CalendarState 新增 `liveDayTransactions`、账本列表/本日摘要/空态全部改用、记账分类选择器网格同滤 + 重载时清理 `drillDownParent`/`selectedCategory` 残留引用。此修对所有删除路径免疫，不限本次入口。
- 删除确认弹窗文案去掉已失效的「若已被交易使用，将无法删除」死胡同表述。

### 验证
- 模拟器 QA 走查：转移路径（明细保留并归入目标分类「待分类」、金额守恒）、一并删除路径（明细与分类同删）、无明细路径（原确认弹窗）、预设分类只给编辑不给删（设计如此）全部符合预期。
- 崩溃修复共三轮：首轮 QA 抓到 broom 崩溃误判为唯一根因，二修（弹层值快照+先关弹层）后 2/2 仍崩；按红线停手跑 self-check 命中已知坑「NSManagedObject 删除后访问崩溃场景 C」，lldb 附着复现拿到真实堆栈定位账本页渲染已删对象，三修（渲染边界活性过滤）后崩溃路径 2 轮 + 转移路径 1 轮复验全绿（删除后停留 15 秒、逐层退回账本、切日期、重开记账页均无异常）。

## [2026-09-18] 账本「记一笔」键盘自动弹出恢复

> 东林反馈：财务模块点击记账以前键盘会自动弹出，现在不弹了。纯 iOS 零后端发版。

### 修复
- **键盘不再自动弹出（根因）**：9-14 修「账本选昨天记到今天」时（e5c7ff3a2）给记账弹层加了预填日期，并顺带做了「带预填日期的入口键盘默认收起」的设计；而账本 Tab「记一笔」恒带预填日期（月历总有选中日，含今天），导致最常用入口键盘永远收起。修法 = 删除按 presetDate 区分键盘初始状态的逻辑，所有入口恢复键盘先行；日期预填功能（选哪天记哪天）不受影响。

### 验证
- iPhone 17 模拟器实测：账本页点「记一笔」，自定义数字键盘自动弹出、金额框聚焦光标，全程无崩溃；presetDate 预填链路未动，选哪天记哪天不受影响。

## [2026-09-17] 桌面小组件三修（徽章掉行/胶囊折行/行内标签）+ 编辑器恢复长按选中复制（flomo 同款）

> 东林反馈桌面三症状：待办组件「逾期」徽章掉到标题下一行叠压文字、收支组件分类胶囊「数码 ¥1,658.83」折成两行、想法随机漫步正文把 #标签 渲染成普通文本；另反馈编辑器选中文字后复制粘贴菜单弹不出来。纯 iOS 零后端发版。

### 修复
- **待办组件行布局**：勾选交互平级化改版时 Link 的 label 直接铺多个子视图（TupleView），系统不按一行排——徽章掉行叠压、标题竖向裁切。修法 = label 内容包显式 HStack。
- **收支组件分类胶囊**：三个胶囊并排接近组件满宽，中间的「数码 ¥1,658.83」被压缩折成两行。修法 = 文字钉单行 + 最小缩放 0.75 兜底。
- **想法记忆组件行内标签**：正文纯 Text 渲染不认 #标签。修法 = 按 InlineTagDetector 同源正则切段、标签段品牌色高亮；底部标签胶囊与正文高亮去重（同 App 内卡片「不重复列」规则）。
- **编辑器选字复制（真机「选中文字后复制粘贴弹不出来」根治）**：7 月为治 Token 点按误弹自动填充菜单，编辑器把系统 UIEditMenuInteraction 替换成自定义实例——该替换在 iOS 26 上让整个选字菜单再也弹不出来。修法 = 撤掉替换、恢复系统原生长按选字菜单（flomo 同款 复制/剪切/粘贴）；Token 场景由 canPerformAction 过滤（完整 Token 仅复制、任务标记不可复制）+ writingToolsBehavior=.none + inputAssistantItem 清空继续兜住；「转为任务」不进菜单（工具栏等价按钮本就存在）。

### 验证
- 模拟器编辑器双击选词 → 系统菜单弹出（剪切/拷贝/粘贴）→ 拷贝后剪贴板逐字比对 = 选中内容；合成触摸长按进不了 UITextView 选字手势，长按路径待真机复验。点正文进编辑器（P0）无回归；行内标签卡片高亮、查看全文/收起正常。
- 组件渲染用东林截图原始数据（去换日币+逾期 / 数码 ¥1,658.83 / 摩卡#碎碎念）按真实组件尺寸 swiftc 探针复刻比对全过。
- 子代理通道今日故障（reasoning-level-missing），本轮主会话亲自验收。

## [2026-09-17] 目标共创（一起想清楚）P0 全链路：澄清 → 路径 → 草案 → 原子保存

> 对应方案：docs/_common/plans/2026-09-17-Holo目标共创-完整开发计划.md（P0 任务 0–7）。**后端 goal_workshop purpose 需发版一次**（未部署，待授权）；iOS 默认关闸（goalWorkshopV1 服务端开关控制），不影响现有任何行为。

### 新增（iOS，36 文件）
- **会话状态机与契约**（`GoalWorkshopModels`/`GoalWorkshopValidator`）：understanding→exploring→choosing→reviewing→saved；事实四级分级（你说/记录/推测/未知）+ 推断可纠正；迟到/重复响应按 sessionID+revision 丢弃；3 问上限 + 5 请求预算；日期严格 yyyy-MM-dd（ICU「/」等价分隔符陷阱用回格式化比对根治）。
- **持久化**（`GoalWorkshopSessionMO`/`GoalPlanRevisionMO` + 两 Store）：会话 payload 版本化信封、未知版本隔离；单调 revision 守卫；iCloud 同 id 副本去重；决策版本幂等记录。Goal 实体新增 `sourceSessionID`（逻辑 ID，CloudKit 兼容可选字段）。
- **编排**（`GoalWorkshopCoordinator`/`GoalWorkshopPromptBuilder`）：额度预检（≥2 才进场）+ 每轮复查；429/断网/取消保留会话可恢复；非法输出一次受控重试（不占流程预算）；同轮重复提交拒绝。
- **UI**（四卡 + 流程容器）：问题卡（一次一问+为什么+跳过）/路径卡（2-3条含代价取舍）/定义卡（成功标准+假设+里程碑+第一步）/恢复卡（继续/放弃/另建）；入口：目标列表新建菜单「一起想清楚」、HoloAI 旧规划入口（开闸时指向同一会话）、目标详情「一起想清楚怎么调整」（P0 只给建议不写回）。
- **确认页补字段**（`GoalDraftReviewView`）：期望结果/动机/期限/待补充信息/成功证据/关键假设（可删）/所选路径代价——旧确认页缺失字段全部补齐；保存失败具体报错不静默。
- **原子保存**（`GoalWorkshopCommitService`）：Goal+任务+习惯+决策版本+会话回执单事务一次 save；同会话重复提交/双击/跨设备同逻辑 ID 幂等返回已有回执；旧 `saveDraft`（手建+旧 AI 草案）复用同一事务——**根治部分写入**。

### 后端（7 文件 + 9 测试，未发版）
- 新 purpose `goal_workshop`：独立路由/限流桶（10分/40日）/归 chat 额度池；提示词 v1（分阶段/一次一问/路径取舍/事实边界/可零习惯/契约 JSON/不声称保存）；强制 metadata_only（管理员开正文捕获也不落原话）；`goalWorkshopV1` 开关默认关；多语言（繁中/英文）支持；mock 契约回声供联调。

### 验证
- 后端 `npm test` **423/423 全绿**；iOS 全量 HoloTests **1174 测试仅 2 败**（均为在档的 Agent 域锁屏已知回归，与本功能无关）。
- 目标共创专项：状态机/校验器 2、存储 10、编排 9、原子提交 12（含失败注入/幂等/跨设备归并）全部通过；UI 旅程 3/3（全流程/退出恢复/关闸回退）。
- 冒烟与视觉：既有验收走查通过；共创各卡片视觉检查通过（无裁切/重叠/乱码）。

### 遗留（待东林决策/环境）
- 后端发版（需授权，发布后跑 scripts/eval-goal-workshop.mjs 真实模型 40 场景评测，P0 门槛 ≥36/40）；真机门禁 `AICrashDeviceReproUITests`；CloudKit schema 上报（新实体+Goal.sourceSessionID）；繁中/英文词条批次；真机验收。

---

## [2026-09-17] AI 建任务「9月20日」识成「9月17日」根治 + 一句话多提醒支持

> 东林反馈：输入「9月20日 早上9点要去换日币，头天晚上提醒我一下，当天早上8点再提醒一次」，任务落成 9月17日（当天）早上9点，且两个提醒时间全部丢失。**后端意图识别（提示词）需发版一次**；iOS 端纯本地改动待真机验收。

### 根因（后端日志 + swiftc 探针双实锤）
- **后端 LLM 本次返回其实是对的**（ai_call_logs id=5028：dueDate="2026-09-20 09:00"、reminderDate="2026-09-19 20:00"），错在 iOS 本地解析环节。
- **NLDateParser 不认识中文绝对日期**：解析器只认「今天/明天/后天/周X」相对日期，遇到「9月20日」找不到日期关键词，走「提取到钟点就落到今天+该时间」的兜底——9月20日+9点 → 9月17日 09:00，与现象完全吻合（探针复现）。凡是 LLM 漏填/填了中文格式日期、落到原文兜底解析的路径全部踩中。
- **确认卡执行路径丢了原话**：立即执行路径带 originalInput（原话兜底），但用户点确认后的 `confirmPendingTask → route(result)` 没传，兜底永远不生效。
- **多提醒无落点**：意图槽位只有一个 reminderDate，且 `handleCreateTask` 不解析它为提醒、硬编码「提前15分钟」；「头天晚上+当天早8点」两个自定义提醒被整体丢弃。

### 修复
- **NLDateParser 支持绝对日期**（`NLDateParser.swift`）：新增「M月d日 / yyyy年M月d日 / N号」解析；无年份且本年已过去自动进位下一年（9月说「1月5日」→ 来年1月）；「2月30日」非法组合拦截不静默进位成 3月2日。
- **确认卡执行路径补原话透传**：Coordinator 生成确认卡时把原话存进 renderData（`originalInput`），确认执行时传给 IntentRouter——LLM 漏填时间/日期时原文兜底与立即执行路径对齐。
- **一句话多提醒落地**（iOS + 后端提示词）：
  - 后端 `intents.json`/`defaultPrompts.json`（v31）：create_task 新增 `reminderDates` 槽位（逗号分隔绝对时间列表），明确「头天/前一天晚上=事件日前一天20:00」「当天早上N点=事件日当天N点」换算；dueDate/reminderDate/reminderDates 一律输出绝对日期时间（消除旧示例「今天 22:00」中文格式的自相矛盾——它会诱导 LLM 返回本地解析不了的形式）。
  - iOS：`ReminderSlotParser` 汇总 reminderDates+reminderDate → `TaskReminder(triggerDate:)` 绝对模式提醒（通知服务本就支持，直接生效）；用户给了明确提醒就不再默认塞「提前15分钟」；任务卡片 footer 显示多个提醒时间。
- 同步：`IntentDescriptor.swift` 经生成器再生成；后端 414 测试全绿；iOS SubtaskParserTests 新增 11 例（含本次事故原话回归用例）全绿。

### 验证与发版状态
- **后端 v31 已发版生产**（4299077cc，部署脚本全流程 + 健康绿；发版提交只含 5 个提示词/测试文件，未卷入并行会话在途文件）。
- 修复后全链路复验（模拟器新包 + 生产 v31，QA 两轮）：
  - 一句话 → **1 张确认卡**（旧提示词曾拆成「去换日币」+「去换日币提醒」两个任务，v31「不要拆成多个任务」规则根治）；
  - 确认卡 footer 同时显示「日期：2026-09-20 09:00 · 提醒：2026-09-19 20:00、2026-09-20 08:00」（旧版有提醒时日期不可见的设计盲区一并修复）；
  - 任务落库：截止 9月20日 周日 09:00（UI/IntentRouter 日志/CoreData 三重佐证），提醒 2 项（19日20:00 头天晚上 + 20日08:00 当天早上，triggerDate 绝对模式，通知服务直用）；
  - 后端 ai_call_logs 实证：v31 生效后 LLM 输出 `reminderDates: "2026-09-19 20:00,2026-09-20 08:00"` 单任务。
- 测试：后端 414 全绿；iOS SubtaskParserTests 新增 11 例全绿（含本次事故原话回归用例）；Debug 构建绿，30 分钟运行零崩溃。

### 遗留 → 提醒明细弹窗（东林拍板「小弹窗展示明细」，同日交付）
- 任务详情「日期与时间」面板原只显示提醒计数徽标（「提醒 2」），AI 建的多提醒看不到每条具体时间。现：提醒头部行可点（右侧「>」箭头），底部弹窗「提醒明细」逐项列出——绝对提醒显示完整时刻（「9月19日 周六 20:00」）+「独立提醒」副标，相对提醒显示提前量+「截止时间前提醒」副标，行内可删、外层计数徽标实时联动并持久化；footer 附语义说明。QA 四截图亲验（明细/删除/联动/布局）全过。
- 真机验收：iOS 改动待装新包后东林真机复验（模拟器已全绿；按红线装包时记录产物路径与时间戳）。

---

## [2026-09-17] 小组件点击交互根治 + 点组件拉起后深链导航落地

> 纯 iOS 零后端发版。东林反馈两问题：①点待办组件仍直接拉起 App（此前 P2 的可交互勾选「没生效」）；②拉起后停在原界面，到不了组件对应页面。模拟器两轮走查全绿（五大入口冒烟 + 深链导航 + 组件点击实点），待真机验收。

### 根因
- **勾选按钮「没生效」**：P2 代码已上线（aca25df57），但待办/习惯组件把勾选 `Button(intent:)` 嵌在整卡 `Link` 的 label 内部——WidgetKit 中外层 Link 接管整卡命中区域，点小圆圈也变成拉起 App。属结构错误，非功能缺失。
- **拉起后不导航**：`holo://` 深链解析链路完整（scheme 注册 / onOpenURL / 解析 / 目标映射全在），但唯一消费者 HomeView 只活在「今天」tab——停在「对话/我的」tab 时它已被卸载，`ContentView.handleDeepLink` 除 `.goalDetail` 外全部 `default: break`，跳转目标滞留。
- **QA 抓到的第三处**：App 停在「个人/设置」覆盖层（sheet）时，HomeView 深链切常驻栈发生在覆盖层下面，用户看到的还是原界面。

### 修复（5 文件）
- **`HoloTodoWidget` / `HoloHabitWidget` 结构重排**：勾选圆圈独立 Button 与「打开 App」的 Link 平级铺开（行文字/标题/进度条/圆环各自挂 Link，空态与 small 习惯卡保留整卡 Link）。intent 逻辑零改动。
- **`ContentView.handleDeepLink` 补根层分发**：非 goalDetail 目标统一把宿主切回「今天」（iPhone `selectedTab` / iPad `sidebarSelection`），HomeView 重建后自动领取 pendingTarget 落到对应模块；对小组件/通知/内部跳转所有来源统一生效。pendingTarget 仍由 HomeView 消费后自清。
- **`HomeView.handleDeepLink` 入口收覆盖层**：深链到达先统一收起个人/设置 sheet、iPad 页面层、看板 cover，再切常驻栈；目标自身要开的层（dailyReminder/weeklyBrief 看板、编辑器 sheet）在各 case 里再开。
- **`DeepLinkState.handle` 权益闸门去网络阻塞**：导航前不再 `await refreshStatus()`（弱网最多等 30 秒才动），改读 `HoloEntitlementState` 本地快照（冷启动即从磁盘恢复，弱网不误判）；正式权益仍由冷启动/回前台既有刷新通道保持准确，非 Plus 弹付费墙 + 购买续跳逻辑不变。

### 验证（模拟器两轮，iPhone 17 Pro Max 无头）
- 组件实点：勾选圆圈原地生效（App 进程未被拉起、列表刷新、App 内数据一致）；行文字拉起直达对应模块（Plus 态）。
- 深链导航：从 AI 页 / 习惯页 / 个人页 / 设置页出发的 `holo://tasks|habits|ai` 全部落到对应模块（第一轮个人/设置覆盖层分支 FAIL，修复后复验三场景全 PASS）。
- 五大入口冒烟（交付红线）：今日/看板、HoloAI（停留 3s + 发消息）、想法、任务、财务/设置全过，零新增崩溃 .ips。

### 遗留问题同步修复（同日第二批，东林拍板「同步修」）
- **组件标题「剩 N 项」口径与列表不一致**：标题原只算「今天到期」（`totalToday - completedToday`），列表却是「今天到期 + 逾期」（与任务页「今日」筛选同口径），全逾期日出现「剩 0 项」配 3 条待办。改为 `pendingCount` 存储字段、快照生成时传「未完成去重全量计数」（var+默认值保旧快照解码兼容，样本同步传 3 自洽）；逾期行本身已带红徽章，标题不再单独标注。QA 复验：「剩 4 项」配 4 条逾期，与 App 内任务页「已过期 4」一致。
- **免费用户点组件弹付费墙、关闭后深链丢失**：`HoloPlusActionCoordinator.requirePlus` 新增可选 `onDismiss`（默认 nil，既有调用方零影响；fullScreenCover 无下滑关闭，「关闭」按钮是唯一路径），`DeepLinkState` 的 desktopWidget 门控购买成功（resume）与直接关闭（onDismiss）都续跳到目标页。QA 区分性验证两路径全 PASS：内容态行文字（holo://tasks/habits）关墙落对应模块页；锁定态整卡（holo://ai）关墙落 AI 页——均为该 URL 的正确落点。
- **测试方法沉淀**：免费态验证中组件存在「Plus 快照 → 冷启动写回 free → reload 变锁定态」的形态漂移窗口，点击发生在窗口前后会得到不同 URL 与落点；验证组件点击必须先截图记录点击瞬间形态再断言落点。
...
### 修复
- **全新安装首次启动卡死在启动页（三方锁死锁）**：`CoreDataStack.persistentContainer` 的 getter 持着状态锁调用 `buildContainer()`，而 store 异步加载的完成回调需要拿同一把锁置位就绪标记，`viewContext` 配置又会同步等待 CoreData 内部装载队列——全新安装时「后台话题回填线程 + 主线程 prepareIfNeeded」并发首建，三方互等永久卡死（主线程等锁 / 回填线程持锁等 CoreData / 加载回调等锁）。老安装不触发仅因回填一次性标记已写过、两线程不再同时进门，属时序侥幸而非不会发生。修复三件套：①容器构建改独立 `buildLock` 串行（并发首建唯一胜者、后来者二次检查后复用），状态锁只在瞬时读写时持有、绝不横跨 CoreData 调用；②锁序固定 `buildLock → lock` 单向获取，结构上杜绝锁倒置；③`viewContext` 的 `automaticallyMergesChangesFromParent` / `mergePolicy` 配置挪进加载完成回调（装载完成后再配置，消除对 CoreData 内部队列的同步等待，时序语义也更正确）。

### 验证
- 两轮「擦净模拟器 + 全新安装」首启：均约 10 秒走出启动页到新手引导（修复前 100% 永久卡死，全程未使用任何绕过手段）；第二次冷启动 8 秒直接进首页（引导状态正确记忆）；新手引导→首页→AI 页（未授权引导态渲染正常、不闪退）回归通过。
- 全程零崩溃：DiagnosticReports 新增 .ips 为 0，进程日志 fatal/crash/exception/deadlock 关键字 0 命中。
- 踩 `CoreDataStack.shared` 的四个测试套件（AnniversaryDaysCalculation / ChatMessageRepositoryOrphanCleanup / ChatMessageRepositoryCacheRecovery / ReceiptBookingKernel）逐套隔离回归全绿（隔离跑以避开已记录在案的 R4-1 同进程测试污染）。
...
### 修复
- **正文行距放宽**：中/大卡正文原为系统默认紧凑行距，中文多行观感粘连；按字号三成补行距（大卡 14pt 字号约 +4pt，中卡 13pt +4），大卡底部淡出带高度随行距同步，末行淡出不露半截。
- **标签压分隔线根治**：分隔线原 overlay 画在「含上边距的页脚容器」顶部——恰为标签行底部、零间隙，全部空隙都留在线的另一侧，视觉上标签整个压线；改为先内边距再挂 overlay，线上移至页脚文字上方：标签与线留出一段呼吸位（实测约 13pt），线与「回到那一天」文字间约 9.7pt。
- **文案**：页脚「回到那天 →」改「回到那一天 →」，简中/繁中/英文三语词表同步。

### 入库说明
- 同文件随车入库此前已走查未提交的「大卡按渲染尺寸自适应排版」改造（`contentMarginsDisabled` 关双层系统留白 / GeometryReader 排版参数 iPhone 恒定、仅 iPad 温和放大 / 正文区占满剩余空间把日期/标签/页脚钉底 + 溢出裁切贴底淡出），东林真机在跑的即该版本。
- 小组件共享词表整文件入库：含并行会话写入的 6 条暂无代码引用的惰性词条（早上好/等待中等），闲置无害。

### 验证
- 全量编译通过；专用验证模拟器装包 + Plus 摆拍权益 + 手写多行样本快照，桌面大号卡截图逐条验收：文案/标签-线间距/行距三处全过，整卡无破版（行间空隙实测约 8.3pt、标签-线 13pt、线-文字 9.7pt）。中号卡行距为同一代码路径，因验证机自动化受限未单独走查，待真机顺手确认。
...
## [2026-09-16] 想法图片链路双修：iCloud 原图下载链路重构 + 新建带图想法必崩根治

> 纯 iOS 零后端发版。模拟器三场景回归全过，待真机验收。

### 修复
- **iCloud 原图「一直报错」根治**：原三层兜底在 iCloud 场景实为三重冗余——`loadTransferable` 遇云端原图本就会触发系统下载，前两层失败即系统下载通道不可用（无网/蜂窝受限），第三层同通道必败，用户感受是「干等后直接报错」。重构为探测先行四级链路：禁止联网快速探测（本机已有直接取回）→ 确认原图在云端才弹「正在下载」提示（30s）→ 放行网络正式下载 → 失败后走 `PHAssetResource` 资产资源通道兜底（共享相簿/编辑后变体等 `requestImageDataAndOrientation` 盲区）。下载成功主动收 Toast；失败提示细分为「图片原图存放在 iCloud，但下载失败，请检查网络或连 Wi-Fi」，不再误导「换一张图片」（云端图换一张照样失败）。想法/任务/反馈/AI 聊天四处选图共用同一链路。
- **新建带图想法必崩（SIGTRAP）**：选图 2 秒防抖落库后清空暂存列表（图片转正），清空又触发一次防抖自动保存，「无文字+暂存已空」被误判为空草稿 `hardDelete`，而附件正异步挂向这条已删除的想法——界面刷新读到已失效对象，UUID 强转空值即崩。修复：新增转正在途标志，空内容删除分支遇在途转正保留草稿；转正完成后刷新编辑附件区（顺带修复此前「缩略图中途闪没」）。lldb 条件断点（仅 UUID 空值桥接时中断）抓到 `persistContent → hardDelete` 实锤证据链；对照包判别实验确认该崩溃为既有 bug，与本次加载器改动无关。

### 验证
- 模拟器三场景回归全过：新建+选图（原必崩场景，草稿落库/缩略图持续显示/补文字保存成功）、新建+选图等 8 秒直接保存、编辑已有想法追加图片（双缩略图正常）。
- 编辑与新建双路径对照、不含修复的对照包复现崩，定位过程见崩溃报告 Holo-2026-09-16-19*.ips。

---

## [2026-09-16] iPad 完整可用性改造阶段 0-2：内容宽度契约 + 侧边栏两态 + 财务/AI 工作台

> 方案：`docs/ipad-adaptation/plans/2026-09-16-Holo-iPad-完整可用性改造方案.md`。纯 iOS 零后端改动。

### 新增
- **布局决策契约（阶段 0）**：新增 `HoloLayoutPolicy.swift` 纯逻辑层——三层宽度契约（窗口宽/主内容实得宽/各栏宽）落为可单测函数：侧边栏常驻线（窗口−232≥600）、双栏就绪线（内容宽≥860 = 360 列表 + 440 详情 + 60 间隔）、列表栏 360–520 钳制；standalone 38 断言全绿（`HoloTests/Utils/HoloLayoutPolicyStandaloneTests.swift`）。
- **内容宽度环境 `holoContentWidth`（阶段 1）**：由 ContentView 模块宿主区注入（iPad=窗口−侧边栏实占；iPhone 不注入恒 nil→所有宽屏判定恒 false，手机端零变化）；21 个业务视图的 expanded/双栏/列数/字号档判定全部迁移到内容宽语义，`expandedWidthThreshold` 从全窗口 1024 降为内容宽 860。关键行为变化：13 寸竖屏（1024）常驻侧边栏后 792 不再强行双栏；用户收起侧边栏（60pt 窄条）后同窗口自动重评双栏。
- **侧边栏两态（阶段 1）**：常驻全宽 232pt（图标+文字）↔ 收起窄条 60pt（仅图标，目的地始终可见可点）；窄条底部「展开边栏」、常驻底部「收起边栏」；用户手动选择优先于自动策略（旋转/分屏自动重估但尊重干预），侧边栏形态切换不重建任何业务状态（常驻栈/滚动/筛选/AI 对话保留）。iPad mini 竖屏（744）与分屏窄窗默认窄条。
- **`HoloListDetailSplit` 重写并真正启用（阶段 1）**：由全窗口宽度 46% 死比例改为内容宽驱动+最小栏宽钳制（列表 360–520、详情弹性撑满）；任务列表与想法列表两处手写双栏统一迁移到该组件（组件此前为零调用死代码）。
- **财务账本双栏工作台（阶段 2A）**：宽屏账本左列保持日期/筛选/滚动原位，右侧新增 `FinanceTransactionDetailPane` 记录详情面板——无选中显示本日支出/收入摘要+「轻点左侧一笔交易」引导（无空白占位）；轻点交易右栏展示金额/分类/账户/日期/名称/备注/标签/分期/AI 来源，带编辑/复制/删除入口（复用现有编辑 sheet 与删除确认流），左栏对应行浅色高亮联动；记录被删或日期切走自动回落无选中态（从当日列表实时解析，不展示过期详情）。窄屏/iPhone 行点击仍直接进编辑 sheet（语义不变）。
- **AI 依据/报告侧栏（阶段 2B）**：宽屏（内容宽≥860）下聊天中的深度分析报告与周期回放从全屏盖板改为按需右侧栏（520pt，顶部标题+×关闭），聊天消息与输入框保持可见，追问挂在侧栏内（ReportDetailRoute 同一套追问与财务证据深链）；关闭侧栏恢复全宽聊天列。窄屏/iPhone 维持全屏 cover；跨档旋转时同一份选中状态在侧栏与全屏间无缝迁移。

### 修复
- **任务过滤行右缘硬切**：横向滚动胶囊行补右缘渐隐过渡与右留白，最后一枚胶囊不再被排序按钮直线压切（iPad 双栏窄列下尤其明显）。
- **习惯磁贴列数双扣侧边栏**：`tileColumns` 此前在全窗口宽上再手动扣 232；迁移内容宽语义后修正为只扣内容边距（48），列数判定与真实可用宽一致。

### 验证
- 布局契约 standalone 38 断言全绿；全量编译通过（iPad Pro 13 模拟器 destination）。
- 一轮 QA（两台设备×竖横屏×侧边栏两态 12 场景）：侧边栏两态、单双栏判定逐场景核对全过，宽度政策在任务/想法模块确认全局生效，无崩溃。
- 新词表 9 条三语补齐（收起边栏/展开边栏/本日支出/本日收入/报告详情等）。

### 遗留
- 阶段 3 余项（知识树/长廊/健康/设置宽度复核、键盘指针细节）与阶段 4（双设备 CloudKit 真机验收、`UIRequiresFullScreen` 移除——需先完成动态布局回归）未在本批。
- Cmd+, 在「iPhone 硬件键盘兼容层」与「iPad 菜单栏命令」双轨注册会报一条系统日志（同一动作双注册，功能无损），待后续统一。
- 2C（今天↔任务上下文）经核查现状已满足（看板统一 TaskDetailView sheet + repository 同源数据），无代码改动。

---

## [2026-09-16] 记忆低确认成本方案 P1-P6 全量交付：每日确认任务下线 + 五路决策 + 按需澄清 + 「不再使用」接墓碑

### 新增
- **每日确认任务全线下线（P1）**：`HoloMemoryAttentionPolicy` 成为「是否打扰用户」唯一判定入口，收件箱/确认队列/长廊三处口径统一；「待确认 N 件 / 去确认 / 几十秒清完」任务式 UI 从个人页、聊天、长廊全部移除（开关默认下线，显式写 false 可回滚）；回执改为一次性首启说明「Holo 会根据你的记录逐渐了解你，你可以随时查看或纠正」，旧确认回执不再反复唤起、新观测不再产生任务式确认回执。
- **五路统一决策 v4（P2）**：每条记忆由唯一裁决器路由到「确定事实 / 限定建议 / 继续观察 / 按需澄清 / 丢弃」——用户明确陈述自动记住不再二次确认；结构化有界事实（财务聚合/健康统计）自动采用；单次现象继续观察；高影响无支持推断（疾病/心理诊断类）直接丢弃不能靠点击转正；敏感自由文本与第三方信息默认不形成长期派生记忆（一次聊天≠无限期存储同意）。decision metadata v2 挂载到记录（容错信封：未知枚举原样保留、缺字段保守降级）；旧 v3 采用矩阵降为回滚路径（三套件钉位锁定）；召回通道只放行确定事实、建议通道只放行限定建议（语气强制「从记录看/可能」）；shadow 日志双算只记差异。
- **按需澄清（P4）**：只在当前规划/聊天真正依赖且证据无法自解时，问一个最小问题（复用现有 unknowns 渲染轨道，聊天/Voice/Context Plan 三入口同政策）：单流程最多 1 问、普通入口滚动 7 天全局最多 1 问、同题跳过冷却 30 天（证据实质变化才解锁）；回答默认最窄适用范围，只有明确长期表达才成为长期记忆；「不要使用」直接写语义墓碑。零新增串行模型调用。
- **「观察中」折叠组（P5）**：记忆管理页新增默认折叠、无计数徽章的「观察中」分组（文案：这些内容目前不会用于回答，Holo 会继续观察）；「会谨慎参考」徽章区分限定建议与明确记忆；自动采用不显示为「你已确认」。

### 修复
- **「不再使用」接语义墓碑（P5）**：从「只删除当前记录」升级为「先写语义墓碑再置 suppressed」——后台再次萃取同锚点同命题会被拦截，不能因换 ID 或同义改写复活（与「忘记」路径同构，两处旧契约断言已翻转锁定）。
- **存量迁移可续跑（P3）**：调和器仅在完整成功后推进版本号（修复异常路径误标完成导致迁移断点丢失）；v4 判丢弃的存量候选归档留痕不再滞留待确认；迁移 journal（前后基线快照，仅 metadata）落盘供对账；旧确认回执一次性消化；用户已表态记录分毫不动。
- **压缩治理覆盖观察类（P2）**：observe 类候选纳入 freshness 归档不再无限积累；未确认记录不得借 permanentFact 逃避衰减。

### 工程验证
standalone 4 套件 208 断言全绿；XCTest 桥接全量 **100 套件 0 失败**；冻结评测 21/21 路由全中（含假设语气/引用他人/临时选择/过去状态/第三方敏感五类对抗样本）；四个灰度开关（v4 决策/收件箱下线/按需澄清/shadow）支持热切回滚。纯 iOS 零后端改动，无需 ECS 部署。实施记录与验收清单：`docs/_common/plans/2026-09-16-Holo记忆低确认成本方案-P0-P6实施记录.md`

---

## [2026-09-16] 记忆低确认成本方案 P0：基线与测试护栏（纯工程，零用户行为变化）

### 新增
- **五路决策契约（`HoloMemoryDecisionPolicy.swift`）**：方案 §5/§6/§10 的数据契约落地——来源权威性/证据核验/影响等级/持久化权限/使用权限/打扰策略六维枚举 + decision metadata v2 容错信封（未知枚举原样保留为 `.unrecognized` 不猜语义、缺 key 一律保守降级、未知 schemaVersion 无损封存），P2 决策逻辑的冻结节拍器。
- **21 条五路决策冻结 fixtures**：覆盖主决策矩阵 16 场景 + 假设语气/引用他人/临时选择缺有效期/过去状态写成现在/第三方敏感五类对抗样本；其中「明确记忆请求」「假设语气判别」「引用语气判别」3 条如实标注 `requiresContractProof`——现有字段无法稳定推导，是方案 §13.1 规定的 P2/P4 灰度门禁项。
- **缺陷锁定测试（双通道验证：standalone 61 断言 + XCTest 实跑 2 条 0 失败）**：①「adviceEligible candidate 被收件箱当作待确认」——收件箱口径只看 `state == candidate` 不读 admission，基线快照量化出差值即缺陷规模，P1 修复时连同断言翻转；②「不再使用」只 `deleteRecord` 不写墓碑、同锚点内容可自由重生——与「忘记」路径的墓碑拦截（现行正确行为）形成对照锁定，P5 修复时翻转。
- **基线快照工具（`HoloMemoryDecisionBaselineSnapshot`）**：candidate 的 admission 分布统计 + 记录稳定身份摘要（顺序无关、版本变更可检出），P0 量化现状、P3 迁移前后对账共用，仅记录 metadata 不含记忆正文。
- **四个 feature flag**（`memoryDecisionPolicyV4Enabled` / `memoryDailyConfirmationInboxDisabled` / `memoryJustInTimeClarificationEnabled` / `memoryDecisionPolicyShadowLoggingEnabled`）默认全关，为 P1-P6 灰度与回滚预埋，不改任何现行行为。

### 修复
- **XCTest 桥接脚本两处根治（共享工具链）**：①无法识别的 @main 文件由「整体中止」改为「告警跳过」——在途测试文件不再阻断所有人再生桥接；②只桥接真正挂在 HoloTests target 里的文件，且套件名改从启动器体内的 `套件名.main()` 调用提取（头部/尾部两种启动器形态通吃）——修复全量再生把未挂载套件桥进 target 导致测试构建崩溃的问题，同时补齐 7 个已挂载但漏桥的在途套件。


## [2026-09-16] 健康模块运动会话级分析上线 + 主页阅读动线重排（睡眠优先）+ 硬编码大清理

### 新增
- **运动会话级分析（一步到位）**：HKWorkout 从「每日总分钟数」升级为保留单次明细——每次运动的类型、起止时间、时长、距离、卡路里、平均/最高心率（取自训练自带统计）。日聚合改由会话列表折叠，全模块口径统一。
### 新增
- **运动会话级分析（一步到位）**：HKWorkout 从「每日总分钟数」升级为保留单次明细——每次运动的类型、起止时间、时长、距离、卡路里、平均/最高心率（取自训练自带统计）。日聚合改由会话列表折叠，全模块口径统一。
- **健康主页「运动」指标卡 + 运动详情页**：主页指标行新增运动卡（2×2 网格第三位）；详情页含当日会话列表卡（一行一次训练，点击进明细）、7 天统计与趋势。单次运动明细弹层：六格统计、**心率曲线**（含均值虚线）、**心率五区间分布**（Z1 热身→Z5 极限，按最大心率百分比 220−年龄分区，读不到生日回退 190 并标注估算）、**公里配速**（距离≥1km 才产出防误差放大）、数据来源。心率明细懒加载（仅打开弹层才查询），周统计不拉心率样本。
- **HoloAI 运动分析升级**：`health.workout` 数据集（会话级一行=一次训练）注册进动态目录并接通取数——云端深度分析的工具目录由快照自描述生成，运动数据（含距离/心率/配速口径说明）自动可查，**无需后端发版**；本地 `workout_summary` 增加总距离/总卡路里/平均心率指标与逐次训练证据。顺手修复 `health.energy`/`health.distance` 两个「已注册却取不到数」的断头路（云端快照此前恒为空）。

### 优化
- **健康主页阅读动线重排（第一性原理梳理）**：新序=今天怎么样(三环)→各项具体(指标)→Holo 怎么看(核心洞察)→最近趋势→跨域关联(生活闭环)→身体状态入口→脚注；**「数据源状态卡」只在部分连接异常时常驻**（正常态由头部副标题承载，不再打断内容流）；指标行顺序改为 **睡眠→步数→运动→站立/活动**，与身体分数权重（睡眠 0.45 最重）和今日看板顺序统一，2×2 网格四张卡。
- **7 天趋势卡上移并升级**：从垫底的「仅睡眠」升级为睡眠/步数/运动三指标可切换（默认睡眠）。
- **指标详情页粒度动线**：当日大环→当日结构（睡眠阶段+时间轴 / 步数 24h 分布 / 运动会话列表）→7 天统计→7 天趋势（去掉双层重复标题）→洞察→关联，同粒度相邻不再日/周来回跳。
- **睡眠阶段卡口径标注**：该晚有小睡时标题旁显示「含小睡」胶囊，解决与时间轴卡（只画主睡眠）数字对不上的困惑。
- **模拟器 mock 全量按日种子化**：四指标与运动会话同一天跨启动稳定，QA 截图可比可复现。

### 修复
- **硬编码大清理**：删除无引用遗留组件 HealthMetricCard.swift；移除「授权了但从不读取」的 appleStandTime；模拟器假数据门收紧为 `DEBUG && 模拟器`（Release 模拟器构建不再显示假数据）；体征页整页、洞察链路（状态文案/兜底文案/证据与汇总文案/日期格式）等约 90 处硬编码中文迁移三语词表；趋势图星期标签写死 zh_CN 的 locale 移除（英文用户曾看到「周四」）；运动类型名界面层本地化（AI 证据摘要保持中文口径分离）。
- **口径常量集中（单一事实源 `HealthThresholds`）**：目标值（1万步/8h 睡眠/12 站立/30 分钟）、睡眠质量 7h/6h 分档（看板与详情页两处）、AI 工具裸写的 8h/6h、14 天分析窗口（原 4 处重复定义）、异常阈值（<3000 步/连续 3 天）、咖啡关键词补英文 "coffee" 匹配。
- readTypes 新增 `.heartRate`（心率区间/曲线）与生日特征（最大心率分区），权限文案已覆盖运动无需变更。

### 测试
- 新增 HealthWorkoutTests 10 条（会话折叠/配速换算与格式化/区间分桶含间隔封顶/最大心率估算回退/mock 按日稳定）+ 看板快照顺序与三环徽章口径测试；周边睡眠/活动分析器回归全绿；QA 无头模拟器两轮走查（中文动线+英文冒烟）。

---

## [2026-09-15] 习惯「累计打卡次数」总账上线 + 坏习惯连续天数假 3,650 根治

### 新增
- **习惯详情页「累计」格**：统计摘要区新增第四格「累计」，永不归零的全历史总账——连续天数会断签清零、月度统计每月清零，唯独累计只增不减；且不随详情页时间范围选择器变化（切「近7天」看近期表现时，累计始终是总账）。口径：打卡型=历史已完成次数（取消重打不双计、补签计入）；计数类=历史数值总和（与统计页当月累计同口径）；坏习惯与测量类不显示（坏习惯看「连续克制」，常驻犯错总数体验不好）。
- **习惯磁贴「累计X次」文字徽章**：磁贴名称旁（火焰连续徽章右侧）直书「累计X次」，打卡成功的瞬间数字即 +1，不用点进详情页；一眼区分「连续」会断、「累计」一直在。首次实现用 ∞ 图标被东林否决（普通用户看不懂），改为直书文字。

### 修复
- **新建坏习惯 0 记录时连续天数显示 3,650 天（走查发现，东林拍板根治）**：连续天数逐天回查没有设起点，坏习惯口径是「没打卡=克制成功」，新习惯一天记录都没有就从当天一路"克制"到十年前的回溯上限。修复=回溯物理下限压到习惯创建日（创建之前不存在「克制」），每日与周/月频率两处同根因一起修（周/月同样会凭空累计到 520 周/120 月上限）；好习惯计算路径零改动。

### 验证
- 新增 15 条口径单测（累计 8：取消不算/补签计入/数值求和/iCloud 重复行不双计/坏习惯测量类不显示；连续 7：创建日封顶/犯错中断/周月不凭空累计/好习惯三场景不回归）+ 习惯既有 19 条回归全绿；隔离 worktree 全量构建绿；模拟器 QA 两轮：详情页四格与切范围累计不动、磁贴四类习惯徽章形态、打卡即时 +1、布局无挤压、全程无崩溃。纯 iOS 改动、无后端发版；待东林真机验收。

---

## [2026-09-15] 记忆长廊两修复：周视图「0–7」折叠按钮误翻页根治 + 轴视图无法滚动根治

### 修复
- **周视图点「0–7」折叠按钮莫名横滑一天、多点几次才生效（东林报）**：横向翻页手势挂在整个泳道卡片上，把「0–7」按钮/日期头/凌晨摘要按钮全盖住——点击时的轻微触点位移被 DragGesture 抢走致按钮点击被吞；且快速点击的 predictedEndTranslation 预测位移可达 44pt+，被误判成翻页指令。修复=翻页手势只挂网格滚动区（按钮区彻底无翻页手势，点击 100% 可靠）+ 翻页判定加实际位移 ≥24pt 门槛（预测位移再大也翻不了页）。
- **轴视图无法上下滑动（东林报；实测快慢滑全失效、仅程序化滚动有效）**：「长按空白拖动建任务」的 SwiftUI LongPress+Drag 序列手势挂在 ScrollView 内容上，在 iOS 26 上把滚动通道整体压死——highPriorityGesture/simultaneousGesture/收紧 maximumDistance 三种姿势均无效（两轮 QA 复现 + 手势隔离实验实锤：摘掉手势滚动立刻恢复）。修复=手势下沉改用 UIKit UILongPressGestureRecognizer（TimelineLongPressBridge 按小时行铺在空白背景），与滚动 pan 天然并行：手指一动长按即失败、滚动不受影响，静止 0.5 秒才成立进入建任务；长按成立期间 scrollDisabled 临时锁滚防「边滚动画选区」。任务块/日程块/折叠带热区在上层照常命中。
- **建任务保存返回后轴不即时刷新（修滚动时新暴露的潜伏问题；建任务手势此前从未走通，故首次被走到）**：轴数据为 @State 仅在出现/切日期/切折叠时加载，表单关闭不重拉。修复=表单关闭（newTaskDraft 置空）时重新加载数据。

### 验证
- 全量编译绿；模拟器三轮 QA：周视图连点「0–7」3/3 生效、日期列头零横移、横滑翻列与纵向滚动正常；轴视图快/慢/反向滑动全通且无误触「建任务」提示条，长按拖选→选区→表单→保存全流程走通（保存后任务块即时出现、重启仍在）；任务块点击/凌晨折叠展开收起/「此刻」回正全过。纯 iOS 改动、无后端发版；待东林真机验收（遗留观察：输入法组合态未上屏直接返回的保存路径轴不刷新，低概率；凌晨展开后视口跳回当前小时、「接下来」条常驻为既有设计待拍板）。

---

## [2026-09-15] 今日看板两修复：「今天的安排」点任务行闪退根治 + 「＋开始一件事」补 AI 引导语

### 修复
- **「今天的安排」点任务行闪退（东林报：点一次被弹回、再点必闪退）**：新版今日看板是全 App 唯一把任务详情页整页推入的入口，而详情页按弹窗设计、自带下拉关闭守卫（sheetDismissGuard）——推入场景下守卫劫持了整个今日看板的关闭代理，首次点击详情刚打开即被「保存并关闭」弹回；二次点击时导航栈已空、removeLast 无保护直接崩溃。修复=任务详情回归全 App 统一的弹窗卡片呈现（TasksView/旧看板同款）：Dispatcher 新增 taskDetailSelection（sheet 仅持任务 id、呈现时取最新对象），删除 taskDetail 推入路由与崩溃点。
- **「＋开始一件事」跳 Holo AI 无引导（东林报；方案 §5.2 原有要求实现遗漏）**：跳转前预填引导语「我想开始一件事：」到 AI 输入框（只预填不自动发送，补后半句发送即走情境规划→方案卡→加入进行中的事），三语文案入库；ChatView 补 onChange(of: prefillText) 兜底消费——AI 页常驻、.task 只跑一次，页面已存在时外部预填原不生效（看板快捷记录同通道一并受益）。

### 验证
- 全量编译绿；模拟器走查：弹窗形态/反复开关 3 轮无闪退/弹窗内删除任务正常/预填且不自动发送/二次预填全过，全程无崩溃。纯 iOS 改动、无后端发版；待东林真机验收（遗留观察：一次未复现的「关详情连带关看板」判为测试工具点击重复投递误触）。

---

## [2026-09-14] 「今天」看板 Matter 化 + 首页光球素四定稿——从模块汇总页升级为行动优先的每日工作台（T0-T5 全链交付）

### 新增
- **「现在最值得推进」确定性主行动**：新增 HoloTodaySnapshot 统一快照（冻结日边界、局部失败不空白）与 HoloTodayFocusResolver 纯函数排序器（P0 进行中日程→P1 90 分钟内→P2 逾期/atRisk Matter 已确认动作→P3 今日到期→P4 加入今日→P5 习惯窗口；stale 投影抑制、linkedTask 同实体去重、稍后仅会话级降级），页面打开零网络零模型请求。
- **「今天」行动优先新页**（todayCommandCenterEnabled 默认开，关闭即完整恢复旧看板）：今日判断+主行动卡（查看任务/加入今日/查看日程/和 Holo 梳理四类真实出口）→ 进行中的事（0 件不隐藏，1 完整卡+多件紧凑行，右上全部 N 件）→ 今天的安排（日程+任务统一序列，Matter 轻标签）→ 保持状态（习惯/健康紧凑一行可展开）→ 今日概况（支出失败显示 -- 不误报 ¥0）；iPad 双栏、 loading 骨架、VoiceOver 顺序、Reduce Motion 兼容。
- **方案卡单项即时加入**：每条建议旁直接「加入待办」，真实落库即回执；回执升级 V2（带真实任务 ID+来源键，跨重启幂等、按 createdAt 淘汰、任务被删后如实降级「原任务已删除可重新加入」）；删除「无日期条目≥2 自动并成主任务」的旧规则；多条目仍按方案主题归清单。
- **任务-Matter 双向补链**：创建后即时链、激活后按 V2 回执补链（两种顺序均闭环）；loop→task 以链接行 sourceRevision 记 loopID 真实对应，不用标题猜；新增 HoloMatterLinkedEntityChangeCoordinator 监听类型化任务变更（完成/删除/改期/归档），自动推进 Matter revision 与投影。
- **Matter 普通回答带背景**：scoped Chat 生成前注入当前 Matter 最小快照（confirmed 与 suggested 分区、stale 不注入旧内容、上限 10 条）；请求级冻结 matterID，回答期间切换事项迟到对账不串。
- **Next Action 变成可执行对象**：openLoopAction 携带真实 loop entityID、已链任务升级 linkedTask；详情页与今天页共用同一动作解析；全部 Matter 详情入口（首页/列表/今天/方案卡）「和 Holo 讨论」统一接线；Open Loop 省略号改为点按即弹菜单。

### 变更
- **首页光球素四定稿（东林逐轮拍板）**：品牌渐变零改动，环线细化降调（白弧 0.55/0.38/0.26）、删高光层与末端光点，状态摘要「今天有 X 项安排」以弧形铭文嵌入球内下环带（ArcInscriptionText 逐字弧排）；摘要绝不压球心、球外无文字。探索稿（月相钟/行星卫星/天气球/显影文字/素一二三）存档 docs/homepage/design-today-orb/。
- 首页导览文案更新（「从今天开始」）；旧看板组件全部保留供灰度回滚。

### 验证
- 新增 XCTest 39 项（Resolver 20 + ContextPlan 单项 11 + 补链 8）与 UI 纵向切片走查全绿；Matter/Todo 回归 55 项全绿；standalone 9 套件 343 断言全过；全工程 Debug 构建绿；模拟器实装走查通过。纯 iOS 改动、无后端发版；真实日本旅行 16 步旅程待东林真机验收（§13.5）。

---

## [2026-09-14] 睡眠「整晚睡眠时间轴」重设计——小睡不再混进夜晚，条带回到真实时间轴（东林报「时间分布乱七八糟，别人也看不懂」）

### 修复
- **白天小睡被并入夜晚**：归晚窗口（前一日中午→当日中午）把窗口内所有睡眠一律视为一晚，白天午睡混入后「入睡」显示 12:00、时间轴被拉成 20.5 小时、真正的夜睡眠被挤到半张图；新增睡眠会话切分器（睡着段间隔 <3 小时归同一觉，最长者为夜间主睡眠），时间轴、入睡/起床/夜醒/REM/深睡集中全部只算主睡眠，小睡单独一行灰字带过（「白天另有小睡 1 次 · 约 1 小时 20 分，未计入上图」）。
- **条带与标注两套坐标系**：分段色条原是首尾相接顺铺（无样本空隙被跳过），夜醒红字却按真实时间位置飘——图和数字各说各话；重写为按真实比例定位、无样本处如实留白，删除全部无解释红字标注。
- **口径自相矛盾**：小睡把「夜晚中点」算歪后，深睡集中算出 0%，统计行当「无数据」显示「—」、解读文案却下结论「偏后半夜」；统一为 0% 是合法值（深睡确实全在后半夜），仅真正无数据才显示「—」。
- **细条纹与假宽度**：几秒的微小样本被强制画成 2pt 宽制造假条纹；改为同阶段相邻样本细缝自动缝合、<40 秒清醒段不再单独画。

### 优化
- **卡片信息重排**：入睡/起床两个时刻带标签放在条带两端（取代裸数字），整点刻度（00:00、02:00…）取代看不懂的「中点时刻」，新增五色图例（深睡/核心/REM/清醒/在床），统计行改为夜醒/REM/入睡耗时/深睡集中四项（入睡/起床不再重复）；起床时刻按真实离床点计（赖床在床段如实计入，不再截断）。
- **夜醒口径修正**：主睡眠内清醒 ≥2 分钟才计一次，末次醒来后的赖床清醒不算夜醒（那是起床）；真机与模拟器共用同一构建器，口径永不打架；AI 健康工具的入睡/起床字段自动继承主睡眠口径（顺带修正「入睡 12:00」混进 AI 数据的问题）。
- **繁英本地化补齐**：时间轴卡 24 条文案补齐繁体/英文（此前繁英用户看到的是简体兜底）。

### 验证
- 睡眠相关三测试套件 22 项全绿（新增会话切分器 9 项：小睡切分/3 小时聚类/尾巴链式吸收/末醒锚点等）；全工程 Debug 编译绿；模拟器走查 9 项全过（mock 含小睡夜晚：入睡 23:47/起床 08:15/夜醒 2 次/REM 4 段/小睡行/刻度/图例逐项核对，期间发现起床被尾巴截断规则误报为 07:47，已改链式吸收并以真实离床点复验通过）。纯 iOS 改动，无需后端发版；待东林真机验收。

---

### 修复
- **悬浮「＋」不再无视选中日期**：账本页右下角「记一笔」打开时原来从不带日期、一律默认今天——先点了昨天的格子再点＋，账就记到今天；现改为账本 Tab 把当前选中日期传给记账弹层（presetDate），选哪天默认记哪天；账户 Tab 无日期语境保持默认今天，避免把残留选中日带进新账。

### 优化
- **带预设日期的入口键盘默认收起**：从日期格进入记账（选中格子点＋、长按格子快速记账）时数字键盘默认收起，打开即完整表单，预填日期一眼可核对——原先键盘自动弹出盖住日期行，日期被带错也不易察觉；其余入口日期固定是今天，保持键盘先行的快速记账路径不变。

### 验证
- 模拟器实测三入口通过：选 13 号点＋→「9月13日 星期日」；长按 12 号→「9月12日 星期六」；无预设入口键盘照旧弹出且为今天。顺带核实上轮「键盘挡住日期行且滚不出来」系测试操作误报，表单滚动正常。纯 iOS 改动，无需后端发版。

---

## [2026-09-14] 任务清零庆祝卡文案不再绑定时段——早上清零不再说「享受你的夜晚」（东林截图指出）

### 修复
- **庆祝卡副行写死晚安话术**：任务页「今天全部完成」庆祝卡副行原写死「享受你的夜晚吧」，早上 9 点清零也照说；改为不绑定时间的普适句「今天剩下的时间都是你的」，凌晨（0–6 点）单独致意「夜深了，早点休息」；有次日任务的「明天还有 X 项等着你」预告分支不变。
- **时段口径收拢为一份**：新增共享 DayPeriod 定义（CalendarHelpers，凌晨/早上/下午/晚上四段边界唯一定义），首页与看板问候语改为同源取用（行为不变），后续任何「按时段说话」的文案都从这里取时段，不再各写各的。

### 验证
- 模拟器全工程 Debug 编译绿；问候语单元测试 8/8 绿（含全天段边界覆盖用例）；繁体/英文翻译同步补齐、废弃键清除。纯 iOS 改动，无需后端发版。

---

## [2026-09-14] AI 建任务自动归清单——主题任务不再散在「全部」（东林反馈「日本旅行任务全堆在全部」）

### 新增
- **对话建任务自动归清单**：AI 判断任务主题（如旅行、装修等多件事准备）时填 listName，App 先匹配已有清单（精确 > 唯一模糊命中），无命中自动创建并回执「已创建清单「X」并放入」；单条琐碎待办（买东西、取快递、缴费）不建清单，防止清单泛滥。
- **情境规划落卡归组**：方案卡一键落任务（≥2 条）按方案标题归入清单（同样先匹配后创建），不再散在「全部」；单条落卡保持默认位置。
- **判定与执行分离**（沿用「AI管归组、算法管判定」拍板）：AI 只负责取原话主题词，匹配/创建/名字清理（剥引号限长 16 字）由本地 TodoListNameResolver 纯逻辑承担，可独立测试。

### 修复（后端提示词，需发版——已发版）
- create_task 提示词原句「指定清单填 listName（已有清单名）」无示例、且限定「已有」，模型基本从不填写（清单归组链路三断点之首）→ 改为「主题性待办填 listName，取原话主题词，App 自动匹配或建清单；单条琐事不填」；以置换示例策略通过 6200 字符防膨胀红线（删山姆购物示例、并入带 listName 的旅行示例，一份字符预算教两件事）。

### 验证
- 后端全量 404/404 绿（含意图注册表跨端一致性对拍、提示词长度红线、评测语料覆盖）；已发版生产并容器内 grep 验证新提示词生效（220ce02fc，分支 1.0.4）。
- iOS 新增 9 单测绿（清单名清理/精确命中/唯一模糊/多命中新建/垃圾输入落默认/自动创建/二次同名复用不重建成）+ 既有计划时间段回归 14/14 绿；全工程 Debug 编译绿。

---

## [2026-09-11] Matter「进行中的事」首版纵向切片 M0–M3（1.0.4 大需求第一刀）

### 新增
- **「进行中的事」数据底座（M0）**：HoloMatter/HoloMatterOpenLoop/HoloMatterLink/HoloMatterEvent 四实体（ID 逻辑外键、随现有 Core Data/CloudKit 链路同步）+ 唯一写者 HoloMatterRepository（原子事务/幂等/冲突优先级：用户显式 > 业务回执 > 系统确定性 > 模型推断）+ 生命周期/进行阶段/关注状态三维正交状态机 + 五级灰度开关（默认全关，依赖只能逐级开启）。
- **方案卡 → 事情激活（M1）**：情境规划方案卡新增「开始整理」入口；只校对标题与日期（未知日期不默认今天）；同来源重复点击幂等返回同一件事（红线：重复 Matter=0）；命中同名已有事项让用户选「更新到已有的/仍新建」；建立后原位变绿色回执可进详情。
- **事情详情页（M1）**：固定阅读顺序八区——标题/HOLO 判断（可溯源）/现在最值得做（最多一条，无依据不凑数）/还没解决（已确认实线、AI 猜测虚线弱化永分离）/已经解决/相关内容/最近变化（事件流可撤销）/和 Holo 讨论这件事。完成/归档必须用户亲手确认（模型永不可触发），完成后退出首页焦点位、可重新打开不建副本。
- **首页焦点卡（M3）**：插在今日日程条与今日看板入口之间；多件事只展示关注最高的一件，其余进「查看全部」列表；小屏自动降为紧凑一行条；没有进行中的事时整块消失。关注状态由确定性规则计算（日期窗口/已确认问题缓急），AI 只解释原因不能决定颜色。
- **事情内对话与持续对账（M2）**：详情页「和 Holo 讨论这件事」带显式 matterID 进聊天（顶部胶囊可退出，退出后不泄漏上下文）；「酒店订好了」唯一对应时自动更新+绿色回执可撤销；歧义（东京/京都）弹选项追问绝不猜；对账失败/429 不影响普通聊天、不写假状态；迟到 proposal（revision 过期）一律拒绝。
- **后端 matter_reconciliation purpose（M2，需发版）**：独立路由/限流桶/计费口径，归 chat 配额池；typed proposal JSON 契约 + 注入防护（输入是数据不是指令）；metadata-only 日志强制集（标题/问题原文永不落日志）。
- **合成数据种子**：`-MatterDemoSeed` 启动参数（Debug 双门禁）一键种出「国庆日本旅行」完整演示态，模拟器走查可重复。

### 修复（实施过程测试逼出）
- 撤销按钮把 openLoopID 误当 eventID 传给回滚接口导致静默失败（数据级取证定位）；
- 完成/归档确认弹层遗漏未实现（原型转码丢失）；
- 详情页底部固定按钮遮挡内容（safeAreaInset 改 VStack 分区）；
- 事件流文案泄漏 UUID（payload 补可读标题 + 展示层兜底）；
- iOS 26 Menu 菜单项 XCUITest 点击失效（完成提为头部直接按钮，产品上也更顺）。

### 验证
- iOS 单测 36/36 绿（隔离内存库）；后端 Matter 测试 11/11 绿 + 既有回归 59/59 绿；全工程 Debug 编译绿；XCUITest 纵向切片全绿（焦点卡→详情→撤销回滚→完成→焦点退出）。
- 新增文案三语（简中/繁中/英文）已全量补齐主 App 与小组件词表。
- 未完成：CloudKit Production schema 部署（真机上报 Development 后进行）、真机验收、240 组模型留出评测（内部灰度前门禁）。

---

## [2026-09-11] 1.0.3 发版前置：CloudKit Production schema 增量部署完成 + 开发包同步判定修复（运维+一修）

### 改动
- **云端数据库结构增量部署到生产环境**：本次随 1.0.3 上线的新字段/新实体全部就位——情境规划的规划运行态（聊天消息新字段）、想法主题 V3（主题/想法/关系表）、目标 v3、事物事项、人生计划、记忆锚别名与墓碑、洞察反馈、重复规则、回收站批次、分类归纳规则等 18 个新实体+33 个既有类型的加字段；程序化核对部署差异**净删除为零**（只有新增，存量数据不受影响），已在生产环境复核关键字段就位。到此 1.0.3 的发版硬前置全部解除。
- **顺带修复一个同步判定缺陷**：开发签名包的云端服务声明常写为通配符，此前的检查只认字面「CloudKit」导致这类包被误判为「不支持同步」、静默退化为纯本地存储；现通配符同样放行。正式版不受影响（走另一分支）。

### 备注
- 模拟器按设计永不启用 iCloud 同步（防测试数据污染真实云端），因此 schema 上报必须由真机跑一次最新开发包完成；本次已通过东林真机完成上报（部署记录见 docs/appstore-preflight/CloudKit-schema部署指南.md）。

---

## [2026-09-10] 卡顿专项第二批+多语言文案补齐（纯 iOS）

### 改动
- **图片消息气泡磁盘探测缓存**：聊天气泡每次重画都会探测一次图片文件是否存在，全页重算被放大成对每条消息的磁盘扫描；现按文件存在性缓存（含否定结果），重绘不再碰磁盘。
- **回放卡文本清洗缓存**：周期回放洞察卡一屏多张、每张十几个文本段，全页重算时逐段重跑正则清洗；现按原文缓存清洗结果（纯函数，同输入恒同输出）。
- **新文案三语补齐**：本版本各功能（健康颗粒度/情境规划/截图识别/小组件互动等）新增的全部用户可见文案补齐简中/繁中/英文词表，含小组件词表；顺带入库 1.0.3 提审截图素材（v2）与视觉识别评测记录。

### 边界
纯 iOS；两处缓存均为纯函数缓存，无行为变化。

---

## [2026-09-10] 想法同步副本根治——想法域修复引擎+读路径去重收口（纯 iOS）

### 背景
库迁 App Group 后 iCloud 同步产生过全量副本，此前已根治财务/习惯/通用域，想法域因数据关系最复杂（标签、主题归属、图片附件、引用、任务链接都挂在想法上）留到最后收口。副本表现为：想法列表出现重复卡片、标签重复、同一主题里同一想法出现两次。

### 改动
- **想法域专用修复引擎**：副本合并保留最早原件；副本身上的标签/主题归属并回保留项；同 id 的挂子行（标签挂载/附件等）随副本级联删除，副本独有的改挂到保留项名下（否则陪葬）；引用关系双侧改指保留项；任务只保链接不删任务行；内容有冲突的保守保留不自动合并；重复执行幂等不误伤。
- **读路径去重**：想法/主题仓库全部列表查询按 id 去重，修复器清理存量前界面也不再显示重复。
- 挂入启动修复链（与财务/习惯同一条链路），发现即清。

### 验证
- standalone 测试十场景全过：副本合并、标签主题并回、同 id 挂子行随删、独有挂子行改挂、引用双侧改指、任务链接保留、内容冲突保留、同步身份延迟、未保存编辑、幂等。
- 纯 iOS，无后端发版、无 schema 变更。

---

## [2026-09-10] 个人情境规划一致性与可信交互七阶段（iOS 侧，后端已发版）

### 背景
情境规划在真实使用中暴露五类一致性问题：同一句话两次规划结果不同（后端路由抖动）、生成失败只有含糊报错、退出聊天再进来草案卡变空白或永远转圈、方案卡的「依据」点不回原记录、日期表述有歧义且默认全勾选保存。

### 改动（七阶段）
1. **路由稳定**：AI 解析结果携带后端路由来源（稳定层/模型分类），确定性路由不再被模型分类覆盖。
2. **类型化失败**：规划失败按失败类型给明确文案与下一步指引，不再一律「规划失败了」。
3. **持久运行态**：规划进行中的阶段状态落库——草案未出也显示真实进度（识别/取数/生成），退出重进、冷启动都恢复；运行卡可主动停止；彻底告别「三个点转圈到天荒地老」。
4. **云端异步**：情境规划云端任务通道（轮询+状态流，内部账号灰度，生产用户仍走同步路径）。
5. **证据回源**：方案卡「查看依据」每一条可点回原记录（任务/财务/习惯/想法/健康/日程六域深链），逐条核对。
6. **日期三态**：待办条目日期明确「今天/具体日期/无日期」三态显示，默认不再全部勾选，无日期条目给计数。
7. **可信表达**：校验器拦截两类虚报——「把缺失数据当零」与「无依据的个性化判断」，发现即阻断不进方案卡。

### 验证
- 后端 366/366 全绿×2 + 生产 31 条语料路由 31/31 实测（已随 579953b17 发版生产）；稳定器同句 100 次回归一致。
- iOS Debug+Release 双构建绿；规划单测 43 条（新增 9 条）全绿；端到端场景 169.9 秒完整走通。
- 验收清单：docs/_common/plans/2026-09-09-HoloAI个人情境规划一致性与可信交互实施方案/验收清单-20260910.md（可逐条对照真机验收）。

### 边界
- 聊天消息新增本地字段（规划运行状态存档，轻量迁移，老数据自动兼容）。
- 云端异步仅内部账号灰度；云规划冷启动续跑=诚实报错+可重试。
- 发版前置：需 CloudKit 增量部署聊天消息实体新字段（与财务项目批次同流程）。

---

## [2026-09-10] 云端深度分析纳入健康数据——方案B落地（iOS 侧收尾，后端已发版）

### 背景
此前「涉健康的问题只走本地分析轨道」，但本地分析是多轮工具调用、容易失败；锁屏/离开 App 时的云端深度分析因快照不含健康数据，回答不了健康问题。东林拍板方案 B：健康域以日聚合摘要进云端快照。

### 改动
- 云端深度分析快照纳入健康域：睡眠（含结构富字段：阶段/效率/入睡起床/夜醒）、步数、站立、活动、能量、距离的日聚合摘要；沿用快照密文上送+完成即焚通道，仅日聚合行，不含分钟级原始样本。
- 隐私说明弹层文案同步更新：明确「深度分析与周期回放上传的数据都包含健康与活动摘要（每日汇总，不含原始明细）」。
- 沿用既有 v2 隐私同意（健康与活动摘要授权），不新增授权项。
- 后端侧已随 9-10 发版上线：云端健康分析日志改 metadata-only，不落健康数据明文。

### 验证
- 模拟器走查通过；涉健康问题不再依赖本地轨道，锁屏云端分析可正常回答。待真机验收。

---

## [2026-09-09] 健康模块颗粒度升级一期+二期——睡眠结构特征+时间轴/分布/体征三张新卡（纯 iOS）

### 背景
健康数据此前只读到日总量（睡眠总时长、步数总数），HealthKit 底层的分段明细（睡眠各阶段、小时级步数）没有用起来；页面侧睡眠详情只有总时长，静息心率/HRV/呼吸频率等体征没有专属页面。

### 改动
一期（数据底座 + AI 工具链）：
- 睡眠结构特征四项进 AI 查询指标目录：REM 段数（合并重叠段+过滤 5 分钟碎片）/REM 潜伏期/深睡前半夜占比/入睡潜伏期。
- 三个新数据集：小时级活动分布（最长静坐/活动窗/晚间占比，白天窗 8-21 时）、能量（千卡）、距离（公里），本地与云端分析同源同结构。
- 每个字段带「数据身份证」：口径说明（需 Watch / 缺失≠0 / 看 7 天趋势 / 与自己比），AI 引用时如实交代数据边界，不夸大。

二期（页面可视化）：
- 睡眠详情三态：有分期=四色睡眠时间轴卡（夜醒标注/统计行/结构解读）+分阶段结构；无分期有时长=引导卡（说明如何获得分期数据，不再整卡隐藏）。
- 步数详情新增 24 小时分布卡：24 柱+最长静坐红色高亮带+活动窗/峰值/晚间占比。
- 新增「身体状态」体征页（健康看板卡片进入）：静息心率/HRV/呼吸频率 30 天曲线+个人基线虚线+偏离标注。

### 验证
- 新增 3 个单元测试套件（睡眠结构/活动分布/时间轴构建）全绿；XCUITest 四屏走查（看板入口/时间轴卡/分布卡/体征页）全绿+截图目检通过。
- 观察项：HRV 偏离统一用红色表示，后续可按指标语义分色（待东林意见）。

### 边界
纯 iOS 改动，无后端发版、无数据迁移；健康授权新增读取静息心率/HRV/呼吸频率三项，需在系统健康授权里允许后生效。

---

## [2026-09-10] 桌面小组件「想法随机漫步」标签排版修复（纯 iOS）

### 背景
东林真机反馈：大号「想法随机漫步」卡片正文较短时，标签胶囊（#短期记忆 等）被顶到卡片最底部，与正文之间空出一大截，视觉上脱节。

### 根因
布局里正文之后插了 Spacer 把标签钉底——标签是正文的附属信息，跟正文走才对；钉底只适合「回到那天」这类页脚落款。

### 改动
- `HoloWidgets.swift`「想法随机漫步」：标签改为紧跟正文时间下方（大号 12pt / 中号 10pt 间距），「回到那天 →」+分隔线仍钉在卡片底部作落款；大号与中号两规格一并修正（同源问题）。
- 顺带全量排查其余 7 个小组件（语音启动/快捷控制台/本月收支/今日待办/今日习惯/目标进度/纪念日倒数）全部尺寸与锁屏配件：无同类「附属信息脱节」问题；待办进度条、目标预测脚注、纪念日签名句等底部元素属页脚设计，判定不改。

### 验证
- HoloWidgets target 编译 BUILD SUCCEEDED（iPhone 17 模拟器）。
- 待东林真机验收（重新添加小组件或等刷新后看新排版）。

---

## [2026-09-10] 品牌升级：slogan「人生数据库」全链路落地

### 背景
东林定稿品牌 slogan「人生数据库」。全仓库盘点后替换各处旧定位句（「你的个人数据助理」「让每天的记录，长成一个真正懂你的 AI」「HOLO · 记下此刻」等），统一为新口径。

### 改动
- **App 内**：新人引导首屏定位句、AI 聊天页新用户开场改为「你的人生数据库」；「关于 Holo」页新增 slogan 落款位（此前空白）；想法分享卡品牌尾注「HOLO · 记下此刻」→「HOLO · 人生数据库」，并修复 HOLO/Holo 大小写两个版本并存的旧问题（统一为 HOLO）。
- **AI 人格**：persona preamble 与 chat 主身份句轻改（「你是 Holo——用户的人生数据库，也是陪伴用户的生活助理」），陪伴语气与全部安全边界不动；后端 defaultPrompts.json、iOS PromptManager 镜像、PROMPT_GUIDELINES.md 三处同步。**需后端发版生效**。
- **官网**：页面标题、首页大标题改为「Holo · 你的人生数据库」，原主句降级为价值副句保留；HoloAI 模块标签与页脚落款同步。
- **App Store 提审材料**：简中/英文/繁中三语的宣传文本、简短描述、完整描述写入「人生数据库」口径；App 名称与副标题保留 ASO 关键词不动（拍板：品牌句不占搜索词位）。两份提审材料口径不一致问题一并对齐（旧材料加批注指向新口径）。
- **内部文档**：README（已有）、CLAUDE.md、HoloPRD 产品愿景与一句话定位统一口径。

### 验证
- 全量编译 BUILD SUCCEEDED（iPhone 17 模拟器）；改动 Swift 文件逐一语法解析通过。
- 词表 JSON 合法性校验通过，新 key 三语（简中/繁中/英文）齐备，旧 key 无残留；iOS 测试目录无旧文案断言依赖。
- 后端 prompts 测试 24/24 绿（含 persona 注入断言同步更新）。
- 提交纪律：主词表文件与他人 iCloud/健康在途改动交错，按 hunk 外科手术只暂存本次改动（50 行），他人工作区改动原样保留。

---

---

## [2026-09-09] iCloud 同步「假成功」根治 + iCloud 空间满专门提示 + 同步诊断页（纯 iOS）

### 背景
东林定位到真实用户案例：iCloud 存储已满、上传全部失败，App 却提示「同步成功」。根因：同步状态只看「同步引擎事件结束」、不校验事件是否带错误，失败时照样写「已上传本机数据」并刷新「最近同步时间」；错误只存内存（重启即丢、被后续下载成功清掉），且全仓库无一处代码认识「配额超限」（quotaExceeded）错误码。

### 改动
- **失败绝不写成功状态**：事件以错误结束时不再刷新最近同步时间、不写「已上传」类文案；副文案在有未解除错误时优先显示「最近同步失败」。同步错误与状态文案持久化，App 重启不丢。
- **错误清除规则定死为「仅上传方向成功」**：下载成功只说明云端→本机通了，不代表积压数据传了上去，不会误清上传错误（含配额满）。
- **iCloud 空间满专门识别与指引**：逐层拆解错误（含 partialFailure 包裹、underlyingError 链）识别 quotaExceeded；命中时错误行显示「iCloud 空间已满，本机数据未上传」，并展示清理路径指引（设置 → 顶部你的名字 → iCloud → 管理账户存储，清理后自动恢复上传）。手动同步的探针反馈不再让用户干等 12 秒超时：上传失败立即报真实原因，配额满时明说「重试解决不了」。苹果无公开 API 查询剩余空间，只能事后如实上报，此为平台限制。
- **同步诊断页**（设置 → iCloud 同步 → 同步诊断）：当前状态摘要（账号状态/CloudKit 能力/最近成功/最近失败/当前错误）+ 最近 20 条错误流水（方向/时间/CloudKit 错误码/原文，仅存本机），用户报障时可一眼分清配额满、未登录还是网络问题。
- **修复既有误导文案**：账号状态检查成功不再清除同步错误；签名未启用 CloudKit 不再进「最近错误」行（能力缺失≠同步失败）。
- 新文案 26 条补齐简中/繁中/英文三语。

### 验证
- 单测 13 条全绿：错误识别（直接/partialFailure 包裹/underlying 链/取消/非配额）、错误流水截断（上限 20 条）、状态机（失败不写成功时间与文案、import 成功不清上传错误、export 成功才清错、系统取消不算失败、重启恢复错误状态）、进行中事件只更新进度。
- iOS Simulator Debug 构建通过（主 App + 小组件 target）；新支撑文件已挂 HoloWidgets 编译清单（与 ICloudSyncStatusService 同域共享）。
- 模拟器走查：设置页 iCloud 卡片新增「同步诊断」入口渲染正常，诊断页（状态摘要 + 错误记录空态）布局正常、可正常关闭。配额满的完整链路需真机触发（模拟器无 CloudKit），行为由单测锁定。
- 边界：纯 iOS 改动，无后端发版、无 CloudKit schema 变更、无数据迁移；数据本身始终安全存本机，本次只是让「没传上去」如实可见。

---

## [2026-09-09] HoloAI 对话滚动与键盘贴底体验优化（纯 iOS）

- 点击输入框会立即回到最新消息，并在键盘完整升起过程中持续贴底，不再停在旧位置或需要二次滑动。
- 短对话始终贴近输入栏；最后一条消息与底部保持稳定留白，返回最新按钮出现时也不再改变列表高度、引发内容跳动。
- 消息列表改用确定性布局，避免复杂卡片离屏后因高度估算出现大片空白；键盘改变可视区域时使用变化前的位置判断，只跟随原本位于底部的用户，不打断正在浏览历史消息的用户。
- 验证：滚动 standalone 回归测试通过，iOS Simulator Debug 构建通过，`git diff --check` 通过。完整 Xcode 测试因测试 worker 未能启动而中断，未将其计为通过；待真机手感验收。
- 边界：本次仅调整 iOS 聊天交互，无后端改动、无数据迁移。

---

## [2026-09-09] 修复启动后个人情境提取遇重复来源导致闪退

- 真机多份崩溃日志指向 `HoloPersonalContextValidator.validate`：来源 ID 重复触发 `Dictionary(uniqueKeysWithValues:)` 致命错误；此前其他模块的重复键修复未覆盖此处。
- 提取与校验统一使用来源索引：完全相同的快照折叠，同 ID 不同快照判定为冲突；冲突批次以可捕获错误退出，不调用模型、不推进游标、不删除原文。反证校验同样拒绝冲突来源并检查修订。
- 验证：两组 standalone 测试共 113 条断言通过；签名 Debug 真机构建成功并覆盖安装。真机首次启动观察超过一分钟，无新增崩溃日志；第二次冷启动被锁屏阻止，未完成验证。
- 边界：冲突批次等待来源修复后重试；重复数据最初的写入原因尚未确认。本次仅 iOS 修复，无后端部署或数据迁移。

---

## [2026-09-09] 云端深度分析「_search 虚拟字段」兑现 + 未知字段静默判空根治（后端发版 + iOS 对齐）

### 背景
东林真机反馈：问「上次买猫粮和猫砂是多久之前，需要补货吗」，云端深度分析答「账里没翻到猫粮猫砂的购买记录」，但 App 内搜索明明能搜到 13 条「猫」系记录（含 7 月 21 日猫砂 ¥72）。生产日志定位到任务 3a71c6db（5 轮，工具全部「正常返回空」），并在生产容器内用同款提示词/引擎/模型完整复现。根因：提示词 v19（8 月 4 日，commit 0ac0944d3）教模型「按商品关键词筛选必须用 `_search` 虚拟字段」，但该字段从未在任何引擎实现——云端引擎把不存在的字段静默当空值处理，模型拿到「没有匹配的数据」信以为真，言之凿凿输出「账里没记录」。8 月 4 日起所有需要关键词翻账的云端深度分析（补货/复购/商品追踪类）全部静默误报。

### 改动
- **云端引擎兑现 `_search`**（cloudAnalysisQueryEngine.js）：实现为「目录声明为 text 的全部字段拼接匹配」（财务域即分类/账户/备注合并文本/项目，与提示词承诺的「分类、备注、商户、标签」一致）；仅允许 contains；工具目录补 `_search` 用法示例，模型每轮可见。
- **未知字段静默判空 → 显式报错**：filter 引用数据集未声明字段时返回 UNKNOWN_FIELD（可恢复错误，点名拼错的字段并指引 `_search` 用法），模型可自我纠正；聚合内 filters 同样校验。此前 `row[不存在的字段]` 被当空字符串过滤，返回 status=empty，模型无法区分「字段拼错」和「真没数据」——这是本次误报的机制根因。
- **iOS 本地引擎对齐**（HoloDataTool.swift）：校验器放行 `_search`（仅 contains），行匹配按行内全部文本字段拼接实现，两端语义一致；此前本地靠 unknownField 报错让模型碰壁换路，算「因祸得福」的隐性兜底，现转为显式能力。
- **测试**：后端 342/342 绿（新增 3 个回归：`_search` 命中备注与分类／`_search` 仅限 contains／未知字段报 UNKNOWN_FIELD 不再返回空）；iOS HoloFinanceToolTests 全组绿（新增 2 个用例：`_search` 跨字段命中备注且计数正确／校验器 `_search` 仅限 contains 且目录外字段继续拒绝）。顺手修复存量发版闸门破损：1cc30301b（财务项目一期）加提示词内容后超 prompts.test.js 长度红线（5950→6200，按既有上调惯例补记）。

### 影响与验收
- 后端已发版生产：云端深度分析的关键词检索类问题恢复正常；已生成的错误报告不会重算，重新发起即可。
- 生产真实链路复验通过（同快照同问题）：`_search` contains「猫砂」返回「7月21日 宠物 猫砂 -¥72」（修复前静默空），最终结论正确给出猫砂购买时间与补货建议，并主动提示猫粮可能以品牌名记账（与账内实况一致——13 条猫系记录确无猫粮）。
- iOS 改动（HoloDataTool.swift + 测试）在工作区未提交（与在途改动同文件交错），随下次 iOS 发版带上；本地轨道此前未被静默误报波及，属对齐加固。

---

## [2026-09-08] 云端深度分析体验三修 + 本地分析卡死根因链根治（纯 iOS）

### 背景
东林真机反馈云端深度分析三问题：①云端任务实际成功，聊天里却提示「深度分析已中断」且状态来回横跳；②问「电费趋势」，数据依据里混进房租 28000、红包礼金 5616 等无关分类；③数据依据不可点击，无法逐笔核对。排查中又发现新问题：本地分析杀 App 恢复后永久卡死「思考中」，一并根治。生产日志佐证：投诉那单云端 7 轮正常完成（claims=3），纯客户端状态误判。

### 改动
- **假失败根治**：源是「对账兜底」按「无本地任务档案超 90 秒」判死——云端分析设计上不落本地档案、真实耗时 1-10 分钟，90 秒必被冤枉，且与云端轮询互相覆盖形成状态横跳。对账兜底对「有云端任务在途」的消息豁免；云端任务改为发起成功即登记（顺带修复上传中被杀无法冷启动恢复的盲区）。
- **证据只留结论引用的**：云端证据池「查过什么就有什么」，探索阶段的全量分类桶（房租/红包）混进核对列表。现按每条发现自带的引用编号过滤（模型未给引用时保留原列表兜底）；空分类分组名渲染「未分类」（原渲染成空的「」）。
- **证据可点**：云端证据的下钻字段此前硬编码为空、全部渲染静态文本。现财务证据接通账单复核页下钻（分类+近 180 天窗，逐笔核对），收藏夹入口一并补齐（此前收藏夹里连本地报告证据也是静态）；行样本证据从摘录提取共有类别词预过滤，直达该分类明细，混类抽样保持全量不瞎猜。
- **本地分析卡死根因链根治**（四环相扣，全部有实证）：①agent 请求超时写死 60s < 模型单轮时延（实测 90s+）→ 长轮次首试必超时，改为 agent_loop 单独 180s；②超时重试撞自己还在算的服务端步锁（TTL 24h）连吃 409 → 步锁退避预算 2/4/8 共 14s ≪ 剩余时延，加到 5 次约 62s（延迟序列可注入供测试）；③耗尽后旧代码静默上抛无人收尾 → runLoop 落 failed 终态如实中断；④活跃任务 3 分钟无落盘进展由对账兜底终结——任何形态的挂死（含一处未破的低概率恢复竞态）最多 3 分钟可见收尾，不再无限转圈。
- **测试**：单测 59 绿（证据翻译 13 / 对账兜底 8 / 步幂等 6 / 调度器回归 32）；新增杀 App→恢复 UI 复现驱动（TEST_RUNNER_HOLO_REPRO=1 门禁防误跑烧额度）；额度自重置后活体复现两轮，第二轮完整重演事故链、新代码当场落明确终态。

### 影响与验收
- 纯 iOS 改动（12 文件），无后端发版、无 CloudKit schema 变更。可选后端优化待拍板：服务端步锁 TTL 24h 偏长，可调小（如 10 分钟）加快异常自愈。
- 模拟器 E2E：云端分析跨 90 秒生死线无假失败、证据仅剩结论相关项且全部可点、点证据跳账单复核页（时间窗=快照口径）逐笔核对；本地分析中途杀 App 恢复不再永久转圈。待真机验收。

---

## [2026-09-08] 财务搜索结果行补时间·账户元信息——默认账户也显示 + 跨年日期带年份（纯 iOS）

### 背景
东林真机反馈：财务搜索（如搜「电费」）结果只有分类名和金额，不知道每笔消费什么时候发生、发生在哪个账户。排查确认两层原因：①搜索结果行复用 TransactionRowView 默认参数，不显示日期；②App 全局规则「默认账户不显示账户名」（其他列表页有账户上下文，省略降噪），而东林的电费多记在默认账户——光打开旧逻辑依然看不到账户。

### 改动
- **TransactionRowView 新增搜索元信息行**（`showsDate` 非 compact 分支）：分类名下方一行小字「时间 · 账户 · 备注（如有）」，样式对齐现有备注行（12pt 次要色）。该分支下账户**不受「默认账户省略」规则限制**——搜索是跨账户查询场景，账户是关键上下文；其他调用方默认参数不变，零影响。
- **日期格式跨年智能补年份**：与当前同年显示「8月3日 20:30」；跨年老交易自动带年份「2025年7月5日 20:30」（本地化模板，简中/繁中/英文自适应），消除搜索结果跨年时的年份歧义。账本流水页（DetailTabView 排序列表）等既有 showsDate 场景顺带受益。
- **FinanceSearchView 结果行打开 `showsDate: true`**：普通搜索与 AI 精确查询共用同一列表，一处生效。

### 影响与验收
- 纯 iOS 改动（2 文件 25 行），无后端发版、无 CloudKit schema 变更；数据层 `searchTransactions` 返回完整托管对象，零改动。
- 日期模板三语输出验证：zh_CN「8月3日 20:30 / 2025年7月5日 20:30」、en_US「Aug 3 at 20:30 / Jul 5, 2025 at 20:30」。
- 模拟器实走：造数后搜「电费」两条结果均显示「8月3日 20:30 · 现金」「7月5日 20:30 · 现金」（两笔均在默认账户「现金」，旧逻辑下完全不显示）；点结果行正常进编辑页。待真机验收。

---

## [2026-09-08] 个人情境规划接入真实聊天入口 + 会话续接 + 同轮多待办合并（iOS，三个月改版第一期收尾）

### 背景
东林真机反馈三问题：①规划回复溯源不了、徽章点不动；②第二轮补充信息被拆成 4 个独立任务（应 1 主任务+子条目）；③「已为你处理 4 件事」与「任务待确认」同屏矛盾。排查确认根因：P0 建成的规划管线（HoloContextChatPlanner + ContextPlanChatCard 草案卡）从未接入聊天流程——`plan()` 全仓库零调用、`.contextPlan` 消息无生产者，草案卡是不可达死 UI，用户看到的「规划」实际是普通记忆聊天（用记忆却无署名）。

### 改动（iOS）
- **规划入口接线**：ConversationCoordinator 识别 contextual_planning 意图后真正调起规划器，产出 `.contextPlan` 草案卡消息——「查看依据」快照（你说过/你的记录/推测）、「因你的情况调整了」、逐项勾选保存从此在真实聊天可见。生成失败/闸关闭/预算耗尽自动落回普通流式聊天，不阻断对话。
- **规划会话续接**：最后一条 AI 消息是未解决草案卡时会话进入续接态——非写消息走 followUp 重新生成草案（不再被意图分类拐去直接建任务）；点「已安排好/本次不用」或中途聊别的自动退出续接态；明确写意图仍走写链路。
- **同轮多待办合并**：一句话里多条 create_task（全部无日期或同一天、标题不含分隔符、模型未自带 subtasks）合并为 1 张「主任务+子条目」确认卡，确认后建 1 任务 + N 条 CheckItem；日期各异的条目是独立时间锚点，保持独立。草案卡保存同样合并（goalSummary 派生主任务标题）。判定逻辑独立为 `TaskGroupMergePlanner` 纯模块。
- **记忆署名兜底**：注入了记忆但模型没吐 `[[HOLO_MEMORY_IDS]]` 标记时，按内容词对账补署名（`HoloMemoryAttributionReconciler`，宁多署不漏署）；模型吐了标记仍以标记为准。
- **标签可点性**：无跳转目标的意图标签不再渲染成按钮样式（弱化显示），草案卡消息不再叠加意图标签。
- **话术对齐真实状态**：「已为你处理 N 件事」只在全部执行完成后出现；待确认批次改「识别到 N 项，待你确认」，混合批次分开计数（消息正文与多卡汇总头两处）。

### 测试
- 新增 standalone 套件 ChatTaskGroup（20 断言）：合并判定（无日期合并/异日独立/分隔符标题不并/尊重既有子条目/同日上提/单条不并）、主任务标题（剥「帮我」/16 字截断/枚举原话整体）、署名对账（命中/不误署/空输入）。
- `scripts/run-personal-context-standalone.sh` 登记；全量 9 套件通过。主 App 编译通过。

### 影响与验收
- 纯 iOS 改动，**不需要后端发版**（personal_context_planning prompt 已在生产）。
- 待真机验收：规划问句出草案卡（含依据）、追问续接重生成、多待办合并确认卡、记忆署名徽章、话术三态。

---

## [2026-09-07] 通用个人情境规划 P0——方案卡可信化 + A/B 评测基线（iOS+后端，三个月改版第一期）

### 背景
三个月改版方案（`docs/_common/plans/2026-09-07-Holo-Agent能力诊断与三个月改版方案.md`）拍板执行，P0 目标：让 AI 的方案真的用上「你的情况」且可核对。本轮修复诊断报告点名的四个在途问题中的三个半，并搭建 P0 验收的端到端 A/B 评测基线。

### 改动（iOS）
- **回执防虚报**：方案卡「添加到待办」逐项真实执行并回报回执，失败不记回执、卡片如实显示「N 项未成功」，失败项重试只补失败部分，不再虚报成功（对账逻辑下沉 `HoloContextPlanExecutionAdapter.reconcileReceipts` 纯函数，standalone 测试锁定）。
- **依据区可核对**：方案卡「查看依据」改为展示生成时从本机检索命中固化的命题原文快照（非模型复述），带认识状态徽标（你说过/你的记录/推测）；模型编造的引用不产生伪造快照。
- **纠正跨会话兑现**：「已安排好/本次不用」持久化（runID 键），重启后不再回到可保存态。
- **planEffects 契约**：方案卡新增「因你的情况调整了」区块，只展示带个人依据的变化（增加/取消/调序/改时/方案选择）；无个人依据的方案卡不显示该标题，不给通用建议贴「懂你」标签。
- **保存权限闸**：保存前复查控制闸/遗忘/修订（`HoloContextChatPlanner.canSave`），背景被忘记或更改后旧草案不得保存，卡片显示「方案背景已变化」。

### 改动（后端，**需发版**）
- `personal_context_planning` prompt：要求输出 planEffects（每项变化连接依据的 contextID）；加无关性约束（与请求无因果关系的情境不得进方案）；answerText 长度控制（300 字内，防截断）。
- `personal_context_planning` maxTokens 默认 4000→8000（实测 4000 被长回答截断致 JSON 解析失败）。**发版时须核对服务器 .env 是否显式设置了 `HOLO_PERSONAL_CONTEXT_PLANNING_MAX_TOKENS`，如有需同步改为 8000。**

### A/B 评测基线（首轮+第二轮，24 目标 68 输入）
- 冻结样本八类覆盖（责任/时间依赖/多人对象/旧状态/反证/资料不足/少提醒/偏好），三版本设计（原事实/条件改变/无关信息），A 臂通用聊天 vs B 臂情境管道 + 隐藏标签 judge 三层评分。
- **达标**：红线 0 违反（零容忍）；planEffects 输出率 100%；反事实响应 95%→**100%**；情境应用与配对净收益 judge 满分（个性化增益成立）。
- **未达标**：无关稳健性 48%→**67%**（门槛 90%）。定性：无关偏好被规划模型编进行动步骤；但评测管道无检索层（iOS 真实管道有检索信号过滤先拦截无关记录），67% 属保守口径。prompt 约束带来真实增益。**P0 遗留**：harness 补检索模拟后重测；发版不因此受阻（新能力在内部账号闸后，残留影响=方案卡偶尔多一条无关小建议，非致命）。

### 影响与验收
- iOS 侧不依赖后端发版即可运行（planEffects 缺失时诚实降级）；「因你的情况调整了」区块与截断修复需后端发版后生效。
- 方案卡在内部账号闸后，真机验收走内部账号。
- 评测：`scripts/eval-personal-context-ab.mjs`（分片并行+断点续跑）+ `scripts/merge-ab-results.py`；结果与口径说明见 `docs/_common/eval/personal-context/ab/`。

---

## [2026-09-07] iCloud 同步链路完善——第二台设备数据自动出现 + 同步状态可信化（纯 iOS，P0）

### 背景
用户报告 iPhone（1.0.1）数据在 iPad 全新装机后不同步。排查确认 1.0.1 三个缺陷：①全 App 只有习惯等 5 处监听 iCloud 远程变更，其余模块「数据已下载到本地但界面永不刷新」（用户端表现为空态，重启 App 才出现）；②同步状态展示自欺——打开设置页会冲掉真实同步记录、「操作被取消」这类良性错误当红字常驻、手动同步按钮递交请求就报成功；③同步引擎没跑起来时（如系统设置里 App 的 iCloud 权限被关）完全静默。

### 改动
- **`CloudImportRelay`（新增，统一中继）**：全局唯一监听 CloudKit 同步引擎的 setup/import 事件完成（云端数据真正落地），防抖 2 秒广播 `holoCloudDataDidSync`；各模块订阅一条通知重拉数据，不再各自维护监听+防抖。刻意不用 NSPersistentStoreRemoteChange——那条通知本地保存时同样会发，会让全部模块在每次写入后多跑一轮无意义重拉。
- **八模块接入远程刷新**：任务（联动日程对账引擎级联刷新）/目标/纪念日（含通知重排）/想法列表/聊天（仅空会话时重拉，不打断浏览）/回收站/记忆长廊（仅空列表时重拉，防滚动位置被打断；多模块连发通知合并为一次刷新）/财务（沿用种子修复监听，补 `financeDataDidChange` 广播驱动账本与分析页重查）。与既有习惯模块修复互补，覆盖第二台设备「首启加载早于后台导入」的完整时序。
- **同步状态可信化**：打开设置页不再冲掉真实事件记录（`lastEventDescription` 可选化，未发生事件时回落账号态文案）；「操作被取消」（CKError 2）不再进「最近错误」，事件成功即清除旧错误；手动同步按钮即时反馈改为「已递交同步请求」，真实结果等探针上传事件落地后报「同步完成」或 12 秒超时提示。
- **同步自检指引**：启动即静默拉账号状态（不再等打开设置页）；账号可用但从未完成过任何同步事件时，设置页显示「同步尚未开始 + 系统设置检查指引」，不再静默装死。
- **首次同步可见**：任务/想法列表空态在「首次同步尚未完成」时显示「正在从 iCloud 恢复数据」提示，把「数据在路上」和「同步坏了」区分开。

### 影响与验收
- 纯 iOS 改动，无后端发版；**发版前置**：`docs/appstore-preflight/CloudKit-schema部署指南.md` 已更新 1.0.2 部署清单（8/25 后的新实体与字段须部署 CloudKit Production，否则线上相关类型数据静默同步失败——同时是线上 1.0/1.0.1 用户的修复前置），交 GPT 执行。
- 主工程 build_sim 通过；全量回归 973 过、14 失败经三组对照实验全部定责为环境与既有问题，与本改造无关：7 个想法富内容单测全套件运行时交叉干扰（单跑三次全绿）、3 个 UI 用例为模拟器旧数据污染（全新模拟器即过）、3 个 UI 用例在改动前基线提交上同样失败（分支既有）、1 个截图用例为套件顺序脆弱性（单跑即过）。
- 待真机双设备（同 iCloud 账号）验收：iPad 全新装机后台导入完成后各列表自动出现、无需重启。

---

## [2026-09-07] HoloAI 收起洞察卡概率性整屏空白——逐帧稳定校验根治（纯 iOS）

### 背景
对话页收起周期回放卡（「收起详细内容」）后概率性整个对话框清空成空白页，第三次复发。前两轮修复（b914d015 收起钳制、bab56a055 展开重锚定）都是事件驱动的一次性补偿，只压住了各自那一条时序，没收住整类问题。

### 根因
收起让内容一次性缩短数千 pt，偏移停在按旧几何看「合法」的位置上、落到最终内容之外，表现为整屏空白。事件驱动钳制有两个永久盲区：①`setOffset` 重入保护会把布局期间同步到来的 contentSize KVO 直接丢弃，最后一次收缩无人处理；②`defaultScrollAnchor(.bottom)` 框架级再定位移动偏移时不产生 contentSize 事件，钳制无从触发。哪条盲区被踩中纯看帧对齐，所以概率性复现。

### 改动
- **`ChatOffsetSettlingPolicy`（纯逻辑，独立可测）**：内容收缩后的稳定窗口策略——逐帧校验「偏移在内容合法范围内」，修正过或用户接管滚动则重置稳定计数，连续 12 帧无越界即收窗，另设 90 帧硬上限防病态震荡。
- **`ChatScrollController` 接入**：新增 `beginOffsetSettling()`（CADisplayLink 短窗口逐帧钳越界）；收起路径 `requestOffsetClamp()` 与 contentSize 收缩分支（含消息删除）都开窗。拖动/惯性期间不与手指抢位置，松手由 UIScrollView 自行回弹。
- 与展开方向的 scrollTo 重锚定（bab56a055）互补：展开靠重锚定落到真实内容上，收起靠逐帧校验兜住所有时序。
- 踩坑速查表新增同族条目（事件驱动一次性钳制在大体积收缩下必然有盲区）。

### 影响与验收
- 纯 iOS 改动，无后端发版。
- `ChatScrollBehaviorStandaloneTests` 新增 3 组策略回归（多帧收缩重开窗/用户接管让位/硬上限收窗），swiftc 独立套件全绿；主工程 build_sim 通过。
- 模拟器实走 7 轮压测（卡底收起/快收起/卡中收起/惯性中收起/拖顶收起/极速连收），逐轮截图像素方差检测零空白；待真机验收。

---

## [2026-09-07] iPad 开屏适配——启动画面按设备分流（方案A拍板落地，纯 iOS）

### 背景
东林报告 iPad 端开屏没适配。根因：开屏是一张 1290×2796 的 iPhone 比例整图 aspectFill 铺满，iPad 竖屏被裁上下约三分之一（构图不完整）、横屏只剩中间一条窄竖条且放大发糊；加上 iPad 分屏/台前调度下窗口比例多变，固定比例整图天然防不住裁切。东林拍板方案A：iPad 改「纯色底+居中标志」，一次根治；iPhone 保持整幅插画。

### 改动
- **新增居中标志素材 StartupSplashMark**：以现有 HoloFaceLineArt 白线稿为底染成「上金橙(#F2A66B)→下珊瑚(#F0786B)」垂直渐变（与开屏图 Holo 字样色系一致），柔化额头光晕边缘、清除底部杂点，透明底 PNG。
- **启动画面按设备分流（资源层）**：StartupSplashArtwork imageset 改为 iphone=整图 / ipad=4×4 纯色图（与开屏底色一致）；新增 StartupSplashMark imageset：iphone=4×4 透明占位 / ipad=标志图；LaunchScreen.storyboard 新增居中标志层（310pt 宽+比例约束，水平垂直居中）。
- **技术要点（长期坑）**：LaunchScreen.storyboard 里 hidden 属性的机型变体（trait variation）在系统渲染启动画面时会被忽略——启动画面按设备出图，图片资源的 idiom 分流是唯一可靠机制。

### 影响与验收
- 纯 iOS 资源改动，无后端发版。
- 全新 iPad 模拟器实拍开屏=纯色底+居中标志 ✓；iPhone 模拟器实拍=整幅插画与改动前完全一致 ✓；横屏与竖屏同布局（居中约束与方向无关），留真机确认。
- commit 0c2675347 已推送 release/1.0.2。真机验收注意：iOS 会缓存启动画面，装新包后若仍见旧画面，删 App 重装或重启手机即可刷新。

---

## [2026-09-06] 删除回收站闭环 + 短想法告知（东林两项拍板落地，纯 iOS）

### 背景
想法模块体检（见上条）遗留两个拍板点，东林当日拍板：①单条删除保留 30 天（进回收站可自助恢复）；②短想法（<10 字）不参与 AI 整理，但要让用户知道。

### 改动
- **单删建批次进回收站**：RecycleBinService 新增 `makeSingleItemBatch`（scope="single"，与模块清空共用同一套展示/恢复/30 天过期清理链路）；想法删除（ThoughtRepository.delete）与任务删除（TodoRepository.deleteTask）都改为建单条批次，回收站列表显示「删除想法/任务 · 标题（剩余 30 天）」，点进详情可一键恢复。任务的 restoreTask 同步补清 deletedBatchId。
- **三处删除确认文案改回可恢复口径**：「删除后将进入回收站并保留 30 天，可在最近删除中恢复」——此前「无法恢复」的过渡文案随实现兑现而回退。
- **短想法告知**：编辑器退出时若正文不足 10 字且自动整理开启，轻提示「内容较短，暂不自动整理；补充内容后会自动整理」；每个编辑器会话只提示一次，纯图想法不提示（心智上不期待文字整理）。
- 词表新增 6 条（批次摘要×2/图片想法/无标题/短想法提示/删除可恢复文案回退复用既有 key），随多语言批次提交。

### 影响与验收
- 纯 iOS 改动，无后端发版。
- 模拟器已实测：删除想法→回收站出现「删除想法 · 被那个谁谁谁看到了（30天）」批次（任务单删批次同样可见）→批次详情正常；恢复动作走既有回收站恢复链路（十轮 QA 已验收），真机复验即可。
- 想法模块相关 45 个单测全绿。

---

## [2026-09-06] 想法模块完整体检——图片链路根治 + 删除文案纠偏 + 整理管道三处 P1（纯 iOS）

### 背景
东林报告「想法模块上传照片不成功，整个模块不可用」。排查确认：想法照片为纯本地功能（从无上传环节），真正根因是选图加载失败被静默吞掉（相册 iCloud 原图未下载等场景「选完毫无反应」）+ 纯图片想法被当空草稿整条丢弃。遂按东林拍板升级为「全项目图片链路 + 想法模块全功能」专项体检，完整台账见 docs/thoughts/2026-09-06-想法模块完整体检-台账.md。

### 改动
- **图片链路根治（4 处共用方）**：新建统一加载器 PhotoLibraryImageLoader（Data 失败回退系统转码管线，仍失败给用户明确提示，不再静默跳过）；想法附件、任务附件、反馈截图全部接入，一次修复三处受益。
- **纯图片想法合法化**：只加图不打字也能保存（此前整条丢弃）；新建模式暂存改存原始数据，与编辑模式同走 2048 压缩管线（此前被 1024 预览封顶）。
- **相机权限被拒反馈**：想法编辑器、任务详情页补齐提示（对齐任务模块已有做法）。
- **删除确认文案纠偏（P0）**：单条删除实为软删但不进「最近删除」列表（模块清空批次才进）、用户无法自助恢复；三处文案（想法卡片/滑动删除/任务详情）从「可在最近删除中恢复」改为如实的「删除后将无法恢复，30 天后从设备彻底清除」。是否让单删也进回收站待东林拍板。
- **整理管道三处 P1**：①编辑已整理想法后状态回 pending 并在退出编辑器时重新排队（此前编辑过的想法旧标签失效却永远不会重排）；②配额暂停跨天自动复位（兑现「明天自动续做」，不再依赖重启）；③429 一律不自动重试（此前被 1/2/4 秒盲重试 4 倍放大冲击设备限流）。
- **整理管道四处 P2**：冷启动一律恢复 processing（防永久卡「整理中」）；软删想法正文不再上传；MODEL_OUTPUT_INVALID 加入不可重试名单（防同 payload 重放 4 倍计费）；批量进度先去重再计数（防 banner 永不消失）。

### 影响与验收
- 纯 iOS 改动，无后端发版；图片本就不走后端。
- 模拟器实测六场景全过（新建选图/纯图保存/详情/画廊/编辑模式加图/列表同步）；想法模块 41 个单测全绿。
- 真机验收清单见台账第六节（重点：iCloud 未下载原图的照片、纯图想法、已整理想法编辑后重排）。

---

## [2026-09-05] 账户详情页改版——余额卡压扁 + 对账状态条 + 本月收支提前（纯 iOS）

### 背景
东林反馈账户详情页两处问题：① 余额下方常驻一行「建议对账：与银行 App 核对一次，余额更可信」，口吻像在质疑用户记的账不可信，且「从未对账」状态会永远挂着、看着像警告实际却是可点的按钮；② 布局失衡——头像大卡五层竖排占半屏、「现金」在页内出现三次（导航栏 + 卡内大标题 + 类型标签），管理工具（预算，Plus 权益）排在核心回顾信息（本期统计）前面。HTML 设计稿两文案方案对比后，东林拍板方案 A（中性事实 + 动作指引）。

### 改动
- **余额卡压扁**：64pt 大图标 + 大标题 + 余额 + 提示 + 标签的五层竖排，改为「身份行（36pt 图标 + 账户名 + 类型/默认标签）+ 36pt 大字余额 + 通栏分隔线 + 对账状态条」三段式，卡片高度约砍半；去掉与导航栏重复的卡内大标题。
- **对账状态条按状态分级**：从未对账 → 灰色「尚未对账 · 点此核对余额 ›」（动作词橙色，整行可点进对账弹层，从说教改邀请、不再像警告）；已对平 → 绿色正反馈「已对平 · x 天前 · 此后 N 笔」保留；账目基准变动 → 红色「账目有变动，建议重新对账」保留，且成为页面唯一警示。
- **本月收支提前**：原「本期统计」改名「本月收支」，从预算之后提到预算之前——先回答「这卡多少钱、这个月用得怎么样」，再看「管得怎么样」；信用卡账单卡仍紧跟余额卡。
- **卡片间距收紧**：卡片间留白 32pt → 16pt（东林反馈留白过松，对齐设计稿档位），交易记录首组直接进入首屏。
- 三条新文案（尚未对账 / 点此核对余额 / 账目有变动建议重新对账）已走词表流水线补齐繁体与英文（xcstrings 随多语言批次提交）。

### 影响与验收
- 纯 iOS 改动，无后端发版；对账弹层与对账逻辑本身不变，仅详情页呈现。
- 模拟器已走查：新版布局与设计稿一致，状态条点击正常弹出对账，「已对平/账目变动」两态未在模拟器造数据（写对账锚点会经 iCloud 同步真机），随真机验收。
- 真机验收：从未对账的账户应显示灰色邀请句且可点进对账；对平后变绿色正反馈；本月收支位于预算之前。

---

## [2026-09-04] 想法分享卡——暖纸手账风长图，全部内容完整呈现（纯 iOS）

### 背景
把一条想法（正文、图片、标签、心情、时间）排成一张精美的图片分享出去。按东林拍板：方案 A「暖纸手账」风格；长文与图片**全部内容、不做任何截断**（「都发出去的是图片了，截断没有意义」）；品牌尾注默认带、可一键取消。

### 改动
- 新增 `ThoughtShareCard`（暖米纸底 + 宋体正文 + 行内 # 标签着色 + 拍立得照片 + 虚线缝线 + 金线侧脸品牌尾注）与 `ThoughtShareSheet`（深色舞台：缩放预览 + 尾注开关（记忆偏好）+ 分享图片 + 保存到相册）。
- 宽度固定、高度随内容生长的「长图」：正文与图片完整呈现；微信/朋友圈对长图原生折叠、点开看全文由平台处理。
- 入口：想法卡片「…」菜单与详情页右上菜单新增「生成分享卡」。
- 导出走 ImageRenderer，动态钳制渲染分辨率 ≤8000px（实测超 8192px 上限会输出全透明空图）；预览降采样上屏、导出保持全量清晰。
- 存相册补 `NSPhotoLibraryAddUsageDescription` 权限声明。

### 影响与验收
- 纯 iOS 改动，无后端发版。
- 真机验收：长笔记（数千字）生成分享卡无截断；尾注开关即时生效且记住偏好；保存到相册成功；带图想法的拍立得排版正常。

---

## [2026-09-04] 想法卡片「…」卡顿第三层根因根治——内容渲染与状态重算解耦（纯 iOS）

### 背景
想法卡片点「…」卡顿此前已修两轮（渲染输入缓存 + 无障碍文本缓存），真机仍有感知卡顿。插桩实测定位到第三层根因：点「…」弹菜单时系统会强制整个列表重算一遍（弹层动画结束时机），而每张卡片在重算时仍要重新解码一遍内容 JSON 并重建朗读文本缓存键——成本与笔记长度成正比，长笔记 × 多张可见卡片叠加即为主线程可感卡顿。

### 改动
- 卡片正文预览抽为独立子视图 `ThoughtContentBody`，只依赖内容字符串两份值类型输入：SwiftUI 在状态型重算（弹菜单、整理队列状态变化）时因输入相等直接跳过其 body，JSON 解码与富文本排版整体消失，卡片重算降为常数成本、与笔记长度无关。
- 朗读文本新增按原始内容字符串为键的进程级二级缓存（`accessibilityText(richJSON:fallbackPlainText:)`），重算时无需解码即可命中，富文本键零拷贝桥接。
- 模拟器插桩验证：修复前每次弹菜单全列表各卡重复解码（208 字实测 2.32ms/卡，线性放大）；修复后重算完全不再触碰内容解码，实测 0.00ms。

### 影响与验收
- 纯 iOS 改动，无后端发版；渲染结果与无障碍语义不变。
- 真机验收：长笔记列表点「…」应无感知卡顿（此前长笔记场景必卡）。

---

## [2026-09-04] 回放摘要归纳云端化——断开不再中止+隐私「用完即焚」口径上墙（后端已发版）

### 背景
周期回放的累计摘要归纳此前由客户端在线直调 AI：断网、锁屏、切后台都会把生成掐断（近 14 天生产实测 74 次撞 30 秒中止，中止前烧掉的 token 照常计费，用户重试再花一遍）。本次把它迁入深度分析/周期回放已经验证过的云端异步任务底座，同时按东林要求把「云端用完即销毁」的隐私承诺写进用户可见文案。

### 改动（后端 29f854ae5 已发版；iOS fcb882401 随下次发版生效）
- **后端**：云端任务新增 `replay_digest` 类型——执行器单轮生成，走 replayDigest 服务端提示词与路由（成本治理后的 low 思考档同样生效）；限流沿用摘要独立额度桶（10/分、30/天）不挤占深度分析；不消耗会员额度、不打完成推送（后台静默维护）；token 用量独立入账（purpose=cloud_replay_digest）。
- **iOS**（`HoloReplayDigestService`）：摘要归纳优先走云端任务轨道（建任务→加密上传→轮询→领取→回执），锁屏/断网/切后台不再中止归纳；云端不可用、隐私未升级 v2 或任何失败自动回落原直调通道，用户无感。
- **隐私四层落实（代码行为，非文案修辞）**：① 隐私说明弹窗新增条目「回放的长期摘要维护也在云端完成，本期内容用完即删，云端不留存」，且仅对已同意云端处理的用户启用；② 素材上传即 AES-256-GCM 加密落库，数据库零明文；③ 云端生成完成，输入侧素材密文立即销毁；④ App 把结果落到本地后回执，服务端随即销毁结果密文副本；超时/失败时 App 主动令服务端整行销毁，另有 7 天无人认领兜底清理。
- 测试 287/287 全绿（新增执行器三例 + start 端点白名单一例）；xcodebuild 模拟器构建通过。

### 影响与验收
- 后端兼容两种方式（云端任务 + 直调回退），无发版顺序依赖；iOS 改动随下次 App 版本生效。
- 真机验收：生成一次周期回放后摘要正常更新；后台 AI 指标页可见 cloud_replay_digest 记录与 token 用量。

## [2026-09-04] AI 成本治理第一批——思考档位矫正+缓存前缀治理+token 全量计量（后端已发版）

### 背景
对近 14 天生产日志逐条计价（DeepSeek v4-flash 分时价：输出 ¥4.5-9/M 是输入的 3-6 倍，思考 token 按输出计费）：可测消耗约 ¥0.30/天，另有约一半消耗藏在计量盲区。三大浪费点：回放摘要默认满档思考（completion 的 87% 是思考 token 且 54% 调用撞超时中止）、意图识别分类任务开深度思考（思考占 81%）、意图提示词的分钟级时间变量打断缓存前缀（命中率仅 20%）。此外最贵的长流式调用（insight 等）因日志 50KB 截断用量永久失踪，云端分析完全不记账。

### 改动（后端 91efd0fe6，已发版）
- **回放摘要 reasoningEffort=low**、**意图识别 reasoningEffort=none**（与想法整理同机制，质量由确定性意图直判+端侧澄清兜底）。
- **意图提示词 v29**：日期/时间变量从提示词开头移到全提示词最末尾——前缀日内稳定，缓存命中率预期 20%→90%+。
- **计量盲区修复**：ai_call_logs 迁移 19 新增 prompt/completion/cached/reasoning_tokens 四独立列；流式路径增量捕获 usage（仅含 usage 的块才解析，不进热路径）；云端深度分析/周期回放补写 ai_call_logs（cloud_* 前缀）；admin AI 指标页新增输入/输出 token 列（含缓存命中与思考占比），可在后台直接看成本结构。
- 附带：重新生成 IntentDescriptor.swift 修复既有漂移（早前后端 intents.json 变更未同步生成产物）。
- 估算整体效果：单位成本下降约 45%-55%（每活跃用户每月 AI 成本从 ~¥5-7 降至 ~¥2.5-3）。

---

## [2026-09-04] iPad 适配第三轮——长廊通铺根治+13寸主视觉分档放大+首页头部对齐（自评 92/100）

### 背景
东林反馈 iPad 适配仍不理想，要求按 90 分标准迭代到位。第三轮以 iPad Pro 13 寸模拟器全页面竖横双拍体检：R1 现状 78 分，主要扣分点为记忆长廊整页通铺（前两轮兜底均未生效）、首页竖屏留白失衡、头部问候/齿轮不跟内容列。

### 改动（纯 iOS，无后端）
- **长廊通铺根治（真根因）**：洞察 tab 内容上的 `.containerRelativeFrame(.horizontal)` 按容器全宽给自己定宽，无视 720 列提议，并沿安全区传播链把 tabContent→CalendarRootView→整页全部撑回全屏——日历页签（日/周/月/轴/时间线）跟着通铺，前两轮的外层兜底因此全部失效。删除该修饰符后长廊竖横屏均恢复列宽（iPhone 靠布局提议本就占满，行为不变）。诊断方法：`.border` 三色描边 + PIL 像素测距 + 摘除 bisect。
- **首页主视觉分档放大**（`HomeView.heroScale`）：iPhone 1.0 不变；iPad 11 寸 1.25；13 寸级（宽 ≥1000pt）1.45——竖屏上下留白恢复均衡。
- **首页头部对齐**（`HomeView`）：问候语+设置齿轮包进 520 列（与底部导航条同框），不再贴屏幕边缘。
- **记忆详情页 + 洞察骨架屏**：同类 containerRelativeFrame 通铺一并修复，详情页改「背景通铺+内容限宽」模式。
- **审计基建**（`HoloIPadAudit*UITests`，7 个用例类）：iPad 全页面竖横截图走查可复用；注意 `XCUIDevice.orientation` 必须在 `launch()` 之后设置才生效。

### 验证
- iPad Pro 13 寸（iOS 26.3 模拟器）竖屏 13 页 + 横屏 4 页逐一实拍：全部限宽居中、无破版；横竖旋转往返正常；全程无崩溃。自评 92/100（扣分：快捷键待真机外接键盘实测、其余页面横屏未逐张拍、11 寸未实测）。
- **iPhone 回归实测零变化**：hero 1.0、首页/长廊布局与改前一致。
- 改动与多语言一期在途改动同文件交错（MemoryGalleryView/HomeView），已按「HEAD+仅本改动」外科拆分提交，多语言在途工作区改动分毫未动。
...
### 背景
1.0.1 发版前工作区积压 412 个未提交改动。经与东林拍板：1.0.1 不带多语言（String(localized:) 标记与词表均不随版，后端语言链路不发布）；其余完工功能按批次收口入库。四批功能此前各自完成并验证过（档案在案），本次做归属分拣+提交+干净 worktree 全量回归。

### 四批提交
- **余额对账一二期三批收口**（082a1003d）：账户对账锚点四态状态机+独立对账页；对账调整流水统计口径全链排除（22 处）；账单余额列导入核对+一键以账单为准；期初余额修改警示；云恢复后 SeedRevivalRepair 防抖挂载；严格预算/自定义 Prompt 云恢复设置挂载。
- **纪念日仪式感 P0+P1**（9c0ff2d13）：三步向导（场景/日期/仪式）+点亮庆祝+详情三态（庆祝/翻转数字/里程碑轨道）+列表 Hero 分组+农历（ChineseLunarCalendar）+里程碑推送（100/365/520/1000/2000/3650 天）+桌面与锁屏小组件重设计。
- **记忆出处三路修复**（6198f4584）：证据 summary 回填用户原话（截 200 字+角色标记隔离防注入）；出处页深链路由（HoloMemoryEvidenceRouter）可点跳原模块；summary 缺失时实时查会话原文兜底。
- **想法双修**（383415e29）：「待确认」徽章可点直达详情 AI 确认区（滚动锚定）；「…」展开卡顿根治（无障碍文本 NSCache 进程缓存+日期格式器缓存）。

### 明确不随 1.0.1 的
- 多语言一期全部改动（约 300 文件 String(localized:) 标记、Localizable.xcstrings 词表、日期格式模板化）：词表与后端语言链路不上，显示行为不变。
- 上架素材在途批：App 图标全套、docs/app-store 截图、截图渲染脚本、HoloAppStoreScreenshotSeeder 改动（东林在途，勿混提交）。

### 验证
- 工作区整体验证：Debug 编译 BUILD SUCCEEDED（含全部在途改动，确认无半成品挡路）。
- 干净 worktree（detached HEAD）编译+全量测试：见下方补记。

## [2026-09-03] 设置页日历来源选择重设计——列表收口一行+独立分组选择页+组级全选+修清零回弹

### 背景
东林反馈设置页「日历」区块把手机上全部系统日历按名称字母序平铺（真机 13 个：个人/订阅节假日/携程/大麦/抖音订单等混在一层），大圆圈勾选样式噪音大，且把设置页撑爆。按拍板的方案 B 实施：低频配置收进独立选择页，设置页只留汇总入口；同时 QA 走查揪出一个交互语义 bug 一并修复。

### 改动（纯 iOS，无后端）
- **设置页收口**（`ScheduleSettingsSection`）：删除 13 行平铺列表与底部 AI 说明，收成一行「显示的日历 · 已选 X/Y ›」汇总行（副标题「勾选后才在 Holo 展示、供 AI 读取」），点击 push 独立选择页（`navigationDestination`），设置页其余内容回到首屏可见。
- **独立选择页**（新建 `CalendarSourcePickerView`）：顶部浅橙说明卡承载 AI 可读性告知（移到勾选发生处）；日历按来源分组——「Holo 专属」置顶（带「任务同步写入」徽章、不设组级按钮）→ 各账户组（组名=系统来源名）→「订阅」组垫底（组头带「默认不勾选」）；组头右侧「全选/全不选」按钮随组内状态切换；勾选改 Apple 日历同款轻量对勾（选中橙勾/未选浅灰勾），替代原大圆圈。
- **数据层**（`ScheduleStore`）：`ScheduleCalendarInfo` 新增 `isHolo`（title=同步引擎专属日历且可写）与 `sourceTitle`（EKSource.title）字段；`refreshCalendars` 排序由字母序改为语义序 Holo→账户（按来源名）→订阅垫底，同来源连续供选择页切组；新增 `setSelection` 组级勾选。
- **修：全部取消勾选后重启被悄悄重置回默认**（`ScheduleStore`）：激活时的默认勾选补写逻辑把「空勾选集」当「从未配置」，与「用户主动全不选」语义撞车。新增 `selectionConfigured` 标记（用户手动勾选/组级操作即置位），补默认前先查标记——主动清零后重启不再回弹，全新用户首启默认勾选不受影响。
- **测试**（新增 `HoloUITests/CalendarPickerWalkUITests`，已挂 pbxproj）：主路径/交互/画面 14 项走查用例（含深色模式），供后续回归复用。

### 验证
- QA 全量回归 14 项全部 PASS（iPhone 17 Pro / iOS 26.3 模拟器，XCUITest 通道）：主路径 5 项（汇总行/分组结构/组头状态机/单行勾选/返回计数一致）、交互 5 项（清零到 0 不崩、Holo 取消勾回、总开关收展、权限被拒降级、日历页回归）、画面 4 项（深色模式对比度/说明卡不溢出/间距对照设计稿/对勾样式）；截图存档 `/tmp/holo_picker_walk/`。
- 清零回弹修复经 XCUITest UI 层双复验：清零到 0/4 重启 App 后保持「已选 0/4」不回弹；彻底清除配置（删容器 plist+清 cfprefsd 缓存）模拟全新用户，首启仍自动补默认勾选 2/4（Holo+日历，订阅与生日未勾）。注意模拟器强杀 App 后勾选值会滞留 cfprefsd 内存，验证首启行为必须连缓存一起清。
- `ScheduleSyncPlannerTests` 20 用例全绿，数据层无连带破坏。
- 真机待验项：长日历名截断、13+ 日历滚动、多账户分组堆叠（模拟器仅 4 日历受限）；分组名显示系统来源名原文（模拟器为 "Default"/"Other"，真机 iCloud 账户显示「iCloud」）。
...
### 背景
应东林要求对记忆萃取模块做产品+工程双体检（继想法模块 P2 后同规格），三轮调研覆盖 iOS 全链路/后端流水线/感知面与消费面。工程侧发现四处可直接修复的问题；另有人格化分类（星型档案五层）与存在感（三级时刻）两套产品方案已单独交付东林，待拍板后排期。本条目只含已实施修复。

### 改动
- **涉健康跨域推断恢复敏感确认门**（`HoloCrossDomainFusionService`）：跨域融合 prompt 一直要求模型对涉及 health 的结论标记 `sensitiveLocal`，但客户端自 7-17「收敛确认边界」改动起硬编码 `sensitivity = .normal`，标记被解码后从未消费——涉健康的跨域推断（如「睡眠偏短与餐饮支出偏高并发」）即使重复出现也自动生效、不经用户确认。现恢复消费模型标记：标记敏感的融合记忆落 `.sensitive`，按激活政策进待确认队列（域萃取侧健康客观数据不一刀切标敏感的 v2 政策不变）。
- **校验拒绝当日止损**（`HoloMemoryObservationScheduler` + `HoloMemoryLiveObservationCoordinator`）：模型输出被本地校验拒收时，原先仍按瞬态失败走指数退避（1/2/4…分钟）当天反复重试——同输入同 prompt 重试大概率同样被拒，一个表现差的领域可烧光当日全部静默额度。新增终态错误 `HoloMemoryCommitValidationRefused`：此类失败退避直接推到下一个 UTC 日界，当日不再重试。
...
- 涉健康跨域推断的待确认卡片需真机观察：出现频率为每周最多一条，走原「想和你确认的」队列。

## [2026-09-03] 记忆长廊「轴」档六项体验优化——滑动分层+凌晨折叠+初始定位+前瞻条+空态可建+日轴互跳

### 背景
东林对「轴」档提出三个问题：①滑动手感不可预期（不知道什么时候能滑、什么时候滑不动）；②凌晨 0–7 点没人用、是否压缩；③轴与日档功能相似如何区分。代码诊断：①根因是三条隐形规则在抢手指——整个轴面挂「长按 0.3s+拖 5pt 建任务」（按住 0.3s 后滑动即被劫持）、任务块上下缘有 14pt 隐形调时把手（长按 0.25s+拖 2pt 即接管，2pt 极易误触）、打开默认停在凌晨 0 点（滑一屏空刻度无反馈）；②周档网格早有「凌晨 0–7 折叠」交互可对齐；③本质差异存在（日=全模块记录回放、轴=计划排程可编辑）但未传达。东林拍板：名字不改，其余全部实施，含互跳隐藏问题。

### 改动（纯 iOS，无后端）
- **手势分层**（`TimelineReplayView`）：建任务长按 0.3s→0.5s，长按成立即震动 + 顶部浮出「上下拖动选择时间」提示条，拖出选区后提示自然让位——「能不能滑」的分界线从此可感知；调时把手收紧为长按 0.4s + 拖 8pt（原 0.25s/2pt 误触率高），长按成立有震动，调整中块体描边加粗 + 上下缘显示强调线（把「正在拖哪条边」画出来）。
- **凌晨折叠**（对齐周档交互语言）：0–7 点默认压成一条摘要带（与凌晨相交的任务/日程计数显示「N 项」，点击展开/收起），展开态收起入口固定在 0 点行；偏好持久化 `holo.memoryGallery.timeline.collapseMorning`；纯凌晨块折叠时隐藏（计数入摘要带），跨 7 点的块保留白天部分从带底起画；折叠态拖拽吸附下限收到 7:00（凌晨排事先展开）。
- **初始定位**：打开轴自动滚到当前时刻（今天）或首个事件前一小时（历史日，无事件落 9 点）；今天右上角常驻「此刻」回正按钮；切换折叠/日期后重新锚定不飘。
- **前瞻条**（今天）：顶部「接下来 HH:mm · 标题」条，任务与系统日程合并取最早一条，点击滚动定位到该块——强化轴「看安排」的定位，与日档「回看记录」区隔。
- **空态可建任务**：无任务无日程时不再显示孤立空态文案，轴照常铺出（刻度+折叠带），9 点位置放不拦截触摸的引导（「长按空白处拖动，直接排一件事」）——原来文案让去设时间段但页面无入口的断点消除。
- **布局抽纯逻辑**：分钟↔像素双向映射抽为 `TimelineAxisLayout`（折叠/展开两态，白天段「高度=时长」恒成立），View 只留手势与渲染。
- **日→轴互跳**：日档（含周/月）事件详情弹层，待办原记录带时间段时显示「在轴上查看排布」，点击关弹层→聚焦该天→切轴档（`CalendarEventDetailSheet` 回查 plannedStart，`CalendarRootView` 注入跳转闭包）——把两边串成互补而非各说各话。

### 验证
- App 主 target BUILD SUCCEEDED；新增 `HoloTests/Views/MemoryGallery/TimelineAxisLayoutTests.swift` 10 条契约测试全绿（双向映射可逆、折叠关键点、白天比例不变、吸附下限、跨界块渲染口径、拖拽换算比例；已挂 pbxproj）。

### 补修（东林真机反馈：长按后上下拖动无法选择时间，选区不出现）
- 根因一（手势仲裁）：建任务/调时间手势用 `.gesture()` 挂在 ScrollView 内容上，长按成立后一开始拖动即被 ScrollView 的滚动手势夺走，选区逻辑收不到任何拖动事件——这正是「长按提示出现了、但拖不出选区」的原因。两处改 `.highPriorityGesture`：长按成立后 ScrollView 让位；未成立（<0.5s 就滑）时本手势不认、滚动完全不受影响。
- 根因二（坐标系错位）：拖拽换算读的是全局窗口坐标，选区渲染用的是轴内容坐标，两者差着导航/横幅/档位条高度与滚动偏移。给轴面声明 `.coordinateSpace(name:)` 并让 DragGesture 指定同一命名空间，拖拽坐标与渲染坐标同源，错位一次消除。
- 模拟器实测（iPhone 17 Pro, iOS 26.3）：长按→拖动→松手弹出新建任务表单、时间段自动带入拖出的区间，全链路恢复。注意 Mac↔模拟器窗口坐标换算存在分钟级误差，选区吸附精度以真机为准。

## [2026-09-02] 周期回放云端化体检五连修——同周期复用+云端落档+摘要截断+环比窗口+素材截断标注

### 背景
东林 15:39 手动发起 8 月回放后出现两条同标题「一个里程碑落地的八月」、描述不同的报告。服务端取证还原完整链路：9/1 11:01 后台月初回退已自动生成过一份 8 月洞察，其标题随「历史累计摘要」（replayHistory digest）并入全局摘要；15:39 手动回放走云端轨道（9/1 云端化统一），绕过了本地轨道的全部缓存/查重防线，直接重新生成并再扣一次 memoryInsight 额度，且素材里的累计摘要未按本期截断——AI 沿用旧标题重写正文，产出「同标题不同描述」的第二份。体检另发现两处口径问题：上期对比窗口的排他边界未对齐（月对比永久丢上月末一天、周对比丢周日，环比变化被系统性夸大）；想法素材截断不告知（textContents 会被砍至 5 条但 totalCount 仍为全量，模型把 5 条样本的主题外推成「全部想法几乎都在说X」）。

### 改动（iOS 端；提示词与输出预算分档随后端 ae782879 已发版上线）
- `HoloPeriodReplayCoordinator`：发起回放前先查同周期 ready 洞察并现场比对素材指纹（snapshotHash），一致则直接落卡复用——不生成、不扣额度；数据变了（补录账单/补打卡等）才走正常生成。云端结果落洞察档案并发 `memoryInsightDidGenerate`，与本地轨道同一真相源（摘要归纳/记忆候选提取/长廊展示/后续复用全部吃到）；云端结果补齐 `fillModuleHints`+偏好重排后处理链，两条轨道零漂移。
- `HoloCloudAnalysisService`：TaskContext 携带 snapshotHash（含杀 App 冷启动恢复的持久化），finalizer 回传供落档。
- `MemoryInsightContextBuilder`：累计摘要仅当覆盖范围完全早于本期时带入（堵住本期旧标题/旧结论泄漏给本次生成）；上期对比窗口抽为共用纯函数 `previousPeriodWindow`，上期末端对齐排他边界落在本期 start（财务/习惯两处同修，月/周/自定义环比基数不再缺天）；想法素材带 `textContentsTruncated` 截断标注（含 token 裁剪路径置位）。
- 后端（已上线 ae782879）：`memory_insight_generation` v11 加标题唯一性条款（总标题不得与卡片标题相同、卡片间不得重复）与样本外推禁令；月/季/自定义洞察输出预算 4096→≥8192（`config.insightMaxTokensFor`，本地端点与云端 executor 共用——思考型模型 reasoning 与正文共享预算，4096 下长周期内容会截断导致「额度已扣但拿到失败卡」）。

### 验证
- 后端 282/282 全绿（含新增 `insightMaxTokensFor` 用例）；App 主 target BUILD SUCCEEDED；上期窗口「月 31 天/周 7 天/自定义等长」经独立脚本验证通过；三条窗口单测已写入 `HoloPeriodReplayJobTests`（测试 target 暂被工作区在途的 bridge 半成品挡住编译——`ThoughtClusterEngineStandaloneTests` 未挂 pbxproj，非本次引入，待该批收尾后可跑）。真机验收点：装新包后再发起 8 月回放应秒出且不扣额度（复用档案，实际复用的是 9/1 自动生成那份——15:39 云端那份在旧包上未落档）；补一笔 8 月数据再发起才会重新生成，且不再出现同标题两份。
...
### 背景
东林走查反馈：①总览页柱状图拖动查看时「不跟手，有错位」，高亮带叠在 Y 轴刻度栏上；②触摸气泡只有金额和余额，不知道看的是哪一天。根因：Swift Charts 的 `position(forX/Y:)` 返回的是绘图区（plot area）内坐标（`FinanceAggregation.swift` 里既有注释也写明了这一口径），但总览趋势图把它直接当全图坐标用，少补了 plotFrame 偏移（约一个 Y 轴刻度栏宽 ≈ 3 天），高亮带与气泡整体左移；明细页折线图当年补过偏移所以无此问题。

### 改动（纯 iOS，无后端）
- `TrendChartView`：高亮带、触摸气泡、断口截断标注三处统一补回 `plotFrame.minX/minY` 偏移；触摸态由「label 反查索引」改为直接存索引（顺带消除跨年同月标签重复时选错年月的隐患）。
- `FinanceAggregation` 新增共享工具 `ChartTooltipDateLabel`：气泡日期行按数据点间距自适应粒度——日粒度「8月9日 周日」、周粒度「9月1日 当周」、月粒度「2026年9月」。
- `LineChartView`（明细页）：气泡同样加日期行。

### 验证
- UI 自动化实测（种子数据 8 月 39 笔，iPhone 17 Pro / iOS 26.3）：总览柱状图横向 4 个位置轻点，高亮带与气泡全部正落指尖，气泡日期行「8月9日 周日 / 8月15日 周六 / 8月21日 周五 / 8月26日 周三」星期几全对；明细页拖动中抓拍，气泡带日期且与下方定位到的当日明细金额一致（-¥118 = 28+32+58）。

## [2026-09-02] 账本页日历与汇总卡间距修复

### 背景
东林走查反馈：账本页日历收起时，日期行与下方收支汇总卡几乎贴死。根因是收起态日历区域高度写死 80pt，而周历内容实际约 81pt——当初为消除日期下方空白调得过紧，内容底边直接顶住卡片顶部。

### 改动（纯 iOS，无后端）
- `FinanceLedgerView` 收起态日历区域高度 80 → 90，日期行与汇总卡之间留出约 10pt 呼吸间距；月历展开过渡起点同步变为 90 → 280，交互与动画节奏不变。

### 验证
- 模拟器（iPhone 17 Pro / iOS 26.3）进账本页实拍：日期行与汇总卡间距恢复，深浅色双模式确认；纯常量改动。

## [2026-09-02] emoji 搜索排序重做 + 口语补充词层

### 背景
东林反馈财务新增科目时 emoji 搜「车」找不到。排查确认 emoji 库本身是全量的（1898 个，🚗 在库），但两个真实缺陷叠加：①搜索结果按目录顺序返回，搜「车」时 🚗 排第 33 位（消防员/骑车/火车全在前面），搜「汽车」也只排第 9（官方词典给警车也标了「汽车」）；②官方中文词典缺日常口语词，「交通/出行/打车/买菜/房租/工资」等全部搜不到。

### 改动（纯 iOS，无后端）
- **排序重做**（`EmojiCatalog.search`）：命中关键词越短越核心（搜「车」时「汽车」优于「过山车」）；同长度时人工补充词优先于机器标注（🚗 压过同样标注「汽车」的 🚓 警车）；再按目录顺序保持稳定。
- **口语补充词层**（新文件 `EmojiSearchSupplement`）：叠加在生成索引之上、不改生成物（`EmojiSearchIndex` 仍由脚本生成勿手改）。首批覆盖：🚗（车/汽车/car/交通/出行/打车/代步）、打车/公交/摩托/单车，以及记账高频场景词（吃饭/外卖/买菜/超市/房租/工资/旅游/通勤/bike）。以后补词只加这个文件。

### 验证
- 新增 `EmojiCatalogSearchTests` 8 用例全过（pbxproj 手动挂载至 HoloTests/Utils，测试目录不自动收录）。
- 模拟器实测完整路径：财务 → 分类管理 → 新增科目 → Emoji 搜「车」，56 个结果第一行 🚗🛻🚕🚌🚲；搜「打车」第一位 🚕；搜「交通/出行/买菜/房租/工资」全部命中。
- 遗留观察：东林真机曾显示搜「车」无结果，与代码行为不符（疑拼音字母入框），待新包复测确认。

## [2026-09-01] 新用户引导回归修复

### 改动
- **目标入口补全**：今日看板无目标时的「设个目标」提示改为真实按钮，直接进入「创建目标」表单；创建成功后即时切换为目标进度提示。
- **记忆命名统一**：个人页及长期记忆详情页统一使用「长期记忆」，与「记忆长廊」区分。
- **引导文案修正**：AI 数据处理说明中的「问题」改为更准确的「信息」。
- **法律文档显示修复**：优先使用 iOS 中文字体，避免隐私政策和服务条款正文在模拟器中显示为方框。

### 验证
- 全新 iOS 26.3 模拟器完成新人引导及主要模块回归。
- 验证目标入口可进入创建表单，个人页和详情页均显示「长期记忆」。
- Debug Simulator 构建成功，`git diff --check` 通过。

## [2026-09-01] Plus 付费墙重设计 + 两态金线徽章 + 免费版图文修复

### 拍板与背景
东林两轮反馈驱动：①免费账号在个人页看到「Holo Plus」卡片，图文不符；②付费界面太丑、没有购买欲。设计过程先出四候选（星芒系徽章）被否——「太丑、星星没有品牌寓意、看不出想表达什么」；转向「Holo logo 为核心」出三方案，最终拍板**方案一+方案二混搭**（麦穗桂冠 + 金框印章 PLUS 缎带），素材处理授权全权代办；附加效果拍板「点亮两态」+「扫光动效仅付费墙」。

### 改动（纯 iOS，无后端）
- **免费图文修复**：个人页卡片免费态标题改「免费版」，副标题「升级解锁 2 倍 AI 额度与全部小组件」，CTA「升级 Holo Plus」；已订阅态展示不变。
- **付费墙重写（转化结构：价值先行→权益→方案→一键购买）**：hero 深卡价值主张（金色渐变字标）；权益速览（免费值划除 → Plus 橙色）；方案选择卡（年订默认选中，「立省 N%」徽章与折合月价由真实 StoreKit 价格推导，不硬编码）；主购买渐变大按钮（按钮含所选方案价格；商品若配置 intro offer 自动切换「开始 X 天免费试用」文案）；自动续订披露/恢复购买/法务链接按 3.1.2 保留。
- **两态徽章（HoloPlusEmblem 重写）**：以一笔画侧脸为唯一符号——免费态银灰素描肖像章（细灰环、未点亮），Plus 态金色麦穗桂冠 + 金线侧脸 + PLUS 缎带 + 光晕；个人页/会员中心按真实权益自动切换，付费墙恒 Plus 态并开扫光。
- **素材**：新增 `HoloFaceLineArt.imageset`——用 Pillow 从 Icon-1024 抽线稿（亮度/饱和双通道→alpha，smoothstep 软边，阈值滤掉底部渐变噪点，内切圆角遮罩去图标边框），白色透明底；SwiftUI 侧用渐变 mask 上色，金/银一图两用。
- **会员中心重排**：渐变「升级 Holo Plus」大按钮上移到权益对比表之前，升级动作第一屏可达。
- **主题 token**：HoloPlusTheme 新增 ctaGradient / wordmarkGradient / planSelectedTint，配色仍统一从 theme 取。

### 调试基建（DEBUG-only，Release 不编译）
- 启动环境变量 `HOLO_DEBUG_AUTO_SURFACE=paywall|membership`：启动后自动弹对应界面（锁屏/无 idb 环境的自动化截图通道）；`HOLO_DEBUG_SAMPLE_PLANS=1`：商品读不到时用本地配置真实价渲染方案卡；方案卡数据驱动化（PlanCardData）。

### 验证
- iOS 编译零错误；模拟器截图验证：免费态卡片图文一致、会员中心新布局、付费墙完整结构、两态徽章、扫光动效命中帧。
- 方案卡价格区在模拟器为本地样例数据（与 ASC 配置同价 ¥12/¥128），真机显示实时商品。

### 验收路径（东林真机）
1. 个人页看免费素描章 → 点卡片进会员中心（渐变升级按钮已提前）→ 进付费墙：金徽章+扫光、方案卡真实价格、大按钮含价格。
2. 沙盒购买年订走一遍（年订默认选中）。
3. 待拍板：年订 ¥128 相对月付仅省 11%，建议 App Store Connect 调至 ¥88 左右（约省 39%，不需发版；当前无存量订阅用户，零成本窗口）。

---

## [2026-09-01] 周期回放云端化——云端任务基建统一（方案 C：健康数据随回放上云）

### 拍板与背景
东林验收反馈「周期分析失败+点继续生成闪退」并拍板：**所有多轮/长耗时任务统一走云端轨道**（方案 C——回放素材含健康与活动摘要上云，即焚语义不变，隐私文案升级 v2 并对老用户重新确认）。「继续生成」闪退随本地恢复入口的退役面大幅缩小，待真机日志进一步定位存量问题。

### 改动（后端已发版；iOS 已提交待真机验收）
- **云端任务类型化（后端）**：任务表加 task_type（migration #18）；`deep_analysis`=多轮 Agent 循环（存量行为），`period_replay`=回放单轮生成。执行器单轮分支复用 insight 服务端提示词，结果原文回传（MemoryInsight schema 校验以 iOS 解析器为单一真相源）。
- **额度同一套**：云端回放消耗 memoryInsight 池（预订-提交台账，生成失败自动释放）；额度不足任务失败回落 iOS 本地链落标准额度卡，无双扣。
- **推送文案区分**：深度分析→「深度分析完成」；回放→「回放已生成」。
- **iOS 云端轨道接入**：发起回放→素材（含健康摘要）加密上传→云端单轮生成→推送/打开 App 自动落卡；进度卡显示「云端生成中，锁屏或离开 App 都不影响」。云端失败/超时自动回落本地生成链（用户无感）；杀 App 冷启动恢复领取同深度分析。
- **隐私 v2**：同意版本化（v1=仅分析数据→v2=回放含健康摘要）；新文案明确「生成周期回放时上传的数据包含健康与活动摘要（如睡眠、步数）」；v1 老用户下次发起时重新确认一次。
- 后台定时洞察的云端化（M3）待本批验收后接入。

### 验证
- 后端 286/286 测试绿（新增：period_replay 生成/额度不足不调模型/失败释放/素材缺失、start 端点 taskType 白名单）。
- 生产 E2E 真模型：含健康摘要（睡眠 6 小时 42 分/日均步数）+TIMA 大额事件的月度素材→32 秒生成标准回放卡片 JSON，健康与大额事件全部进入内容。
- iOS 编译零错误；模拟器 UI 自动化通道受阻（AX 失效），链路验证以 E2E+代码走查为准，真机验收兜底。

### 验收路径（东林真机）
1. 发起回放（任一周期）：首次会弹升级后的隐私说明（含健康摘要说明）→ 同意 → 本次本地、下次云端；第二次发起 → 状态卡显示「回放云端生成中」。
2. 发起后立即锁屏 → 等 1-2 分钟 → 收到「回放已生成」推送 → 点按打开 App 看到完整回放卡。
3. 深度分析照旧可用（v2 同意一次两类任务共用）。

---

## [2026-09-01] 云端分析完成推送通知——APNs 从零接入

### 改动（后端已发版；iOS 已提交待真机验收）
- **「深度分析完成」推送**：云端分析在服务端跑完即推本地通知「深度分析完成——结果已就绪，点按查看」，锁屏/离开 App 的用户被及时叫醒；点通知打开 App 后现有恢复链自动领取结果落卡。
- **后端**：设备令牌表（migration #17）+ 令牌上报端点（64 位十六进制校验+限流）+ APNs 发送器（jose ES256 JWT + HTTP/2 直连 Apple，连接复用、JWT 缓存 40 分钟）+ 执行器完成钩子（fire-and-forget，推送失败不影响任务终态）。
- **环境自动探测**：iOS 端无法可靠自知开发/正式环境——服务端先发正式通道，「环境不符」应答（400 BadDeviceToken / 403 BadEnvironmentKeyInToken，后者为真机开发包实测应答）自动回退沙盒通道，成功后回写环境直发；410 Unregistered（设备已卸载）自动删令牌行。
- **iOS**：发起云端分析时请求通知授权并注册远程推送（权限拒绝不影响分析）；AppDelegate 适配器接收 APNs 令牌并去重上报；**前台收到推送静默**（前台轮询 5 秒领取结果，无需横幅打扰）；entitlements 加 aps-environment。
- 模拟器不支持 APNs（注册失败静默属正常）；后端未配置密钥时整体 no-op。

### 验证
- 后端 276/276 测试绿（新增 8 用例：JWT 头/payload 结构、环境回退、410/403 语义、令牌表生命周期、上报端点）。
- 生产 E2E：假令牌全链路触发 Apple 真实应答——正式通道 403 环境错→自动回退沙盒通道→BadDeviceToken（假令牌预期），证明密钥签名与回退逻辑经 Apple 服务器验证。

### 东林待办（真机验收前置）
1. developer.apple.com → Certificates, Identifiers & Profiles → Identifiers → `com.tangyuxuan.holo-app` → 勾选 **Push Notifications** capability → Save（App ID 未启用该能力时注册推送会被系统拒绝）。
2. 真机装新构建 → 发起深度分析 → 锁屏等推送。

---

## [2026-09-01] 体检遗留四项收口——种子复活根治 + 云备份恢复链 + 两处边角

### 背景
体检报告（见下条）第五节列出的 1.0.2 候选，东林拍板随 1.0.1 一并处理。

### 改动
- **用户删过的默认账户/分类重装后「复活」——根治**。原理：种子逻辑只凭「本地为空」判断新用户，重装后 iCloud 数据要数分钟后才落地，期间老用户被误判、默认数据重铺；用户删过的默认项云端无对应行，去重合并挤不掉它。修复：新增 `SeedRevivalRepair`——真正种数据时记录种子时刻；云端回流行的 createdAt 必早于种子时刻（卸载前写入），即「老用户铁证」；云同步远端变更防抖后清理「名字仍在默认清单、无交易/预算/项目/子分类挂靠」的种子行。账户逐行按铁证识别；分类实体无时间字段，以「交易数据已恢复」作为动手信号（交易到齐后挂靠判断才可靠，宁可不删不误删）。真新用户永远无铁证，默认数据原样保留。
- **严格预算模式 + 自定义 AI Prompt 卸载即丢**：两者原先只存 UserDefaults。修复：保存时同步快照上行 iCloud 键值表（复用昵称同款通道）；本次安装没动过时从云端恢复；老用户首次升级时主动建立云备份。全部幂等，随云同步回流自动触发。
- **网关类 429 不再自动重试**：限流窗口内 1+2+4 秒盲重试加重限流、按请求计额度时可能重复扣。quota 格式 429 原本就不重试，本修覆盖网关类。
- **Ask to Buy（青少年请求购买）无反馈**：`lastErrorMessage` 此前只在付费墙商品列表为空时渲染。修复：有商品时也在按钮下方透出状态文案（等待审批/已扣款同步中等）。
- **健康/敏感记忆本机存储的用户告知**：记忆中心头部说明追加一句——健康与敏感类记忆仅存本设备、不上传 iCloud，卸载重装后不会恢复（隐私设计与数据安全的取舍，至少让用户知情）。

### 验证
- 编译零错误（提交时点）。
- ⚠️ 全量单测此轮出现 7 条「模型配置不兼容」失败，集中在想法域，与本批修复文件零交集，系工作区另一批在途财务改动所致；本批完整验证待该批次收口后复跑。

### 验收路径（东林真机）
1. 删一个默认分类/账户 → 卸载重装 → 等 iCloud 同步完成后：被删的不再回来，其余默认数据正常。
2. 设置严格预算开关、自定义任一 Prompt → 卸载重装 → 云同步后两者应自动回来。
3. 记忆中心顶部应可见「健康与敏感类记忆仅保存在本设备」说明。

---

## [2026-08-31] 发版前深度体检——四路扫描 14 处修复（P0×5 + P1×9），为 1.0.1 收口

### 背景（东林拍板）
App 已上线 App Store，东林遇到「卸载重装后分类学习映射丢失」类致命 bug（上一条已修），要求对「可能伤害用户体验的不合理之处」做全身体检后再发 1.0.1。三轮执行：①亲自审计升级迁移与数据链路；②崩溃 / 网络离线 / 内购额度 / 卸载重装四路并行深扫；③修复 + 编译 + 全量单测验证。

### 已修 P0（致命级）
- **付费用户卸载重装 Plus 永久丢失**：设备身份原先存 UserDefaults（卸载即清，未登录 iCloud 无兜底），且「恢复购买」从不向服务器回传交易凭证——服务器按新设备号查不到权益即判免费，无自救通道。修复：设备身份迁 Keychain 为主存储（重装存活、不依赖 iCloud 登录）；恢复购买后遍历 `Transaction.currentEntitlements` 逐笔回传服务器。
- **跨天日历事件回流必崩**：1.0.1 日历同步功能中，用户在系统日历把镜像事件拖成跨天（如 23:00→次日 1:00），回前台即触发断言闪退且复现。修复：规划层新增 detach 操作（撤镜像 + 清任务时间段 + 轻提示），任务本体保留；补 4 条单测锁死。
- **重装后 AI/语音配置被误删**：历史敏感数据清理迁移的「已完成」标记存 UserDefaults，重装复位后重跑，把 Keychain 里幸存的用户 API Key 当调试残留删掉。修复：完成标记改存 Keychain。
- **四处重名/重复行闪退**：与默认分类同名的一级分类做财务对比分析即崩；CloudKit 双设备同步产生同键多行时字典构造即崩。修复：全部改为合并/取最早行。

### 已修 P1（体验级）
- **断网冷启动付费用户被当免费版**：会员状态零持久化，飞行模式打开 App 全程被付费墙拦、语音截断 60 秒（Plus 应 300 秒）。修复：权益状态落盘快照，冷启动兜底展示，刷新失败不清档位。
- **深度分析点「停止」后额度照烧**：云轨取消后一个隐藏的本地全量分析仍会启动（结果不展示但扣额度跑数分钟）。修复：取消语义终止整条链路。
- **弱网提交反馈锁死弹窗**：提交中禁止下滑关闭且无取消手段，最坏挂数分钟。修复：20 秒超时放行。
- **付款后网络中断文案误导**：扣款已发生却提示「购买未完成」。修复：区分「购买已成功，同步中」。
- **重复数据的修复窗口期**：去重只在启动瞬间跑一次，iCloud 恢复数分钟后才落地，窗口期内用户亲眼看到重复账户/图标折叠。修复：首页图标配置监听云同步到达后防抖重修 + 重载。
- 另修：被取消的云分析轮询不再僵尸空转 15 分钟；回前台自动刷新会员状态；周计划落库后瞬时读失败不再闪退（改为内存直接构建快照）。

### 附带：测试基建收口
- 全量测试此前会整体崩溃：日历一期新增第 17 个工具但测试契约仍写 16，一条过时断言（fatalError）炸掉后续全部测试——此前「测试全绿」有水分。已对齐 17 工具契约。

### 验证
- 编译零错误；全量 820 个单元测试 0 失败（含新增跨天事件 4 条）；已提交推送 0fe4a919。

### 验收路径（东林真机）
1. 卸载重装（关键场景）：重装打开 App 后不重新登录、AI 配置仍在、首页图标不折叠、无重复账户/分类；付费用户 Plus 仍生效、断网打开不弹付费墙。
2. 付费链路：恢复购买后会员状态正确；付款瞬间断网，文案应为「购买已成功，同步中」。
3. 日历同步：任务带时间段自动建镜像事件后，在系统日历把它拖成跨天 → 回 Holo：应收到「已改为跨天，解除同步」轻提示，任务保留、App 不闪退。
4. 弱网（可开飞行模式模拟）提交反馈：20 秒内必有结果反馈，弹窗可关闭。

### 本轮确认不修（1.0.2 候选）
用户删过的默认账户/分类重装复活；严格预算模式与自定义 Prompt 仍存本地卸载即丢；健康/敏感记忆按隐私设计仅存本机（建议加用户告知）；旧格式纯文件附件重装后缩略图永久转圈；网关类 429 重试策略；Ask to Buy 无反馈；启动链路主线程 IO 治理。

---

## [2026-08-31] 拍立得照片堆展开——三列阶梯露出后片，不再糊成一摞

### 背景（东林真机反馈）
七项打磨把照片堆错位收窄后，多张照片几乎同心叠死：后片被顶片完全盖住，只露出两条白边，照片「糊在一块儿」。而卡片两侧各有约 60pt 留白未利用。

### 改动（iOS 已提交待真机验收）
- **层间错位拉开为「左中右」三列阶梯**：3 张以上时中片右移 58pt、左后片左移 56pt（双片 ±52pt），每张照片都露出一条可辨认的竖带；错位仍吃不满行宽，居中不溢出。
- 照片堆容器加高 6pt 适配更深的纵向错位；计数印章提升到照片层之上，防止展开后右上片压住印章。

### 验证
- 编译零错误；模拟器 3 图想法实测：三张照片各露一条竖带、印章无遮挡、caption 不被压。

### 验收路径（东林真机）
1. 记一条带 3 张照片的想法 → 日回放：三张照片应左中右阶梯展开、每张可见，右上角「3 张」印章完整。
2. 横滑翻片：顶片换人后姿态一致，仍不溢出卡片。
...
### 背景（东林圈注）
东林看真机截图圈出两处（想法正文被截到只剩三行、「此刻」分隔线夹在内容中间突兀），随整页走查再补五处，拍板一起修。

### 改动（iOS 已提交待真机验收）
- **想法正文放宽到 6 行 +「轻点查看全文」提示**：想法的价值就是当时写下的那段话，回看时不该被三行截断；超 6 行的卡片尾部出现提示，轻点进详情读全文。记账、习惯等其他模块维持原行数。
- **今天章节脚注去掉「此刻」**：「现在」的语义由列表顶端「今天的记忆还在继续」承担，章节脚注统一为「继续向下，回看前一天」，不再切断阅读。
- **章节头首末同分钟只报一次时间**：一天只有一条记录时不再出现「00:49—00:49」式自我重复。
- **带图想法去掉左侧时间列**：时间保留在拍立得照片下方的手记行里，不再左右显示两遍；照片堆同时拿回整行宽度。
- **拍立得照片堆居中 + 错位收窄**：不再贴屏幕左缘被裁切。
- **筛选按钮只在聚焦章节显示**：章节头是钉顶的，滚动交界处两个头会同屏——现在筛选器只跟正在阅读的那一天走，不会并排出现两个「全部」让人分不清哪个生效；周/月视图不受影响。
- 章节衔接间距收紧一档，阅读节奏连贯。

### 验证
- 编译零错误；展示逻辑 standalone 测试 5/5（新增「首末同分钟只报一次」断言）；模拟器造数走查：6 行截断+提示、拍立得居中不贴边、昨日单条时间单显、滚动后筛选器正确跟随聚焦章节。

### 验收路径（东林真机）
1. 记一条超过 6 行的长想法：日回放卡片显示 6 行 + 「轻点查看全文」，轻点可进详情。
2. 记一条带 3 张照片的想法：照片堆居中、不贴屏幕左缘，卡片左侧没有时间列。
3. 找一个只有一条记录的日子：章节头只显示一个时间，无「HH:mm—HH:mm」重复。
4. 在日回放里滚动到两天交界：屏幕上只有一个筛选按钮，且跟随正在阅读的那天。

---

## [2026-08-31] 问候语未设昵称省略称呼——两页收口统一组装+昵称行占位

### 背景（东林拍板）
未设置昵称时首页问候显示「下午好，你」——兜底代词「你」被硬拼进「问候，称呼」句式，读起来像半截话。拍板方案：没名字就直接不称呼。

### 改动（iOS 已提交待真机验收）
- **统一问候组装（UserDisplayNameSettings.greetingText）**：时段问候+称呼的拼接收口到一个函数——只有真正设置过昵称（非空且非兜底代词）才拼「，某某」，否则只显示时段问候。首页与任务看板页各自重复的时段判断逻辑删除、共用此函数。
- **昵称行占位**：个人页、设置页昵称行及设置页账号卡片，未设昵称时由显示「你」改为灰色占位「未设置」，避免误当代词是真昵称。
- 引导流程依赖「名字≠兜底代词」的判定不受影响（未动 fallbackDisplayName 本体）。

### 验证
- 单测 8/8 过（新增 6 例：四时段、无名字/空白/兜底代词省略称呼、占位取值）；模拟器（iPhone 16e）实测未设昵称显示「下午好」、设「东林」后显示「下午好，东林」。

### 验收路径（东林真机）
1. 未设昵称的账号看首页与任务看板：问候应只显示「下午好/早上好」等，无「，你」。
2. 设置昵称后回首页：应显示「下午好，你的昵称」。
3. 个人页/设置页昵称行：未设置时显示「未设置」占位。

---

## [2026-08-31] 记忆长廊「册页风」图片支持——想法附件照片进日回放

### 改动（iOS 已提交待真机验收）
- **想法附件照片进入记忆长廊**：带照片的想法在日回放里以「册页」形态呈现——米白纸感底色、拍立得样式照片卡（圆角白框+轻投影）、右上角印章红棕「N 张」计数（自绘印章样式）。列表消费已生成的 300×300 缩略图，原图不进长廊，保证滚动性能。
- **事件跳转统一解析器（CalendarEventOriginResolver）**：日历事件跳回原模块详情的逻辑收口——后台线程回查原始实体（不阻塞 UI）、软删除的记录视为不存在、回主线程后统一走 DeepLink 导航。「在 X 模块打开」按钮与想法卡片直跳想法详情共用同一解析，两条入口落点永远一致。
- **事件详情页对接**：详情页跳转改走统一解析器；想法事件可预览附件照片。
- 新增册页造数 fixture 单测；原型稿 `memory-gallery-image-prototype.html` 入库。

### 验证
- 模拟器验收通过：照片卡渲染、多张翻片、跳转想法详情；全量编译零错误。

### 验收路径（东林真机）
1. 记一条带 2 张以上照片的想法 → 记忆长廊日回放找到当天：照片以拍立得卡呈现，右上角有张数印章。
2. 点照片卡翻片、点卡片进想法详情：落点正确。
3. 任意事件的详情页点「在 X 模块打开」：跳转正常；把想法删掉后再试：不应跳转。

---

## [2026-08-31] 账户页列表化改版——收起卡堆拟物，回归信息密度

### 背景（东林拍板）
钱包式账户卡堆（点卡头翻卡/上下滑动/信息联动）视觉拟物、一屏放不下几个账户，东林拍板放弃卡堆方向，改回信息密度优先的列表骨架。

### 改动（iOS 已提交待真机验收）
- **卡堆整体下线**：AccountCardStackView（翻卡容器）、AccountDynamicPanel（卡间联动面板）两个视图删除，相关翻卡/置顶/环境光逻辑一并清退。
- **全部账户 N 行平铺列表**：净资产卡保留（暖深褐材质+品牌橙负债比例条），下方单卡容器内所有账户逐行平铺——点行进详情、长按弹出管理菜单（编辑/归档等），一屏看全、点哪进哪，无翻卡无屏中空白。
- **口径统一**：信用卡行语义「已用额度」取欠款绝对值；普通账户负余额展示与净资产卡「总负债」同一口径；已归档账户单独分组、不计入净资产。
- 财务总览双轴同图原型 `finance-trend-layered-prototype.html` 入库。

### 验证
- 全量编译零错误；待真机验收。

### 验收路径（东林真机）
1. 财务页账户区：应为净资产卡+账户平铺列表，一屏尽量看全所有账户。
2. 点任一账户行进详情；长按行弹管理菜单。
3. 信用卡行应显示「已用额度」；归档账户在列表尾部独立分组且净资产数字不含它。

---

## [2026-08-31] 云端深度分析放量前置——默认开启+隐私弹窗铺满修复

### 改动（iOS 已提交待真机验收）
- **云端分析默认开启（真机验收期）**：`cloudDeepAnalysisEnabled` 由「UserDefaults 手动打开」改为「未配置时默认开」，真机升级后无需手动开 flag 即可体验云端轨道；正式全量时改为服务端 feature_flags 下发并收回默认值。
- **首次隐私说明弹窗铺满修复**：弹窗外壳被 detent 固定半屏而内容偏矮时，上下两截露出系统毛玻璃底、观感「没铺满」。修复：内容撑满容器（顶部对齐）+ `presentationBackground` 统一容器底色为卡片白，双保险杜绝异色露出（与反馈弹窗同一惯例）。模拟器（iPhone 17 Pro / iOS 26.3）实测弹窗白底铺满、确认后云端链路正常。
- **TodoTask taskId 补 CloudKit 默认值**：taskId 原为非可选且无默认值，违反 CloudKit「同步实体字段须可选或有默认值」规则，云同步记录保存会静默失败（8/31 闪退的二段根因）。业务创建时恒显式赋值，默认值仅为合规。
- **QA UI 测试整理**：新增云端分析全流程驱动脚本（记数→发起→隐私确认→云端结果卡全程截图，临时脚本验收后删）；验收测试文件清退 573 行历史临时调试用例。

### 验收路径（东林真机）
1. 升级后直接发起深度分析（不手动开 flag）：应正常进入云端轨道（卡片角标「云端」）。
2. 首次弹出的隐私说明弹窗：白底铺满，上下无毛玻璃空隙。
3. 云同步开启时记录任务：任务应能正常上云（taskId 合规验证）。

---


## [2026-08-31] 记忆长廊「时间门」模态化——卡死/蒙版穿透/拖动不跟手三修

### 背景（东林真机验收反馈）
日档长按日期唤出的「时间门」有三个问题：①门偶发永久卡在屏上退不出去；②蒙版形同虚设，底下的界面照样能点，与门的「日期选择中」状态互相冲突；③上下拖动经常不跟手、切换不了日期。

### 根因与改动（iOS，单文件 DailyReplayView.swift）
- **卡死与穿透同根**：时间门原是关闭触摸测试的纯展示贴图——蒙版只是画上去的一层黑，不拦点击；它的消失只依赖长按手势正常结束，手势一旦被系统中断（来电横幅、误触等）门就永久挂屏，底下全部可点。改为真模态浮层：蒙版拦截一切触摸，**点日期之外的空白处即退出**；门上新增「轻点日期前往 · 点空白处退出」提示行。
- **交互模型重构**：长按日期开门后**松手门留在屏上**（原地松手不再直接关闭），后续交给门自身接手——卡片上上下拖动穿梭（42pt 一天，与长按拖动同口径，不越过今天）、松手或轻点卡片前往所选日期、点空白处退出。无论手势是否被系统中断，门永远有出口，不再可能卡死。
- **跟手**：长按门槛从 0.38s/18pt 放宽到 0.3s/24pt（阈值太紧时「按下就滑」的触摸会被底层列表抢走，门根本开不出来）；穿梭过程中去掉逐帧 withAnimation，数字即时跟手。

### 验证
- 模拟器全流程验收通过：长按开门且松手留存／蒙版下点「全部」筛选不穿透、门正常退出／卡片拖动松手跳转正确（31 日拖回 26 日，天数与拖距吻合）／轻点卡片前往／普通列表滚动无回归。
- 已知边界：模拟器合成输入驱动不了「按住不放继续拖」的一气呵成手势（卡片拖动链路已单独验证），该形态需真机复验。纯 iOS 改动，无后端发版。

### 验收路径（东林真机）
1. 长按章节头大日期：门出现，松手后门留在屏上；点空白处退出。
2. 开门状态下点蒙版下任意按钮/卡片：不应触发任何功能，门退出。
3. 长按后不停顿直接向上/向下拖：门应能开出来且日期跟随换页（本次重点）。
4. 门上拖动后松手、或轻点门：跳到门上显示的日期。

---

## [2026-08-31] 云端深度分析验收三问题修复——备注识别/中文化/时间口径/证据区块

### 背景（东林真机验收反馈）
云端异步分析（二期）真机跑通后结果卡片有三处缺陷：①「音乐」分类里写了备注「TIMA音乐盛典」，AI 没识别出来还瞎猜「可能是乐器或设备」；②正文直接暴露内部字段与调试语句（`spending_breakdown`、`dynamic_query`、「已在 warnings 中声明」），且没有时间范围；③证据区块整个缺失（本地轨道有）。QA 模拟器复现 + Node 引擎还原 + 真模型 E2E 三路定位后修复。

### 根因与改动
- **① 备注「没告诉 AI + 没工具看」双重缺口（后端已发版 741e8dcf）**：备注数据其实一直在上传（快照 `text` 字段），但云端给 AI 的字段目录把 iOS 目录里的中文说明丢掉了（AI 只看到 `text:text`，不知道这是备注），且云端只有聚合查询、没有「看某几笔明细」的工具，AI 无论怎么查都看不到备注原文。修复：快照字段带说明上传 + 目录输出中文说明 + 新增 `snapshot_rows` 行明细工具（AI 可查「音乐分类最大 3 笔」直接拿到行摘录含备注）。
- **②③ 渲染构造缺陷（iOS）**：`finalizeCloudResult` 把模型内部推理原文（含工具名/协议术语）直接当摘要上屏、时间范围和证据硬编码空。重做：摘要改用 AI 写的人话结论（推理原文不再上屏）、卡片标注「近180天」时间口径、云端证据随结果回传并翻译成中文（`交易明细·「音乐」：合计「交易金额」= 3316 元（来源 1 条）`）+ 行样本核对、claim 文本走与本地轨道同一条内部词防线（含英文 metricKey 的整条拦截）。

### 验证
- 后端 268/268 测试绿（新增目录说明/snapshot_rows/证据回传 5 用例）；已部署生产。
- 真模型 E2E（生产端点+TIMA 备注快照）：AI 结论明确写出「TIMA 音乐盛典 ¥3,316」并列出六笔大额备注清单，证据回传 metric 4 条 + rows 行样本——修复前同样的数据 AI 只能瞎猜。
- iOS 新增 `CloudEvidencePresenterTests` 5 用例全绿（口径句翻译/兜底/行样本/上限）；编译零错误。

### 验收路径（东林真机）
1. 发起深度分析（问消费结构/大额支出归因类问题），等云端完成。
2. 结果卡片：正文应是通顺中文（无英文工具名/调试语句）、显示「近180天」口径。
3. 点开详情看「依据」区块：应有中文统计口径（含来源条数）与「已核对 N 条交易明细」行样本。
4. 对有备注的大额支出：AI 结论应能说出备注内容（如「TIMA音乐盛典」）而非猜测。

---

## [2026-08-31] 想法模块离线整理自愈+卡片「…」卡顿修复

### 改动（iOS 已提交待真机验收）
- **断网整理不再误标「整理失败」**：原队列重试仅覆盖约 2.5 分钟（3 次退避），耗尽即 failed 死终态——重启不恢复、批量整理也排除。现在断网时挂起不发注定失败的请求；连接类错误（断网/超时，`APIError.isConnectivityLoss`）重试耗尽后回退 pending（与配额耗尽同构），等条件恢复续做；failed 只留给真正的处理失败（解析失败等），防服务端持续故障无限循环。
- **网络恢复自动续做**：NWPathMonitor 断→通沿重拾 pending 续做；App 启动开监听，回前台读 currentPath 补偿（覆盖「落地后杀 App 重开」「后台驻留时断网」两种时序）。飞行模式写笔记→落地连网自动补标签，全程无需操作。
- **卡片状态如实化**：断网挂起的 pending 显示「等待网络」（wifi.slash）而非假装「整理中」。
- **卡片「…」菜单新增「重新整理」**（仅 failed 状态提供；想法列表+主题详情页两处接线）——存量失败笔记的恢复入口；详情页菜单原有「重新整理」保留。
- **「…」点按卡顿修复（长笔记明显）**：两处重复工作消除——①卡片正文每次重算解析两遍富文本 JSON（UIKit 渲染与无障碍各取一次），改为单次解析；②`ReadOnlyRichTextView.updateUIView` 每次全量重建 attributedText+无障碍文本/动作（长文数十毫秒主线程），改为 Coordinator 缓存渲染输入、未变早退。

### 验证
- 新增 `ThoughtOrganizationQueueTests` 4 用例全绿：连接类错误耗尽回退 pending／其他错误仍标 failed（现状不回归）／断网挂起不发请求+恢复沿续做／在途请求遇断网立即回退不空转重试。
- 想法模块既有 67 测试无回归；编译零错误零警告。纯 iOS 改动，无后端发版。

### 验收路径（东林真机）
1. 飞行模式写一条长笔记→卡片显示「等待网络」；关飞行模式几秒内自动整理完成并带上标签。
2. 翻到此前标「整理失败」的存量笔记→点「…」→「重新整理」。（存量 failed 不在自动恢复范围，需手动触发一次。）
3. 列表里点「…」确认长笔记不再卡顿。

---

## [2026-08-30] 统计总览趋势卡改版——收支柱+余额线双轴同图（资产总览风格）

### 改动（iOS 已提交待真机验收）
- **终稿形态（东林拍板参考资产类 App 的双轴画法）**：每日支出/收入成对圆顶柱贴底走**左轴**（刻度 0/半高/上限），余额线冷蓝走**右轴**（最小/中位/最大三档刻度）——两把尺子都明示，替代此前「三线同图」的暗双尺方案。四轮迭代史：三线同图→分区叠加（假数据原型验证通过但稀疏真数据露馅）→纯柱状图（余额撤出）→双轴同图。
- **峰值日高亮**：单日收支最大的那天整柱实色，其余柱稍收。
- **尖峰断口截断**：单日尖峰超过次高值 2 倍且有效天数 ≥5 时，柱轴上限压缩到次高值附近并对尖峰柱做「断口」（主柱+留白+小帽+真实值标注），避免一根针毁掉其余天数的比例；稀疏月份（如只有两三天有量）不截断。
- **交互**：点按/横向拖动查看单日（柱后淡色高亮带+当日明细气泡，含当日余额）；竖向手势交还页面滚动不挡刷页；点空白收起。
- 图例红绿蓝三点+「余额较期初 ±」一句话；LegendItem 防折行加固；OverviewTabView 移除旧 BalanceChartScale 传参（该结构仅剩单测引用）。

### 验收路径（东林真机）
1. 统计→总览，看 8 月：两根轴刻度是否清晰、余额蓝线与红绿柱是否互不打架。
2. 点按/拖动图表看单日明细气泡；竖向滑动确认页面照常滚动。
3. 有大尖峰的月份看断口截断形态；顺手核实 8/28 那笔 ¥21,800 若为转账/还款，后续从收支统计剔除（趋势卡会更干净）。

---

## [2026-08-31] 云端分析第三/四轮对抗审查（攻击者+用户旅程视角）——结果确认制与重启自愈

### 发现与修复（换视角：不再从实现侧查对错，从攻击面与旅程缝隙找问题）
- **P0-R1 结果「先删后返」竞态**：领取接口先删云端密文再返回——响应在网络回程丢失（电梯/切换基站）时结果永久丢失。改为**确认制**：领取不删、客户端落地本地后 POST /ack 回执才销毁；未确认结果留 7 天，冷启动恢复可再次领取（幂等）。生产端到端验证：连领两次同结果→ack→再领为空。
- **P0-R2 服务器重启 running 任务永久卡死**：启动恢复只扫 queued，中断的 running 无人接管。启动时重置孤儿 running 回队列重新执行。生产验证：执行中重启容器→「重置 1 个中断任务」→自动重跑→完成且结论与数据分布吻合。
- **R3 任务过期后的无限重试**：7 天清理后 GET 返回 404，轮询原逻辑无差别重试到 15 分钟超时；改为识别 404 立即回落本地。
- **R4 协议校验失败分支补轮次日志**（此前该轮次无迹可查）。
- 静态确认安全的边界：taskId UUID 不可枚举+设备所有权校验、快照内文本的提示注入被输出校验器拦截、时间篡改被历史窗口策略护栏拦截、工具目录只发数据集元信息（不发原始数据进上下文）、云端分析中发起新分析走本地单承载门。

### 验证
- 后端 264/264 绿（新增 R1 确认制/R2 重置测试）；iOS 编译零错误。
- 生产端到端：R1 三步验证（重领/确认/销毁）+ R2 重启自愈全流程。

---

## [2026-08-31] 云端分析两轮对抗性自审——引擎结构对齐修订（根治复合问题循环）

### 两轮自审发现与修复
- **第一轮（静态走查）3 处**：503 服务禁用的错误类型映射修正（backendError 而非 serverError）；云端轨道分支的 progressPoller 泄漏；**杀 App 后云端任务无人领取的 P0**（taskId 持久化 UserDefaults + 冷启动 recoverIfNeeded 恢复轮询——云端结果密文暂存 ≤7 天，没有恢复入口等于丢失）。
- **第二轮（mock 真实规模数据动态验证）1 个根因级缺陷**：云端查询引擎输出为自造结构，与提示词约定的 iOS 端返回格式不一致——模型读不懂结果，复合问题（财务+习惯跨域）陷入重复查询循环，12 轮耗尽失败。**重写为 iOS 同构**：metricKey=`dynamic.{source}.{id}.{group}`（sanitize 小写）、formula、comparison（all→null）、evidenceText 格式、HoloDataToolResult 信封（错误也走 error 字段）。
- 执行器补逐轮可观测日志（轮次/status/工具请求与成败，无用户内容）。
- iOS 轮询超时 5→15 分钟（云端复合分析实测 6-10 分钟，等待不该比本地紧）。

### 验证（mock 真实规模：4 域 1980 行 250KB 快照，生产真模型）
- 修复前：复合问题 12 轮耗尽 failed；修复后：**6 轮完成**（约 6 分钟），循环消失。
- 结论质量：跑步 119/180 天≈66% 与 mock 数据分布精确吻合；对快照口径限制（无法单独切三个月）诚实披露而非编造。
- 快照大小 250KB 远低于 2MB 上限；后端 263/263 绿；iOS 编译零错误。

---

## [2026-08-30] 云端异步深度分析二期 M2b/M2c——iOS 全链路接入+生产启用，端到端验收通过

### 改动（iOS 已提交待真机验收）
- **快照聚合器**（新增 `Cloud/HoloCloudAnalysisSnapshotBuilder`）：发起时把十类数据域（财务/任务/习惯/想法/目标/记忆/对话/纪念日/洞察/档案）的结构化行数据打成快照 JSON；取数复用本地 Agent 工具的同一分发器（语义零漂移）；字段定义直接引用各工具静态目录；近 180 天窗口+单域行数上限适配后端 2MB 限制。**健康域按合规决策不进快照**（涉健康的分析自动走本地轨道）。
- **云端客户端与编排**（新增 `Cloud/HoloCloudAnalysisClient/Service`）：建任务→上传→轮询领取→结果落消息（复用 Agent 消息管道）；锁屏/离开期间轮询随进程暂停、云端持续执行，回前台自动恢复领取；任一环节失败自动回落本地轨道（用户无感）；5 分钟超时取消并回落；同一时间只承载一个云端任务。
- **隐私文案首启 sheet**（新增 `CloudAnalysisPrivacySheet`，定稿文案）：首次发起时出现一次（本次仍本地分析，下次生效），确认后不再打扰。
- **双轨开关**：`HoloAIFeatureFlags.cloudDeepAnalysisEnabled`（UserDefaults，默认关）；周计划与追问仍走本地轨道。

### 改动（后端 dd70f69c 已发版：执行器创建顺序修正+注入白名单补齐）
- 生产密钥启用时暴露的初始化顺序错误（执行器创建早于 providers 声明）修正；loadConfig 补执行器注入键（测试隔离用）。

### M2c 生产启用与端到端验收（真实模型）
- ECS 已配置独立加密密钥（32 字节 base64，env_file 注入，不回显）；云端分析服务自动启用。
- **端到端烟测通过**：建任务→上传 3 笔账目快照→云端真实模型 3 轮分析（约 60 秒）→结论正确（92 元合计+逐笔明细+数据断言+解读）→结果领取。
- **即焚验证（数据库层）**：两个完成任务的问题与快照密文均已销毁；已领取的结果密文已删、未领取的等 7 天兜底清理——「分析结束即删除、结果只回传一次」的隐私承诺由真实行为兑现。

### 验收路径（东林真机）
1. 装新包，`defaults write` 开启 cloudDeepAnalysisEnabled（或等正式放量开关）。
2. 发起一次深度分析→首次弹隐私说明 sheet→「知道了」→本次本地跑完。
3. 再次发起→状态卡「加密上传→云端深度分析中」→**立刻锁屏**→几分钟后解锁回 App→结果自动出现；期间可杀 App 重开验证领取兜底。
4. 检查设置与隐私政策文案口径（随正式放量同步核对 App 隐私标签）。
...
### 背景
- 一期止血包根治「断开白算」后，锁屏期间推进仍受 iOS ~30s 后台窗口约束；二期把深度分析执行搬上云端根治（东林拍板「先止血后上云」）。两轮对抗性审查通过后开工，设计稿见 `docs/superpowers/specs/2026-08-30-cloud-async-analysis-design.md`。
- 审查关键修正：「Holo 不存储用户数据」全局表述不成立（长期记忆在存）→ 承诺限定「本次分析上传的数据」；「分析结束即销毁」必须代码化（任务完成主动清除输入侧密文+step 缓存）而非文案修辞；第一版排除健康数据（合规保守项，可推翻）；快照传数据域全集保留动态分析循环（拒绝预挖降级）。

### 改动（后端已发版 ee089361，功能禁用态上线）
- **任务表**（migration #16 `agent_cloud_analysis_tasks`）：快照/问题/失败原因/结果全部 AES-256-GCM 密文落存（独立密钥，AAD=taskId），数据库零明文。
- **端点四件套**：`POST /v1/ai/agent/cloud/start`（创建+限流）、`PUT .../:id/snapshot`（快照密文落存，仅 uploading 态，≤2MB）、`GET .../:id`（状态/结果回传即销毁密文）、`DELETE .../:id`（取消整行销毁）。设备所有权隔离，跨设备 404。
- **隐私契约代码化**：完成/失败立即销毁问题与快照密文；结果回传一次后销毁；7 天无人认领过期兜底清理（复用 step 清理节拍）。
- **零风险上线**：`HOLO_CLOUD_ANALYSIS_ENCRYPTION_KEY` 未配置时端点统一 503、服务正常启动——ECS 配置密钥后自动启用，无需再发版。

### 验证
- 8 个新测试：生命周期/密文落盘断言/所有权隔离/销毁彻底性（完成+失败+取消+过期）/重复上传拒绝/禁用态；全量 256/256 绿。
- 生产验收：migration #16 应用、公网健康、端点禁用态 503 确认。
- **后端已发版**（影响：新端点+新表，密钥未配置时对外无行为变化）。

### 待办（M2）
- 云端执行器（Node 版 runLoop+快照工具查询+完成即焚钩子+APNs 推送）。
- iOS 双轨接入（feature flag+失败回落本地轨道）+ 隐私文案首启页 + 隐私政策与 App 隐私标签核对。
- ECS 配置 `HOLO_CLOUD_ANALYSIS_ENCRYPTION_KEY`（32 字节 base64）后端点自动启用。

---

## [2026-08-30] 锁屏分析止血包——断开不白算 + 恢复哨兵不假转圈 + 遥测上云

### 背景
- 8/30 17:33 锁屏深度分析事故（后端日志钉死）：深度分析第一步连续 4 次在精确 ~30.5 秒处被手机端断开（锁屏后 App 仅剩 ~30s 后台窗口），每次断开后端把「客户端断开」连坐传导给模型调用——模型边算边被掐、幂等缓存永远建不起来，重试全部从头算全部作废，形成「边算边掐」死循环。更严重的是解锁后恢复链并未真正拉起（10+ 分钟无请求），界面却显示「自动进行中」假转圈；且关键诊断证据只存手机本地，排查只能靠推演。
- 复核确认：额度池按 runId 幂等本就无双扣；当日撞 429 的是普通对话 50 次/天日限（全天正常用量打满），与锁屏重试无关，限流数字调整另行决策。

### 改动（后端已发版 4e2a3efe）
- **断开不白算**（`HoloBackend/src/app.js`）：`agent_loop` 的 step 幂等请求不再把客户端连接信号传给上游模型调用——锁屏/切后台断开后模型照常算完并落幂等缓存，手机重发同 stepID 直接命中秒回。锁屏期间从「每个窗口从头算永远失败」变为「每个窗口推进一步」。其余 purpose（含流式 chat）保持断开即止。
- **Agent 遥测上报端点**（新增 `POST /v1/ai/agent/telemetry` + `agentTelemetryStore` + migration #15）：iOS 端锁屏租约/执行恢复/任务终态等技术诊断事件批量上报落库（14 天滚动保留，字段白名单无用户内容，事件 id 幂等去重）。出障后远端可直接还原时间线。
- 后端 248/248 测试全绿，生产部署验收通过（migration 应用 + 端点烟测 accepted:1 + 公网健康）。

### 改动（iOS，待真机验收）
- **恢复哨兵**（`HoloBackgroundContinuationManager`）：恢复链触发后快照等待态 job，10 秒复查 state 与 updatedAt 均未变化且无活跃执行 = 恢复未兑现——落 `waitReason = .resumeStalled`，消息卡片从「会自动继续」切换为「恢复没成功 · 点立即继续」的如实文案（复用暂停卡按钮链路）；哨兵随恢复 Task 生命周期自动取消重建，不堆叠。同步落 `agent_resume_stalled` 遥测事件。
- **waitReason 新枚举值 `resumeStalled`**：只改写等待原因不改变 state，等待态恢复不按 waitReason 过滤，不挡后续自动重试；恢复成功时写路径自然覆盖。
- **遥测增量上报器**（新增 `HoloAgentTelemetryUploader`）：App 前台化/冷启动时按 watermark 增量上报本地事件（≤100 条/批），失败静默下次再试。
- PauseNotifier 补 `resumeStalled` 对冲通知分支。

### 验证
- 后端：新增 2 个断开场景测试（断开算完落缓存重发命中免重算 / 非 agent_loop 保持断开即止）+ 5 个遥测端点测试，全量 248/248 绿。
- iOS：Debug 构建成功零新增警告；HoloAgentSchedulerTests / ContinuationTests / ContinuedProcessingTests 全过（含 3 秒完整网络恢复链路）。
- **后端已发版**（影响：agent_loop 断连处理语义 + 新增遥测端点与表，其余接口零变化）；iOS 待真机验收——发起深度分析→锁屏→等两分钟→解锁，预期任务自动续上且结果完整，额度不再因分析重试缩水。

---

## [2026-08-28] 财务模块视觉精致化——账户卡堆去重收口 + 统计总览趋势卡三线同图

### 背景
- 账户页真机走查反馈：展开卡余额出现两遍、卡身收支胶囊与动态面板三格重复、负余额卡面无负债语义（同一笔钱净资产卡算负债标色、卡面却是白色正常余额）、「› 卡片视图」按钮命名反直觉、悬浮橙色把手条像界面残留；添加账户与右上角 + 功能重复，且虚线框曾被卡片叠压读作排版错误。
- 统计总览趋势图为「每日双柱 + 蓝色余额点线 + 左右双 Y 轴」：点串感强、双轴数字费解、蓝色与全局暖色调出戏；首版单序列改版又被否——余额是总览核心信息，须与收支同图。

### 改动
- **账户卡堆**（`AccountCardStackView` / `AccountCardMaterial` / `AccountDynamicPanel` / `AccountListView`）：展开卡金额只在卡身出现一次，卡头右侧改账户类型英文排印；删除悬浮把手条（橙描边光晕已标示当前卡）。负余额统一负债语义——卡面大字、收起卡、管理列表、动态面板净收支全部使用暖橙红（与净资产卡「总负债」同色），负号规范为「-¥42.50」。收支统计下放动态面板，卡身回归银行卡面语言（大余额 + 芯片 + 水印），本月零收支时面板收拢为一行；信用卡三胶囊改「可用 ¥xx」排印行。图标底色染各账户强调色、收起卡叠压阴影减轻、切换按钮「卡片视图/卡堆视图」改「列表/卡堆」。添加账户唯一入口收敛到导航栏 +（补无障碍标签），堆底虚线卡删除。
- **统计总览趋势卡**（新增 `TrendChartView`，删除 `BarChartView`）：支出/收入/余额三线同图——余额线品牌橙替代原蓝、渐变水位垫底；monotone 单调插值平滑曲线、无逐日数据点（catmullRom 的过冲与急坠消除）；余额经缩放映射同轴、右轴刻度移除（精确值走触摸气泡）；分段切换控件移除，各线独立 series 防跨系列串线。
- **总览其余两卡**：周期汇总卡补「周期汇总」标题行（日期范围 + 笔数，修正排他日期边界显示）；分类排行自绘红胶囊换系统分段控件。
- **测试同步**：UI 回归断言随新文案与卡片结构更新，并做置顶卡动态适配；测试滚动/点击稳健性修复（拖拽避开卡堆翻卡手势区与底部 tab 栏、移除两段式置顶交换竞态）。

### 验证
- iPhone 17 Pro 模拟器（iOS 26.3）三轮截图走查：负债展开卡、收起卡堆叠压、管理列表、动态面板零态、深色模式、趋势卡三线效果与平滑度。
- HoloUITests `testFullAcceptance` 47 项检查全部 PASS、0 失败。
- 纯客户端改动，**无需后端发版**；待真机验收。

---

## [2026-08-25] 账户卡堆翻卡动效修复——卡身跟卡滑动 + 过渡期不再盖住「添加账户」

### 背景
- 真机反馈两个动效问题：①翻卡时卡壳在平滑滑动，但卡身内容（余额大字、橙色把手条）原地淡入「等着卡片贴上来」；②过渡期间最后一张收起卡会短暂盖住「添加账户」按钮。

### 改动
- **卡身转场**（`AccountCardStackView.swift`）：展开卡下半身挂 `move(edge: .top) + opacity` 转场并外挂 `clipped()` 裁掉越界段——置顶时从卡头下方滑出、收起时滑回，与卡壳位移动画共用同一条 spring，不再是两层各动各的；「收起卡只渲染卡头」的原架构保持不变。
- **层级不参与弹簧插值**：卡片与添加卡的 `zIndex` 挪到 `.animation(_:value:)` 之外——层级瞬间按目标态生效，翻卡过渡期间不再出现「飞卡层级没换过来、盖住途经卡/添加卡」的错乱。稳定态几何本就正确（添加卡层级全场最高、位于最后一张卡下方），遮挡只发生在动画期。
- **添加卡补同步动画**：增删账户导致添加卡挪位时，原本是瞬移而卡片走弹簧，两层错位；现补上与卡片同一条 `value: order` 弹簧。

### 验证
- iOS Debug Simulator 构建成功；HoloUITests `testFullAcceptance` 49 项检查全部 PASS（含收起卡 64pt 步进、「添加账户」存在/可见/位于最后一张卡下方的三项几何断言、点卡头置顶、上滑/下滑翻卡、长按管理菜单全流程）。
- 纯客户端改动，**无需后端发版**；待真机复验快速连续翻卡时「添加账户」始终可见、置顶卡身随卡滑出。
...
## [2026-08-25] 提审前功能收口——报告收藏追问、分析场景、账户卡堆与长廊头部

### 改动
- **HoloAI 分析入口**：空状态新增场景面板，按财务、习惯、任务、健康、想法、目标等真实数据域展示可直接发起的分析问题；入口文案和配色支持深色模式。
- **报告档案闭环**：深度分析与周期回放统一进入报告档案，支持场景筛选、关键词搜索、收藏列表，以及从报告详情继续追问；追问结果保留与原报告的关系，避免上下文断裂。
- **财务账户卡堆**：账户页升级为纵向卡堆，统一银行卡材质、动态余额/额度信息和详情入口；保留账户编辑、余额调整、归档与删除能力。
- **记忆长廊头部**：收紧导航密度并强化报告入口，完善洞察跳转与日期排序，避免从卡片进入错误记录。
- **预算/纪念日对话卡**：补齐结构化卡片渲染、确认创建和详情跳转，统一已删除状态反馈。
- **验收基础设施**：加入 App 锁单元测试、UI 验收目标与测试宿主隔离，防止测试进程启动真实业务服务；补充深色模式、报告收藏和全屏返回手势守则。
- **本地验收产物隔离**：`accept_r5/` 与 `.accept_r5/` 截图目录加入 `.gitignore`，保留本机验收证据但不进入发行提交。

### 验证
- iOS Release 真机配置无签名构建成功，主 App 与 Widget 均使用 `MARKETING_VERSION=1.0`、`CURRENT_PROJECT_VERSION=21`。
- 完整单元测试（跳过会长时间不结束的 `HoloLocalAgentRuntimeTests` 桥接用例）实际执行 767 项、0 失败；App 锁定向测试 7 项、0 失败，HoloTests/HoloUITests 测试目标编译成功。该 Agent Runtime 用例在全量运行中超时，不以通过冒充。
- App Store 静态预检已核对 Privacy Manifest、HealthKit/CloudKit/App Group 权限、Apple Health 可见说明、订阅价格/自动续订/恢复购买/隐私与条款入口。
- 真机 Sandbox 内购、Apple Health 授权录屏、signed Archive/TestFlight 与 CloudKit Production schema 仍按提交流程执行，不以本地构建替代。

---

## [2026-08-24] 数据清理与回收站——模块清空/清空所有数据 + 30 天可恢复 + 恢复冲突预检 + 已清除感知

### 背景
- Holo 此前没有「清空所有记录」能力，删除都是逐条硬删、删了就没了。东林需求：财务/想法/任务/习惯四模块各可清空（健康只读系统库不参与），设置页可清空所有数据；被删数据 30 天内可恢复，恢复与新数据冲突要有明确处理。方案定稿于 docs/plans/2026-08-24-data-cleanup-recycle-bin-plan.md（D1-D7 东林拍板：财务清空前选范围；全局清空含聊天/报告/记忆/周计划、需输入「清空」二字确认；想法入口在列表「…」菜单；回收站仅设置页统一入口；AI 记忆不连带清但点击已删数据要有感知；单条删除不进回收站）。

### 改动
- **统一软删除基建**：约 30 个业务实体新增 `deletedAt/deletedBatchId` 字段（程序化模型工厂统一接入，`SoftDeletable` 协议集中声明于 Models/SoftDeletable.swift，deletedAt 建索引）；新增 `RecycleBinBatch` 批次实体（随 CloudKit 同步——A 设备清空 B 设备回收站可见可恢复）。`deletedBatchId != nil` = 清空批次（回收站展示可恢复）；nil = 单条软删（不展示、30 天后物理清理）。
- **RecycleBinService + RecycleBinRestoreEngine**：模块清空/清空所有数据（后台分批打标记）→ 事件批次 → 恢复（依赖联动：交易的账户/打卡的习惯/任务的清单软删时一并恢复）→ 立即清除（物理删+附件文件+任务通知取消+空批次自清）→ 30 天过期清理（挂 App 启动维护链）→ 旧软删标记（isSoftDeleted/deletedFlag 无时间戳）一次性迁移根治「永远躺库」积压。
- **恢复冲突预检**（东林点名规划）：软删除下同 id 唯一无主键冲突，冲突=「内容撞车」——财务复用导入去重指纹（日期|金额|类型|分类|账户）、任务=标题+截止日、习惯=名字+类型、想法=全文；疑似重复默认跳过保留现有数据，恢复预览页逐条对比可勾选强制恢复，系统绝不自动删任何一边。
- **全库查询过滤**（防「清空后 AI/小组件还读到旧数据」灵异）：三路并行改造约 40 个文件——Repository 层、AI 服务层直查（UserContextBuilder/IntentRouter/TaskAnalysisContextBuilder/MemoryInsightContextBuilder/DailySenseStateBuilder/Agent Tools 四数据源/MemorySignalDataAdapter/FlexibleQueryExecutor/LifePlanGenerationService 等）、周边（小组件快照/周报/任务通知/纪念日生成器）、长廊 KVC 聚合；旧 `deletedFlag == NO`/`isSoftDeleted == NO` 谓词统一迁移到 `deletedAt == nil`（单一事实源），聊天卡删除态判定同步换轨。
- **入口**：财务→设置 Tab「清空财务数据」（范围选择：仅交易/全部，D1）；任务→归档管理页；习惯→设置页；想法→列表「…」菜单（D4）；设置页新增「数据管理」子页（数据概览+最近删除回收站+清空所有数据输入「清空」确认页，D3/D5；替代原「清除观点数据」硬删入口）。「清空所有数据」含四模块+纪念日+目标+聊天+AI 报告+长期记忆（含敏感库独立 store+记忆墓碑）+周计划；不含健康/账号订阅/偏好设置，与「删除账号」在文案上明确区分。敏感记忆库 ControlState 为运行状态单例，清空时物理重置自动重建。
- **已清除感知（D6）**：深链统一规则「目标不存在或 deletedAt != nil → 提示」——财务交易深链弹 toast（原静默）、习惯深链 toast（原永久转圈）、想法详情「该想法已被删除」占位（原空白页）、记忆证据跳转前校验拦截（原跳走后无反馈）、任务深链文案改「该任务已被删除」（TaskDetailView「30 天可恢复」空头承诺同步改为「删除后无法恢复」，D7）；聊天卡删除态预填扩展到纪念日卡+卡片 footer 统一「已删除」角标；日历回放原记录回查补软删判定。
- **恢复后联动**：任务提醒重排、各模块 dataDidChange 广播、聊天页常驻内存经 ChatMessageRepository.shared 重载（报告 Tab onAppear 自然刷新）。

### 验证
- iOS Debug 编译绿（模型层/服务层/过滤批/UI 批四轮全量编译验证）。纯客户端改动，**无需后端发版**；CloudKit 新字段随 App 发版更新 Apple 侧 schema（见 docs/appstore-preflight/CloudKit-schema部署指南.md）。
- 待真机验收路径：①财务设置→清空（仅交易档）→账本清空/账户分类还在→回收站事件卡→整批恢复；②清空后重新导入同账单→恢复→冲突预览默认跳过；③想法「…」/任务归档页/习惯设置各自清空；④设置→数据管理→清空所有数据（输入「清空」）→各模块空→AI 记忆还在→点记忆证据→「该记录已被清除」；⑤回收站立即清除/29 天边界（可改设备时间）；⑥清空任务后通知不再发、恢复后提醒重排；⑦聊天卡对已删交易/任务/纪念日置灰+「已删除」角标；⑧多设备（iCloud 同步）A 清 B 恢复。

---

## [2026-08-24] 应用锁（FaceID / 系统密码）——设置自主开关 + 5 分钟宽限 + 后台快照遮罩

### 背景
- Holo 存有记账、想法、长期记忆等高隐私数据，此前拿到解锁状态手机的人可直接翻看。东林拍板（方案 docs/plans/2026-08-24-app-lock-plan.md）：D1 系统验证（密码=手机解锁密码，无忘记密码负担）；D2 三档锁定时机可选（立即/1分钟/5分钟），默认 5 分钟内免重验；D3 后台快照遮罩随锁一并做。

### 改动
- **新增 `AppLockSettings`/`AppLockManager`/`AppLockOverlayView`**（Services/Security + Views/Security）：设置页「隐私与安全」分区新增「应用锁」开关（开启前预检设备是否设密码，通过后现场验证一次才生效；取消验证弹回）+「锁定时机」三档菜单。
- **解锁链路**：`LAContext.evaluatePolicy(.deviceOwnerAuthentication)`——FaceID 优先、失败/不可用自动转手机密码键盘；取消/失败绝不放行，锁屏按钮可重试；开启后用户又在系统里关掉设备密码的极端情况（passcodeNotSet）重置开关并放行，不把用户锁死。
- **锁定判定双防误伤**：只有真正进过后台（didEnterBackground）才算「离开过」，下拉控制中心/系统弹窗不算；宽限判定抽成纯函数并配 7 条单测（固定时间戳，HoloTests 四处登记 pbxproj）。
- **显示双通道（防两类闪泄）**：①HoloApp 根部 ZStack 常驻闸门层——开启即锁在冷启动首帧前同步确定，绝不先闪内容再上锁；②独立系统级 UIWindow（层级高于一切 App 内容）——热回锁定时盖住 fullScreenCover/sheet（付费墙、CSV 导入预览等系统级浮层无法绕过锁）。
- **后台快照遮罩**：willResignActive 即盖不透明遮罩（仅 App 标识），多任务切换器看不到页面内容。
- **工程配置**：pbxproj 两处补 `INFOPLIST_KEY_NSFaceIDUsageDescription`（缺失时真机调 FaceID 会被系统直接杀 App）。
- 后台任务（Agent 续跑、洞察生成、提醒排程）完全不受影响，锁是纯界面层；深链/通知点击在锁定期挂起，解锁后直达目标页（现有 DeepLinkState 机制天然兼容）。

### 验证
- iOS Debug 编译绿（App + 测试 target，新文件零警告）；AppLockManagerTests 7/7 通过（宽限边界/立即档/时间戳缺失保守锁/配置持久化/脏数据回落默认档）。测试方法须用 async：同步 @MainActor 测试在 hosted test 下对象于测试线程释放会触发 MainActor backdeploy dealloc 崩溃（malloc double-free），已按项目既有惯例修正。
- 纯客户端改动，无需后端发版。
- 待真机验收路径（FaceID 真机）：①开启开关→立即弹验证，取消→弹回；②设备未设密码→开启被拦有提示；③杀进程重开→首帧即锁屏无内容闪现；④切后台 5 分钟内回→免验证直接进入，超 5 分钟回→自动弹 FaceID；⑤取消验证停锁屏，点「解锁」重试，任何取消路径不放行；⑥FaceID 失败转手机密码键盘；⑦锁定期点通知→解锁后直达；⑧多任务切换器只见遮罩；⑨锁屏期 Agent/提醒照常跑；⑩深浅色/英文键盘锁屏样式正常。

---

## [2026-08-24] 洞察胶囊链路根治——点击直达洞察内容 + 已读时机后移 + 观察与记忆按时间排序

### 背景
- 东林报障三连：①首页「新洞察」胶囊点进记忆长廊再返回，胶囊直接消失；②点过去找不到提示的洞察在哪；③长廊「观察与记忆」每次都先展示财务、无时间顺序，不知道新洞察在哪。
- 排查结论（结构级断裂，非偶发）：7/13 胶囊上线时点击会切到长廊洞察 tab 并展开回放卡（`focusWeeklyInsight` 定位闭环）；7/29 长廊撤除回放卡时把消费代码一起删了，洞察内容此后只在 AI 对话页展示——但胶囊仍跳长廊（一个没有洞察内容的页面），且点击瞬间就 markRead + 隐藏胶囊。双重断裂：入口指向空页 + 没看到内容入口就没了。

### 改动
- **已读时机后移**（方案 §7.5「已读=看过内容」语义归位）：`HomeView.handleScheduleTap` 与系统洞察通知点击都不再提前 markRead；已读改由 ChatView 把洞察卡片落地展示后落（`ChatViewModel.openScheduledInsight`），随后刷新首页候选让胶囊让位下一条。跳转链路断了胶囊就还在，不再出现「没看到内容、入口却消失了」。
- **胶囊直达洞察内容**：`.memoryInsight(insightId:)` 深链改由 ChatView 消费（同 MemoryGalleryView 聚焦新记忆的 .task + onChange 双消费模式）；`HoloPeriodReplayCoordinator.presentCachedInsight` 把已生成的洞察直接落成一张完成的回放卡片——不触发生成、不受额度预检拦截（额度拦新生成，不拦查看已有结果）、不走重试/息屏接管链路。
- **普通洞察胶囊同构修复**：今日/本月洞察胶囊原先跳长廊洞察 tab（同样没有内容），改为带 insightId 走同一条 Chat 直达链路；并补未读过滤（`readAt == nil` 才投递），与周观察胶囊口径一致——看过即让位。
- **「观察与记忆」按时间排序**：领域分组从固定枚举顺序（财务常驻第一组）改为按组内最新一条记忆的 updatedAt 倒序，从上往下=从新到旧；组头补「N 条 · M月d日更新」标注，最新内容位置一眼可见。组内本就 updatedAt 倒序，不动。

### 验证
- iOS Debug 编译绿（模拟器目标）；HoloTests 无既有用例覆盖此链路，无失配。
- 纯客户端改动，无需后端发版。
- 待真机验收路径：等首页出现「上周洞察/今日洞察」胶囊 → 点击应直接进 AI 对话页并看到该洞察的回放卡片（秒出，无生成转圈）→ 返回首页胶囊应已让位（消失或换下一条候选）；长廊「观察与记忆」第一个分组应是最近有变化的领域且组头带更新日期。

---

## [2026-08-23] HoloAI 对话框补齐 4 项能力——预算设置、纪念日创建/修改、目标达成/暂停、建任务指定清单（P0+P1 批）

### 背景
- 意图覆盖差集盘点（2026-08-23）：任务/记账/习惯/目标/想法录入对话框已覆盖，断档集中在「设置/管理类」操作——纪念日只能读不能写、预算/账户完全断开、目标不能改状态。东林拍板 P0（目标状态更新、建任务指定清单）+ P1（预算设置/调整、纪念日创建/编辑）本期实施。

### 改动（按意图注册表规范两端同步）
- **新意图 ×3**（后端 `intents.json` v27→v28，生成器重出 `IntentDescriptor.swift`）：
  - `set_budget`：设置/调整总预算或分类预算（复用记账的分类匹配链：`categoryCandidate` + 预览 + 学习到的映射），`period` 支持 month/year/week（默认 month）；已有预算自动转为调整（update 而非重复创建）。挂默认账户，与预算设置页行为一致。
  - `create_anniversary`：新建纪念日/生日/倒数日/里程碑（`typeCandidate` 决定默认图标与是否每年重复），确认后卡片可点跳纪念日详情（复用既有 `anniversaryDetail` 深链）。
  - `update_anniversary`：修改已有纪念日的日期或名称，唯一匹配直接执行（与 `update_task` 同构：精确名 > 唯一包含，无/多命中文本追问）。
- **字段扩展 ×2**：`update_goal_field` 的 field 新增 `status`（completed 达成 / paused 暂停，多目标歧义走既有 goalChoice 选择卡）；`create_task` 新增可选 `listName`（精确名 > 唯一模糊匹配清单，匹配不到放默认位置并在回复中说明，不阻断创建）。
- **确认交互**：预算与纪念日创建走「待确认卡 → 确认/取消/失败重试」模式（与记账/任务同构，含 confirming 中间态防双击）；新卡 `BudgetChatCard`（总/分类预算 + 周期展示，确认后不可点——预算无详情页，杜绝假交互）与 `AnniversaryChatCard`（类型 emoji + 日期，确认后整卡跳详情）。目标状态与纪念日修改为直接执行（低风险可逆，与 update_task 一致）。
- **路由防误吸**：三个新意图全部进 `shouldRouteToDeepAgent` 的执行意图排除清单（否则会被深度分析 Agent 吞掉不执行）；`set_budget` 不进 `financeIntents`（避免混入交易确认谓词导致多卡消息误操作）。
- **评测语料**：`seed-v1.json` 新增 11 条样本（含反向边界：「预算还剩多少」应走查询而非 set_budget、「下个月有什么纪念日」不应误吸到 create_anniversary）。
- **红线重划**：意图 prompt 长度红线 4950→5710（v28 实际 ~5679，按惯例「结构变更允许重划红线并升版本」）。

### 验证
- 后端 241/241 测试绿（含意图注册表对拍/评测语料覆盖/prompt 红线三重护栏）；iOS Debug 编译绿、测试 target 编译绿、ChatCardDataTests 等相关测试绿。
- **需要后端发版**（`intents.json` + prompt 版本 v28 在后端，客户端 prompt 从后端拉取；发版前 iOS 新版识别不到新意图——App 内 DEBUG 兜底 prompt 已含新意图，开发机可直接验证）。
- 待真机验收话术：「这个月餐饮预算调到 2000」「10月3号是爸妈结婚纪念日，帮我记一下」「把结婚纪念日改到10月5号」「英语目标达成了」「减肥目标先暂停一下」「把明天的会议提醒放到工作清单」（清单名需已存在）。
- 已知边界（第一期未做）：纪念日确认中途 App 被杀无实体级对账（Anniversary 实体无 AI 来源标记，重启后卡片回 pending 可重点，极小窗口内可能重复创建，重复项可手动删除）；预算确认是幂等的（重复=再调整一次，无风险）；完成/暂停的目标暂不能通过对话重新激活（匹配只在活跃目标中）。
- 顺手修复：`HoloAgentResultRendererTests.swift:317` 参数顺序笔误（并行会话遗留，挡整个测试 target 编译）。

---

## [2026-08-22] 长廊日历日回放——上滑回看更早日期（列表反转为自最新向过去）+ 撤无意义下拉刷新

### 背景
- 东林报障：日历日回放从今天（如 22 日）往上滑想看 21 号，列表不允许滑动、直接触发顶部下拉刷新，而刷新后内容毫无变化。排查结论：日回放窗口只有「向未来（向下读到今天）翻页」的单侧能力，初始窗口只有今天一天，更早日期根本不在列表里；上滑到顶的手势被根视图挂的 `.refreshable` 截胡。且东林的直觉方向是「上滑=看更早」，与原升序结构（向下=走向未来）相反。
- 连续两轮「顶部插入更早内容+位置保持」方案真机翻车：一轮 scrollTo 异步锚定（打开定位到窗口最早日 8/10 而非今天）；二轮照聊天页 KVO contentSize 增量补偿（更糟，链式插入把首屏推到 7 月，且滑动卡顿）。结论：SwiftUI 下大体积顶部插入的位置保持没有可靠解——已在踩坑速查表结案为结构级规则。

### 改动（三轮定稿：反转列表结构）
- **列表自最新向过去排列**：`DailyReplayView` 章节改为降序——今天固定在列表顶部，打开即今天（零定位逻辑，结构上不可能定位错）；**上滑回看更早的一天**（与直觉及聊天页一致），向下是走向更晚直到今天。翻页只有向过去一个方向：最早章节接近视口时在**列表底部**追加更早 12 天（`DailyReplayPageWindow.earlierPage` 纯函数）——底部追加不动视口，零补偿零锚定零 scrollTo，也不存在链式触发。初始即带 12 天缓冲。
- **方向语义全面对齐**：空白日滑动手势（上滑=前一天、下滑=更晚封顶今天）、时间门穿梭（向上拖=更早）、页脚文案（「继续向下，回看前一天」）与「今天的记忆还在继续」终点卡（移至今天上方的未来位）。
- **撤三档下拉刷新手势**：`CalendarRootView` 根视图的 `.refreshable` 移除（本地数据无可刷新之物，且截胡上滑手势）；失败重试横幅按钮继续走 `refreshForCurrentScale` 不受影响。
- 删除二轮引入的 `DailyReplayScrollPreserver` 补偿桥（结构性不再需要）。
- **打开卡顿（1-2 秒）与快速滑动卡顿治理（东林反馈后深度排查）**：排查确认打开时在主线程一次性做三件重活——①±60 天（120 天）四模块全量查询；②记账/想法逐笔的 Core Data 关系惰性加载（N+1：每笔交易查 2 次分类/账户、每条想法查 1 次话题，几百次小查询）；③与长廊洞察 Tab 的加载链并发挤主线程。修复：**首屏只拉 ±20 天**（覆盖日回放窗口+近程翻页+切周，数据量降 2/3），首帧渲染 0.4s 后全量 ±60 天错峰补齐（合并去重）；取数通道加进行中标记，续载/预载/重试并行触发时只跑一个；`getTransactions(from:to:)` 与 `fetchThoughts(from:to:)` 加关系预取（`relationshipKeyPathsForPrefetching`），一次查询带回归类/账户/话题，消灭 N+1。快速滑动的另一个撞车源（滑到数据边缘触发主线程续载）随全量预载错峰到位后基本不再出现（触发点退到距边 46 天）。

### 验证
- `DailyReplayPresentationStandaloneTests` 10/10 绿（翻页无下限跨月、空白日导航新方向含今天封顶）；Debug + Release 双配置编译绿。
- 纯 iOS，无需后端发版。
- 待真机复验：①打开定位在今天，打开瞬间的 1-2 秒卡顿明显缓解（首屏数据量降 2/3 + N+1 消除）；②上滑连续回看更早，快速滑动无撞车式卡顿；③滑到很远的历史日期也能继续往前翻；④时间门/日期选择器/月历「回放这一天」定位正常；⑤空白日上滑跳前一天。
- 已知边界：打开后 0.4 秒内立即切月档，月历可能先显示部分格子、约 1 秒内补全（切月档会主动触发全量拉取兜底）。
- 后续候选（未动）：长廊洞察 Tab 的加载链（MemoryGalleryViewModel.refresh 全量记忆条目+高亮/里程碑检测）与日历侧检测器重复跑，是打开卡顿的另一半来源； repos 全量后台化（需打破「viewContext 主线程」架构约定）需单独立项。
- 注意：阅读方向与此前版本相反（原「向下=进入下一天」），现「上滑=回看更早」——与报障时的直觉方向一致。

---

## [2026-08-22] Agent 锁屏假失败收口——灵动岛卡片语义修正 + 对冲通知防误读（验收反馈批）

### 背景
- 东林真机验收首轮反馈：发起分析后离开 App，收到「分析失败」提示、下拉可见失败记录、灵动岛显示失败；进 App 实际是暂停态，点继续后再退出 App 分析成功。三个问题：①灵动岛假失败；②对冲通知标题「已暂停，未失败」被快速阅读误读成「失败」；③任务终态后通知中心的暂停提示不立即消失。另确认「被收回后重新申请后台执行权」已生效（第二次锁屏跑完）。

### 改动
- **系统卡片失败样式只留给真失败**：此前任务让位等待/暂停/取消/被抢占时都以「未完成」标志闭合后台任务卡片，系统将其定格为失败样式且无法程序化清除——这是灵动岛假失败的根源。现统一为仅任务真正失败（failed 终态）才以失败样式闭合，其余全部按正常样式静默消失（四处闭合点：系统收回/执行收尾/取消/暂停）。
- **对冲通知与 App 内暂停文案去掉「失败」字样**：标题统一「已暂停 · 进度已保存」（原「已暂停，未失败」的否定式反而在快速阅读时制造「失败」联想），通知中心/App 卡片/测试断言同步。
- **终态立即清暂停通知**：任务到达终态的瞬间清除通知中心可能残留的暂停提示（原要等下一次状态同步，存在「点继续跑完、通知还留着暂停」的窗口）。

### 验证
- Debug + Release 双配置编译绿；Agent 相关 54/54 测试全绿（系统收回路径断言更新为正常闭合）。
- 纯 iOS，无需后端发版。
- 待真机复验：①锁屏等待/暂停场景灵动岛不再出现失败样式；②通知标题「已暂停 · 进度已保存」；③点继续跑完后通知中心的暂停提示立即消失；④真正失败（如额度耗尽/超截止）时失败样式仍正常显示。
- 说明：「第一次没跑完」本身是系统行为（锁屏后台时间有限或锁屏断网等待让位），关键是不再显示失败且回 App 无缝续跑——本轮已闭环；若复现异常请配合抓一次日志定位具体是网络等待还是系统收回。

---

## [2026-08-22] Agent 锁屏高可用——断网自动重连/状态语义统一/等待不再锁死输入框

### 背景
- 东林报障：锁屏下深度分析状态混乱——「有时提示任务失败，点进去却能继续」「有时锁屏干脆不能分析」，要求统一状态语义（失败就是失败）、锁屏可分析且保证成功率、网络中断自动重联不等用户、文案让用户感知没卡住。日志证据：锁屏后网络请求被系统掐断（-1005）、Continued Processing 执行权被收回后任务停摆。
- 三轮改造（五项基础改造 → 深度 review 补修 → 用户旅程走查），共 14 处修复、12 个文件；顺带修复 Agent 调度测试基线雪崩（32 测 30 败，2026-08-19 v21 深度升级批次遗留）。

### 改动
- **网络中断原地等待自动重连**：runLoop 遇可恢复网络错误不再退出任务，落盘等待态后就地指数退避（3s 起步、封顶 20s、累计预算 90s）自动重发——服务端步骤幂等保证重发拿回同一结果，不重复扣分析次数、不重复执行。等待期间后台保护被收回会正确转入暂停语义；仅超 30 分钟绝对截止才判真失败。
- **锁屏后台执行权可失而复得**：删掉「被系统收回过一次就永久失去申请资格」的旧资格门——它是「锁屏不能继续分析」的直接根因（第一次收回后每次锁屏只剩约 30 秒保护）。重新提交安全：旧系统卡片已闭合定格为历史，用户任务恢复执行时重新接管后台保护权。
- **网络恢复自动唤醒**：新增全局网络监听（NWPathMonitor），「断→通」瞬间自动拉起所有等待中的分析任务，不再依赖用户回前台/解锁/重开页面。
- **三个「假失败」来源全部堵死**：① runAnalysis 把可恢复等待态渲染成「深度分析出错」卡片（消息层假失败，旧代码遗留）；② 任务持后台保护正常让位时锁屏系统卡片以失败样式定格且无对冲通知；③ 取消信号被误当网络错误触发无意义重试。等待类状态一律显示「进行中·进度已保存·会自动继续」。
- **等待不再锁死输入框**：三处连锁修复（收尾逻辑/消息刷新反向写锁/停止按钮可见性）——等待网络或系统资源期间可正常发新消息（新深度分析按既有规则自然接管，普通聊天互不干扰），停止按钮始终可见可点。本地通知对冲补发且加前台抑制（盯屏幕时不弹横幅打扰）。
- **时长轮换活文案**：分析等待 25/50/90 秒三档（「模型正在深入思考，Holo 没有卡住」→「这一轮要核对的数据比较多」→「可以锁屏或离开，进度不会丢」）；断网等待 15/45 秒两档（「正在自动重连，无需手动操作」）；修正两处等待时长计算基准失真（时间戳停在启动时刻/等待重试反复重置起点）。
- **测试基线修复**：22 处测试夹具补空结论修复 Debug 契约严格模式雪崩；三个过时契约测试更新（网络等待/持久化取消/CP 重申请=2）；两处测试时序修正（补执行启动步骤+等待登记）；discover 前置探查补工具支持门（并行会话遗留待办收尾，生产零行为变化）；健康锁屏既有红测结案——根因是 7-26 上线的 discover 前置会先写一条非健康结果进断点档案，撞破 7-19 老用例「档案全空」断言，属测试期望过时非锁屏行为回归（健康 DEVICE_LOCKED→等待、零写入语义完好），断言收窄为「health 结果必须为空 + 仅允许 discover 前置结果」。

### 验证
- Debug + Release 双配置编译绿、零警告；测试 54/54 全绿（调度 32 + 后台执行权 15 + 步骤幂等 6 + 流式恢复 1），其中网络中断测试完整验证「断网→等待→自动重发→一次执行内完成」链路。
- 后端 241 项测试全绿 + 本地起服务实测三场景（掐断后重发拿回一致结果且模型只算一次、处理中重发 409 退避后成功、额度池拦截正常）；后端幂等缓存 24 小时远大于 90 秒等待窗口。
- 纯 iOS 改动，无需后端发版。
- 待真机验收：①锁屏反复测试不出「深度分析出错」；②断网 1~2 分钟恢复后自动续跑；③等待期间可发消息、停止键可见可停；④文案随等待时长轮换；⑤飞行模式 30 分钟以上才显示真失败。
- 已知边界：锁屏系统失败卡片无法程序化清除（苹果 API 只有成功/失败两态），通知中心的历史失败卡片属定格记录，以对冲通知与 App 内状态为准。
...
### 背景
- 2026-08-12 提出、08-14 五决策定稿（只罚不奖 / 债务只传导一期 / 下限 0 / 周月年同规则 / 不追溯）；08-18 东林补充拍板：超支判定按原始预算口径（防连环滚债，完整用例见方案 §3.2.1）、不设 Plus 门槛、超支通知一期不做、开关从全局修订为账户粒度。
- 方案文档：`docs/finance/plans/2026-08-18-严格预算模式方案.md`。

### 改动
- **严格预算模式（账户粒度开关）**：设置页「严格预算模式」按账户逐行开关，各自独立起算；开启后该账户总预算当期超支部分计入下期额度（有效额度 = 原始额度 − 上期超支，下限 0）。只罚不奖（结余不结转）、债务只传导一期（下期不超支即恢复原额度）、历史不追溯（开启当期不受此前超支影响）。分类预算不受影响（分类严格经评估一期不做，见方案 §10）。纯实时计算不落库，关闭开关即完全还原；文案口语化：「超支多少，下个月的预算额度就扣多少（最低扣到 0）；省下的钱不累积，下月不超支就自动恢复原额度。」
- **三处展示带结转拆解**：账户详情预算卡 / 账本页预算卡 / 今日看板预算卡——进度条与剩余额基于有效额度，被扣时显示「上月超支结转 −¥x · 原额度 ¥x」，用户能看懂「设了 10000 为什么显示 7000」。
- **AI 四条链路全部理解预算超支**：① 对话工具 budget_status 快照带结转字段与文案；② 深度分析上下文 FinanceBudgetItem 加原始额度/结转字段；③ 记忆洞察 BudgetPerformanceSummary 同步（「总预算已超支」证据带结转说明）；④ 记忆信号修复口径 bug——原用原始额度判定预算偏离（阈值 115%），严格模式下感知不到真实超支，改用有效额度。后端洞察 prompt 为「整块 JSON 上下文注入」模式，客户端加字段即生效，无需后端发版。
- **顺手修复 AI「预算表现分析」恒为空 bug**：`FinanceAnalysisContextBuilder.buildBudgetPerformance` 查预算传随机空 UUID 永无匹配（周/月两分支恒失败），改走跨账户聚合口径。

### 验证
- iOS Simulator Debug 全量构建通过（一期 / AI 二批 / 账户粒度三批各验证一次）。
- 纯客户端改动，不涉及后端发版；CoreData schema 无改动、无迁移。
- 待真机验收：账户开关交互、跨月结转数值、三处说明文案、AI 对「预算为什么变少」的解释。
- 已知小尾巴：Widget 预算卡数值自动切有效额度但无结转说明文案；单测未跑（HoloTests target 在 HEAD 本就编不过，另有会话遗留）。

---

## [2026-08-17] 任务模块完成撤回四项体验修复 + 任务首页 Hero 仪表改版入库

### 背景
- 东林反馈四条：①完成任务后进度条先到 100%、撤回横幅一消失又跳回 0%；②切换清单时长名字胶囊被截断、几秒后才恢复；③撤回期内点圆按钮无反应，误触场景不友好；④要求自查同模块其他体验问题。
- 本批同时把任务首页 Hero 仪表改版（进度头条/庆祝卡/时间分组/清单收纳 Menu/卡片重设计，原型 `task-home-redesign-prototype.html`）检入库——此前该改版只存在于工作区未提交，真机一直在用这个形态，本次修复正是建立在其上。

### 改动（四项修复）
- **进度条 100%→0% 跳变**：根因 `getTodayTaskProgress()` 复用了只查未完成任务的 `getTodayTasks()`——已完成数恒为 0、任务完成即退出总数。改为独立查询（今天到期的任务不分完成与否全部计入分母），撤回横幅期乐观计数与落库后真实计数衔接平滑；任务统计页「今日进度」卡片同步修正（此前永远 0%）。
- **清单胶囊截断闪变**：SwiftUI Menu 更换 label 内容时沿用旧宽度提案，长清单名被压成省略号直到下次重排。清单胶囊加 `.fixedSize()` 按内容自适应，杜绝闪变。
- **撤回期内点圆按钮 = 撤回**：任务列表与今日看板两处统一——3 秒撤回窗口内再点完成圈直接撤回完成，不必去找底部横幅按钮；看板圆圈在撤回期内同步显示完成态（此前横幅说已完成、圆圈仍为空，视觉自相矛盾）。
- **撤回期内反勾子任务联动撤回**：子任务全勾自动触发父任务「完成中」时，撤回期内反勾任一子任务会连带给父任务撤回（此前会无视反勾照常落库完成）。

### 验证
- iOS Simulator Debug 全量构建通过；纯客户端改动，不涉及后端发版。
- 待真机验收：进度条全程数值连贯、清单胶囊切换无截断、撤回期点圈即撤。
- 遗留待办：子任务「连续添加多条无效」问题两轮静态排查无果，需真机复验路径「新建任务→添加行→回车→继续输入→回车」。

---

## [2026-08-17] 任务模块边界修复——来源想法跳转、今日过期判断与新增默认上下文

### 改动
- **来源想法可追溯**：任务详情中的「来自想法」改为可点击，关闭任务页后跳转到对应的具体想法。
- **今日任务不再提前过期**：全天任务统一按当天结束判断过期；首页 Holo 胶囊当天显示「今天」，定时任务仍显示实际时刻。
- **新增任务自动填充上下文**：从「今日」新增默认填充今天，从具体清单新增自动归入当前清单；每日看板的新增入口同步默认今天。

### 验证
- `TodoTaskDatePolicyStandaloneTests` 4/4 通过。
- iOS Simulator Debug 全量构建通过。
- 纯客户端改动，不涉及后端发版。

---

## [2026-08-17] 任务模块交互重构——任务页三合一（查看=编辑=新建）

### 背景
- 用户反馈四条：①列表卡片只有点文字能进详情；②详情页只读、须再点编辑，页面无存在必要；③子任务添加逻辑看不懂；④整体动线不清晰。交互诊断与 HTML 原型见根目录 `task-module-redesign-prototype.html`（诊断 → 流程对比 → 三屏原型 → 交互规格 → 功能保留核对表）。
- 核心方案：砍掉只读详情页，「查看」与「编辑」合并为一个任务页，改标题从 7 步降到 3 步。不新增、不删减任何功能，纯界面交互层重构，数据层零改动。

### 改动
- **TaskDetailView 重写为任务页（查看/编辑/新建三合一）**：原只读详情页与 AddTaskSheet 编辑表单合并。标题/描述点按即编辑；属性区统一「点一行弹一层」——时间（日期/全天/提醒/重复）、清单、优先级、状态、目标（只读）、来自想法（只读追溯）；优先级四档平铺（取消「更多属性」折叠）；顶部不再有「保存」按钮，返回即自动保存；删除任务收进右上角「⋯」菜单（取消底部大红按钮，防误触）。
- **子任务交互重做**：区块底部常驻「＋ 添加子任务」输入行；唯一提交动作 = 回车或行尾「＋」（取消「输入后添加」提示与「添加」胶囊双轨）；提交后保持聚焦连续录入，空内容回车收起键盘；行删除由常驻红色垃圾桶降为灰色「✕」。进度条、全部完成庆祝动画、点文字原地改名、勾选全部保留。
- **TaskCardView 整卡热区**：卡片任意位置（留白、元信息行、子任务平铺区）点按进入任务页；完成圈、子任务勾选、纪念日来源徽章保持独立点击。
- **新建与编辑同一页面**：底部 Tab「＋」、首页深链、每日看板的新建入口统一进入空白任务页（自动聚焦标题）；返回三分支：有标题→创建、有内容无标题→「放弃/继续编辑」确认、完全空白→直接关闭。
- **AddTaskSheet 删除**（2078 行，功能全量并入 TaskDetailView，含语音拆解/附件/重复规则/清单管理全套）；调用点迁移：TasksView、HomeView、KanbanTaskSection。列表/搜索/看板/DeepLink 的详情跳转因构造签名不变零改动。

### 验证
- iOS Debug 全量构建通过；纯客户端改动，无需后端发版。
- 自查修复两处：新建模式时间行颜色对不存在任务的强解包（潜在闪退）；子任务回车提交后连续录制的聚焦保持逻辑。
- 待真机验收：整卡点击与左滑手势并存的跟手度、子任务连续录入节奏、新建返回三分支。
...
### 背景
- 纪念日模块交互走查发现 10 项问题：编辑表单漏接右滑返回（全 App 表单 sheet 唯独纪念日没接）、表单删除一点就删且孤儿任务、编辑保存后详情页不刷新、底部 Tab「+」在纪念日页无响应等；「纪念日改了但已生成的待办任务不同步」作为设计缺陷一并根治。

### 改动
- **iOS**：新建/编辑表单补接右滑返回手势（与其他表单 sheet 一致：无改动直接关、有改动弹「放弃修改」确认；取消按钮同逻辑；拦截系统下拉关闭统一走确认）。
- **iOS**：表单内删除按钮改为确认弹窗（「删除纪念日及关联任务 / 仅删除纪念日 / 取消」，与列表删除行为一致）——原为一点直接软删且关联任务残留孤儿。
- **iOS**：纪念日变更→关联任务同步根治（`AnniversaryTaskGenerator.syncTasks`）：未完成任务是「计划」跟着源数据走——名称/图标/备注变更就地刷新任务文案；日期/类型/每年重复变更软删重判生成；提醒或生成开关关闭清任务、打开立即生成；已完成任务是「历史」永不重排。
- **iOS**：详情页数据源改响应式（`@ObservedObject`），修复编辑保存后详情不刷新；纪念日被删除时详情页自动关闭（原会僵在原地展示已删数据）。
- **iOS**：底部 Tab「+」在纪念日页由无响应改为弹「新建纪念日」（经通知转发给纪念日页自挂的 sheet，避免双层 sheet 叠层拦截触摸）。
- **iOS**：修复表单「nil=不修改」语义吞掉「清空备注」「恢复默认色」的存量 bug（编辑路径物化最终值存入）。
- **iOS**：主题色补「恢复默认」入口（对齐图标的同款交互）；列表页返回/加号按钮触控区 32→44 对齐其他页面。

### 验证
- iOS Debug Simulator 全量构建通过（提交集合在独立 worktree 基于 HEAD 验证编译）
- 同批包含 Emoji 图标库在纪念日模块的接入与基建组件（`EmojiCatalog` / `EmojiIconPicker` / `EmojiSearchIndex`）——与走查修复在同批文件交织无法拆分；其余模块（习惯/目标/财务科目/想法主题）的 Emoji 推广代码已随后续提交落库
- 纯客户端改动，不涉及后端发版

## [2026-08-15] 想法模块知识树 v1——主视图化/主题详情页/AI 分类透明化

### 背景
- 2026-08-15 东林拍板主线方案（原型确认 + 基于线上数据修正两轮）：主题 = 书架（一条想法只进一个）、关键词 = 便利贴（可贴多个）；界面语言统一为「主题 / 关键词」，不再暴露斜杠路径。旧知识树抽屉与主视图并存混乱、AI 分类只付成本不给反馈是两大断层。

### 改动
- **iOS**：知识树升为想法模块主视图——「时间流 | 知识树」平级切换；旧抽屉整体移除，独有能力迁移（标签全局重命名/删除 → 主题详情页关键词长按菜单；主题重命名/删除 → 主题详情页 ··· 菜单）；知识树卡片统一两列等高网格。
- **iOS**：主题详情页（新）——主题图标 + 关键词管理 + 最近想法；管理入口从设置页收拢进知识树。
- **iOS**：AI 分类透明化——整理结果带置信度分层（≥0.75 自动归档、<0.75 进「待确认池」集中轻确认）；分类理由落库（`Thought.topicAssignmentReason`）并在待确认池/详情页展示。
- **iOS**：想法详情页补主题归属区块（展示与修改）；`recentAITags` 补传近 90 天标签（接通后端复用机制，缓解标签发散）；整理空响应改为抛错走队列重试（后端 `thought_organization` 关思考模式配置另行发版，双保险的另一半）。
- **iOS**：主题图标 emoji 化——`Topic.iconEmoji` 字段 + 三级默认策略（用户自选 > Onboarding 预设 > 关键词启发式 > 哈希兜底），详见下方 Emoji 图标库条目。

### 验证
- iOS Debug Simulator 全量构建通过（提交集在独立 worktree 基于 HEAD 验证编译）
- 二期（主题 AI 综述、打标注入用户档案）需后端发版，另行同步

## [2026-08-15] Emoji 图标库全 App 推广——目标/纪念日/习惯/财务科目统一接入

### 背景
- 想法模块（知识树 v1）引入了 Emoji 图标选择器后定稿推广方案：个人化模块（目标/纪念日）默认 Emoji 化，习惯与财务科目以「可选项」融入（存量单色图标不动、增量可选 Emoji），全 App 选图标的体验统一为一套组件。

### 改动
- **iOS**：统一图标选择器组件——从主题选择器提取通用组件 `EmojiCatalogGrid`（单页连续滚动，无分类切换；顶部搜索框 + 最近使用 + 分组小节标题，可内嵌）与 `EmojiIconPickerSheet`（弹层壳）；`EmojiCatalog` 数据源改为由 Unicode 官方 emoji-test.txt (15.1) 脚本生成的全量库（1898 个 fully-qualified emoji，仅排除肤色变体，与 iOS 键盘策略一致），`isEmojiIcon` 判定（SF Symbol/资产名均纯 ASCII，含非 ASCII 即 Emoji），最近使用记录全 App 共享；新增 emoji 搜索——中英文关键词索引（`EmojiSearchIndex`，中文来自 CLDR zh 官方注释含派生的国旗/ZWJ 注释、英文来自 Unicode 官方名称），搜索框实时过滤，无结果显示空态。
- **iOS**：想法模块换用统一组件（删 `TopicIconPickerSheet` 本地实现，行为不变）。
- **iOS**：目标模块——`Goal` 实体新增 `iconEmoji` 字段（Core Data 编程式模型，轻量迁移）；领域枚举新增默认 Emoji 映射（学习📚/健康💪/职业💼/财务💰/生活✨/项目🚀/其他🎯）；列表行大图标改为 Emoji（手选 > 领域默认），创建/编辑表单新增图标选择行（含恢复默认）；小徽章与领域选择器保持 SF Symbol 语义位不变。
- **iOS**：纪念日模块——类型默认图标由 SF Symbol 改为 Emoji（生日🎂/纪念日❤️/倒数日⏳/里程碑🚩）；表单类型 chip 选中态从实色底改为浅底+描边（避免彩色 Emoji 压实色底）；新增图标选择行；存量数据的老默认 SF Symbol 视为默认（编辑保存后自然升级 Emoji，不编辑不受影响）；详情页中心图标兼容两种体系。
- **iOS**：习惯模块——渲染协议 `iconImage` 加 Emoji 分支（磁贴/详情/统计/看板/AI 卡片全场景生效）；选择器加「经典/Emoji」页签（当前已是 Emoji 时直接落在 Emoji 页）；打卡完成态磁贴的 Emoji 用白色圆片托底（Emoji 无法像单色图标那样变白，避免彩色压实色习惯色背景）。
- **iOS**：财务科目——渲染统一入口 `categoryIconGlyph` 加 Emoji 分支（记账列表/分类网格/统计图例/预算/AI 聊天卡片头等全部场景自动生效）；科目图标选择器 `IconPickerGrid` 加「经典/Emoji」页签（新增/编辑科目两处入口同时获得），Emoji 页固定高度内滚动保证分类 tab 常驻。
- **存量策略**：默认科目/默认习惯的单色图标一律不迁移不替换，用户手选 Emoji 后按 Emoji 渲染；AI 记账卡片头部图标保持现状（按分类名映射 SF Symbol，本就不读用户科目库，渲染层已兼容 Emoji、后续如需跟随用户科目即插即用）。

### 验证
- iOS Debug Simulator 全量构建通过
- 通知链路核查：待办/纪念日通知不引用实体 icon 字段，无影响
- 纯客户端改动，不涉及后端发版

## [2026-08-15] Holo Plus 付费墙全量补齐——分期/周期账单/预算/跨月对比接线

### 背景
- 权益表承诺的付费功能与实际拦截对账后发现：付费墙场景（`HoloPlusGateContext`）只做了定义，财务类功能入口从未接线，免费用户可无限制使用财务分期、周期账单、预算、跨月类别对比。

### 改动
- **iOS**：财务分期接付费墙——记账表单「分期」设置行非 Plus 点击弹付费墙（购买成功自动回开分期设置）；AI 对话识别的分期卡片在点「确认」入账时拦截（识别免费、入账收费，与 AI 配额模式一致）。
- **iOS**：周期账单接付费墙——财务设置「每月起始日」与信用卡账单日/还款日 Stepper 对非 Plus 变为只读展示（带小锁），点击弹付费墙；已配置的周期对统计/AI 口径继续生效（存量只读）。
- **iOS**：预算管理接付费墙——账户页菜单「预算设置」、空预算卡片、「添加」分类预算、点击已有分类预算行四处入口统一走 `gateBudget`（非 Plus 弹付费墙，购买成功回开原入口）；存量预算的展示与超支提醒不受影响。
- **iOS**：统计增强（跨月类别对比）接付费墙——统计页「类别」Tab 对非 Plus 显示小锁，点击弹付费墙不切换；总览/明细保持免费。
- **iOS**：新增付费墙场景 `billingCycle` / `budget` / `advancedStatistics` 文案；统一边界原则：非 Plus 对 Plus 权益=只读（存量数据可见、不可改配置）。
- 其余权益确认无缺口：AI 聊天/智能记账/智能任务/记忆洞察走后端配额拦截（QUOTA_EXCEEDED 卡片含升级入口）；桌面小组件扩展端已按 entitlement 对免费用户显示锁定态。

### 验证
- iOS Debug Simulator 全量构建通过
- 绕过路径核查：分期写入仅两 UI 入口可达（均已拦）、CSV 导入不支持分期字段、记账周期起始日唯一写入点已拦、预算设置唯一入口在账户页（四处已拦）
- 纯客户端改动，不涉及后端发版

## [2026-08-15] AI 数据读取补全——修假数据/打通打卡备注/财务口径对齐

### 改动
- **iOS**：修复想法分析的「每日趋势」假数据——原用「总数÷天数」均分伪造曲线喂给 AI，改用真实的按日计数（`getThoughtCountByDay`），无记录日如实补 0。
- **iOS**：Agent 目标工具补读 `summary`/`motivation`，AI 做目标分析时能看到用户写的一句话摘要与动机（此前仅界面可见）。
- **iOS**：删除目标记忆信号里恒真的 `isUserCreated` 防御判断（`"suggestion"` 从未写入，判断恒真；未确认建议本就不进目标库）。
- **iOS**：打卡备注全链路打通——数值记录弹层与打卡当日可填备注（限 100 字）；AI 对话记录体重/数值习惯/打卡时自动把原话中数值与习惯名之外的上下文存为备注（清洗后 >2 字才存）；习惯分析、健康洞察、Agent 习惯工具三处 AI 链路读取近期备注（解释数据波动的关键归因文本）。
- **iOS**：财务口径对齐记账周期——AI 财务分析/洞察/自然语言查账/Agent 财务工具的「本月/上个月」跟随全局记账周期设置（与统计页同一口径），月度分解标签改为「8月6日起」式周期起始日；Agent 账户快照注入各信用卡的账单日/还款日/额度；预算表现的月窗口比对同步换口径。
- **iOS**：纪念日接入 AI（含修复）——聊天上下文注入**全量**纪念日事实（标题+日期+下一次+倒计时，每年重复的按年换算），周期洞察的「生活事件」纳入周期内纪念日；意图识别上下文同步注入纪念日清单并声明路由规则。修复「离妈妈生日还有多久」被意图识别误判为需要澄清、永远到不了回答环节的问题（意图识别上下文里此前没有任何纪念日数据）。
- **iOS**：任务详情的「来自想法」区块显示选中转任务时的原文选区（`sourceTextSnippet` 此前只写不读），想法✅→任务→原文选区形成闭环。

### 验证
- iOS Debug Simulator 全量构建通过（基于磁贴化提交 6db339de 之后的最新工作区）
- 测试 target 编译失败为 HEAD 既有问题（`HoloAgentEvalModels` 等类型缺失，与本批无关，已在独立 worktree 用 HEAD 验证）
- 不涉及后端改动；后端相关项（想法整理回读当前主题归属、主题综述）列入知识树二期发版批次

## [2026-08-15] 想法编辑器体验巡检与编辑入口修复

### 改动
- **iOS**：编辑器工具栏改为不透明背景，压缩视觉占用，并用图标表达文字样式；颜色面板移除“重置颜色”，增加黑色选项。
- **iOS**：统一编辑器、详情页和列表卡片的正文排版基线，改善字号、行距、动态字体和深色模式下的阅读体验。
- **iOS**：统一 `@引用`、`#标签` 和任务标记的结构化渲染、候选查询、点击跳转、保存重进及长标题展示；任务作用范围持续以下划线显示。
- **iOS**：补齐 Token、任务范围、粘贴、撤销/重做、中文输入法组字和候选面板的边界处理，避免引用身份或任务下划线丢失。
- **iOS**：修复短文无法直接编辑的阻断问题：单击仍查看详情，双击卡片任意位置直接进入编辑器，并自动聚焦正文，避免再次点击才能输入。

### 验证
- iOS Debug Simulator 构建通过并安装验证
- 短文、长文双击直达编辑器验证通过；进入后无需再次点击正文即可输入
- 单击进入详情、引用插入与跳转、任务下划线保存重进路径已验证
- `git diff --check` 通过
- 不涉及后端改动和后端发版

## [2026-08-14] HoloAI 任务条目对话式增删改

### 改动
- **iOS**: 新增 `modify_task_items` 意图：在对话里对最近关联的任务追加/删除条目（「还要买E」「不买B了」「把B换成C」），替换=删旧+加新，走与创建任务一致的待确认卡片（新增项绿色＋、移除项红色删除线）
- **iOS**: 已确认任务卡片新增「补充条目」入口：点击锚定目标任务并预填输入框（锚定一次性消费），消除多任务/间隔较久时的指代歧义
- **iOS**: 意图识别上下文注入「最近对话关联的任务」备忘单（标题+现有条目确切名称），AI 据此识别条目增删并在删除时引用条目原名
- **iOS**: 删除条目按名称精确匹配优先；模糊兜底仅在唯一命中时执行，未匹配项如实提示，不冒误删风险
- **iOS**: 修复 SubtaskParser 单项过滤误伤——追加/删除单项条目被丢弃导致「单项追加不生效、确认后卡片条目消失」；create_task 路径行为不变
- **后端**: `intent_recognition` 规则新增 `modify_task_items` 定义与 2 条 few-shot（commit 2475e015 已发版生效）；prompt 长度护栏 4700→5100

### 验证
- iOS Debug 构建通过（xcodebuild BUILD SUCCEEDED，含单项修复复验）
- HoloBackend 207 测试全绿；已部署生产并验证（公网健康 ok、容器内新规则生效、hono 4.13.1 无回退）
- 提交组合经独立 worktree 编译验证
- 真机功能验证进行中（首轮发现单项 bug 已修复，待复测）

---

## [2026-08-14] 修复 AI 内容举报展示与审核查看链路

### 改动
- **iOS**: 举报弹窗新增被举报 AI 原文预览，正文过长时可上下滚动查看，避免用户无法确认具体举报内容
- **iOS**: 举报弹窗支持中等/大尺寸切换，修复表单内容超出弹窗高度后标题被顶出的问题
- **管理后台说明**: 补充举报记录的查看入口；管理网页通过 SSH 隧道访问 `/admin/reports`，公网 JSON 接口为 `/v1/admin/reports`

### 验证
- iOS Debug 真机目标构建通过
- HoloBackend 举报专项测试 7 个用例全部通过
- `git diff --check` 通过
- 不涉及后端代码改动和后端发版

---

## [2026-08-14] 修复 HoloAI 任务卡片确认状态与双向删除同步

### 改动
- **iOS**: 修复任务卡片生成后、确认后以及退出重进后的卡片状态缓存不同步，确认按钮现在能保持正确状态并正常响应
- **iOS**: 修复从任务模块删除日程后 HoloAI 卡片不刷新的问题；任务模块与 HoloAI 的删除状态双向同步
- **iOS**: 统一财务卡片和任务卡片的消息状态发布机制，避免数据已更新但界面不重绘
- **iOS**: 排查其他 HoloAI 卡片，移除习惯打卡、心情和体重卡片无实际动作的点击入口，避免出现假交互
- **测试**: 增加任务卡片确认状态恢复、删除状态刷新和历史消息预填删除态测试
...
## [2026-08-14] 修复记忆长廊周历 0–7 点折叠与对齐

### 改动
- **iOS**: 重做记忆长廊周历的 0–7 点折叠/展开交互，将入口固定在时间轴表头，避免展开后内容挤压变形
- **iOS**: 折叠摘要、日期表头和事件网格复用同一列宽与基线，确保凌晨摘要与日历日期严格对齐
- **iOS**: 展开状态恢复完整 24 小时可滚动时间轴，折叠状态保留凌晨摘要，降低浏览和切换成本

### 验证
- 周历轴逻辑 standalone 测试通过（5 个用例）
- iOS 模拟器 Debug 编译通过
- 不涉及后端发版

---

## [2026-08-14] 修复深色模式下嵌套卡片边界不清

### 改动
- **iOS**: 账本页的月度支出、月度收入和预算卡片改用独立的嵌套卡片表面，在深色模式下通过背景层级、描边和轻阴影明确区分卡片边界
- **iOS**: HoloAI 对话日志中的内容卡片同步使用嵌套卡片样式，避免同类问题在其他页面重复出现
- **iOS**: 新增统一的 `HoloNestedCardStyle` 和 `NestedCardBackground` 颜色资源，后续嵌套信息块复用同一视觉规则

### 验证
- iOS 模拟器 Debug 编译通过
- iPhone 16e 深色模式账本页验证通过
- 不涉及后端发版

## [2026-08-13] 习惯统计看板大重构——驾驶舱 + 洞察 + 整月色块条全貌

东林反馈：习惯统计页"用户无法一眼看全貌"。旧版是按月滚动的仪表板，三个根本问题导致看不了全貌：①顶部概览只有 3 个干瘪数字（习惯数 / 完成率 / 最佳连续），缺趋势和环比；②每个习惯折叠只露一周 7 格，想看整月必须手动展开，而且**同一时间只能展开一个**，习惯越多越累；③只能看单月，看不到"我整体在进步还是退步"这种跨月对比。

### 改动（融合方案：全局 → 结论 → 明细一条龙）

**一、顶部驾驶舱卡**（替换旧总览卡）
- 本月完成率大数字 + 比上月涨跌徽章（环比，绿↑红↓）
- 今日完成 X/N（仅当查看当前月时显示，看历史月自动隐藏——"今日"在历史月无意义）
- 近 6 个月完成率趋势柱图，当月高亮，一眼看出进步方向
- 最佳连续天数 + 坚持习惯数

**二、洞察卡**（全新）
- 🔥 本月最佳坚持的习惯（完成率最高）
- ⚠️ 最需关注的习惯（完成率最低，至少 2 个习惯才显示）
- 📈 整体完成率与上月环比，系统主动总结，省去用户逐条对比

**三、习惯全貌列表**（折叠态升级——解决"看不了全貌"的核心一刀）
- 每个习惯折叠态从「一周 7 格」升级为「整月色块条」，所有习惯一眼并排对比
- 标题区新增完成率%，点击展开仍是月历 + 数值趋势图（展开态保持不变）

### 关键设计决策
- **色块条统一用品牌暖橙热力图色阶（二值）**：完成=橙、未完成=空档、坏习惯超标=红。所有习惯一致语义，不随各自颜色变化——避免并排时多种颜色打架，视觉统一高级。每个习惯的标识色只留在图标区。
- 坏习惯语义保留（与详情页、月历一致）。
- 数据层小改：习惯展示项补本月完成率；新增轻量"月完成率"方法供趋势/环比用（不算连续天数，避免切月卡顿）。

### 涉及文件（共 9 个 iOS 文件）
- 新增：`HabitHeatmapStrip.swift`（色块条）、`HabitStatsCockpitCard.swift`（驾驶舱）、`HabitStatsInsightCard.swift`（洞察）
- 改造：`HabitStatsView.swift`（页面重排）、`HabitStatsExpandableCardView.swift`（折叠态换色块条 + 标题完成率）
- 数据层：`HabitStatsState.swift`、`HabitRepository+Stats.swift`
- 删除：`HabitStatsSummaryCard.swift`、`HabitWeekStripView.swift`（被替代）

### 验证与发布
- iOS 工程编译 `BUILD SUCCEEDED`
- 视觉/交互待真机验收（色块条空档色对比度、深色模式、趋势柱图、切月/展开/空状态）
- 不涉及后端发版，纯前端改动

---

## [2026-08-06] 修复键盘弹出挤压/遮挡输入框的全局体验问题

东林反馈：在想法模块首页点搜索框，键盘弹起会把输入框"压缩"，导致看不到自己输入的内容。这是一类典型的键盘避让（系统如何为键盘腾位置）处理不当的问题，全局排查后一次性修复多处。
...
**三、调整余额（`AdjustBalanceSheet.swift`）**
- 主容器从固定 `VStack` 改为可滚动 `ScrollView`，键盘弹出时能上滚看到被遮的内容。
- 数字键盘补「完成」收键盘按钮（金额 + 备注两个输入框）。

**四、习惯打卡·数值输入（`HabitCardView.swift` + `HabitQuickCheckInView.swift`）**
- 卡片入口、快捷打卡两处的数字键盘，统一在键盘正上方补「完成」按钮。

**五、周期项目编辑 / 一次性购买编辑（`SpendingProjectsView.swift`）**
- 金额、周期数等数字键盘统一补「完成」按钮。
...
## [2026-08-05] 语音输入额度按会员档位统一 + 免费用户体验修复

修复一个影响免费会员的体验问题：想法/任务模块的语音输入写死 5 分钟上限，但免费会员实际只有 1 分钟额度——免费用户能录满 5 分钟，保存时才被后端拒绝、直接报错，白录 4 分钟。同时全局排查发现聊天模块反向中招（Plus 会员也只能录 1 分钟，权益没给足）。

### 根因
三个录音入口（想法、任务、聊天）的时长上限是**各自硬编码写死的**，完全不读会员档位。而真正权威的额度在后端：免费 60 秒、Plus 300 秒。前端 UI 上限和后端实际额度脱节，导致免费用户被「UI 允许录 5 分钟、后端只认 1 分钟」的矛盾伤害。

### 改动

**一、录音时长统一按会员档位（根治）**

1. **新增单一额度来源**：`HoloEntitlementState.currentAsrMaxSeconds`（优先读服务端下发的 `quotas["asr"].maxSeconds`，兜底按档位 60/300）。
2. **三个录音入口删掉硬编码**：想法、任务、聊天统一调用上面的额度来源。现在免费用户在任何模块录音都自动到 1 分钟停、Plus 用户到 5 分钟停，三处行为完全一致。

**二、修复 60.x 秒边界误判（让"自动停后正常转写"真正生效）**

3. **对抗性审查发现的真 bug**：客户端录音倒计时每 200ms 轮询一次，到点自动停时实际录音文件常是 60.1~60.2 秒。后端时长校验是**严格零容差**的 `<= 60`，60.x 秒会被判超时、报错弹付费墙——修复彻底失效。
4. **修复**：客户端预校验 + 后端校验都加 **1 秒容差**（`> 61` 才算超时），吸收轮询抖动。60 秒是体验上限，不是物理硬约束。

**三、免费用户到时后的柔和升级入口**

5. 免费用户录到 1 分钟自动停、转写结果出来后，结果页底部出现一行小字「升级 Holo Plus，可录制 5 分钟语音」，点击进付费墙。分寸：不遮挡结果、不打断确认、Plus 用户和主动停止的用户都不显示。

**四、文案与展示一致性**

6. 会员中心、个人页的「语音」标签以前把"识别次数"和"单条时长"混为一谈（标签叫"语音"，值只显示时长 5分钟/60秒，次数完全没露出）。统一改为「语音识别」+ 次数口径（50/天 vs 20/天），与 HoloAI、任务三个标签口径一致。
7. 付费墙文案空格对齐（`60秒` → `60 秒`）、想法整理配额耗尽文案两处措辞对齐。

### 涉及文件（共 10 个 iOS 文件 + 后端 1 处）
- iOS：`HoloEntitlementState`、`HoloBackendSpeechRecognitionProvider`、`VoiceInputViewModel`、`VoiceInputSheet`、`ThoughtEditorView`、`AddTaskSheet`、`HoloMembershipCenterView`、`PersonalView`、`HoloPlusPaywallView`、`ThoughtListView`
- 后端：`HoloBackend/src/app.js`（`validateAsrDuration` 加 1 秒容差）

### 验证与发布
- iOS 工程编译 `BUILD SUCCEEDED`
- 后端已发版上线（`1f8a021`，含 1 秒容差修复 + 测试用例同步调整）
- iOS 客户端等下次 App 发版

---

## [2026-08-03] 想法编辑器自动保存 + 空行光标修复

想法编辑器两项体验修复：空行光标变大；编辑中途退出会弹「放弃/继续编辑」且「继续编辑」无法回到编辑页。改为自动保存，彻底去掉弹窗。

### 改动

**一、自动保存（去掉未保存弹窗）**

1. **根因**：编辑器是「显式保存」模式——只有点「保存」按钮才落库；右滑退出时有未保存修改会弹「放弃/继续编辑」，而「继续编辑」按钮存在 bug 无法回到编辑页。用户的核心诉求是「有什么数据都给用户保存下来」。
2. **改造为自动保存**：
   - **防抖落库**：内容变化后停顿 2 秒自动保存一次（复用项目内 debounce 模式），避免逐字写入的性能开销。
   - **草稿状态机**：新建模式下首次有内容时 `create` 拿到草稿 ID，后续改动 `update` 同一条；编辑模式直接 `update` 已有记录。
   - **退出兜底**：新增 `onDisappear` 在退出时强制落库当前内容，确保防抖未触发的修改不丢。
   - **空的不存**：内容为空（去空格换行后）不创建记录；已创建的空草稿退出时自动 `hardDelete` 回退。
3. **去掉弹窗**：取消按钮、右滑退出、查看标签三个出口全部改为直接退出（数据已自动保存），移除 `unsavedChangesAlert`、`showDismissAlert`、`hasUnsavedChanges` 等相关逻辑。
4. **AI 分类只触发一次**：自动保存期间不重复 enqueue AI 分类（避免消耗配额），仅在草稿首次创建时触发一次。
5. **通知节流**：防抖中间保存不发 `.thoughtDataDidChange`（避免 Widget 快照、列表刷新等重链路频繁触发），仅在退出时发一次。

**二、空行光标变大修复**

6. **根因**：`baseAttributes` 里 `NSParagraphStyle.lineSpacing = 8`（文字行间距），空行时光标高度被这个额外行间距撑高约 8pt；有文字的行由字形撑住行高，不受影响。
7. **修复**：`refreshTypingAttributes` 设置 `typingAttributes` 前，把段落样式复制一份并将 `lineSpacing` 清零，使光标高度仅由字体决定，与有文字行一致。已渲染文字的行间距不受影响（那些用完整 paragraphStyle 存在 attributedText 里）。
...
## [2026-08-03] 想法编辑器换行修复 + 知识树折叠展开 + 任务描述框同类问题修复

修复三个体验问题：想法编辑器「写多了之后回车没反应、再按一次跳两行」；知识树标签无法折叠展开、点主题直接跳走；全局排查发现任务描述框存在同类隐患一并修复。

### 改动

**一、编辑器换行 bug 修复**

1. **根因**：编辑器（`UITextView`）在 `layoutSubviews` 里根据「内容是否超出可视区域」反复开关 `isScrollEnabled`（true↔false）。每次开关都会让文本控件重建内部排版引擎，光标的视觉刷新被打断——换行符已插入数据层，但屏幕上光标没动（「第一次没反应」）；下一次操作又触发重建，积压的换行一起显示（「跳两行」）。写多了才触发，是因为内容要超过一屏才会开始翻转开关。
2. **修复**：移除 `layoutSubviews` 里对 `isScrollEnabled` 的动态切换，始终保持编辑态默认值。编辑器高度改由 `dynamicHeight` 绑定驱动外层 `.frame(height:)` 撑到与内容等高，frame 等于内容时文本控件自身无可滚动空间，滚动手势自然交给外层 SwiftUI ScrollView，不再需要翻转开关。
3. **补强**：`makeAttributedText` 补齐 Markdown 块解析对末尾换行的吞掉（解析器跳过空行导致 `第一行\n\n第三行` 这类中间空段被压缩），文本转换忠实保留尾部换行。

**二、任务描述框同类问题修复**

7. **全局排查**：想法编辑器修复后，全项目排查所有文本输入控件，确认仅任务添加页（`AddTaskSheet`）的描述输入框存在同类隐患——`TextEditor` 按内容高度在 132 边界动态切换 `.scrollDisabled`，底层同为 `UITextView.isScrollEnabled` 反复翻转，可能在任务描述写多后触发一样的光标卡顿。
8. **修复**：移除动态 `.scrollDisabled`，让 `TextEditor` 始终允许内部滚动。内容少时框高跟随内容变矮、框内无可滚动空间，触摸自然交给外层 `ScrollView`，与想法编辑器同一套解法。

**三、知识树折叠/展开**

4. **未归类 / 主题标题行**：原点击直接跳转筛选并关闭抽屉，无法折叠。改为标题行主体（图标+名称+数量）点击 = 筛选该分组，右侧独立的小箭头（chevron）点击 = 折叠/展开子标签，两者分离互不干扰（与 iOS 文件 App 惯例一致）。
5. **无子标签时不显示折叠角标**：主题/未归类下没有标签时，不画 chevron，避免无意义的可点击元素。
6. **默认状态**：未归类默认展开（方便看到待归类标签），主题默认收起。
...
7. **CoreData 弃用索引写法升级**（59 处）：程序化定义的数据库表结构用了 iOS 11 后弃用的 `NSAttributeDescription.isIndexed`，统一改为苹果推荐的 `NSEntityDescription.indexes`（实体级别声明索引）。新增 `CoreDataStack.applyIndexes(to:on:)` 工具方法统一生成索引，避免 8 个建模文件各自手写。索引是本地 SQLite 层的性能优化，不涉及数据 schema 与 CloudKit，对存量数据零影响。

### 验证与发布
- iOS 工程全量编译（clean build）`BUILD SUCCEEDED`，0 error；**全量 warning 清零**（并发类、isIndexed 弃用类全部清零，增量与全量均为 0）
- 纯 App 端改动，**不涉及后端发版**
...
### 改动
1. **会员中心改为固定权益对比表（iOS）**：免费版与 Plus 共用同一行坐标，统一图标、列宽和行高，移除旧横向轮播和多余遮块。
2. **补齐 Plus 价值说明**：语音启动、快捷控制台、财务、随机漫步四类桌面小组件分别列为独立权益，免费版显示不可用，Plus 显示可用。
3. **修正个人页额度展示**：免费版不再错误显示 Plus 的 30/天、5 分钟、50/天，入口卡片按真实权益显示对应额度；付费墙摘要同步为 4 类桌面小组件。
4. **统一 Plus 视觉规范**：入口卡、会员中心、徽章复用暖橙主题，并补充设计说明文档。

### 验证与发布
- Holo Plus 相关 Swift 文件语法解析通过
- `git diff --check` 通过
- 上一轮 Holo Debug 模拟器全工程构建成功；本轮重跑受本机无可用 Simulator runtime 的环境问题阻塞
- 纯 App/UI 改动，**不涉及后端发版**
...
### 改动
1. **账户提升为一级 Tab（iOS）**：`FinanceTab` 枚举新增 `accounts` 并置于首位；`AccountListView` 改造为完整 Tab 根视图（自带返回、导航容器），原「设置 → 账户管理」入口移除，避免一个功能两个入口。
2. **账户交易记录复用统一组件（iOS）**：账户详情页的交易行不再手写，改用全财务统一的 `TransactionRowView`——分期期数（如「3/12期」）现在会正常显示，点击可进编辑，长按有「编辑/复制/删除」菜单（分期删除同样支持「仅删此期/删除全部分期」），与账本页交互完全一致。
3. **预算分类支持一级分类（iOS）**：预算分类选择器原只能对二级分类做分类预算，选一级分类需长按/右键。现改为：点一级分类行 = 选中整个大类（预算自动覆盖其下所有子分类），点右侧箭头 = 展开选具体子项；一级分类行新增范围说明副标题（如「含 打车·地铁·加油 等 5 项」），选中提示也标明「覆盖全部子分类 / 仅此项」，用户无需理解层级概念。
4. **分期跨年标注（iOS）**：分期标签今年保持「3/12期」，明年及以后自动带上年份（如「2027·11/12期」），依据该期交易自身日期判断，长期分期不再混淆归属年份。
5. **修复周期项目补账往前找历史账单的 bug（iOS）**：固定支出模块的周期性项目（如月度订阅）自动补账逻辑存在三处缺陷，导致用户 7 月的分期会在 7 月之前的月份生成流水。根治方案：补账起点钉死在项目开始日期（不再依赖可能被污染的「下次扣款日」游标）；按「年-月」维度判断该期是否已建（不再用精确到秒的日期比较，避免编辑/迁移后重复补账）；进度只数「已发生」流水（date ≤ 今天），与余额/统计口径对齐；并自动清理开始日期之前的历史脏流水（用户手工记账不带项目标记，不受影响）。

### 验证与发布
- iOS 工程全量构建成功（`BUILD SUCCEEDED`），无新增 error
- 纯 App 端改动，**不涉及后端发版**
- 账户交易记录、预算分类选择、分期补账均经逻辑核对

---

## [2026-08-02] 移除想法编辑页多余的「标签」展示栏

想法编辑页正文下方原有一个「标签」栏：它不是输入框，只是把正文中已打的 `#标签` 再抄一遍展示出来，并提示「正文中输入 # 添加」。信息与正文重复，没有提供正文之外的能力，体验冗余。直接移除。

### 改动
1. **删除编辑页标签展示栏（iOS）**：移除 `ThoughtEditorView` 中的 `tagsSection`（只读展示区）及其引用。标签的添加方式不变——仍由在正文里输入 `#` 触发候选浮层完成；标签数据保存逻辑（保存时扫描正文 `#` 写库）不受影响。

### 验证与发布
- 纯 App 端 UI 删除，**不涉及后端发版**
- 详情页的标签展示、列表筛选、AI 归类等下游功能均未改动，不受影响
...
### 改动
1. **称呼自然语言解析**：`HoloProfileSnapshotBuilder` 增加无冒号模式，「称呼我为东林」「叫我东林就好」等写法都能识别为 `preferredName`，不再强制要求模板格式。
2. **编号列表目标识别**：新增「引子句 + 编号项」的目标提取逻辑（识别「最近的目标是 1.xxx 2.xxx」这类写法），并把含健身、戒烟等关键词的目标自动归类为健康习惯；标准 `## 关注领域` 模板用户的解析逻辑不变。
3. **修复分析对话与记忆洞察的原文兜底**：`HoloProfilePromptRenderer` 的兜底逻辑从「仅聊天注入原文」改为「结构化全空时所有场景都注入原文」；`MemoryInsightContextBuilder` 的兜底判断从「rendered 是否为空」修正为「snapshot 是否有结构化内容」，让解析失败时用户原文仍能送达。
4. **修复 Agent 工具空档案处理**：`HoloProfileTool` 与 `HoloProfileDataSource` 增加 `rawMarkdown` 字段，结构化全空但用户写了原文时，工具返回原文兜底而非报「无档案」。

### 验证与发布
- 独立 Swift 脚本验证：称呼「东林」被正确识别，3 个目标全部提取，chat/analysis/insight 三个场景原文均送达
- Holo Debug iOS Simulator 全工程构建成功（`BUILD SUCCEEDED`），无新增 error
- 纯 App 端改动，**不涉及后端发版**
- 正常模板用户无回归：结构化数据完整时走原有渲染路径

---

## [2026-08-01] 修复习惯次数按钮缺失与操作区内容裁切

习惯首页卡片在横向空间不足时会优先保留中间说明文字，导致右侧今日数值、单位和操作按钮被压缩成残线或纯色块；次数类习惯的 `-1` 又只在已有今日记录后才出现，使用户误以为减号功能丢失。

### 改动
1. **右侧操作区完整显示**：调整习惯卡片的空间分配优先级，计数、测量和打卡控件不再被中间说明文字压缩；空间不足时由说明文字缩放或截断。
2. **次数类稳定显示双按钮**：`-1 / +1` 始终同时显示，今日没有可撤销记录时 `-1` 置灰禁用，新增记录后立即启用，避免按钮突然出现和卡片布局跳动。
3. **保持原有记录语义**：`+1` 继续新增一次记录，`-1` 继续撤销今日最近一笔记录，不改变历史数据、统计口径和坏习惯超标判断。

### 验证与发布
- Holo Debug iOS Simulator 全工程构建成功（含小组件扩展）
- `git diff --check` 通过
- 纯 App 端界面修复，不涉及 HoloBackend，无需部署后端

---

## [2026-08-01] 任务模块子任务体验升级：改名、连续录入、防丢、语音拆解

针对新建/编辑任务页的「检查清单」做了一轮产品体验整改：统一术语、优化录入流畅度、防止意外丢字，并让语音输入具备 AI 自动拆解能力。

### 改动
1. **「检查清单」统一改名为「子任务」**：表单区标题与导航标题改为「子任务」，消除「检查清单 / 子任务」两套术语混用的认知负担（数据模型英文标识符 `CheckItem` 保持不变，不影响已存数据）。
2. **回车连续录入子任务**：在「添加子任务」输入框按回车提交后，光标自动留在输入框，无需手动重新点击，可一条接一条连续录入。
3. **子任务防丢**：输入框失焦时若仍有未提交内容，自动入列；新建任务态沿用暂存机制，保存时随主任务一并写库，避免打一半切走或误关闭导致丢字。
4. **语音输入自动拆解子任务**：新建任务页的语音输入接入现有意图识别通道（复用聊天页同款 AI 能力），说一句包含多个并列事项的话（如「明天去山姆买牛奶、鸡蛋和纸巾」）即可自动拆成「1 个主任务 + N 个子任务」，标题与子任务自动填入表单；处理期间麦克风按钮显示 loading，拆不出子任务或调用失败时退回「塞入任务描述」的原行为，不比改造前更差。

### 验证与发布
- Holo Debug iOS Simulator 全工程构建成功（含小组件扩展）
- 纯客户端改动，复用现有 AI 意图识别接口，**不需要后端发版**
- 建议真机验收两个场景：① 连续打多条子任务（回车流转）；② 语音说一句包含多个并列事项的话，确认自动拆出子任务

---

## [2026-08-01] Holo Plus 权益与真机验收闭环：免费 / Plus 可切换，额度与小组件真实生效

此前个人页虽然可以显示免费或 Plus，但开发验收状态只影响本地展示，没有形成服务端权益、额度扣减和小组件门控的统一闭环，导致切到免费版后小组件仍可用，HoloAI 与语音识别额度也长期不变化。

### 改动
1. **真机验收支持自由切换**：Debug 内部验收账号可以在会员中心切换免费、Plus、跟随购买，并重置当前验收额度；免费与 Plus 使用独立额度账本，来回切换不会互相污染。
2. **权益改为服务端真相源**：新增订阅状态、购买同步、内部验收和额度重置接口；App 启动、购买、语音识别和 HoloAI 请求后都会刷新服务端权益，不再只相信本地显示状态。
3. **额度按成功动作扣减**：免费版 HoloAI 3 次/天、语音识别 20 次/天且单条最长 60 秒；Plus 分别为 30 次/天、50 次/天且单条最长 300 秒。失败和重试不扣额度，Agent 多步骤任务按一次完整运行计数。
4. **小组件纳入会员门控**：免费版小组件不再读取并展示真实财务或想法快照，而是显示 Plus 解锁状态；点击后回到 App 统一付费墙。切换权益时会主动刷新 WidgetKit 时间线。
5. **统一额度错误体验**：后端返回结构化额度错误，App 能区分次数耗尽和语音过长，展示对应文案并打开正确的升级入口。
6. **生产迁移安全加固**：保留生产已有的 Agent/App Attest 迁移 7/8，订阅表使用 9/10；启动时校验所有已应用 migration checksum，防止不同分支复用编号后静默漏建表。
7. **Debug / Release 边界明确**：免费/Plus 验收控件和本地 StoreKit 配置仅服务内部 Debug 验收，Release 不展示内部切换入口，也不携带商业 Prompt 正文。

### 验证与发布
- HoloBackend 测试 167/167 通过
- Holo Debug 与 Release 通用 iOS Simulator 全工程构建成功，小组件扩展随主工程编译通过
- 后端已部署至生产，release commit `949bc9f99ae3637b1043d22fdceda833fd99174f`，公网 health、release status、订阅状态和三类真实 intent 验收均通过
- 真实 App Store 购买凭证校验仍是独立上线项；不影响内部免费/Plus切换与额度验收

---

## [2026-08-01] 修复 HoloAI 首次进入卡顿与数据卡片二次滑入

HoloAI 首次进入时，页面本身的上滑转场、历史消息异步加载、复杂数据卡片首轮测量和列表自动回底会在同一时间发生，导致界面短暂卡动，部分卡片还会像从底部再次滑入。

### 改动
1. **首屏消息原子呈现**：历史消息和数据卡片先在不可见状态完成首轮布局与底部定位，避免未稳定的中间位置直接暴露给用户。
2. **转场结束后统一展示**：首屏内容复用全屏模块的统一转场时长，等页面进入动画结束后关闭隐式动画一次性显示，不再叠加第二段卡片位移。
3. **保留后续聊天行为**：该时序只作用于首次进入，后续消息追加、流式回复、历史分页和手动滚动逻辑保持不变。

### 验证
- `ChatScrollBehaviorStandaloneTests` 通过，覆盖转场期间、转场结束后及异常时长的首屏展示时序
- Holo Debug 通用 iOS Simulator 全工程构建成功
- `git diff --check` 通过
- 真机冷启动后的最终动画手感仍需人工验收

### 性质
- 纯 App 端首屏呈现修复，随下次 App 发版生效
- 不涉及 HoloBackend，无需部署后端

---

## [2026-07-30] 全局 UX 走查整改 — 数据信任 / 一致性 / 体验打磨

对想法、目标、任务、习惯、健康、看板、财务、AI 聊天、首页、个人、设置等全部主要界面做了一轮深度走查，按「数据信任 → 系统一致性 → 体验打磨」三个优先级整改。

### 数据与信任硬伤
1. **看板「今日支出」接真实数据**：顶部进度卡之前一直显示写死的 `¥--`，现在接入当日真实支出；并修正了「今日支出 ¥0」误用红色（没花钱不该标红）。
2. **写操作失败不再静默**：新建全局提示组件，接管记账保存、习惯打卡、想法图片上传、任务自动保存等 13 处「失败只打日志、用户无感」的场景——现在失败会弹提示，任务编辑保存失败时还会保留你的改动不丢。
3. **目标列表补加载态**：冷启动进「我的目标」不再先闪一下「还没有目标」再被列表替换，改为先转圈加载、数据就绪再显示。

### 系统一致性
4. **想法模块三个「功能断头路」**：底部假 Tab（点了不切页只弹编辑器）改为右下角浮动按钮；补回「已归档」查看入口（原来归档后全 App 找不到）；删除残留的半成品心情功能与死代码。
5. **卡片样式统一**：全 App 卡片圆角收敛到设计系统标准（习惯卡片从 28 收到 16、设置卡片从 10 收到标准档位等），切来切去更像同一个 App。
6. **功能色统一**：聊天用户气泡从突兀的系统蓝改为中性灰（与品牌橙有区分但不割裂）；习惯超标、设置危险操作、任务日期等全部替换为品牌色，深色模式下表现一致。

### 体验打磨
7. **测量类习惯**：补上可见的撤销入口（原来只能长按，发现不了）。
8. **习惯记录**：每行的常驻红色删除按钮改为长按出菜单，不再满屏红色。
9. **空态图标**：习惯空态的「打勾」图标改为「加号 / 空托盘」（打勾=完成，用在空态语义矛盾）。
10. **健康详情**：「假洞察」改为基于真实数据的诚实陈述（原来永远显示「晚饭后散步」固定文案）；健康首页指标卡片补了箭头暗示可点击。
11. **设置页**：从废弃的 NavigationView 升级到 NavigationStack，iPad 横屏下更稳定。

> 本次改动均为前端 UI，不涉及后端发版。

---

## [2026-07-30] 修复跨模块长文本被状态标签与固定控件挤压

上一轮统一交互后继续走查了个人、记忆、HoloAI、习惯、健康、财务、观点和设置中的窄屏排版。主要问题是标题、说明与状态标签被放在同一横排，固定控件优先占宽后，中文会被压成逐字换行或发生裁切。

### 改动
1. **个人页与记忆长廊**：“Holo 记住的你”及记忆摘要优先获得完整文字宽度，状态、反馈标签移到独立行，不再把正文挤成竖排。
2. **HoloAI 与深度分析卡片**：公共卡片标题、说明和分析标题优先展示，状态标签在文字区域下方排列；详情建议标题也不会再被优先级标签压窄。
3. **习惯、健康与财务适配窄屏**：长习惯名空间不足时将“统计/看板”整体下移；健康详情状态标签可自动换到标题下方；财务分类排行移除固定总宽度，避免小屏裁切。
4. **设置与观点细节收口**：开关行、每日状态卡和记忆设置补齐正文宽度优先级与行数边界，固定开关、圆点和箭头不再侵占主要信息。

### 验证
- `git diff --check` 通过
- 在仅包含本次 15 个界面文件的干净副本中，Holo Debug 通用 iOS Simulator 全工程构建成功
- 本次为布局修复，未执行自动化截图与真机动态字号回归；需按个人页、记忆长廊、HoloAI 卡片、习惯统计设置、健康详情和财务分类排行进行人工验收

### 性质
- 纯 App 端布局改动，不改变数据、点击行为或导航逻辑
- 不涉及 HoloBackend，无需部署后端

---

## [2026-07-30] 统一非任务模块交互体验：保留页面上下文、补齐数据真相与跨模块闭环

本次集中处理财务、习惯、健康看板、观点、记忆、目标和设置之间影响使用连续性的交互断层。核心原则是：跨模块操作能回到原位置、无数据不冒充零数据、取消不产生隐式写入、同一能力只保留一个明确入口。

### 改动
1. **跨模块导航保留上下文**：首页驻留页面改为统一路由栈。从记忆、目标或聊天进入财务、习惯等模块后，返回时原页面、筛选条件、子页签、月份和展开状态继续保留，不再每次重建页面。
2. **观点编辑取消语义统一**：取消按钮、下滑关闭和切换标签不再偷偷保存；存在未保存内容时统一询问是否放弃，只有明确点击保存才写入。
3. **每日看板展示真实状态**：健康无权限、设备不支持或当天无数据时显示明确状态，不再误报为“0 / 不足”；预算卡改为读取当前账户的真实今日支出，加载完成前不展示伪金额。
4. **财务与习惯状态连续**：财务分析和习惯统计的页签、月份及展开状态提升到模块级持有，模块内切换或跨模块往返后不再重置。
5. **记忆入口与来源跳转收口**：记忆管理统一进入“记忆长廊”，移除重复记忆中心；记忆日历中的交易、习惯、待办和观点事件可以回到对应原始内容，原内容不存在时给出可见提示。
6. **目标操作形成闭环**：目标详情补齐关联任务/习惯跳转、完成状态、删除确认和错误反馈，避免点击无响应或失败后静默。
7. **设置与可访问性统一**：首页右上角改为明确的设置图标；移除没有实际生效闭环的 Holo One 自定义设置；全局语义字体接入系统动态字号，改善大字体环境下的可读性。

### 验证
- 新增驻留页面路由 standalone 测试，覆盖入栈、返回、去重及返回既有页面，全部通过
- Holo Debug 通用 iOS Simulator 全工程构建成功
- `git diff --check` 通过
- 完整 HoloTests target 仍受项目既有 standalone `@main` 混编及 Agent 测试依赖成员配置阻塞，本次不表述为全量测试通过

### 性质
- 纯 App 端交互与状态管理改动，随下次 App 发版生效
- 不涉及 HoloBackend，无需部署后端
