...
## [2026-08-01] 新建/编辑任务页统一卡片化，消除视觉割裂

新建/编辑任务页此前存在明显的视觉割裂：上半部分（标题、描述、语音按钮）直接摊在页面背景上，没有卡片外壳；下半部分（时间、子任务、附件、状态/清单属性）则各自手写了一套卡片样式，圆角还不统一（时间/子任务用 8pt，附件用 16pt），导致「上面空荡荡、下面突然冒出一堆块」的突兀感。

### 改动
1. **统一卡片样式**：将标题/描述/语音区、时间、子任务、附件、状态/优先级/清单属性、删除按钮共 6 个顶层块全部改用项目现成的 `holoCard()` 标准卡片组件（16pt 圆角 + 0.5pt 半透明描边 + 轻投影），整个页面从上到下使用同一种视觉语言。标题/描述区作为打字主战场，内边距略宽（横 16 / 竖 14）。
2. **清理历史死代码**：删除从未被调用的旧视图 `descriptionSection`、`listSection`，保持文件整洁，避免后续维护误读。
3. **嵌套子项不动**：`metadataSection` 内部的状态行、清单行、优先级选择器等靠外层卡片提供背景，本身不带卡片样式，避免「卡片套卡片」。

### 验证与发布
- Holo Debug iOS Simulator 全工程构建成功
- 纯客户端视觉样式调整，**不涉及后端发版**
- 建议真机验收：新建任务页从上到下各块视觉一致；切换到编辑任务页确认删除按钮、属性块同样统一；检查深色模式下卡片描边与投影是否自然
