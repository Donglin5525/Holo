...
## [2026-09-17] 桌面小组件三修（徽章掉行/胶囊折行/行内标签）+ 想法卡片补「拷贝正文」双入口

> 东林反馈桌面三症状：待办组件「逾期」徽章掉到标题下一行叠压文字、收支组件分类胶囊「数码 ¥1,658.83」折成两行、想法随机漫步正文把 #标签 渲染成普通文本；另反馈想法不能复制（详情页下线批次丢的能力）。纯 iOS 零后端发版。
...
- **想法复制**：详情页砍除批次为保「点正文进编辑器」关闭了正文触摸，选字复制随之中招；iOS 长按「操作菜单」与「原生选字」互斥（contextMenu 实测抢占）。本期交付两条一键全文复制：卡片长按菜单 + 「…」菜单各补「拷贝正文」；「长按直接选字」的让位改造待东林拍板。

### 验证
- 模拟器真装全流程：长按 /「…」两条路径拷贝后剪贴板逐字比对 = 全文；点正文进编辑器（P0）无回归；行内标签卡片高亮、查看全文/收起正常。
