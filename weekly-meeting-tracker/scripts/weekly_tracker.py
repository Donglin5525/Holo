...
def extract_time_nodes(text, sheet_date):
    ...
    for m in re.finditer(r'下周([一二三四五六日天])', body):
        ...
        if wd is not None:
            # sheet_date 是周五（weekday=4），下周一 = sheet_date + 3
    ...
    # "下周" (无具体星期)
    for m in re.finditer(r'下周(?![一二三四五六日天])', body):
        days_ahead = (7 - sheet_date.weekday()) + 2  # 默认下周三
        dt = sheet_date + timedelta(days=days_ahead)
        context = body[m.start():min(m.end() + 20, len(body))]
        candidates.append((dt, context, m.end()))
...
def status_reached(current_status, expected_statuses):
    """
    判断当前状态是否达到或超过任一期望状态。
    返回: 'reached' | 'progressed' | 'not_reached'
    """
...
def load_and_validate(excel_path):
    """
    Step 1: 加载并校验 Excel。
    返回: (workbook, sheet_dates_sorted, warnings)
    """
...
def generate_ai_judgment(prev_data, curr_data, prev_sheet_name, prev_sheet_date, curr_sheet_date):
    """
    Step 2-4: 解析时间节点、比对状态、生成 AI 判断。

    返回: {req_id: judgment_text}, summary_stats, delayed_list
    """
    judgments = {}
    stats = {
        '按期推进': 0,
    ...
    for req_id in curr_data:
        if req_id not in prev_data:
            stats['新增需求'] += 1
            judgments[req_id] = 'ℹ️ 新增需求｜本期新增，无历史数据可比对。'
    ...
    for req_id, prev_record in prev_data.items():
        ...
        # 解析时间节点
        node_date, actions, raw_match = extract_time_nodes(follow_text, prev_sheet_date)
        ...
        if node_date is None or not actions:
            ...
            if req_id in curr_data:
                ...
                if curr_status != prev_status:
                    judgments[req_id] = (
                        f'ℹ️ 无明确节点｜上期跟进记录未提及具体时间节点，'
                        f'本期状态为「{curr_status}」（上期：{prev_status}），状态有变更。'
                    )
                else:
                    judgments[req_id] = (
                        f'ℹ️ 无明确节点｜上期跟进记录未提及具体时间节点，'
                        f'本期状态仍为「{curr_status}」，状态无变化。'
                    )
            else:
                stats['需求已移出'] += 1
                stats['无明确节点'] -= 1
                judgments[req_id] = (
                    f'🔴 需求已移出｜编号 {req_id} 在本期跟踪列表中未找到，'
                    f'请确认是否已关闭或转移。'
                )
            continue
        ...
        node_date_str = f'{node_date.month}/{node_date.day}'
        action_desc = '、'.join(actions[:2])
        ...
        if node_date > curr_sheet_date:
            stats['节点未到'] += 1
            judgments[req_id] = (
                f'⏳ 节点未到｜上期({prev_sheet_name})预计 {node_date_str} {action_desc}，'
                f'节点尚未到达，待下期验证。'
            )
            continue
        ...
        if result == 'reached':
            stats['按期推进'] += 1
            judgments[req_id] = (
                f'✅ 按期推进｜上期({prev_sheet_name})预计 {node_date_str} {action_desc}，'
                f'本期状态已变为「{curr_status}」，符合预期。'
            )
        elif curr_status == prev_status:
            stats['未按期推进'] += 1
            judgments[req_id] = (
                f'❌ 未按期推进｜上期({prev_sheet_name})预计 {node_date_str} {action_desc}，'
                f'本期状态仍为「{curr_status}」，延期 {delay_days} 天，建议关注。'
            )
            delayed_list.append({
                '编号': req_id,
            ...
            stats['有进展未达预期'] += 1
            judgments[req_id] = (
                f'⚠️ 有进展未达预期｜上期({prev_sheet_name})预计 {node_date_str} {action_desc}，'
                f'本期状态为「{curr_status}」（上期：{prev_status}），'
                f'已推进但尚未达到预期阶段。'
            )
...
def write_output(excel_path, wb, curr_sheet_name, curr_data, judgments, output_path=None):
    ...
    # 设置列宽
    col_letter = openpyxl.utils.get_column_letter(ai_col)
...
def print_summary(stats, delayed_list, prev_sheet_name, curr_sheet_name):
    """打印汇总统计"""
    print('\n' + '=' * 60)
    print(f'📊 周会需求进度追踪汇总（{prev_sheet_name} → {curr_sheet_name}）')
    print('=' * 60)

    print(f'\n本期需求总数: {stats["本期需求总数"]}')
    print(f'有明确节点的需求: {stats["有明确节点的需求"]}')
    print()
    print(f'  ✅ 按期推进: {stats["按期推进"]}')
    print(f'  ❌ 未按期推进: {stats["未按期推进"]}')
    print(f'  ⚠️  有进展未达预期: {stats["有进展未达预期"]}')
    print(f'  🔴 需求已移出: {stats["需求已移出"]}')
    print(f'  ⏳ 节点未到: {stats["节点未到"]}')
    print(f'  ℹ️  无明确节点: {stats["无明确节点"]}')
    print(f'  🆕 新增需求: {stats["新增需求"]}')

    if delayed_list:
        print('\n' + '-' * 60)
        print('🚨 未按期推进需求清单（建议周会重点讨论）：')
        print('-' * 60)
        for item in delayed_list:
            print(
                f'  [{item["编号"]}] {item["需求名称"][:30]}'
                f'  | 当前: {item["当前状态"]}'
                f'  | 预期: {item["预期日期"]} {item["预期动作"]}'
                f'  | 延期: {item["延期天数"]}天'
                f'  | IT: {item["IT跟进人"]}'
            )
...
def main():
    ...
    parser.add_argument('--output', '-o', help='输出文件路径（默认在原文件同目录生成 _AI分析结果.xlsx）')
    ...
    if len(date_sheets) < 2:
        if len(date_sheets) == 1:
            print(f'ℹ️  仅有 1 个 sheet ({date_sheets[0][0]})，这是首期数据，无法进行跨周比对。')
            print('    请在下周添加新的 sheet 后再次运行。')
        else:
