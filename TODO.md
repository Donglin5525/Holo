...
| 记忆长廊日历视图 | ✅ P1+P2+P3 完成 | 周历(列表/网格)+月历(热力/徽章)+模块筛选+待办维度+当天详情+观点呈现+健康保底。详情跳原模块/完整健康图层 待后续（方案见 `docs/_common/plans/2026-07-05-Holo记忆长廊日历视图（周历+月历）方案.md`） |
| 本周观察（首页胶囊） | ✅ 核心完成（Phase 0-3/5/6） | 首页胶囊统一入口 + MemoryInsight 分级（light3d/full7d）+ consent 最终防线 + 有效记录日 builder + ScheduleRanker 排序层；Phase 4 ChatView 展示区块 + Phase 6 Release 授权入口待东林决策（方案见 `docs/_common/plans/2026-07-07-Holo本周观察-最终实施方案.md`） |
| 记忆默认生效 | ✅ 已完成 | 普通记忆校验通过后自动采用免确认；仅敏感/身份/永久事实/假设/首次跨域保留确认；健康客观统计不再一刀切误拦；存量误拦记忆启动时自动重评估转正；个人页记忆回执胶囊（方案见 `docs/_common/plans/ADR-2026-07-16-记忆默认采用与敏感确认边界.md`） |
| 观点模块 | 🚧 进行中 | 标签统一识别（AI 平等复用用户认可标签）✅ 已完成并部署 ECS；标签/主题全局管理（长按重命名+删除+防再生）✅ 已完成；编辑器行内 # 标签与 @ 引用（结构化 Token + 多级标签 + 引用快照 + 阅读态渲染）✅ 已完成（方案见 `docs/thoughts/plans/2026-07-18-Holo观点编辑器行内标签与引用方案.md`）；独立标签管理页（搜索/批量）/ 7 领域大类 / 语义匹配（认同义标签）/ Topic 层级重构待立项 |
