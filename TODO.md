...
| 项目 | 说明 | 状态 |
|------|------|------|
| 异常检测引擎 | 消费突增检测、习惯断连预警、任务堆积告警、预算超支预警 | 📋 待开发 |
| 主动触发机制 | 定时触发（每日/周/月自动生成洞察）+ 事件触发（里程碑达成、首次预算达标等）+ 异常触发（检测到异常模式时主动推送） | 📋 待开发 |
| 洞察卡片 UI | 洞察列表页 + 卡片组件（数据可视化/建议/行动按钮）+ 未读标记 | 📋 待开发 |

#### 阶段 C：个性化洞察（~2-4 周）

| 项目 | 说明 | 状态 |
|------|------|------|
| 跨域关联分析 | 消费-习惯关联（加班周外卖增多）、习惯-任务关联（运动月任务完成率更高）、情绪-消费关联等 | 📋 待开发 |
| 洞察持久化与反馈循环 | 洞察归档回顾（上月建议这个月做到了吗？）、用户反馈（有用/没用 → 调整生成策略）、洞察去重（避免反复说同一件事） | 📋 待开发 |
