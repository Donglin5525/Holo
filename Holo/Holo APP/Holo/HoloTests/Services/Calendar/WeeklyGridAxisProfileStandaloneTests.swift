...
struct WeeklyGridAxisProfileStandaloneTests {
    ...
    private static func testHeightTiers() {
        ...
        expect(profile.segments.map(\.height) == [42, 42, 57, 84, 111, 131, 131],
    ...
    private static func testWeekUsesMaximumDailyDensity() {
        ...
        expect(profile.height(for: 10) == 111, "同一小时应取七天中的单日最大事件数")
        expect(profile.height(for: 11) == 57, "其他小时应独立计算密度")
        expect(profile.height(for: 12) == 42, "空小时应保持基础高度")
    }
    ...
    private static func testCumulativeOffsetsAndMinutePosition() {
        ...
        expect(profile.top(for: 7) == 0, "首小时应从 0 开始")
        expect(profile.top(for: 8) == 57, "下一小时应从前一动态高度之后开始")
        expect(profile.top(for: 9) == 141, "累计坐标应包含前面所有动态小时高度")
        expect(profile.yPosition(hour: 8, minute: 30) == 99, "半点应位于当前动态小时的中点")
        expect(profile.totalHeight == 183, "总高度应为所有动态小时高度之和")
    }
    ...
    private static func testEventsRemainIndividuallyVisibleWithinLimit() {
        ...
        expect(layout.displayItems.count == 4, "四条以内应全部逐条展示")
        expect(layout.displayItems.map(\.displayTitle) == events.map(\.title),
               "同一小时应按发生顺序展示真实标题")
        expect(layout.displayItems.allSatisfy { !$0.isOverflow && $0.events.count == 1 },
               "四条以内不应生成 +N 摘要")
    }
    ...
    private static func testOverflowContainsCompleteHourList() {
        ...
        expect(layout.displayItems.count == 5, "六条事件应展示四条明细和一个溢出入口")
        expect(overflow?.displayTitle == "还有 2 条", "溢出入口应显示准确隐藏数量")
        expect(overflow?.events.count == 6, "溢出入口应携带该小时完整事件清单")
        expect(overflow?.top == 111 && overflow?.height == 17,
               "溢出入口应落在 131pt 封顶小时内")
    }
