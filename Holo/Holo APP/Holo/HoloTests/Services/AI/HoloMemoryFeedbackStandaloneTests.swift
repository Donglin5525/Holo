...
struct HoloMemoryFeedbackStandaloneTests {
    ...
    static func main() async throws {
        ...
        expect(tombstones.count == 1, "不再使用应写入语义墓碑（低确认成本方案 §8.5）")
        expect(decisions == [.rejected], "不再使用应标记为用户拒绝（suppressed，不再召回）")
        expect(deletedRecordIDs.isEmpty, "不再使用不得只做删除（同锚点重生会被墓碑拦截）")
        let forgotten = await forgetStore.snapshot(id: original.id)
        expect(forgotten?.state == .suppressed, "不再使用后保留 suppressed 记录（审计可回溯）")
