...
//    -o /tmp/holo_agent_persistence_test && /tmp/holo_agent_persistence_test
//
...
struct HoloAgentPersistenceStoreTests {
    ...
    static func main() async {
        await testJobStore_upsert后load能读到()
        await testJobStore_updateState更新状态与时间()
        await testCheckpointStore_按jobID查最新()
        await testResultStore_upsert后按jobID查到()
        await testJobStore_cleanup按retention清理过期()
    ...
    private static func testJobStore_upsert后load能读到() async {
        let dir = makeTempDir()
    ...
    private static func testJobStore_updateState更新状态与时间() async {
        let dir = makeTempDir()
        let store = HoloAgentJobStore(directory: dir)
        ...
        let target = await store.load().first { $0.id == "job-2" }
    ...
    private static func testCheckpointStore_按jobID查最新() async {
        let dir = makeTempDir()
        ...
        let latest = await store.latestForJob(jobID: "job-3")
        expect(latest?.id == "cp-2", "job-3 的最新 checkpoint 应是 cp-2，实际 \(latest?.id ?? "nil")")
    }

    // MARK: - ResultStore

    private static func testResultStore_upsert后按jobID查到() async {
        let dir = makeTempDir()
        let store = HoloAgentResultStore(directory: dir)
        let result = HoloAgentResult(
            id: "res-1", jobID: "job-4", title: "洞察", summary: "摘要",
            claims: [], evidenceIDs: [], memoryCandidateIDs: [],
            status: "completed",
            generatedAt: Date(timeIntervalSince1970: 1000), updatedAt: Date(timeIntervalSince1970: 1000)
        )
        try? await store.upsert(result)
        let found = await store.forJob(jobID: "job-4")
        expect(found?.id == "res-1", "应按 jobID 查到 res-1")
    }

    // MARK: - Cleanup

    private static func testJobStore_cleanup按retention清理过期() async {
        let dir = makeTempDir()
        let store = HoloAgentJobStore(directory: dir)
        ...
        expect(removed?.count == 2, "应清理 2 个（old-done, old-fail），实际 \(removed?.count ?? -1)")

        let remainingIDs = Set((await store.load()).map { $0.id })
