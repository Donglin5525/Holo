...
nonisolated enum IntentDescriptorRegistry {
    ...
    static let descriptors: [IntentDescriptor] = [
        IntentDescriptor(ids: ["record_expense"], summary: "记支出。填 amount、note、categoryCandidate、primaryCategory+subCategory（从标准科目表选：餐饮按当前时间选餐次，05-10早餐/10-16午餐/16-21晚餐/21点后夜宵；表里没有的品类留空，禁编表外名）；明确或相对日期填 transactionDate（YYYY-MM-DD），可选 normalizedCategoryCandidate/semanticCategoryHint；用户明确提到进行中项目名时填 projectCandidate（照抄项目名，未提及不填）。工资+金额走 record_income。", requiredFields: ["amount"], examples: ["\"昨天麦当劳35\" → intent: \"record_expense\", extractedData: { amount: \"35\", note: \"麦当劳\", categoryCandidate: \"麦当劳\", primaryCategory: \"餐饮\", subCategory: \"午餐\", transactionDate: \"2026-06-02\", normalizedCategoryCandidate: \"快餐\", semanticCategoryHint: \"餐饮\" }", "\"给爷爷买了两百块的彩票\" → intent: \"record_expense\", extractedData: { amount: \"200\", note: \"给爷爷买彩票\", categoryCandidate: \"给爷爷买彩票\", primaryCategory: \"人情\", subCategory: \"送礼\", semanticCategoryHint: \"人情\" }", "\"奶茶22，算装修的\" → intent: \"record_expense\", extractedData: { amount: \"22\", note: \"奶茶\", categoryCandidate: \"奶茶\", primaryCategory: \"餐饮\", subCategory: \"饮品\", normalizedCategoryCandidate: \"饮品\", semanticCategoryHint: \"餐饮\", projectCandidate: \"装修\" }", "\"酸汤肥牛234\"（当前时间 22:30） → intent: \"record_expense\", extractedData: { amount: \"234\", note: \"酸汤肥牛\", categoryCandidate: \"酸汤肥牛\", primaryCategory: \"餐饮\", subCategory: \"夜宵\", semanticCategoryHint: \"餐饮\" }"]),
        IntentDescriptor(ids: ["record_income"], summary: "记收入。填 amount、note、categoryCandidate；明确或相对日期填 transactionDate（格式同支出）。", requiredFields: ["amount"], examples: []),
        IntentDescriptor(ids: ["set_budget"], summary: "设置/调整预算。填 amount；分类预算填 categoryCandidate（不填=总预算）；period 为 month/year/week，默认 month。", requiredFields: ["amount"], examples: ["\"这个月餐饮预算2000\" → intent: \"set_budget\", extractedData: { amount: \"2000\", categoryCandidate: \"餐饮\", period: \"month\" }"]),
        IntentDescriptor(ids: ["create_task"], summary: "建待办/提醒。填 title；确定日期填 dueDate（YYYY-MM-DD 或 YYYY-MM-DD HH:mm，一律绝对日期）；用户说了具体钟点（如晚上10点）必须填 reminderDate 和 dueDate 的 HH:mm，时段换 24 小时制。多个提醒时间填 reminderDates（逗号分隔，每个 YYYY-MM-DD HH:mm）；相对基准换算见规则段。并列待办填 subtasks，title 概括，description 补充。主题性待办（旅行、装修等多项准备）填 listName，取原话主题词，App 自动匹配或建清单；单条琐事不填。", requiredFields: ["title"], examples: ["\"提醒我今天晚上10点给猫换水\" → intent: \"create_task\", extractedData: { title: \"给猫换水\", dueDate: \"2026-09-17 22:00\", reminderDate: \"2026-09-17 22:00\" }", "\"9月20日 早上9点要去换日币，头天晚上提醒我一下，当天早上8点再提醒一次\" → intent: \"create_task\", extractedData: { title: \"去换日币\", dueDate: \"2026-09-20 09:00\", reminderDates: \"2026-09-19 20:00,2026-09-20 08:00\" }", "\"十一要去日本旅行，帮我列个准备清单\" → intent: \"create_task\", extractedData: { title: \"日本旅行准备\", listName: \"日本旅行\", subtasks: \"办签证,订机票,订酒店\" }"]),
        ...
        IntentDescriptor(ids: ["update_goal_field"], summary: "改目标字段。填 goalTitle、field（title/deadline/summary/desiredOutcome/motivation/status）、value；status 的 value 填 completed（达成）或 paused（暂停）。", requiredFields: ["goalTitle","field","value"], examples: ["\"把英语目标截止日期改成年底\" → intent: \"update_goal_field\", extractedData: { goalTitle: \"英语\", field: \"deadline\", value: \"年底\" }", "\"英语目标达成了\" → intent: \"update_goal_field\", extractedData: { goalTitle: \"英语\", field: \"status\", value: \"completed\" }"]),
        IntentDescriptor(ids: ["link_task_to_goal"], summary: "任务关联目标。填 taskTitle、goalTitle。", requiredFields: ["taskTitle","goalTitle"], examples: []),
        IntentDescriptor(ids: ["link_habit_to_goal"], summary: "习惯关联目标。填 habitName、goalTitle。", requiredFields: ["habitName","goalTitle"], examples: []),
        IntentDescriptor(ids: ["log_metric_value"], summary: "记目标或习惯的数值，填 value；提及目标名/习惯名填 targetHint。", requiredFields: ["value"], examples: []),
        IntentDescriptor(ids: ["toggle_goal_visibility"], summary: "切换目标AI可见性。填 goalTitle、enable。", requiredFields: ["goalTitle","enable"], examples: []),
        IntentDescriptor(ids: ["create_anniversary"], summary: "新建纪念日/生日/倒数日。填 anniversaryTitle、anniversaryDate（YYYY-MM-DD）；类型明显时填 typeCandidate（birthday/anniversary/countdown/milestone）。", requiredFields: ["anniversaryTitle","anniversaryDate"], examples: ["\"10月3号是爸妈结婚纪念日\" → intent: \"create_anniversary\", extractedData: { anniversaryTitle: \"爸妈结婚纪念日\", anniversaryDate: \"2026-10-03\" }"]),
        IntentDescriptor(ids: ["update_anniversary"], summary: "改已有纪念日/生日的日期或名称。必填 anniversaryKeyword；改日期填 anniversaryDate，改名称填 anniversaryTitle。", requiredFields: ["anniversaryKeyword"], examples: []),
        ...
        IntentDescriptor(ids: ["contextual_planning"], summary: "个人情境规划（只读）：结合我的情况安排/准备一件具体的事（来访、备考、搬家、必花钱、决策）。单一记账/建任务/查数据不是。", examples: ["\"帮我想想下周末爸妈来怎么安排\" → intent: \"contextual_planning\", mode: \"query\", extractedData: {}"]),
