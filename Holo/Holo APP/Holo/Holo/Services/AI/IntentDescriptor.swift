...
nonisolated enum IntentDescriptorRegistry {
    ...
    static let descriptors: [IntentDescriptor] = [
        ...
        IntentDescriptor(ids: ["create_task"], summary: "建待办/提醒。填 title；确定日期填 dueDate（YYYY-MM-DD 或 YYYY-MM-DD HH:mm）；用户说了具体钟点（如晚上10点）必须填 reminderDate 和 dueDate 的 HH:mm，时段换 24 小时制。并列待办填 subtasks，title 概括，description 补充。指定清单填 listName（已有清单名）。", requiredFields: ["title"], examples: ["\"明天去山姆买牛奶、鸡蛋和纸巾\" → intent: \"create_task\", extractedData: { title: \"去山姆购物\", subtasks: \"买牛奶,买鸡蛋,买纸巾\" }", "\"提醒我今天晚上10点给猫换水\" → intent: \"create_task\", extractedData: { title: \"给猫换水\", dueDate: \"今天 22:00\", reminderDate: \"今天 22:00\" }"]),
