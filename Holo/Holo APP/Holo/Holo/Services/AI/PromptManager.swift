...
final class PromptManager {
    ...
    private let templates: [PromptType: String] = [
        ...
        - 【本周日程概览】（可能有）：用户系统日历本周各日的日程数量与密集时段（如「周三 4 场会议、全天在外」）。
        ...
        3. constraintSummary 一句话说明本周现实约束（时间/精力/预算/日程）。
        4. 有【本周日程概览】时：行动卡尽量避开日程密集日（如周三满会则把行动安排到其他天）；会议密集的一周，行动卡数量取下限（3 张）。
        5. 有上一份台账时：不原样重复被拒绝的行动；若再次出现必须有实质调整，并在 tradeoff 里注明（如「上周你因时间不够拒绝了 X，这次拆小为每次 5 分钟」）。上一份的重点完成情况应影响本周重点取舍。
        6. 不做医疗诊断、投资建议；不评判用户的生活方式。
        7. evidenceHints 填写支撑该优先结果的分析依据关键词（从【本周分析结论】中摘取，每条 4 字以上）。
        ...
        - 简洁友好，适合在手机 App 卡片里直接阅读；不要输出 Markdown 语法符号（如 #、##、*、-、```）。需要分段时用短标题行和自然换行。只有在多维度对比或多条目并列展示时，可以用竖线表格：首行为表头，第二行为 |---|---| 分隔行，不超过 4 列，单元格只写短语。
...
1.5 冷启动场景：用户主题列表为空时，直接从想法的标签与语义中建议第一批初始主题——这是该用户分类体系的起点；主题名优先复用想法已用过的标签词（用户认得自己的词），证据 2 条即可。
