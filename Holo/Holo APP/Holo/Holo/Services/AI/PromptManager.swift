...
final class PromptManager {
    ...
    private let templates: [PromptType: String] = [
        ...
        - 适合在手机 App 里自然阅读；不要输出 Markdown 标题、代码块或表格。确实需要列举时，只使用简单的「- 」或「1. 」列表
        - 第一段直接回答用户的问题，不寒暄、不复述问题，不使用“基于以上数据，我的观察如下”之类开场
        - 短问题用一段自然短文回答；只有同时涉及多个主题时才使用短标题，不要固定套用“事实/变化/模式/建议”全套结构
        - 默认只保留一个主结论和最多三个关键点；数字只在支撑结论时出现，不做流水账式枚举
        - 普通回答默认控制在 80-240 字；用户明确要求详细说明或确有必要时，可在正文之后另起「详细分析」一行，再补充依据和次要信息
        ...
        4. 用中文回复。
        5. 不要输出 Markdown 标题、代码块或表格。确实需要列举时，只使用简单的「- 」或「1. 」列表。
        ...
        使用适合 C 端直接阅读的自然文本结构：
        - 第一段直接回答用户最关心的问题，不寒暄、不复述问题，不使用“基于数据，我的观察如下”之类开场
        - 默认只保留一个主结论和最多三个关键点；不要为了完整而逐模块报数，数字只在支撑结论时出现
        - 短分析直接用自然段；只有同时涉及多个主题时才使用短标题，不要固定套用「事实/变化/模式/建议」全套结构
        - 关键数据直接写在句子里，不要用 ** 加粗语法；确需列举时只使用简单的「- 」或「1. 」列表
        - 默认控制在 180-320 字。用户明确要求详细说明或存在必须交代的依据时，在正文之后另起「详细分析」一行，再补充次要信息
