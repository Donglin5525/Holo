...
nonisolated enum HoloQuotaError: Error, Equatable, LocalizedError {
    ...
    var userMessage: String {
        ...
        case .quotaExceeded(let payload):
            ...
            case "asr": feature = String(localized: "语音识别")
            case "naturalLanguageFinance": feature = String(localized: "智能记账")
            case "naturalLanguageTask": feature = String(localized: "智能任务")
            case "memoryInsight": feature = String(localized: "记忆洞察")
            // 深度洞察与对话是两个独立额度池（免费 2/天 vs 15/天），
            // 文案必须指明是哪个池用完，否则用户无从判断。
            case "deepAnalysis": feature = String(localized: "深度分析")
            case "chat": feature = String(localized: "对话")
            case "lifePlan": feature = String(localized: "周计划")
            ...
            switch period {
            case "week": periodLabel = String(localized: "本周")
            case "month": periodLabel = String(localized: "本月")
            case "day": periodLabel = String(localized: "今天")
            default: periodLabel = String(localized: "当前")
            }
            return upgradeAvailable
                ? String(localized: "\(periodLabel)的免费\(feature)额度已用完，升级 Holo Plus 可继续使用")
                : String(localized: "\(periodLabel)的\(feature)额度已用完，请在额度重置后再试")
        case .asrDurationExceeded(let payload):
            let seconds = Int(payload.maxSeconds ?? 0)
            return upgradeAvailable
                ? String(localized: "免费版单次最多识别 \(seconds) 秒，升级 Holo Plus 可识别更长语音")
                : String(localized: "单次语音最长可识别 \(seconds) 秒，请缩短录音后重试")
        }
    }
    ...
    static func deepAnalysisExhaustedMessage(isPlusActive: Bool) -> String {
        isPlusActive
            ? String(localized: "今天的深度分析额度已用完，请在额度重置后再试")
            : String(localized: "今天的免费深度分析额度已用完，升级 Holo Plus 可继续使用")
    }

    /// 洞察额度预检文案：免费版按周（1 次/周）、Plus 按日（1 次/天），周期措辞随之不同。
    static func memoryInsightExhaustedMessage(isPlusActive: Bool) -> String {
        isPlusActive
            ? String(localized: "今天的记忆洞察额度已用完，请在额度重置后再试")
            : String(localized: "本周的免费记忆洞察额度已用完，升级 Holo Plus 可继续使用")
    }

    /// 语音识别（asr 池，免费 20 次/天、Plus 50 次/天）预检文案。
    static func asrExhaustedMessage(isPlusActive: Bool) -> String {
        isPlusActive
            ? String(localized: "今天的语音识别额度已用完，请在额度重置后再试")
            : String(localized: "今天的免费语音识别额度已用完，升级 Holo Plus 可继续使用")
    }

    /// 对话额度（chat 池，免费 15/天、Plus 30/天）预检文案。
    static func chatExhaustedMessage(isPlusActive: Bool) -> String {
        isPlusActive
            ? String(localized: "今天的对话额度已用完，请在额度重置后再试")
            : String(localized: "今天的免费对话额度已用完，升级 Holo Plus 可继续使用")
    }
    ...
    static func goalPlanningExhaustedMessage(isPlusActive: Bool) -> String {
        isPlusActive
            ? String(localized: "今天的对话额度已用完（目标规划也使用对话额度），请在额度重置后再试")
            : String(localized: "今天的免费对话额度已用完（目标规划也使用对话额度），升级 Holo Plus 可继续使用")
    }
}
